...
SpaceportPanel::SpaceportPanel(PlayerInfo &player) : player(player), port(player.GetPlanet()->GetPort())
{
...
void SpaceportPanel::Step()
{
  if(GetUI().IsTop(this) && port.HasService(Port::ServicesType::OffersMissions))
  {
    ...
    if(!mission) mission = player.MissionToOffer(Mission::LANDING);
    if(mission) mission->Do(Mission::OFFER, player, &GetUI());
    else player.HandleBlockedMissions(Mission::SPACEPORT, GetUI());
  }
}
...
{
  const Interface *ui = GameData::Interfaces().Get(Screen::Width() < 1280 ? "spaceport (small screen)" : "spaceport");
...
{
  if(!port.HasNews()) return nullptr;

  std::vector<const News *> matches;
  const Planet             *planet = player.GetPlanet();
  for(const auto &it : GameData::SpaceportNews())
    if(!it.second.IsEmpty() && it.second.Matches(planet)) matches.push_back(&it.second);
