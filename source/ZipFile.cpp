...
ZipFile::ZipFile(const std::filesystem::path &zipPath) : basePath(zipPath)
{
  ...
  for(const std::filesystem::path &path : ListFiles("", true, false))
  {
    ...
    if(topLevel.empty()) topLevel = *zipPath.begin();
    else if(*zipPath.begin() != topLevel) return;
  }
  topLevelDirectory = topLevel;
}


ZipFile::~ZipFile() { mz_zip_reader_end(&zipFile); }


std::vector<std::filesystem::path>
ZipFile::ListFiles(const std::filesystem::path &directory, bool recursive, bool directories)
{
  ...
  for(mz_uint i = 0; i < count; i++)
  {
    ...
    if(!mz_zip_reader_file_stat(&zipFile, i, &file_stat)) continue;
    std::filesystem::path zipEntry       = file_stat.m_filename;
    bool                  isValidSubtree = Files::IsParent(relative, zipEntry);
    bool                  isRecursive =
        std::distance(zipEntry.begin(), zipEntry.end()) == std::distance(relative.begin(), relative.end()) + 1;
...
std::filesystem::path ZipFile::GetPathInZip(const std::filesystem::path &path)
{
  ...
  if(!topLevelDirectory.empty()) relative = topLevelDirectory / relative;
  return relative;
}
