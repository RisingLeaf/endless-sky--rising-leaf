...
const Hazard *Weather::GetHazard() const { return hazard; }


// Whether the hazard of this weather deals damage or not.
bool Weather::HasWeapon() const { return hazard->Weapon::IsLoaded(); }
...
const Point &Weather::Origin() const { return origin; }
...
void Weather::Step(std::vector<Visual> &visuals, const Point &center)
{
  ...
  if(--lifetimeRemaining <= 0) shouldBeRemoved = true;
}
...
Weather::ImpactInfo Weather::GetInfo() const { return ImpactInfo(*hazard, origin, DamageMultiplier()); }


// Check if this object is marked for removal from the game.
bool Weather::ShouldBeRemoved() const { return shouldBeRemoved; }
