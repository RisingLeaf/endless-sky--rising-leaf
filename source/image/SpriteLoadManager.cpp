...
namespace
{
  ...
  void LoadSpriteQueued(TaskQueue &queue, const shared_ptr<ImageSet> &image)
  {
    ...
    {
      queue.Run(
          [image, sprite] { image->LoadDimensions(sprite); },
          [&queue]
          {
            ++spritesLoaded;
...
void SpriteLoadManager::Init(TaskQueue &queue, map<string, shared_ptr<ImageSet>> images)
{
  ...
  for(auto &[name, imageSet] : images)
  {
    ...
    if(IsDeferredFolder(name)) deferred[SpriteSet::Get(name)] = imageSet;
    lock_guard lock(imageQueueMutex);
...
void SpriteLoadManager::PreventSpriteUpload() { preventSpriteUpload = true; }
...
double SpriteLoadManager::Progress()
{
  ...
  if(queuedAllImages)
  {
    if(!totalSprites) spriteProgress = 1.;
    else spriteProgress = static_cast<double>(spritesLoaded) / totalSprites;
  }
...
bool SpriteLoadManager::IsDeferred(const Sprite *sprite) { return deferred.contains(sprite); }
...
void SpriteLoadManager::LoadDeferred(TaskQueue &queue, const Sprite *sprite)
{
  ...
  if(!sprite || dit == deferred.end()) return;

  const string               &name  = sprite->Name();
  const shared_ptr<ImageSet> &image = dit->second;
  if(name.starts_with("land/")) LoadLandscape(queue, sprite, image);
  else if(name.starts_with("thumbnail/") || name.starts_with("outfit/")) LoadThumbnail(queue, sprite, image);
  else if(name.starts_with("star/") || name.starts_with("planet/")) LoadStellarObject(queue, sprite, image);
  else if(name.starts_with("scene/")) LoadScene(queue, sprite, image);
}
...
void SpriteLoadManager::SetRecheckThumbnails() { recheckThumbnails = true; }
...
void SpriteLoadManager::SetRecheckStellarObjects() { recheckStellarObjects = true; }
...
void SpriteLoadManager::LoadLandscape(TaskQueue &queue, const Sprite *sprite, const shared_ptr<ImageSet> &image)
{
  ...
  if(pit != preloadedLandscapes.end())
  {
    for(pair<const Sprite *const, int> &it : preloadedLandscapes)
      if(it.second < pit->second) ++it.second;
...
{
  if(!loadedScenes.insert(sprite).second) return;
