    ...
    if(node.Size() >= 2 && node.HasChildren() && DEFINITION_NODES.contains(node.Token(0)))
    {
      ...
      if(key == "system")
      {
        ...
        if(any_of(
               node.begin(),
               node.end(),
               [](const DataNode &child) noexcept -> bool { return child.Size() >= 3 && child.Token(0) == "pos"; }))
        {
...
GameEvent::GameEvent(const DataNode &node, const ConditionsStore *playerConditions) { Load(node, playerConditions); }
...
void GameEvent::Load(const DataNode &node, const ConditionsStore *playerConditions)
{
  ...
  if(node.Size() >= 2)
  {
    ...
    if(!DataNode::IsConditionName(trueName)) node.PrintTrace("Invalid event/condition name:");
  ...
  for(const DataNode &child : node)
  {
    ...
    if(key == "date" && child.Size() >= 4) date = Date(child.Value(1), child.Value(2), child.Value(3));
    else if(key == "unvisit" && hasValue) systemsToUnvisit.push_back(GameData::Systems().Get(child.Token(1)));
    else if(key == "visit" && hasValue) systemsToVisit.push_back(GameData::Systems().Get(child.Token(1)));
    else if(key == "unvisit planet" && hasValue) planetsToUnvisit.push_back(GameData::Planets().Get(child.Token(1)));
    else if(key == "visit planet" && hasValue) planetsToVisit.push_back(GameData::Planets().Get(child.Token(1)));
    else if(key == "save raw changes") saveRawChanges = true;
    else if(allowedChanges.contains(key)) changes.push_back(child);
    else conditionsToApply.Add(child, playerConditions);
  }
}
...
void GameEvent::Save(DataWriter &out) const
{
  ...
  out.BeginChild();
  {
    if(date) out.Write("date", date.Day(), date.Month(), date.Year());
...
void GameEvent::Disable() { isDisabled = true; }


// All events held by GameData have a name, but those loaded from a save do not.
const std::string &GameEvent::TrueName() const { return trueName; }


// "Stock" GameEvents require a name to be serialized with an accepted mission.
void GameEvent::SetTrueName(const std::string &name) { this->trueName = name; }


const Date &GameEvent::GetDate() const { return date; }
...
void GameEvent::SetDate(const Date &date) { this->date = date; }
...
std::list<DataNode> GameEvent::Apply(PlayerInfo &player, bool onlyDataChanges)
{
  if(isDisabled) return {};
...
const ConditionAssignments &GameEvent::Conditions() const { return conditionsToApply; }


const std::list<DataNode> &GameEvent::Changes() const { return changes; }


bool GameEvent::SaveRawChanges() const { return saveRawChanges; }


// Date comparison.
bool GameEvent::operator<(const GameEvent &other) const { return date < other.date; }
