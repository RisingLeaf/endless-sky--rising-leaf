...
void BankPanel::Step() { DoHelp("bank"); }
...
void BankPanel::Draw()
{
  ...
  if(!mergedMortgages) mortgageRows = mortgageCount;
  ...
  if(salariesIncome || tributeIncome || b.assetsReturns)
  {
    ...
    table.DrawCustom(
        {LABEL[(salariesIncome != 0) + 2 * (tributeIncome != 0) + 4 * (b.assetsReturns != 0)], incomeLayout});
  ...
  std::string amount;
  if(!qualify) amount = "You do not qualify for further loans at this time.";
  else amount = "You qualify for a new loan of up to " + Format::CreditString(qualify) + ".";
  if(qualify && selectedRow >= mortgageRows) table.DrawHighlight(back);
  ...
  if((crewSalariesOwed || maintenanceDue) && player.Accounts().Credits() > 0)
  {
    ...
    for(const Mortgage &mortgage : player.Accounts().Mortgages())
      if(mortgage.Principal() <= player.Accounts().Credits()) info.SetCondition("can pay");
  }
...
bool BankPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  {
    GetUI().Push(DialogPanel::RequestString(this, &BankPanel::NewMortgage, "Borrow how many credits?"));
    DoHelp("bank advanced");
  }
  else if(key == 'a')
  {
    ...
    while(i < player.Accounts().Mortgages().size())
    {
      ...
      if(principal <= player.Accounts().Credits()) player.Accounts().PayExtra(i, principal);
      else ++i;
    }
...
bool BankPanel::Click(int x, int y, MouseButton button, int clicks)
{
  ...
  if(x >= MIN_X && x <= MAX_X && y >= FIRST_Y + 25 && y < maxY)
  {
    ...
    if(x >= MIN_X + EXTRA_X) DoKey(SDLK_RETURN);
  }
...
void BankPanel::NewMortgage(const std::string &str)
{
  ...
  if(amount > 0) player.Accounts().AddMortgage(amount);
