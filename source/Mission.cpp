...
namespace
{
  ...
  const Trade::Commodity *PickCommodity(const System &from, const System &to)
  {
    ...
    {
      r -= weight[i];
      if(r < 0) return &GameData::Commodities()[i];
    }
  ...
  {
    if(node.Size() >= expected + 1) node.PrintTrace("Use a location filter to choose from multiple " + kind + "s:");
    if(node.HasChildren()) node.PrintTrace("Location filter ignored due to use of explicit " + kind + ":");
  }
  ...
  std::string TriggerToText(Mission::Trigger trigger)
  {
    switch(trigger)
    {
    case Mission::Trigger::ABORT:    return "on abort";
    case Mission::Trigger::ACCEPT:   return "on accept";
    case Mission::Trigger::COMPLETE: return "on complete";
    case Mission::Trigger::DECLINE:  return "on decline";
    case Mission::Trigger::DEFER:    return "on defer";
    case Mission::Trigger::FAIL:     return "on fail";
    case Mission::Trigger::OFFER:    return "on offer";
    case Mission::Trigger::STOPOVER: return "on stopover";
    case Mission::Trigger::VISIT:    return "on visit";
    case Mission::Trigger::WAYPOINT: return "on waypoint";
    case Mission::Trigger::DAILY:    return "on daily";
    case Mission::Trigger::DISABLED: return "on disabled";
    ...
    const std::set<const Planet *> *visitedPlanets)
{
  ...
  if(!DataNode::IsConditionName(trueName)) node.PrintTrace("Invalid mission name:");
  ...
  for(const DataNode &child : node)
  {
    ...
    {
      if(child.Size() == 1) deadlineMultiplier += 2;
      if(child.Size() >= 2) deadlineBase += child.Value(1);
      if(child.Size() >= 3) deadlineMultiplier += child.Value(2);
    }
    ...
    else if(key == "cargo" && child.Size() >= 3)
    {
      ...
      if(child.Size() >= 4) cargoLimit = child.Value(3);
      if(child.Size() >= 5) cargoProb = child.Value(4);
      ...
      for(const DataNode &grand : child)
      {
        if(!ParseContraband(grand))
        {
          ...
          grand.PrintTrace(
              "Deprecated use of \"stealth\" and \"illegal\" as a child of \"cargo\"."
    ...
    else if(key == "passengers" && hasValue)
    {
      ...
      if(child.Size() >= 3) passengerLimit = child.Value(2);
      if(child.Size() >= 4) passengerProb = child.Value(3);
    }
    ...
    else if(key == "boarding")
    {
      location = BOARDING;
      for(const DataNode &grand : child)
        if(grand.Token(0) == "override capture") overridesCapture = true;
        else grand.PrintTrace("Skipping unrecognized attribute:");
    }
    ...
    {
      if(child.Token(1) == "offer") toOffer.Load(child, playerConditions);
      else if(child.Token(1) == "complete") toComplete.Load(child, playerConditions);
      else if(child.Token(1) == "fail") toFail.Load(child, playerConditions);
      else if(child.Token(1) == "accept") toAccept.Load(child, playerConditions);
      else child.PrintTrace("Skipping unrecognized attribute:");
    }
    ...
    else if(key == "complete")
    {
      ...
      else if(child.Token(1) == "at")
      {
        ...
        if(loaded.IsEmpty()) child.PrintTrace("The \"complete at\" filter must not be empty. Ignoring this filter.");
        else completionFilter = loaded;
      }
      else {
    ...
    else if(key == "on" && hasValue)
    {
      ...
      if(it != trigger.end()) actions[it->second].Load(child, playerConditions, visitedSystems, visitedPlanets);
      else child.PrintTrace("Skipping unrecognized attribute:");
    }
    else if(key == "color" && child.Size() >= 3)
    {
      ...
      {
        if(child.Size() >= 5) color = ExclusiveItem<Color>(Color(child.Value(2), child.Value(3), child.Value(4)));
        else color = ExclusiveItem<Color>(GameData::Colors().Get(child.Token(2)));
      };
      const std::string &value = child.Token(1);
      if(value == "unavailable") setColor(unavailable);
      else if(value == "unselected") setColor(unselected);
      else if(value == "selected") setColor(selected);
      else child.PrintTrace("Skipping unrecognized attribute:");
    }
    else {
  ...
  if(displayName.empty()) displayName = trueName;
...
void Mission::Save(DataWriter &out, const std::string &tag) const
{
  ...
  out.BeginChild();
  {
    ...
    if(!description.empty()) out.Write("description", description);
    if(!blocked.empty()) out.Write("blocked", blocked);
    if(deadline) out.Write("deadline", deadline.Day(), deadline.Month(), deadline.Year());
    if(cargoSize) out.Write("cargo", cargo, cargoSize);
    if(passengers) out.Write("passengers", passengers);
    if(paymentApparent) out.Write("apparent payment", paymentApparent);
    if(fine) out.Write("illegal", fine, fineMessage);
    if(failIfDiscovered) out.Write("stealth");
    if(!isVisible) out.Write("invisible");
    auto saveColor = [&out](const ExclusiveItem<Color> &color, std::string tokenName) noexcept -> void
    {
      if(!color->IsLoaded()) return;
    ...
    if(hasPriority) out.Write("priority");
    if(isNonBlocking) out.Write("non-blocking");
    if(isMinor) out.Write("minor");
    if(offerPrecedence) out.Write("offer precedence", offerPrecedence);
    if(autosave) out.Write("autosave");
    if(location == LANDING)
    {
      out.Write("landing");
    }
    ...
    if(ignoreClearance) out.Write("ignore", "clearance");
    if(!hasFullClearance) out.Write("infiltrating");
    if(hasFailed) out.Write("failed");
    if(repeat != 1) out.Write("repeat", repeat);
    ...
    if(destination) out.Write("destination", destination->TrueName());
    ...
    for(const auto &it : onEnter)
      if(!didEnter.contains(&it.second)) it.second.Save(out);
    for(const MissionAction &action : genericOnEnter)
      if(!didEnter.contains(&action)) action.Save(out);
    for(const auto &[planet, action] : onLand)
      if(!didLand.contains(&action)) action.Save(out);
...
void Mission::NeverOffer() { toOffer.MakeNever(); }


// Basic mission information.
const EsUuid &Mission::UUID() const noexcept { return uuid; }
...
// variables, etc.
const std::string &Mission::TrueName() const { return trueName; }


const std::string &Mission::DisplayName() const { return displayName; }


const std::string &Mission::Description() const { return description; }
...
bool Mission::IsVisible() const { return isVisible; }


// The colors that should be used to display the mission name if it is shown
// in your mission list.
const Color &Mission::Unavailable() const { return *unavailable; }


const Color &Mission::Unselected() const { return *unselected; }


const Color &Mission::Selected() const { return *selected; }
...
bool Mission::IsValid() const
{
  ...
  if(!completionFilter.IsEmpty() && !completionFilter.IsValid()) return false;
  ...
  for(auto &&it : onEnter)
    if(!it.first->IsValid() || !it.second.Validate().empty()) return false;
  for(const auto &[planet, action] : onLand)
    if(!planet->IsValid() || !action.Validate().empty()) return false;
  for(auto &&it : actions)
    if(!it.second.Validate().empty()) return false;
  ...
  for(auto &&action : genericOnEnter)
    if(!action.Validate().empty()) return false;
  for(const auto &action : genericOnLand)
    if(!action.Validate().empty()) return false;
  if(!clearanceFilter.IsValid()) return false;

  // The instantiated NPCs should also be valid.
  for(auto &&npc : NPCs())
    if(!npc.Validate().empty()) return false;
...
bool Mission::HasPriority() const { return hasPriority; }
...
bool Mission::IsNonBlocking() const { return isNonBlocking; }
...
bool Mission::IsMinor() const { return isMinor; }


int Mission::OfferPrecedence() const { return offerPrecedence; }


bool Mission::IsAtLocation(Location location) const { return this->location == location; }


// Information about what you are doing.
const Ship *Mission::SourceShip() const { return sourceShip; }


const Planet *Mission::Destination() const { return destination; }


const std::set<const System *> &Mission::Waypoints() const { return waypoints; }


const std::set<const System *> &Mission::VisitedWaypoints() const { return visitedWaypoints; }


const std::set<const Planet *> &Mission::Stopovers() const { return stopovers; }


const std::set<const Planet *> &Mission::VisitedStopovers() const { return visitedStopovers; }


const std::set<const System *> &Mission::MarkedSystems() const { return markedSystems; }


const std::set<const System *> &Mission::UnmarkedSystems() const { return unmarkedSystems; }


const std::set<const System *> &Mission::TrackedSystems() const { return trackedSystems; }


void Mission::RecalculateTrackedSystems()
{
  if(!hasTrackedNpcs) return;

  trackedSystems.clear();
  for(const NPC &npc : npcs)
  {
    if(!npc.GetPersonality().IsTracked()) continue;
    for(const auto &ship : npc.Ships())
      if(!ship->IsDestroyed() && ship->GetSystem()) trackedSystems.insert(ship->GetSystem());
  }
  // If all tracked NPCs are destroyed, then we can ignore
  // future calls to recalculate the tracked systems.
  if(trackedSystems.empty()) hasTrackedNpcs = false;
}
...
{
  for(const System *system : systems)
    if(markedSystems.erase(system)) unmarkedSystems.insert(system);
}


const std::string &Mission::Cargo() const { return cargo; }


int Mission::CargoSize() const { return cargoSize; }


int Mission::Fine() const { return fine; }


std::string Mission::FineMessage() const { return fineMessage; }


bool Mission::FailIfDiscovered() const { return failIfDiscovered; }


int Mission::Passengers() const { return passengers; }
...
const int Mission::ExpectedJumps() const { return expectedJumps; }


// The mission must be completed by this deadline (if there is a deadline).
const Date &Mission::Deadline() const { return deadline; }
...
{
  if(clearance.empty()) return false;
  if(planet == destination || stopovers.contains(planet) || visitedStopovers.contains(planet)) return true;
...
const std::string &Mission::ClearanceMessage() const { return clearance; }
...
bool Mission::HasFullClearance() const { return hasFullClearance; }
...
bool Mission::CanOffer(const PlayerInfo &player, const std::shared_ptr<Ship> &boardingShip) const
{
  ...
  {
    if(!boardingShip) return false;

    if(!sourceFilter.Matches(*boardingShip)) return false;
  }
  else if(location == ENTERING || location == TRANSITION)
  {
    if(!sourceFilter.Matches(player.GetSystem())) return false;
  }
  else {
    if(source && source != player.GetPlanet()) return false;

    if(!sourceFilter.Matches(player.GetPlanet())) return false;
  }

  if(!toOffer.Test()) return false;

  if(!toFail.IsEmpty() && toFail.Test()) return false;

  if(repeat && player.Conditions().Get(trueName + ": offered") >= repeat) return false;

  bool isFailed = IsFailed();
  auto it       = actions.find(OFFER);
  if(it != actions.end() && !it->second.CanBeDone(player, isFailed, boardingShip)) return false;
...
{
  if(!toAccept.Test()) return false;

  bool isFailed = IsFailed();
  auto it       = actions.find(OFFER);
  if(it != actions.end() && !it->second.CanBeDone(player, isFailed)) return false;
...
{
  int extraCrew = 0;
  if(player.Flagship()) extraCrew = player.Flagship()->Crew() - player.Flagship()->RequiredCrew();
...
{
  if(completionFilter.IsEmpty() && player.GetPlanet() != destination) return false;
...
{
  if(!waypoints.empty() || !stopovers.empty()) return false;

  // Test the completion conditions for this mission.
  if(!toComplete.Test()) return false;
  ...
  for(const NPC &npc : npcs)
    if(!npc.HasSucceeded(player.GetSystem())) return false;
  // All non-optional timers must be complete.
  for(const MissionTimer &timer : timers)
    if(!timer.IsOptional() && !timer.IsComplete()) return false;
  ...
  for(const auto &ship : player.Ships())
  {
    ...
    if(ship->GetSystem() == player.GetSystem() || (!ship->GetSystem() && ship->CanBeCarried() && ship->GetParent() &&
                                                   ship->GetParent()->GetSystem() == player.GetSystem()))
    {
      continue;
    }

    if(ship->Cargo().GetPassengers(this)) return false;
    // Check for all mission cargo, including that which has 0 mass.
    auto &cargo = ship->Cargo().MissionCargo();
    if(cargo.find(this) != cargo.end()) return false;
  }
...
{
  if(!toFail.IsEmpty() && toFail.Test()) return true;

  for(const NPC &npc : npcs)
    if(npc.HasFailed()) return true;

  return hasFailed;
}


bool Mission::OverridesCapture() const { return overridesCapture; }


// Mark a mission failed (e.g. due to a "fail" action in another mission).
void Mission::Fail() { hasFailed = true; }
...
std::string Mission::BlockedMessage(const PlayerInfo &player)
{
  ...
  if(flagship && player.GetPlanet()) extraCrew = flagship->Crew() - flagship->RequiredCrew();
  ...
  if(bunksNeeded > 0) out << (bunksNeeded == 1 ? "another bunk" : std::to_string(bunksNeeded) + " more bunks");
  if(bunksNeeded > 0 && cargoNeeded > 0) out << " and ";
  if(cargoNeeded > 0)
    out << (cargoNeeded == 1 ? "another ton" : std::to_string(cargoNeeded) + " more tons") << " of cargo space";
  if(bunksNeeded <= 0 && cargoNeeded <= 0) out << "no additional space";
...
bool Mission::Do(Trigger trigger, PlayerInfo &player, UI *ui, const std::shared_ptr<Ship> &boardingShip)
{
  if(trigger == STOPOVER)
  {
    ...
    if(it == stopovers.end()) return Land(player.GetPlanet(), player, *ui);
    ...
    stopovers.erase(it);
    if(!stopovers.empty()) return false;
  }
  if(trigger == ABORT && IsFailed()) return false;
  if(trigger == WAYPOINT && !waypoints.empty()) return false;
  ...
  if(trigger == ABORT && it == actions.end()) it = actions.find(FAIL);
  ...
  if(trigger == OFFER && location == JOB) ui = nullptr;
  ...
  if(it != actions.end())
  {
    it->second.Do(
        ...
        ui,
        this,
        (destination && isVisible) ? destination->GetSystem() : nullptr,
        boardingShip,
        IsUnique());
  }
...
bool Mission::RequiresGiftedShip(const std::string &shipId) const
{
  ...
  std::set<Trigger> requiredActions;
  {
    ...
    if(!stopovers.empty()) requiredActions.insert(Trigger::STOPOVER);
    if(!waypoints.empty()) requiredActions.insert(Trigger::WAYPOINT);
  }
  for(const auto &it : actions)
    if(requiredActions.contains(it.first) && it.second.RequiresGiftedShip(shipId)) return true;
...
const std::list<NPC> &Mission::NPCs() const { return npcs; }
...
void Mission::Do(const ShipEvent &event, PlayerInfo &player, UI &ui)
{
  if(event.TargetGovernment()->IsPlayer() && !IsFailed())
  {
    ...
    else if(event.Type() & ShipEvent::BOARD)
    {
      ...
      if(failed)
      {
        message = "Your " + event.Target()->DisplayModelName() + " \"" + event.Target()->GivenName() +
                  "\" has been plundered. ";
      }
    }
    ...
    if(failed)
    {
      ...
      {
        Messages::Add(
            {message + "Mission failed: \"" + displayName + "\".", GameData::MessageCategories().Get("high")});
      }
    }
  }

  if((event.Type() & ShipEvent::DISABLE) && event.Target() == player.FlagshipPtr()) Do(DISABLED, player, &ui);
  ...
  if((event.Type() & ShipEvent::JUMP) && flagship && event.Actor().get() == flagship)
  {
    ...
    if(Enter(system, player, ui)) UpdateNPCs(player);
  }
  ...
  for(NPC &npc : npcs)
  {
    ...
    if((event.Type() & (ShipEvent::JUMP | ShipEvent::DESTROY)) && isTarget) RecalculateTrackedSystems();
  }
}
...
Mission Mission::Instantiate(const PlayerInfo &player, const std::shared_ptr<Ship> &boardingShip) const
{
  ...
  if(result.waypoints.erase(sourceSystem)) result.visitedWaypoints.insert(sourceSystem);
  ...
  for(auto it = result.stopovers.begin(); it != result.stopovers.end();)
    if((*it)->IsValid()) ++it;
    else it = result.stopovers.erase(it);
  ...
  {
    if(player.GetPlanet()) result.destination = player.GetPlanet();
    else return result;
  }
  ...
  if(!cargo.empty())
  {
    ...
    if(commodity) result.cargo = commodity->items[Random::Int(commodity->items.size())];
    else result.cargo = expandedCargo;
  }
  // Pick a random cargo amount, if requested.
  if(cargoSize || cargoLimit)
  {
    if(cargoProb) result.cargoSize = Random::Polya(cargoLimit, cargoProb) + cargoSize;
    else if(cargoLimit > cargoSize) result.cargoSize = cargoSize + Random::Int(cargoLimit - cargoSize + 1);
    else result.cargoSize = cargoSize;
  }
  // Pick a random passenger count, if requested.
  if(passengers || passengerLimit)
  {
    if(passengerProb) result.passengers = Random::Polya(passengerLimit, passengerProb) + passengers;
    else if(passengerLimit > passengers) result.passengers = passengers + Random::Int(passengerLimit - passengers + 1);
    else result.passengers = passengers;
  }
  ...
  if(deadlineBase || deadlineMultiplier) result.deadline = player.GetDate() + deadlineBase + deadlineMultiplier * jumps;
  ...
  if(player.GetPlanet()) subs["<origin>"] = player.GetPlanet()->DisplayName();
  else if(boardingShip) subs["<origin>"] = boardingShip->GivenName();
  else if(player.GetSystem()) subs["<origin>"] = player.GetSystem()->DisplayName();
  ...
  if(result.paymentApparent) subs["<payment>"] = Format::CreditString(abs(result.paymentApparent));
  // Stopovers: "<name> in the <system name> system" with "," and "and".
  auto getDisplayName = [](const auto *const &item) { return item->DisplayName(); };
  if(!result.stopovers.empty())
  {
    subs["<stopovers>"] = Format::List<std::set, const Planet *>(
        result.stopovers,
  ...
  for(const NPC &npc : npcs)
  {
    ...
    {
      Logger::Log(
          "Instantiation Error: NPC template in mission \"" + TrueName() + "\" uses invalid " + std::move(reason),
  ...
  for(const auto &[trigger, action] : actions)
  {
    ...
    {
      Logger::Log(
          "Instantiation Error: Action \"" + TriggerToText(trigger) + "\" in mission \"" + TrueName() +
  ...
  for(const auto &[system, action] : onEnter)
  {
    ...
    {
      Logger::Log(
          "Instantiation Error: Action \"on enter `" + system->TrueName() + "`\" in mission \"" + TrueName() +
              "\" uses invalid " + std::move(reason),
          Logger::Level::WARNING);
      return result;
    }
  ...
  for(const auto &[planet, action] : onLand)
  {
    ...
    {
      Logger::Log(
          "Instantiation Error: Action \"on land `" + planet->TrueName() + "`\" in mission \"" + TrueName() +
              "\" uses invalid " + std::move(reason),
          Logger::Level::WARNING);
      return result;
    }
  ...
  for(const MissionAction &action : genericOnEnter)
  {
    ...
    {
      Logger::Log(
          "Instantiation Error: Generic \"on enter\" action in mission \"" + TrueName() + "\" uses invalid " +
              std::move(reason),
          Logger::Level::WARNING);
      return result;
    }
  ...
  for(const MissionAction &action : genericOnLand)
  {
    ...
    {
      Logger::Log(
          "Instantiation Error: Generic \"on land\" action in mission \"" + TrueName() + "\" uses invalid " +
              std::move(reason),
          Logger::Level::WARNING);
      return result;
    }
...
int Mission::CalculateJumps(const System *sourceSystem)
{
  ...
  while(!destinations.empty())
  {
    ...
    DistanceMap distance(sourceSystem, distanceCalcSettings.WormholeStrat(), distanceCalcSettings.AssumesJumpDrive());
    auto        it       = destinations.begin();
    auto        bestIt   = it;
    int         bestDays = distance.Days(**bestIt);
    if(bestDays < 0) bestDays = std::numeric_limits<int>::max();
