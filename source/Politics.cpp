...
namespace
{
  ...
  bool EvadesCargoScan(const Ship &ship)
  {
    ...
    if(netIllegalCargo <= 0) return true;
...
bool Politics::IsEnemy(const Government *first, const Government *second) const
{
  ...
  // it is the first one.
  if(second->IsPlayer()) swap(first, second);
  if(first->IsPlayer())
  {
    if(bribed.contains(second)) return false;
    if(provoked.contains(second)) return true;
...
{
  if(!planet || !planet->GetSystem()) return false;

  const Government *gov = ship.GetGovernment();
  if(!gov->IsPlayer())
    return !ship.IsRestrictedFrom(*planet) && (!planet->IsInhabited() || !IsEnemy(gov, planet->GetGovernment()));
...
{
  if(!planet || !planet->GetSystem()) return false;
  if(!planet->IsInhabited()) return true;
  if(HasClearance(planet)) return true;
  if(provoked.contains(planet->GetGovernment())) return false;
...
{
  if(!planet || !planet->GetSystem()) return false;
  if(dominatedPlanets.contains(planet)) return true;

  auto it = bribedPlanets.find(planet);
  if(it != bribedPlanets.end()) return it->second;
...
void Politics::BribePlanet(const Planet *planet, bool fullAccess) { bribedPlanets[planet] = fullAccess; }


void Politics::DominatePlanet(const Planet *planet, bool dominate)
{
  if(dominate) dominatedPlanets.insert(planet);
  else dominatedPlanets.erase(planet);
}


bool Politics::HasDominated(const Planet *planet) const { return dominatedPlanets.contains(planet); }


// Check to see if the player has done anything they should be fined for.
pair<const Conversation *, string>
Politics::Fine(PlayerInfo &player, const Government *gov, int scan, const Ship *target, double security)
{
  // Do nothing if you have already been fined today, or if you evade
  // detection.
  if(fined.contains(gov) || Random::Real() > security) return {};
  ...
  {
    if(target && target != &*ship) continue;
    if(ship->GetSystem() != player.GetSystem()) continue;
    const Planet *planet = player.GetPlanet();
    if(planet && ship->GetPlanet() != planet) continue;
    // Skip parked ships. The spaceport authorities are only scanning the ships you just landed with.
    if(ship->IsParked()) continue;
    ...
    if((!scan || (scan & ShipEvent::SCAN_CARGO)) && !EvadesCargoScan(*ship))
    {
      ...
      if(fine.second) deathSentence = fine.second;
  ...
  if(maxFine < 0)
  {
    ...
    {
      reason = "atrocity";
      if(!deathSentence) deathSentence = gov->DeathSentence();
    }
    else {
  ...
  else if(maxFine > 0)
  {
    ...
    reason  = "The " + gov->DisplayName() + " authorities fine you " + Format::CreditString(maxFine) + reason;
...
void Politics::AddReputation(const Government *gov, double value) { SetReputation(gov, reputationWith[gov] + value); }
