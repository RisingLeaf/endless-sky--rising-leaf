...
void EscortDisplay::Draw(const Rectangle &bounds) const
{
  ...
  for(const Icon &escort : icons)
  {
    ...
    {
      corner.X() += width;
      if(corner.X() + width > bounds.Right()) break;
    ...
    Color color;
    if(escort.isDisabled) color = disabledColor;
    else if(escort.isHostile) color = hostileColor;
    else if(!escort.isHere) color = elsewhereColor;
    else if(escort.cannotJump) color = cannotJumpColor;
    else if(escort.notReadyToJump) color = notReadyToJumpColor;
    else color = hereColor;

    // Draw the selection pointer
    if(escort.isSelected) info.SetCondition("selected");
...
const std::vector<std::weak_ptr<Ship>> &EscortDisplay::Click(const Point &point) const
{
  for(unsigned i = 0; i < zones.size(); ++i)
    if(zones[i].Contains(point)) return stacks[i];

  static const std::vector<std::weak_ptr<Ship>> EMPTY;
  return EMPTY;
}


EscortDisplay::Icon::Icon(
    const std::shared_ptr<Ship> &ship,
    bool                    isHere,
    bool                    systemNameKnown,
    bool                    fleetIsJumping,
...
{
  height = basicHeight;
  if(!system.empty()) height += systemLabelHeight;
}


// Sorting operator. It comes sooner if it costs more.
bool EscortDisplay::Icon::operator<(const Icon &other) const { return cost > other.cost; }


int EscortDisplay::Icon::Height() const { return height; }
...
void EscortDisplay::Icon::Merge(const Icon &other)
{
  ...
  if(system.empty() && !other.system.empty()) system = other.system;
...
void EscortDisplay::MergeStacks(int maxHeight) const
{
  ...
  while(true)
  {
    ...
    for(Icon &icon : icons)
    {
      if(!unstackable.contains(icon.sprite) && (!cheapest || *cheapest < icon)) cheapest = &icon;

      height += icon.Height();
    }

    if(height < maxHeight || !cheapest) break;
