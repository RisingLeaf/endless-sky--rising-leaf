...
struct Frustum
{
  struct Plane
  {
    ...
    Plane(const gm::dvec3 &norm, const double d) : Normal(norm), D(d) {}

    [[nodiscard]] double SignedDistance(const gm::dvec3 &point) const { return gm::dot(Normal, point) + D; }
  };
  ...
  {
    return std::ranges::all_of(Planes, [&](const Plane &plane) { return plane.SignedDistance(pos) > -radius; });
  }
};
