...
bool AudioPlayer::IsFinished() const { return done; }


double AudioPlayer::Volume() const
{
  if(!alSource) return 0.;
...
void AudioPlayer::Move(double x, double y, double z) const { alSource3f(alSource, AL_POSITION, x, y, z); }


SoundCategory AudioPlayer::Category() const { return category; }
...
void AudioPlayer::Stop(bool stop) { shouldStop = stop; }


AudioSupplier *AudioPlayer::Supplier() { return audioSupplier.get(); }


bool AudioPlayer::ClaimSource()
{
  if(alSource) return true;
