...
std::vector<File::ShaderString> File::ReadShader(const std::string_view path)
{
  ...
  while(true)
  {
    ...
    if(shaderType == 'v')
    {
      working.stage = GraphicsTypes::ShaderStage::VULKAN_VERTEX;
    }
    else if(shaderType == 'f')
    {
      working.stage = GraphicsTypes::ShaderStage::VULKAN_FRAGMENT;
    }
    else if(shaderType == 'c')
    {
      working.stage = GraphicsTypes::ShaderStage::VULKAN_COMPUTE;
    }
    else if(shaderType == 'm')
    {
      working.stage = GraphicsTypes::ShaderStage::METAL_COMBINED;
    }
    else if(shaderType == 'x')
    {
      working.stage = GraphicsTypes::ShaderStage::GLSL_VERTEX;
    }
    else if(shaderType == 'y')
    {
      working.stage = GraphicsTypes::ShaderStage::GLSL_FRAGMENT;
    }
    else if(shaderType == 'z')
    {
