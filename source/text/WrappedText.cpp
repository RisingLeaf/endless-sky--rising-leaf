  ...
  if(truncate == Truncate::NONE)
  {
    ...
    for(size_t i = 0; i < words.size(); ++i)
    {
      ...
      if(h == w.y && (i != words.size() - 1 && w.y == words[i + 1].y))
        font->Draw(text.c_str() + w.Index(), w.Pos() + topLeft, color);
      else font->Draw({text.c_str() + w.Index(), {wrapWidth, truncate}}, w.Pos() + topLeft, color);
      h = w.y;
    }
  }
}
...
void WrappedText::Wrap()
{
  ...
  if(currentLineHasWords) word.y += lineHeight + paragraphBreak;
...
int WrappedText::Space(char c) const { return (c == ' ') ? space : (c == '\t') ? tabWidth : 0; }
