  ...
  player(player), ship(ship), bribeCallback(std::move(bribeCallback)), facing(ship->Facing())
{
  ...
  if(!ship->GivenName().empty()) header = gov->DisplayName() + " " + ship->Noun() + " \"" + ship->GivenName() + "\":";
  else header = ship->DisplayModelName() + " (" + gov->DisplayName() + "):";
  ...
  else if(ship->IsDisabled())
  {
    ...
    else if(canGiveFuel || canRepair)
    {
      ...
      if(canGiveFuel && canRepair) helpOffer += "patch you up and give you some fuel?";
      else if(canGiveFuel) helpOffer += "give you some fuel?";
      else if(canRepair) helpOffer += "patch you up?";
      else if(canGiveEnergy) helpOffer += "recharge you?";
  ...
  if(message.empty()) SetMessage(ship->GetHail(player.GetSubstitutions()));
}
  ...
  player(player), object(object), planet(object->GetPlanet()), facing(object->Facing())
{
  ...
  if(planet) header = gov->DisplayName() + " " + planet->Noun() + " \"" + planet->DisplayName() + "\":";
  ...
  else if(planet && player.Flagship())
  {
    if(planet->CanLand())
    {
      ...
      if(planet->CanBribe()) SetBribe(planet->GetBribeFraction());
      if(bribe) SetMessage("If you want to land here, it'll cost you " + Format::CreditString(bribe) + ".");
      else if(gov->IsEnemy()) SetMessage("You are not welcome here.");
      else SetMessage("I'm afraid we can't permit you to land here.");
    }
  }
}
...
void HailPanel::Draw()
{
  ...
  if(ship)
  {
    ...
    {
      if(requestedToBribeShip) info.SetCondition("show pay bribe");
      if(ship->GetGovernment()->IsEnemy())
        if(requestedToBribeShip) info.SetCondition("can pay bribe");
        else info.SetCondition("can bribe");
      else if(!ship->CanBeCarried() && ship->GetShipToAssist() != player.FlagshipPtr()) info.SetCondition("can assist");
    }
  }
  else {
    if(!GameData::GetPolitics().HasDominated(planet)) info.SetCondition("show dominate");
    else info.SetCondition("show relinquish");
    if(hasLanguage)
    {
      info.SetCondition("can dominate");
      if(!planet->CanLand()) info.SetCondition("can bribe");
    }
  }
  ...
  if(ship)
  {
    ...
    if(hasFighters)
    {
      for(const Ship::Bay &bay : ship->Bays())
        if(bay.side == Ship::Bay::UNDER) addFighter(bay);
    }
    for(const Hardpoint &hardpoint : ship->Weapons())
      if(hardpoint.GetSide() == Hardpoint::Side::UNDER) addHardpoint(hardpoint);
    draw.Add(Body(*ship, center, Point(), facing, zoom));
    for(const Hardpoint &hardpoint : ship->Weapons())
      if(hardpoint.GetSide() == Hardpoint::Side::OVER) addHardpoint(hardpoint);
    if(hasFighters)
    {
      for(const Ship::Bay &bay : ship->Bays())
        if(bay.side == Ship::Bay::OVER) addFighter(bay);
    }
  }
  else {
...
bool HailPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  if(key == 'd' || key == SDLK_ESCAPE || key == SDLK_RETURN || (key == 'w' && (mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI))))
  {
    if(bribeCallback && bribed) bribeCallback(bribed);
    GetUI().Pop(this);
  ...
  {
    if(shipIsEnemy || ship->IsDisabled()) return false;
    if(playerNeedsHelp)
    {
      if(ship->GetPersonality().IsSurveillance())
      {
      ...
      else if(player.Flagship()->NeedsFuel(false))
      {
        ...
        else SetMessage("Sorry, we don't have any fuel.");
      }
      else if(player.Flagship()->NeedsEnergy())
      {
        if(ship->Energy()) SetMessage("Sorry, but if we give you energy we won't have enough for our ship.");
        else SetMessage("Sorry, we don't have any energy.");
      }
      ...
      if(bribe) SetMessage("Yeah, right. Don't push your luck.");
      else SetMessage("You don't seem to be in need of repairs or fuel assistance.");
    }
  }
  else if((key == 'b' || key == 'o') && hasLanguage)
  {
    ...
    if((ship && !shipIsEnemy) || (planet && planet->CanLand())) return true;
    ...
    else if(bribe)
    {
      if(!ship || requestedToBribeShip)
      {
        ...
        if(planet) SetMessage(gov->GetPlanetBribeAcceptanceHail());
        else SetMessage(gov->GetShipBribeAcceptanceHail());
      }
      if(ship)
      {
        if(!requestedToBribeShip)
        {
          SetMessage("If you want us to leave you alone, it'll cost you " + Format::CreditString(bribe) + ".");
          ...
          bribed->Bribe();
          Messages::Add(
              {"You bribed a " + bribed->DisplayName() + " ship " + Format::CreditString(bribe) +
        ...
        planet->Bribe();
        Messages::Add(
            {"You bribed the authorities on " + planet->DisplayName() + " " + Format::CreditString(bribe) +
...
void HailPanel::SetMessage(const std::string &text)
{
  message = text;
  if(!message.empty())
    Messages::Add({"(Response to your hail) " + header + " " + message, GameData::MessageCategories().Get("log only")});
}
