...
namespace
{
  ...
  bool ReadPNG(const std::filesystem::path &path, ImageBuffer &buffer, int frame, bool onlyDimensions);
  bool ReadJPG(const std::filesystem::path &path, ImageBuffer &buffer, int frame, bool onlyDimensions);
  int  ReadAVIF(
...
const std::set<std::string> &ImageBuffer::ImageExtensions() { return IMAGE_EXTENSIONS; }


const std::set<std::string> &ImageBuffer::ImageSequenceExtensions() { return IMAGE_SEQUENCE_EXTENSIONS; }


ImageBuffer::ImageBuffer(int frames) : width(0), height(0), frames(frames), pixels(nullptr) {}


ImageBuffer::~ImageBuffer() { Clear(); }
...
void ImageBuffer::Allocate(int width, int height)
{
  ...
  // is set to zero.
  if(pixels || !width || !height || !frames) return;
...
const uint32_t *ImageBuffer::Begin(int y, int frame) const { return pixels + width * (y + height * frame); }


uint32_t *ImageBuffer::Begin(int y, int frame) { return pixels + width * (y + height * frame); }
...
void ImageBuffer::ShrinkToHalfSize()
{
  ...
  for(int y = 0; y < result.height * frames; ++y)
  {
    ...
    for(; aIt != aEnd; aIt += 4, bIt += 4)
    {
      ...
      {
        *out = (static_cast<unsigned>(aIt[0]) + static_cast<unsigned>(bIt[0]) + static_cast<unsigned>(aIt[4]) +
                static_cast<unsigned>(bIt[4]) + 2) /
               4;
      }
    }
  }
...
int ImageBuffer::Read(const ImageFileData &data, int frame, bool onlyDimensions)
{
  ...
  if(!isPNG && !isJPG && !isAVIF) return false;

  bool isAlphaPreMultiplied = data.blendingMode == BlendingMode::PREMULTIPLIED_ALPHA;
  int  loaded;
  if(isPNG) loaded = ReadPNG(data.path, *this, frame, onlyDimensions);
  else if(isJPG) loaded = ReadJPG(data.path, *this, frame, onlyDimensions);
  else loaded = ReadAVIF(data.path, *this, frame, isAlphaPreMultiplied, onlyDimensions);

  if(loaded <= 0) return 0;
  if(onlyDimensions) return loaded;

  if(!isAlphaPreMultiplied)
  {
    if(isPNG || (isJPG && data.blendingMode == BlendingMode::ADDITIVE)) Premultiply(*this, frame, data.blendingMode);
  }
  return loaded;
}
...
namespace
{
  ...
  bool ReadPNG(const std::filesystem::path &path, ImageBuffer &buffer, int frame, bool onlyDimensions)
  {
    ...
    if(!width || !height || width != buffer.Width() || height != buffer.Height())
    {
      ...
      {
        Logger::Log(
            message + "width: expected " + std::to_string(buffer.Width()) + " but was " + std::to_string(width),
            Logger::Level::WARNING);
      }
      if(height && height != buffer.Height())
      {
        Logger::Log(
            message + "height: expected " + std::to_string(buffer.Height()) + " but was " + std::to_string(height),
            Logger::Level::WARNING);
      }
      return false;
    }
...
#pragma GCC diagnostic pop
      ...
      bool                         onlyDimensions)
  {
    ...
    std::string data = Files::Read(path);
    avifResult  result =
        avifDecoderSetIOMemory(decoder.get(), reinterpret_cast<const uint8_t *>(data.c_str()), data.size());
    ...
    if(result != AVIF_RESULT_OK)
    {
      Logger::Log(std::string("Failed to decode image: ") + avifResultToString(result), Logger::Level::WARNING);
      return 0;
    }
    ...
    if(!decoder->imageCount) return 0;
    ...
    for(int i = 0; i < decoder->imageCount; ++i)
    {
      ...
      {
        Logger::Log(
            "Could not get image timing for '" + path.generic_string() + "': " + avifResultToString(result),
            Logger::Level::WARNING);
        return 0;
      }
      if(frameTimeUnit < 0 || (frameTimeUnit > timing.duration && timing.duration)) frameTimeUnit = timing.duration;
    }
    ...
    try
    {
      if(bufferFrameCount > 1) buffer.Clear(bufferFrameCount);
    ...
    if(onlyDimensions) return bufferFrameCount;
    ...
    while(avifDecoderNextImage(decoder.get()) == AVIF_RESULT_OK)
    {
      ...
      if(!repeats[avifFrameIndex]) continue;
      ...
      {
        Logger::Log(
            "Conversion from YUV failed for \"" + path.generic_string() + "\": " + avifResultToString(result),
            Logger::Level::WARNING);
        return bufferFrame;
      }
  ...
  void Premultiply(ImageBuffer &buffer, int frame, BlendingMode blend)
  {
    for(int y = 0; y < buffer.Height(); ++y)
    {
      ...
      for(uint32_t *end = it + buffer.Width(); it != end; ++it)
      {
        ...
        if(blend == BlendingMode::HALF_ADDITIVE) alpha >>= 2;
        if(blend != BlendingMode::ADDITIVE) value |= (alpha << 24);
