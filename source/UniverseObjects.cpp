...
std::shared_future<void> UniverseObjects::Load(
    TaskQueue                                &queue,
    ...
    bool                                      debugMode)
{
  ...
  return queue.Run(
      [this, &player, &sources, globalConditions, debugMode]() noexcept -> void
      {
...
double UniverseObjects::GetProgress() const { return progress.load(std::memory_order_acquire); }
...
void UniverseObjects::FinishLoading()
{
  ...
  startConditions.erase(
      remove_if(
          startConditions.begin(),
          startConditions.end(),
  ...
  for(const auto &category : disabled)
  {
    ...
    else Logger::Log("Unhandled \"disable\" keyword of type \"" + category.first + "\".", Logger::Level::WARNING);
  }
...
void UniverseObjects::Change(const DataNode &node, PlayerInfo &player)
{
  ...
  else if(key == "event" && hasValue)
  {
    ...
    std::list<DataNode> changes   = eventCopy.Apply(player, true);
...
void UniverseObjects::UpdateSystems()
{
  for(auto &it : systems)
  {
    ...
    if(it.first.empty() || it.second.TrueName().empty()) continue;
    ...
    for(const auto &object : it.second.Objects())
      if(object.GetPlanet()) planets.Get(object.GetPlanet()->TrueName())->FinishLoading(wormholes);
  }
}
...
void UniverseObjects::CheckReferences()
{
  ...
  {
    if(deferred.contains(it.first)) it.second.SetTrueName(it.first);
    else return false;
  ...
  for(const auto &it : conversations)
    if(it.second.IsEmpty()) Warn("conversation", it.first);
  ...
  for(auto &&it : effects)
    if(it.second.TrueName().empty()) NameAndWarn("effect", it);
  ...
  for(auto &&it : fleets)
  {
    ...
    if(!it.second.IsValid() && !deferred["fleet"].contains(it.first)) Warn("fleet", it.first);
  }
  // Government names are used in mission NPC blocks and LocationFilters.
  for(auto &&it : governments)
    if(it.second.TrueName().empty() && !NameIfDeferred(deferred["government"], it)) NameAndWarn("government", it);
  // Minables are not serialized.
  for(const auto &it : minables)
    if(it.second.TrueName().empty()) Warn("minable", it.first);
  ...
  for(const auto &it : missions)
    if(it.second.DisplayName().empty()) Warn("mission", it.first);
  ...
  for(auto &&it : outfits)
    if(it.second.TrueName().empty()) NameAndWarn("outfit", it);
  // Phrases are never serialized.
  for(const auto &it : phrases)
    if(it.second.Name().empty()) Warn("phrase", it.first);
  // Planet names are used by a number of classes.
  for(auto &&it : planets)
    if(it.second.TrueName().empty() && !NameIfDeferred(deferred["planet"], it)) NameAndWarn("planet", it);
  ...
  for(auto &&it : systems)
    if(it.second.TrueName().empty() && !NameIfDeferred(deferred["system"], it)) NameAndWarn("system", it);
  // Hazards are never serialized.
  for(const auto &it : hazards)
    if(!it.second.IsValid()) Warn("hazard", it.first);
  // Wormholes are never serialized.
  for(const auto &it : wormholes)
    if(it.second.DisplayName().empty()) Warn("wormhole", it.first);
  // Formation patterns are not serialized, but their usage is.
  for(auto &&it : formations)
    if(it.second.TrueName().empty()) NameAndWarn("formation", it);
  // Any stock colors should have been loaded from game data files.
  for(const auto &it : colors)
    if(!it.second.IsLoaded()) Warn("color", it.first);
  for(const auto &it : swizzles)
    if(!it.second.IsLoaded()) Warn("swizzle", it.first);
  for(const auto &it : messageCategories)
    if(!it.second.IsLoaded()) Warn("message category", it.first);
  for(auto &it : messages)
    if(!it.second.IsLoaded()) NameAndWarn("message", it);
  // Persons can be referred to when marking them as destroyed.
  for(const auto &it : persons)
    if(!it.second.IsLoaded()) Warn("person", it.first);
}
    ...
    bool                         debugMode)
{
  ...
  if(path.extension() != ".txt") return;

  DataFile data(path);
  if(debugMode) Logger::Log("Parsing: " + path.string(), Logger::Level::INFO);
  ...
  for(const DataNode &node : data)
  {
    ...
    else if(key == "interface" && hasValue)
    {
      ...
      {
        std::lock_guard<std::mutex> lock(menuBackgroundMutex);
        if(overwrite) menuBackgroundCache = Interface();
    ...
    else if(key == "phrase" && hasValue)
    {
      ...
      if(overwrite) *phrase = Phrase();
      phrase->Load(node);
    }
    else if(key == "planet" && hasValue)
    {
      Planet *planet = planets.Get(node.Token(1));
      if(overwrite) *planet = Planet();
    ...
    else if(key == "star" && hasValue)
    {
      ...
      for(const DataNode &child : node)
      {
        ...
        if(childKey == "power" && childHasValue) solarPower[sprite] = child.Value(1);
        else if(childKey == "wind" && childHasValue) solarWind[sprite] = child.Value(1);
        else if(childKey == "icon" && childHasValue) starIcons[sprite] = SpriteSet::Get(child.Token(1));
    ...
    else if(key == "message category")
    {
      ...
      if(overwrite) *category = Message::Category();
      category->Load(node);
    }
    else if(key == "message")
    {
      Message *message = messages.Get(node.Token(1));
      if(overwrite) *message = Message();
      message->Load(node);
    }
    else if(key == "disable" && hasValue)
    {
      ...
      {
        if(overwrite) disabled[category].clear();
        if(node.HasChildren())
          for(const DataNode &child : node)
