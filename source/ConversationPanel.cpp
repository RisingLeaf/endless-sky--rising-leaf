  ...
  ship(ship)
{
  ...
  for(const auto &sub : subsToSave)
  {
    ...
    if(it != subs.end() && !it->second.empty()) savedSubs.emplace(*it);
  }
  ...
  if(useTransactions) player.StartTransaction();
...
ConversationPanel::~ConversationPanel() { Audio::Resume(); }


void ConversationPanel::SetCallback(std::function<void(int)> fun) { callback = std::move(fun); }
...
void ConversationPanel::Draw()
{
  ...
  else if(choices.empty())
  {
    ...
    for(const auto &it : choices)
    {
      ...
      if(isHovering && zone.Contains(hoverPoint)) choice = index;

      if(index == choice) FillShader::Fill(center + Point(-5, 0), size + Point(30, 0), selectionColor);
...
bool ConversationPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  if(choices.empty())
  {
    ...
    else if(key >= ' ' && key <= '~')
    {
      ...
      char c = ((mod & SDL_KMOD_SHIFT) ? SHIFT[key] : key);
      // Caps lock should shift letters, but not any other keys.
      if((mod & SDL_KMOD_CAPS) && c >= 'a' && c <= 'z') c += 'A' - 'a';
      if(FORBIDDEN.find(c) == std::string::npos && (name.size() + otherName.size()) < MAX_NAME_LENGTH) name += c;
      else flickerTime = 18;
    }
  ...
  if(key == SDLK_UP && choice > 0) --choice;
  else if(key == SDLK_DOWN && choice + 1 < static_cast<int>(choices.size())) ++choice;
...
bool ConversationPanel::Scroll(double dx, double dy) { return Drag(0., dy * Preferences::ScrollSpeed()); }
...
void ConversationPanel::Goto(int index, int selectedChoice)
{
  ...
  if(choices.empty() && conversation.Choices(node) != 0) node = Endpoint::DECLINE;
  this->choice = 0;
}
...
void ConversationPanel::Exit()
{
  ...
  if(useTransactions) player.FinishTransaction();
  ...
  else if(ship)
  {
    ...
    // TODO: Is there a better / more robust boarding check than relative position?
        ...
        (node != Endpoint::ACCEPT || player.CaptureOverriden(ship)) && ship->GetGovernment()->IsEnemy() &&
        !ship->IsDestroyed() && ship->IsDisabled() && ship->Position().Distance(player.Flagship()->Position()) <= 1.)
    {
...
void ConversationPanel::ClickName(int side) { choice = side; }
...
void ConversationPanel::ClickChoice(int index)
{
  ...
  else UI::PlaySound(UI::UISound::FAILURE);
}
...
{
  if(n < 0 || n >= static_cast<int>(choices.size())) return 0;
  else return next(choices.cbegin(), n)->second;
}
...
int ConversationPanel::Paragraph::Height() const
{
  ...
  if(scene) height += 20 * !isFirst + scene->Height() + 20;
  return height;
}
