...
void AsteroidField::Add(const Minable *minable, int count, double energy, const WeightedList<double> &belts)
{
  ...
  if(!minable || !minable->GetMask().IsLoaded()) return;
...
const std::list<std::shared_ptr<Minable>> &AsteroidField::Minables() const { return minables; }
...
void AsteroidField::Asteroid::Step()
{
  ...
  if(position.X() < 0.) position = Point(position.X() + WRAP, position.Y());
  else if(position.X() >= WRAP) position = Point(position.X() - WRAP, position.Y());

  if(position.Y() < 0.) position = Point(position.X(), position.Y() + WRAP);
  else if(position.Y() >= WRAP) position = Point(position.X(), position.Y() - WRAP);
}
