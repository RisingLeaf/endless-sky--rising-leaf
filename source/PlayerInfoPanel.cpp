...
namespace
{
      ...
      bool                                          drawValues = true)
  {
    ...
    for(const auto &it : list)
    {
      ...
      if(drawValues) table.Draw(it.first);
      else table.Advance();
    }
  }
  ...
  bool CompareSystem(const std::shared_ptr<Ship> &lhs, const std::shared_ptr<Ship> &rhs)
  {
    ...
    if(lhs->GetSystem() == nullptr) return false;
    else if(rhs->GetSystem() == nullptr) return true;
  ...
  {
    return lhs->Attributes().Get("fuel capacity") * lhs->Fuel() < rhs->Attributes().Get("fuel capacity") * rhs->Fuel();
  }

  bool CompareRequiredCrew(const std::shared_ptr<Ship> &lhs, const std::shared_ptr<Ship> &rhs)
  {
    // Parked ships are sorted to the end.
    if(lhs->IsParked()) return false;
    else if(rhs->IsParked()) return true;
  ...
  {
    if(f == &CompareName) return ReverseCompare<CompareName>;
    else if(f == &CompareModelName) return ReverseCompare<CompareModelName>;
    else if(f == &CompareSystem) return ReverseCompare<CompareSystem>;
    else if(f == &CompareShields) return ReverseCompare<CompareShields>;
    else if(f == &CompareHull) return ReverseCompare<CompareHull>;
    else if(f == &CompareFuel) return ReverseCompare<CompareFuel>;
...
PlayerInfoPanel::PlayerInfoPanel(PlayerInfo &player, InfoPanelState panelState) : player(player), panelState(panelState)
{
  Audio::Pause();
...
void PlayerInfoPanel::Step()
{
  if(GetUI().IsTop(this) && !checkedHelp)
  {
    ...
    {
      if(!DoHelp("multiple ships")) DoHelp("fleet management");
    }
    checkedHelp = true;
  }
}
  ...
  if(panelState.CanEdit() && !panelState.Ships().empty())
  {
    ...
    for(const auto &it : panelState.Ships())
    {
      if(!it->IsDisabled() && it.get() != flagship)
      {
        ...
        hasOtherShips  = true;
        if(it->GetSystem() == flagshipSystem) allParkedSystem &= it->IsParked();
      }
    }
    if(hasOtherShips)
    {
    ...
    if(panelState.CanEdit() && panelState.CurrentSort()) interfaceInfo.SetCondition("show save order");
  }

  interfaceInfo.SetCondition("three buttons");
  if(player.HasLogs()) interfaceInfo.SetCondition("enable logbook");
...
{
  bool control = (mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI));
  bool shift   = (mod & SDL_KMOD_SHIFT);
  if(key == 'd' || key == SDLK_ESCAPE || (key == 'w' && control) || key == 'i' || command.Has(Command::INFO))
  {
    GetUI().Pop(this);
  }
  ...
  else if(key == SDLK_UP || key == SDLK_DOWN)
  {
    if(panelState.AllSelected().empty())
    {
      ...
      if(isNewPress)
      {
        if(key == SDLK_UP) panelState.SetSelectedIndex(panelState.Ships().size() - 1);
        else panelState.SetSelectedIndex(0);
      }
    }
    ...
    else if(panelState.CanEdit() && !panelState.AllSelected().empty() && control && shift)
    {
      ...
      if(panelState.ReorderShipsTo(toIndex)) ScrollAbsolute(panelState.SelectedIndex() - 12);
      return true;
    }
    else {
      ...
      if(selectedIndex < 0)
      {
        if(isNewPress) panelState.DeselectAll();
      }
      else if(shift)
      {
        if(panelState.AllSelected().contains(selectedIndex)) panelState.Deselect(panelState.SelectedIndex());
        if(isValidIndex) panelState.SetSelectedIndex(selectedIndex);
      }
      else if(control)
      {
  ...
  else if(panelState.CanEdit() && (key == 'k' || (key == 'p' && shift)) && !panelState.AllSelected().empty())
  {
    ...
    {
      const Ship &ship = *panelState.Ships()[i];
      if(!ship.IsDisabled() && &ship != flagship) allParked &= ship.IsParked();
    }

    for(int i : panelState.AllSelected())
    {
      const Ship &ship = *panelState.Ships()[i];
      if(!ship.IsDisabled() && &ship != flagship) player.ParkShip(&ship, !allParked);
    }
  }
  else if(panelState.CanEdit() && (key == 'a') && !panelState.Ships().empty())
  {
    ...
    const Ship *flagship  = player.Flagship();
    for(const auto &it : panelState.Ships())
      if(!it->IsDisabled() && it.get() != flagship) allParked &= it->IsParked();

    for(const auto &it : panelState.Ships())
      if(!it->IsDisabled() && (allParked || it.get() != flagship)) player.ParkShip(it.get(), !allParked);
  }
  else if(panelState.CanEdit() && (key == 'c') && !panelState.Ships().empty())
  {
    ...
    const System *flagshipSystem = flagship ? flagship->GetSystem() : player.GetSystem();
    for(const auto &it : panelState.Ships())
      if(!it->IsDisabled() && it.get() != flagship && it->GetSystem() == flagshipSystem) allParked &= it->IsParked();
    ...
    if(control)
    {
      ...
      for(Ship *ship : player.GetEscortGroup(group))
      {
        for(size_t i = 0; i < panelState.Ships().size(); ++i)
          if(panelState.Ships()[i].get() == ship) added.insert(i);
      }
...
bool PlayerInfoPanel::Release(int /* x */, int /* y */, MouseButton button)
{
  ...
  if(!panelState.CanEdit() || hoverIndex < 0 || hoverIndex == panelState.SelectedIndex()) return true;
...
void PlayerInfoPanel::DrawPlayer(const Rectangle &bounds)
{
  ...
  table.DrawTruncatedPair("player:", dim, player.FirstName() + " " + player.LastName(), bright, Truncate::MIDDLE, true);
  ...
  if(!combatRating.empty())
  {
    ...
    table.DrawTruncatedPair(
        ...
        Truncate::MIDDLE,
        false);
    table.DrawTruncatedPair("experience:", dim, Format::Number(combatExperience), dim, Truncate::MIDDLE, false);
...
void PlayerInfoPanel::DrawFleet(const Rectangle &bounds)
{
  ...
  for(const auto &column : columns)
  {
    ...
    const Color &columnHeaderColor =
        ((!isDragging && zone.Contains(hoverPoint)) || panelState.CurrentSort() == column.shipSort) ? bright : dim;
  ...
  for(auto sit = panelState.Ships().begin() + panelState.Scroll(); sit < panelState.Ships().end(); ++sit)
  {
    ...
    if(!bounds.Contains(table.GetRowBounds())) break;

    // Check if this row is selected.
    if(panelState.SelectedIndex() == index) table.DrawHighlight(selectedBack);
    else if(panelState.AllSelected().contains(index)) table.DrawHighlight(back);
    ...
    table.Draw(Format::Percentage(std::max(0., ship.Shields()), 0));

    table.Draw(Format::Percentage(std::max(0., ship.Hull()), 0));

    std::string fuel = std::to_string(static_cast<int>(ship.Attributes().Get("fuel capacity") * ship.Fuel()));
    table.Draw(fuel);
    ...
    if(!isFlagship) crewCount = std::min(crewCount, ship.RequiredCrew());
...
{
  if(panelState.Ships().empty()) return;

  // Clicking on a sort column twice reverses the comparison.
  if(panelState.CurrentSort() == shipComparator) shipComparator = GetReverseCompareFrom(*shipComparator);
  ...
  std::shared_ptr<Ship>                                                  lastSelected =
  ...
  if(selectedShips.empty()) return;
  auto it = selectedShips.begin();
  for(size_t i = 0; i < panelState.Ships().size(); ++i)
  {
    if(panelState.Ships()[i] == *it)
    {
      if(lastSelected == *it) panelState.SetSelectedIndex(i);
      else panelState.Select(i);

      ++it;
      if(it == selectedShips.end()) break;
    }
  }
}
...
bool PlayerInfoPanel::Scroll(double /* dx */, double dy) { return Scroll(dy * -.1 * Preferences::ScrollSpeed()); }
...
bool PlayerInfoPanel::ScrollAbsolute(int scroll)
{
  ...
  if(panelState.Scroll() == newScroll) return false;
...
bool PlayerInfoPanel::Scroll(int distance) { return ScrollAbsolute(panelState.Scroll() + distance); }
