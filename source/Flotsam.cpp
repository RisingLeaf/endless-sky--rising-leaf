  ...
  commodity(commodity), count(count), sourceGovernment(sourceGovernment)
{
  ...
  if(count != TONS_PER_BOX) lifetime = sqrt(count * (1. / TONS_PER_BOX)) * lifetime;
}
  ...
  outfit(outfit), count(count), sourceGovernment(sourceGovernment)
{
  ...
  int lifetimeBase = 3'000'000'000 / (outfit->Cost() * count + 1'000'000);
...
{
  Place(source);
  if(source.Bays().size() > bayIndex) position += source.Facing().Rotate(source.Bays()[bayIndex].point);
}
...
void Flotsam::Place(const Body &source, const Point &dv)
{
  ...
  if(outfit && outfit->FlotsamSprite()) SetSprite(outfit->FlotsamSprite());
  else SetSprite(SpriteSet::Get("effect/box"));
...
void Flotsam::SetVelocity(const Point &velocity) { this->velocity = velocity; }


// This is the one ship that cannot pick up this flotsam.
const Ship *Flotsam::Source() const { return source; }
...
const Government *Flotsam::SourceGovernment() const { return sourceGovernment; }


// This is what the flotsam contains:
const std::string &Flotsam::CommodityType() const { return commodity; }


const Outfit *Flotsam::OutfitType() const { return outfit; }


int Flotsam::Count() const { return count; }
...
double Flotsam::UnitSize() const { return outfit ? outfit->Mass() : 1.; }


double Flotsam::Mass() const { return Count() * UnitSize(); }
