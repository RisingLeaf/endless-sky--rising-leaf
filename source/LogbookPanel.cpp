...
void LogbookPanel::Draw()
{
  ...
  FillShader::Fill(Point(Screen::Left() + .5 * SIDEBAR_WIDTH, 0.), Point(SIDEBAR_WIDTH, Screen::Height()), sideColor);
...
bool LogbookPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  if(key == 'd' || key == SDLK_ESCAPE || (key == 'w' && (mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI))))
  {
    GetUI().Pop(this);
  }
  ...
  else if(key == SDLK_UP || key == SDLK_DOWN)
  {
    ...
    for(; i < contents.size(); ++i)
      if(contents[i] == selectedName) break;
    if(i == contents.size()) return true;
...
{
  if((hoverPoint.X() - Screen::Left()) > SIDEBAR_WIDTH) scroll = max(0., min(maxScroll, scroll - dy));
  else categoryScroll = max(0., min(maxCategoryScroll, categoryScroll - dy));

  return true;
}


bool LogbookPanel::Scroll(double dx, double dy) { return Drag(0., dy * Preferences::ScrollSpeed()); }
