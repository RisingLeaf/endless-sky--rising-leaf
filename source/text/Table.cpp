...
void Table::SetRowHeight(int height) noexcept { rowSize.Y() = height; }
...
void Table::SetColor(const Color &color) const { this->color = color; }
...
void Table::Draw(const char *text) const { Draw(text, nullptr, color); }


void Table::Draw(const std::string &text) const { Draw(text, nullptr, color); }


// If a color is given, this field is drawn using that color, but the
// previously set color will be used for future fields.
void Table::Draw(const char *text, const Color &color) const { Draw(text, nullptr, color); }


void Table::Draw(const std::string &text, const Color &color) const { Draw(text, nullptr, color); }


void Table::Draw(double value) const { Draw(Format::Number(value), nullptr, color); }


void Table::Draw(double value, const Color &color) const { Draw(Format::Number(value), nullptr, color); }


void Table::DrawCustom(const DisplayText &text) const { Draw(text.GetText(), &text.GetLayout(), color); }
...
void Table::DrawTruncatedPair(
    const std::string &left,
    const Color       &leftColor,
    const std::string &right,
    const Color       &rightColor,
    Truncate           strategy,
    bool               truncateRightColumn) const
{
  ...
  if(truncateRightColumn) lhs.truncate = Truncate::NONE;
  else rhs.truncate = Truncate::NONE;
...
void Table::DrawUnderline() const { DrawUnderline(color); }
...
void Table::DrawHighlight() const { DrawHighlight(color); }


void Table::DrawHighlight(const Color &color) const { FillShader::Fill(GetRowBounds(), color); }
...
void Table::DrawGap(int y) const { point.Y() += y; }


// Get the point that should be passed to DrawAt() to start the next row at
// the given location.
Point Table::GetPoint() const { return point - Point(0., (rowSize.Y() - font->Height()) / 2); }
...
Point Table::GetCenterPoint() const { return point + center; }


Point Table::GetRowSize() const { return rowSize; }


Rectangle Table::GetRowBounds() const { return Rectangle(GetCenterPoint(), GetRowSize()); }
