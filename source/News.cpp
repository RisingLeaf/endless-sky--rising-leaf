  ...
  for(const DataNode &child : node)
  {
    ...
    if(tag == "location")
    {
      ...
      else if(remove)
      {
        ...
        if(child.HasChildren()) child.PrintTrace("Removing full location filter; partial removal is not supported:");
      }
      else {
    ...
    else if(tag == "name")
    {
      ...
      {
        speakerNames = Phrase{};
        if(child.HasChildren()) child.PrintTrace("Removing all names; removal of individual names is not supported:");
      }
      else {
    ...
    else if(tag == "message")
    {
      ...
      {
        messages = Phrase{};
        if(child.HasChildren()) child.PrintTrace("Removing all messages; removal of single messages is not supported:");
      }
      else {
        messages.Load(child);
      }
...
bool News::IsEmpty() const { return messages.IsEmpty() || speakerNames.IsEmpty(); }
...
string News::SpeakerName() const { return speakerNames.Get(); }


// Pick a portrait at random out of the possible options.
const Sprite *News::Portrait() const { return portraits.empty() ? nullptr : portraits[Random::Int(portraits.size())]; }


// Get the speaker's message, chosen randomly.
string News::Message() const { return messages.Get(); }
