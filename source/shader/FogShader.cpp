...
void FogShader::Init()
{
  ...
  constexpr float vertexData[] = {0.f, 0.f, 0.f, 1.f, 1.f, 0.f, 1.f, 1.f};

  square = graphics_layer::ObjectHandle(GameWindow::GetInstance(), 4, 2 * sizeof(float), vertexData, {}, "fog_quad");
}
...
void FogShader::Redraw() { previousZoom = 0.; }
...
void FogShader::Draw(const Point &center, double zoom, const PlayerInfo &player)
{
  ...
  const double left    = Screen::Left() - GRID * PAD * zoom + fmod(center.X(), GRID) * zoom;
  const double top     = Screen::Top() - GRID * PAD * zoom + fmod(center.Y(), GRID) * zoom;
  int          columns = static_cast<int>(std::ceil(Screen::Width() / (GRID * zoom))) + 1 + 2 * PAD;
  const int    rows    = static_cast<int>(std::ceil(Screen::Height() / (GRID * zoom))) + 1 + 2 * PAD;
  ...
  if(shouldRegenerate)
  {
    ...
    for(const auto &it : GameData::Systems())
    {
      ...
      if(!system.IsValid() || !player.CanView(system)) continue;
      Point pos = zoom * (system.Position() + center);

      const int x = static_cast<int>(std::round(pos.X() - left) / (GRID * zoom));
      const int y = static_cast<int>(std::round(pos.Y() - top) / (GRID * zoom));
      if(x >= 0 && y >= 0 && x < columns && y < rows) buffer[x + y * columns] = 0;
    }
    ...
    for(int y = 1; y < rows; ++y)
    {
      ...
      {
        buffer[x + y * columns] = std::min<int>(
            buffer[x + y * columns],
            std::min(
