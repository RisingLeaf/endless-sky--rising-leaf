...
ShipInfoPanel::ShipInfoPanel(PlayerInfo &player, InfoPanelState state) : player(player), panelState(std::move(state))
{
...
ShipInfoPanel::~ShipInfoPanel() { Audio::Resume(); }


void ShipInfoPanel::Step() { DoHelp("ship info"); }
...
void ShipInfoPanel::Draw()
{
  ...
  {
    if(!(*shipIt)->IsDisabled()) interfaceInfo.SetCondition("can park");
  ...
  else if(!panelState.CanEdit())
  {
    ...
    if(CanDump()) interfaceInfo.SetCondition("enable dump");
  }
  if(player.Ships().size() > 1) interfaceInfo.SetCondition("five buttons");
  else interfaceInfo.SetCondition("three buttons");
  ...
  ClearZones();
  if(shipIt == panelState.Ships().end()) return;
...
bool ShipInfoPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool /* isNewPress */)
{
  ...
  {
    if(shipIt == panelState.Ships().begin()) shipIt = panelState.Ships().end();
    --shipIt;
    UpdateInfo();
  }
  ...
  {
    ++shipIt;
    if(shipIt == panelState.Ships().end()) shipIt = panelState.Ships().begin();
    UpdateInfo();
  }
    ...
    if(shipIt->get() != player.Flagship())
    {
      ...
      auto                          AddToUniques = [&uniqueOutfits](const std::map<const Outfit *, int> &outfits)
      {
        for(const auto &it : outfits)
          if(it.first->Get("unique")) uniqueOutfits[it.first] += it.second;
      };
      AddToUniques(shipIt->get()->Outfits());
      AddToUniques(shipIt->get()->Cargo().Outfits());

      std::string message = "Are you sure you want to disown \"" + shipIt->get()->GivenName() +
  ...
  else if(key == 'c' && CanDump())
  {
    ...
    {
      GetUI().Push(
          DialogPanel::RequestPositiveInteger(
              this,
              &ShipInfoPanel::DumpCommodities,
    ...
    else if(plunderAmount > 0 && selectedPlunder->Get("installable") < 0.)
    {
      GetUI().Push(
          DialogPanel::RequestPositiveInteger(
              this,
              &ShipInfoPanel::DumpPlunder,
              "How many tons of " + Format::LowerCase(selectedPlunder->DisplayName()) + " do you want to jettison?",
              plunderAmount));
    }
...
bool ShipInfoPanel::Click(int x, int y, MouseButton button, int /* clicks */)
{
  ...
  if(shipIt == panelState.Ships().end()) return true;
  ...
  for(const auto &zone : commodityZones)
    if(zone.Contains(point)) selectedCommodity = zone.Value();
  for(const auto &zone : plunderZones)
    if(zone.Contains(point)) selectedPlunder = zone.Value();
...
bool ShipInfoPanel::Drag(double dx, double dy) { return Hover(hoverPoint + Point(dx, dy)); }
...
void ShipInfoPanel::DrawOutfits(const Rectangle &bounds, Rectangle &cargoBounds)
{
  ...
  for(const auto &cat : GameData::GetCategory(CategoryType::OUTFIT))
  {
    ...
    if(it == outfits.end()) continue;

    auto validOutfits = std::ranges::filter_view(
        it->second,
        [](const Outfit *outfit) { return outfit->IsDefined() && !outfit->DisplayName().empty(); });

    if(validOutfits.empty()) continue;
    ...
    {
      start += Point(WIDTH, 0.);
      if(start.X() + COLUMN_WIDTH > bounds.Right()) break;
      table.DrawAt(start);
    }
    ...
    for(const Outfit *outfit : validOutfits)
    {
      ...
      table.DrawTruncatedPair(
          outfit->DisplayName(),
          dim,
          std::to_string(ship.OutfitCount(outfit)),
          bright,
          Truncate::BACK,
          false);
    }
...
void ShipInfoPanel::DrawWeapons(const Rectangle &bounds)
{
  ...
  double        scale  = 0.;
  if(sprite) scale = std::min(1., std::min((WIDTH - 10) / sprite->Width(), (WIDTH - 10) / sprite->Height()));
  ...
  if(maxX > (LABEL_DX - LABEL_PAD)) scale = std::min(scale, (LABEL_DX - LABEL_PAD) / (2. * maxX));
  ...
  for(const Hardpoint &hardpoint : ship.Weapons())
  {
    ...
    if(hardpoint.GetOutfit()) name = hardpoint.GetOutfit()->DisplayName();
    ...
    Color color;
    if(isTurret)
      color = *GameData::Colors().Get(isHover ? "player info hardpoint turret hover" : "player info hardpoint turret");
    else color = *GameData::Colors().Get(isHover ? "player info hardpoint gun hover" : "player info hardpoint gun");
  ...
  if(hasTop) DrawLine(topFrom, topTo, topColor);
...
void ShipInfoPanel::DrawCargo(const Rectangle &bounds)
{
  ...
  if(cargo.CommoditiesSize() && hasSpace)
  {
    for(const auto &it : cargo.Commodities())
    {
      ...
      if(it.first == selectedCommodity) table.DrawHighlight(backColor);
  ...
  if(cargo.HasOutfits() && hasSpace)
  {
    for(const auto &it : cargo.Outfits())
    {
      ...
      if(it.first == selectedPlunder) table.DrawHighlight(backColor);
      ...
      if(!isSingular) name += " (" + std::to_string(it.second) + "x)";
      table.Draw(name, dim);
  ...
  if(cargo.HasMissionCargo() && hasSpace)
  {
    for(const auto &it : cargo.MissionCargo())
    {
      ...
      // Truncate the list if there is not enough space.
      if(table.GetRowBounds().Bottom() >= endY) break;
    }
    table.DrawGap(10.);
  }
...
bool ShipInfoPanel::Hover(const Point &point)
{
  ...
  for(const auto &zone : zones)
  {
    ...
    if(zone.Contains(hoverPoint) && (draggingIndex == -1 || isTurret == dragIsTurret)) hoverIndex = zone.Value();
  }
...
bool ShipInfoPanel::Rename(const std::string &name)
{
...
{
  if(panelState.CanEdit() || shipIt == panelState.Ships().end()) return false;
...
void ShipInfoPanel::Dump()
{
  ...
  {
    Messages::Add(
        {"You jettisoned " + Format::CreditString(loss) + " worth of cargo.",
...
void ShipInfoPanel::Disown()
{
  ...
  if(shipIt == panelState.Ships().end() || shipIt->get() == player.Flagship()) return;

  const auto ship = shipIt;
  if(shipIt != panelState.Ships().begin()) --shipIt;
