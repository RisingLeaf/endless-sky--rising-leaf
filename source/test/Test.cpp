...
namespace
{
  ...
  bool KeyInputToEvent(const char *keyName, Uint16 modKeys)
  {
    ...
    if(event.key.key == SDLK_UNKNOWN) return false;
  ...
  for(const DataNode &child : node)
  {
    ...
    if(key == "key")
    {
      ...
      for(const DataNode &grand : child)
      {
        ...
        if(grandKey == "shift") modKeys |= SDL_KMOD_SHIFT;
        else if(grandKey == "alt") modKeys |= SDL_KMOD_ALT;
        else if(grandKey == "control") modKeys |= SDL_KMOD_CTRL;
    ...
    else if(key == "pointer")
    {
      for(const DataNode &grand : child)
      {
        ...
        if(grandKey == "X")
        {
          if(grand.Size() < 2) grand.PrintTrace(BAD_AXIS_INPUT);
          else XValue = grand.Value(1);
        }
        else if(grandKey == "Y")
        {
          if(grand.Size() < 2) grand.PrintTrace(BAD_AXIS_INPUT);
          else YValue = grand.Value(1);
        }
        else if(grandKey == "click")
        {
          for(int i = 1; i < grand.Size(); ++i)
            if(grand.Token(i) == "left") clickLeft = true;
            else if(grand.Token(i) == "right") clickRight = true;
            else if(grand.Token(i) == "middle") clickMiddle = true;
            else grand.PrintTrace("Warning: Unknown click/button \"" + grand.Token(i) + "\":");
        }
        else {
...
void Test::LoadSequence(const DataNode &node, const ConditionsStore *playerConditions)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    auto               it       = find_if(
        STEPTYPE_TO_TEXT.begin(),
        STEPTYPE_TO_TEXT.end(),
        [&typeName](const std::pair<TestStep::Type, const std::string> &e) { return e.second == typeName; });
    ...
    {
    case TestStep::Type::APPLY:  step.assignConditions.Load(child, playerConditions); break;
    case TestStep::Type::ASSERT: step.checkConditions.Load(child, playerConditions); break;
    case TestStep::Type::BRANCH:
      ...
      if(child.Size() > 2) step.jumpOnFalseTarget = child.Token(2);
    ...
    case TestStep::Type::INJECT:
      ...
      break;
    case TestStep::Type::INPUT: step.LoadInput(child); break;
...
void Test::Load(const DataNode &node, const ConditionsStore *playerConditions)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    if(key == "status" && child.Size() >= 2)
    {
      ...
      auto               it         = find_if(
          STATUS_TO_TEXT.begin(),
          STATUS_TO_TEXT.end(),
          [&statusText](const std::pair<Status, const std::string> &e) { return e.second == statusText; });
      if(it != STATUS_TO_TEXT.end())
      {
        ...
        if(status != Status::BROKEN) status = it->first;
      }
      else {
        status = Status::BROKEN;
...
const std::string &Test::Name() const { return name; }
...
void Test::Step(TestContext &context, PlayerInfo &player, Command &commandToGive) const
{
  ...
  if(status == Status::BROKEN) Fail(context, player, "Test has a broken status.");
  ...
  if(context.callstack.back().step >= steps.size())
  {
    ...
    if(context.callstack.empty())
    {
      ...
      if(status >= Status::KNOWN_FAILURE) UnexpectedSuccessResult();
  ...
  while(context.callstack.back().step < steps.size() && !continueGameLoop)
  {
    ...
    case TestStep::Type::ASSERT:
      if(!stepToRun.checkConditions.Test()) Fail(context, player, "asserted false");
      ++(context.callstack.back().step);
      break;
    case TestStep::Type::BRANCH:
      ...
      else ++(context.callstack.back().step);
      break;
    case TestStep::Type::CALL:
      {
        auto calledTest = GameData::Tests().Find(stepToRun.nameOrLabel);
        if(nullptr == calledTest) Fail(context, player, "Calling non-existing test \"" + stepToRun.nameOrLabel + "\"");
    ...
    case TestStep::Type::INPUT:
      if(stepToRun.command) commandToGive |= stepToRun.command;
      ...
      ++(context.callstack.back().step);
      break;
    case TestStep::Type::LABEL: ++(context.callstack.back().step); break;
    case TestStep::Type::NAVIGATE:
      ...
      ++(context.callstack.back().step);
      break;
    default: Fail(context, player, "Unknown step type"); break;
    }
  }
}


const std::string &Test::StatusText() const { return STATUS_TO_TEXT.at(status); }
...
void Test::Fail(const TestContext &context, const PlayerInfo &player, const std::string &testFailReason) const
{
  ...
  if(!testFailReason.empty()) message += ": " + testFailReason;
  message += "\n";
  ...
  if(context.callstack.empty()) stackMessage += "  No callstack info at moment of failure.";
  ...
  if(!flagship)
  {
    Logger::Log("No flagship at the moment of failure.", Logger::Level::ERROR);
  }
  else {
    ...
    for(auto &&ptr : flagship->GetEscorts())
    {
      ...
      if(!escort) continue;
      if(++escorts <= 5) shipsOverview += "escort " + ShipToString(*escort) + "\n";
      else ++escortsNotPrinted;
    }
    if(escortsNotPrinted > 0) shipsOverview += "(plus " + std::to_string(escortsNotPrinted) + " additional escorts)\n";
  ...
  if(!conditions.empty()) Logger::Log(conditions, Logger::Level::INFO);
  else Logger::Log("No conditions to display at the moment of failure.", Logger::Level::INFO);
  ...
  if(status >= Status::KNOWN_FAILURE) throw known_failure_tag{};
...
void Test::UnexpectedSuccessResult() const
{
  throw std::runtime_error(
      "Unexpected test result: Test marked with status '" + StatusText() +
