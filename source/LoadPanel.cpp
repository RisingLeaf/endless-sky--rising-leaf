...
void LoadPanel::Draw()
{
  ...
  if(loadedInfo.IsLoaded())
  {
    ...
    if(!loadedInfo.GetSystem().empty()) info.SetString("system", loadedInfo.GetSystem());
    if(!loadedInfo.GetPlanet().empty()) info.SetString("planet", loadedInfo.GetPlanet());
  ...
  if(!selectedPilot.empty()) info.SetCondition("pilot selected");
  if(!player.IsDead() && player.IsLoaded() && !selectedPilot.empty()) info.SetCondition("pilot alive");
  if(selectedFile.find('~') != string::npos) info.SetCondition("snapshot selected");
  if(loadedInfo.IsLoaded()) info.SetCondition("pilot loaded");
  ...
  // Draw the list of pilots.
  {
    ...
    for(const auto &it : files)
    {
      ...
      if(drawPoint.Y() < top - fadeOut) continue;
      if(drawPoint.Y() > bottom - fadeOut) continue;
      ...
      double alpha = min((drawPoint.Y() - (top - fadeOut)) * .1, (bottom - fadeOut - drawPoint.Y()) * .1);
      alpha        = max(alpha, 0.);
      alpha        = min(alpha, 1.);

      if(it.first == selectedPilot) FillShader::Fill(zone, Color(.1 * alpha, 0.));
  ...
  if(!selectedPilot.empty() && files.contains(selectedPilot))
  {
    ...
    for(const auto &it : files.find(selectedPilot)->second)
    {
      ...
      if(file == selectedFile) FillShader::Fill(zone, Color(.1 * alpha, 0.));
  ...
  if(!hasHoverZone) tooltip.DecrementCount();
  else tooltip.Draw();
}
...
bool LoadPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  else if(key == 'd' && !selectedPilot.empty())
  {
    ...
    GetUI().Push(
        DialogPanel::RequestStringWithValidation(
            this,
            &LoadPanel::DeletePilot,
  ...
  else if(key == 'a' && !player.IsDead() && player.IsLoaded())
  {
    ...
    if(it == files.end() || it->second.empty() || it->second.front().first.size() < 4) return false;

    sound = UI::UISound::NONE;
    nameToConfirm.clear();
    filesystem::path lastSave = Files::Saves() / it->second.front().first;
    GetUI().Push(
        DialogPanel::RequestString(
            this,
            &LoadPanel::SnapshotCallback,
  ...
  else if((key == SDLK_DOWN || key == SDLK_UP) && !files.empty())
  {
    ...
    if(sideHasFocus)
    {
      ...
      for(; it != files.end(); ++it, ++index)
        if(it->first == selectedPilot) break;

      if(key == SDLK_DOWN)
      {
        const int lastVisibleIndex = (sideScroll / 20.) + 13.;
        if(index >= lastVisibleIndex) sideScroll += 20.;
        ++it;
        if(it == files.end())
        {
          it         = files.begin();
        ...
        if(index <= firstVisibleIndex) sideScroll -= 20.;
        if(it == files.begin())
        {
          it         = files.end();
    ...
    else if(pit != files.end())
    {
      ...
      for(; it != pit->second.end(); ++it, ++index)
        if(it->first == selectedFile) break;

      if(key == SDLK_DOWN)
      {
        ++it;
        const int lastVisibleIndex = (centerScroll / 20.) + 13.;
        if(index >= lastVisibleIndex) centerScroll += 20.;
        ...
        if(index <= firstVisibleIndex) centerScroll -= 20.;
...
bool LoadPanel::Click(int x, int y, MouseButton button, int clicks)
{
  ...
  else if(snapshotBox.Contains(click))
  {
    ...
    if(filesIt == files.end()) return true;
    for(const auto &it : filesIt->second)
    {
      if(i++ == selected)
      {
        ...
        selectedFile            = it.first;
        if(sameSelected && clicks > 1) KeyDown('l', 0, Command(), true);
        break;
      }
    }
  }
  ...
  if(!selectedFile.empty()) loadedInfo.Load(Files::Saves() / selectedFile);
...
{
  if(x >= pilotBox.Left() && x < pilotBox.Right()) sideHasFocus = true;
  else if(x >= snapshotBox.Left() && x < snapshotBox.Right()) sideHasFocus = false;
  ...
  if(!tooltip.ShouldDraw()) tooltip.ResetCount();
...
{
  auto it = files.find(selectedPilot);
  if(sideHasFocus) sideScroll = max(0., min(20. * files.size() - 280., sideScroll - dy));
...
bool LoadPanel::Scroll(double dx, double dy) { return Drag(0., dy * Preferences::ScrollSpeed()); }
...
void LoadPanel::UpdateLists()
{
  ...
  for(const auto &path : fileList)
  {
    ...
    if(!isSnapshot) pos = fileName.size() - 4;
    ...
    if(!isSnapshot) swap(savesList.front(), savesList.back());
  }
  ...
  for(auto &it : files)
  {
    ...
    if(start->first.find('~') == string::npos) ++start;
    sort(
        start,
        it.second.end(),
  ...
  if(!files.empty())
  {
    if(selectedPilot.empty()) selectedPilot = files.begin()->first;
...
{
  auto it = files.find(selectedPilot);
  if(it == files.end() || it->second.empty() || it->second.front().first.size() < 4) return;
...
{
  loadedInfo.Clear();
  if(selectedPilot == player.Identifier()) player.Clear();
  auto it = files.find(selectedPilot);
  ...
  if(failed) GetUI().Push(DialogPanel::Info("Deleting pilot files failed."));
...
void LoadPanel::DeleteSave()
{
  ...
  Files::Delete(path);
  if(Files::Exists(path)) GetUI().Push(DialogPanel::Info("Deleting snapshot file failed."));
