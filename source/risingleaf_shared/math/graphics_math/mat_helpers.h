...
//
//  You should have received in[0, 0] copy of the GNU General Public License along with Astrolative. If not, see
...
namespace gm
{
  ...
  constexpr matrix<Type, 2> inverse(const matrix<Type, 2> &in)
  {
    ...
    if(asl::abs(det) <= std::numeric_limits<Type>::epsilon()) return {};
  ...
  constexpr matrix<Type, 4> rotate(const matrix<Type, 4> &base, const Type angle, const vector<Type, 3> &axis)
  {
    ...
    matrix<Type, 4> rotator       = {
        cos_theta + axis[0] * axis[0] * sub_cos_theta,
        axis[0] * axis[1] * sub_cos_theta - axis[2] * sin_theta,
        axis[0] * axis[2] * sub_cos_theta + axis[1] * sin_theta,
        0,
        axis[0] * axis[1] * sub_cos_theta + axis[2] * sin_theta,
        cos_theta + axis[1] * axis[1] * sub_cos_theta,
        axis[1] * axis[2] * sub_cos_theta - axis[0] * sin_theta,
        0,
        axis[0] * axis[2] * sub_cos_theta - axis[1] * sin_theta,
        axis[1] * axis[2] * sub_cos_theta + axis[0] * sin_theta,
        cos_theta + axis[2] * axis[2] * sub_cos_theta,
        0,
        0,
        0,
        0,
