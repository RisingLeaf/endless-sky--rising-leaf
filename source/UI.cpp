...
bool UI::Handle(const SDL_Event &event)
{
  ...
  while(it != stack.begin() && !handled)
  {
    ...
    if(count(toPop.begin(), toPop.end(), it->get())) continue;

    if(event.type == SDL_EVENT_MOUSE_MOTION)
    {
      if(event.motion.state & SDL_BUTTON_MASK(1))
      {
        handled = (*it)->DoDrag(event.motion.xrel * 100. / Screen::Zoom(), event.motion.yrel * 100. / Screen::Zoom());
      }
      else {
        handled = (*it)->DoHover(
    ...
    else if(event.type == SDL_EVENT_MOUSE_BUTTON_DOWN)
    {
      ...
      if(event.button.button == SDL_BUTTON_LEFT) handled = (*it)->ZoneClick(Point(x, y));
      if(!handled) handled = (*it)->DoClick(x, y, static_cast<MouseButton>(event.button.button), event.button.clicks);
    }
...
TaskQueue &UI::SyncQueue() { return syncQueue; }


TaskQueue &UI::AsyncQueue() { return asyncQueue; }


const std::vector<std::shared_ptr<Panel>> &UI::Stack() const { return stack; }


// Add the given panel to the stack. UI is responsible for deleting it.
void UI::Push(Panel *panel) { Push(std::shared_ptr<Panel>(panel)); }
...
void UI::Pop(const Panel *panel) { toPop.push_back(panel); }
...
// considered.
bool UI::IsTop(const Panel *panel) const { return !stack.empty() && stack.back().get() == panel; }
...
{
  if(!toPush.empty()) return toPush.back();
...
std::shared_ptr<Panel> UI::Root() const
{
  if(stack.empty())
  {
    if(toPush.empty()) return std::shared_ptr<Panel>();
...
void UI::CanSave(bool canSave) { this->canSave = canSave; }


bool UI::CanSave() const { return canSave; }


// Tell the UI to quit.
void UI::Quit() { isDone = true; }


// Check if it is time to quit.
bool UI::IsDone() const { return isDone; }
...
// game.
bool UI::IsEmpty() const { return stack.empty() && toPush.empty(); }
...
Point UI::GetMouse()
{
  ...
  return Screen::TopLeft() + Point(x, y) * (100.f / Screen::Zoom());
}
...
void UI::PlaySound(UISound sound)
{
  ...
  switch(sound)
  {
  case UISound::NORMAL:    name = "ui/click"; break;
  case UISound::SOFT:      name = "ui/click soft"; break;
  case UISound::SOFT_BUZZ: name = "ui/buzz soft"; break;
  case UISound::TARGET:    name = "ui/target"; break;
  case UISound::FAILURE:   name = "ui/fail"; break;
  default:                 return;
  }
...
void UI::PushOrPop()
{
  ...
  for(std::shared_ptr<Panel> &panel : toPush)
    if(panel) stack.push_back(panel);
  toPush.clear();
