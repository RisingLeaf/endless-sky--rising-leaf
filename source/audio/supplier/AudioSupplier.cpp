...
void AudioSupplier::DestroyBuffer(ALuint buffer) { alDeleteBuffers(1, &buffer); }


AudioSupplier::AudioSupplier(bool is3x, bool isLooping) : is3x(is3x), nextPlaybackIs3x(is3x), isLooping(isLooping) {}


void AudioSupplier::Set3x(bool is3x) { nextPlaybackIs3x = is3x; }
...
void AudioSupplier::NextChunk(ALuint buffer, bool spatial)
{
  if(AvailableChunks())
  {
    ...
    alBufferData(
        buffer,
        spatial ? FORMAT_SPATIAL : FORMAT,
        samples.data(),
        sizeof(sample_t) * samples.size(),
        SAMPLE_RATE);
  }
  else {
