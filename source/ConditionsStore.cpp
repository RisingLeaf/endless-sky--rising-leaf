...
ConditionsStore::ConditionsStore(const DataNode &node) { Load(node); }
...
void ConditionsStore::Load(const DataNode &node)
{
  for(const DataNode &child : node)
  {
    ...
    if(!DataNode::IsConditionName(key)) child.PrintTrace("Invalid condition during savegame-load:");
...
void ConditionsStore::Save(DataWriter &out) const
{
  ...
  for(const auto &it : storage)
  {
    ...
    if(it.second.getFunction || it.second.providingEntry) continue;
    ...
    if(it.second.value == 1) out.Write(it.first);
    else out.Write(it.first, it.second.value);
  }
  out.EndChild();
}
...
void ConditionsStore::Add(const std::string &name, int64_t value) { (*this)[name] += value; }


void ConditionsStore::Set(const std::string &name, int64_t value) { (*this)[name] = value; }
...
ConditionEntry &ConditionsStore::operator[](const std::string &name)
{
  ...
  if(ceprov != nullptr) it->second.providingEntry = ceprov;
...
int64_t ConditionsStore::PrimariesSize() const
{
  ...
  for(const auto &it : storage)
  {
    ...
    if(it.second.providingEntry || it.second.getFunction) continue;
    ++result;
  }
  return result;
}
...
const ConditionEntry *ConditionsStore::GetEntry(const std::string &name) const
{
  ...
  if(it == storage.begin()) return nullptr;
  ...
  if(ceProv && name.starts_with(ceProv->name)) return ceProv;
