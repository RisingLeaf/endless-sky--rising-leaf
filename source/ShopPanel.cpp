...
namespace
{
  ...
  bool CanShowInSidebar(const Ship &ship, const Planet *here) { return ship.GetPlanet() == here; }

  constexpr auto ScrollbarMaybeUpdate =
      [](const auto &op, ScrollBar &scrollbar, ScrollVar<double> &scroll, bool animate)
  {
    if(!op(scrollbar)) return false;
...
ShopPanel::ShopPanel(PlayerInfo &player, bool isOutfitter) :
  player(player),
  day(player.GetDate().DaysSinceEpoch()),
  ...
  creditsTooltip(
      250,
      Alignment::LEFT,
      Tooltip::Direction::UP_LEFT,
      Tooltip::Corner::TOP_RIGHT,
      GameData::Colors().Get("tooltip background"),
      GameData::Colors().Get("medium")),
  buttonsTooltip(
      250,
...
{
  if(playerShip) playerShips.insert(playerShip);
...
void ShopPanel::Step()
{
  ...
  if(!checkedHelp && GetUI().IsTop(this) && player.Ships().size() > 1)
  {
    ...
    else if(!Preferences::Has("help: shop with multiple ships"))
    {
      ...
      {
        if(!CanShowInSidebar(*it, player.GetPlanet())) continue;
...
void ShopPanel::Draw()
{
  ...
  static const std::string FIND = "_Find";
  ...
  if(!shipName.empty())
  {
    std::string text = shipName;
    if(!warningType.empty()) text += "\n" + GameData::Tooltip(warningType);
    shipsTooltip.SetText(text, true);
    shipsTooltip.SetBackgroundColor(
        GameData::Colors().Get(
            warningType.empty() ? "tooltip background" : (warningType.back() == '!' ? "error back" : "warning back")));
  ...
  if(dragShip && isDraggingShip && dragShip->GetSprite())
  {
    ...
    float         scale  = ICON_SIZE / std::max(sprite->Width(), sprite->Height());
    if(Preferences::Has(SHIP_OUTLINES))
    {
      ...
      const Swizzle *swizzle =
          dragShip->CustomSwizzle() ? dragShip->CustomSwizzle() : GameData::PlayerGovernment()->GetSwizzle();
  ...
  if(delayedAutoScroll)
  {
    ...
    if(selected != zones.end()) MainAutoScroll(selected);
  }
}
...
void ShopPanel::DrawShip(const Ship &ship, const Point &center, bool isSelected)
{
  ...
  if(thumbnail)
  {
    if(thumbnail->IsLoaded()) SpriteShader::Draw(thumbnail, center + Point(0., 10.), 1., swizzle);
    else loadingCircle.Draw(center);
  }
  else if(sprite)
  {
    // Make sure the ship sprite leaves 10 pixels padding all around.
    const float zoomSize = SHIP_SIZE - 60.f;
    float       zoom     = std::min(1.f, zoomSize / std::max(sprite->Width(), sprite->Height()));
    SpriteShader::Draw(sprite, center, zoom, swizzle);
  }

  // Draw the ship name.
  const Font        &font = FontSet::Get(14);
  const std::string &name = ship.GivenName().empty() ? ship.DisplayModelName() : ship.GivenName();
  Point              offset(-SIDEBAR_CONTENT / 2, -.5f * SHIP_SIZE + 10.f);
  font.Draw(
      {name, {SIDEBAR_CONTENT, Alignment::CENTER, Truncate::MIDDLE}},
      center + offset,
...
{
  if(!GetUI().IsTop(this)) return;

  Mission *mission = player.MissionToOffer(location);
  if(mission) mission->Do(Mission::OFFER, player, &GetUI());
  else player.HandleBlockedMissions(location, GetUI());
}
...
void ShopPanel::ValidateSelectedShips()
{
  ...
  std::set<Ship *> shipPtrs;
  for(const std::shared_ptr<Ship> &ship : player.Ships())
    shipPtrs.insert(ship.get());
  std::set<Ship *> stillValid;
  std::ranges::set_intersection(shipPtrs, playerShips, inserter(stillValid, stillValid.begin()));
...
int ShopPanel::VisibilityCheckboxesSize() const { return 0; }


bool ShopPanel::ShouldHighlight(const Ship *ship) { return hoverButton == 's' || hoverButton == 'r'; }


// Only override the ones you need; the default action is to return false.
bool ShopPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  if(key == 'l' || key == 'd' || key == SDLK_ESCAPE || (key == 'w' && (mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI))))
  {
    if(!isOutfitter) player.UpdateCargoCapacities();
    GetUI().Pop(this);
  ...
  else if(command.Has(Command::HELP))
  {
    ...
    {
      if(isOutfitter) DoHelp("outfitter with multiple ships", true);
  ...
  {
    if(isOutfitter) GetUI().Push(new MapOutfitterPanel(player));
    else GetUI().Push(new MapShipyardPanel(player));
  }
  else if(key == SDLK_LEFT)
  {
    if(activePane != ShopPane::Sidebar) MainLeft();
    else SideSelect(-1);
    return true;
  }
  else if(key == SDLK_RIGHT)
  {
    if(activePane != ShopPane::Sidebar) MainRight();
    else SideSelect(1);
    return true;
  }
  else if(key == SDLK_UP)
  {
    if(activePane != ShopPane::Sidebar) MainUp();
    else SideSelect(-4);
    return true;
  }
  else if(key == SDLK_DOWN)
  {
    if(activePane != ShopPane::Sidebar) MainDown();
    else SideSelect(4);
    return true;
  }
  ...
  else if(key == 'k' || (key == 'p' && (mod & SDL_KMOD_SHIFT)))
  {
    const Ship *flagship          = player.Flagship();
    bool        anyEscortUnparked = any_of(
        playerShips.begin(),
        playerShips.end(),
        [&](const Ship *ship) { return ship != flagship && !ship->IsParked() && !ship->IsDisabled(); });
    for(const Ship *ship : playerShips)
      if(ship != flagship) player.ParkShip(ship, anyEscortUnparked);
  }
  else if(key >= '0' && key <= '9')
  {
    int group = key - '0';
    ...
    else if(mod & SDL_KMOD_SHIFT)
    {
      ...
      std::set<Ship *> added           = player.GetEscortGroup(group);
      bool        allWereSelected = true;
      for(Ship *ship : added)
        allWereSelected &= playerShips.erase(ship);

      if(allWereSelected) added.clear();

      const Planet *here = player.GetPlanet();
      for(Ship *ship : added)
        if(CanShowInSidebar(*ship, here)) playerShips.insert(ship);

      if(!playerShips.contains(playerShip)) playerShip = playerShips.empty() ? nullptr : *playerShips.begin();
    }
    else {
      // Change the selection to the desired ships, if they are landed here.
      playerShips.clear();
      std::set<Ship *> wanted = player.GetEscortGroup(group);
...
bool ShopPanel::Click(int x, int y, MouseButton button, int clicks)
{
  ...
  {
    return ScrollbarMaybeUpdate(
        [x, y, button, clicks](ScrollBar &scrollbar) { return scrollbar.Click(x, y, button, clicks); },
        scrollbar,
        scroll,
        true);
  };
  if(ScrollbarClick(mainScrollbar, mainScroll) || ScrollbarClick(sidebarScrollbar, sidebarScroll) ||
  ...
  if(zoneButton) return DoKey(zoneButton);

  const Point clickPoint(x, y);

  // Check for clicks in the category labels.
  for(const ClickZone<std::string> &zone : categoryZones)
  {
    if(zone.Contains(clickPoint))
    {
      bool toggleAll = (SDL_GetModState() & SDL_KMOD_SHIFT);
      ...
      if(it == collapsed.end())
      {
        ...
        if(toggleAll) collapsed.clear();
        else collapsed.erase(it);
      }
      return true;
    }
  }
  ...
  for(const Zone &zone : zones)
  {
    if(zone.Contains(clickPoint))
    {
      if(zone.GetShip()) selectedShip = zone.GetShip();
      else selectedOutfit = zone.GetOutfit();
...
bool ShopPanel::Hover(int x, int y)
{
  ...
  if(x > Screen::Right() - SIDEBAR_WIDTH) activePane = ShopPane::Sidebar;
  else if(x > Screen::Right() - SIDE_WIDTH) activePane = ShopPane::Info;
  return true;
}
...
bool ShopPanel::Drag(double dx, double dy)
{
  if(dragShip)
  {
      ...
      if(zone.Contains(dragPoint))
      {
        if(zone.Value() != dragShip)
        {
          ...
          for(unsigned i = 0; i < player.Ships().size(); ++i)
          {
            ...
            if(ship == dragShip) dragIndex = i;
            if(ship == zone.Value()) dropIndex = i;
          }
          if(dragIndex >= 0 && dropIndex >= 0) player.ReorderShip(dragIndex, dropIndex);
        }
      }
    }
  }
    ...
    auto scrollbarInterceptSpec = [dx, dy](ScrollBar &scrollbar, ScrollVar<double> &scroll)
    {
      ...
      return ScrollbarMaybeUpdate(
          [dx, dy](ScrollBar &scrollbar) { return scrollbar.Drag(dx, dy); },
          scrollbar,
          scroll,
          false);
    };
    if(!scrollbarInterceptSpec(mainScrollbar, mainScroll) && !scrollbarInterceptSpec(sidebarScrollbar, sidebarScroll) &&
...
bool ShopPanel::Scroll(double dx, double dy) { return DoScroll(dy * 2.5 * Preferences::ScrollSpeed()); }


void ShopPanel::DoFind(const std::string &text)
{
  int index = FindItem(text);
  if(index >= 0 && index < static_cast<int>(zones.size()))
  {
    auto best = next(zones.begin(), index);
    if(best->GetShip()) selectedShip = best->GetShip();
    else selectedOutfit = best->GetOutfit();
...
int64_t ShopPanel::LicenseCost(const Outfit *outfit, bool onlyOwned) const
{
  ...
  if((owned && onlyOwned) || player.Stock(outfit) > 0) return 0;

  const Sale<Outfit> &available = player.GetPlanet()->OutfitterStock();

  int64_t cost = 0;
  for(const std::string &name : outfit->Licenses())
  {
    if(!player.HasLicense(name))
    {
      const Outfit *license = GameData::Outfits().Find(name + " License");
      if(!license || !license->Cost() || !available.Has(license)) return -1;
      cost += license->Cost();
    }
  }
  return cost;
...
ShopPanel::Zone::Zone(Point center, Point size, const Outfit *outfit) : ClickZone(center, size, nullptr), outfit(outfit)
{
}


const Ship *ShopPanel::Zone::GetShip() const { return Value(); }


const Outfit *ShopPanel::Zone::GetOutfit() const { return outfit; }
...
void ShopPanel::DrawShipsSidebar()
{
  ...
  static const std::string YOURS = "Your Ships:";
  ...
  if(shipsHere < 4) point.X() += .5 * ICON_TILE * (4 - shipsHere);
  ...
  for(const std::shared_ptr<Ship> &ship : player.Ships())
  {
    ...
    if(!CanShowInSidebar(*ship, here)) continue;
    ...
    if(isSelected && ShouldHighlight(ship.get())) SpriteShader::Draw(background, point);
    ...
    if(sprite)
    {
      ...
      if(Preferences::Has(SHIP_OUTLINES))
      {
        ...
        const Swizzle *swizzle =
            ship->CustomSwizzle() ? ship->CustomSwizzle() : GameData::PlayerGovernment()->GetSwizzle();
    ...
    {
      const std::string &check = (*checkIt).second.front();
      ...
      if(shipZones.back().Contains(mouse)) warningType = check;
    }

    if(isSelected && playerShips.size() > 1 && ship->OutfitCount(selectedOutfit))
    {
      PointerShader::Draw(
          Point(point.X() - static_cast<int>(ICON_TILE / 3), point.Y()),
          Point(1., 0.),
          14.f,
          12.f,
          0.,
  ...
  sidebarScroll.SetMaxValue(
      std::max(0., point.Y() + sidebarScroll.AnimatedValue() - Screen::Bottom() + Screen::Height()));
...
void ShopPanel::DrawDetailsSidebar()
{
  ...
  FillShader::Fill(Point(Screen::Right() - SIDEBAR_WIDTH - INFOBAR_WIDTH, 0.), Point(1., Screen::Height()), line);
  FillShader::Fill(
      Point(Screen::Right() - SIDEBAR_WIDTH - INFOBAR_WIDTH / 2, 0.),
      Point(INFOBAR_WIDTH - 1., Screen::Height()),
      back);

  Point point(Screen::Right() - SIDE_WIDTH + INFOBAR_WIDTH / 2, Screen::Top() + 10 - infobarScroll.AnimatedValue());

  double heightOffset = DrawDetails(point);

  infobarScroll.SetDisplaySize(Screen::Height());
  infobarScroll.SetMaxValue(
      std::max(0., heightOffset + infobarScroll.AnimatedValue() - Screen::Bottom()) + Screen::Height());
...
void ShopPanel::DrawMain()
{
  ...
  for(const auto &cat : categories)
  {
    ...
    std::map<std::string, std::vector<std::string>>::const_iterator it       = catalog.find(category);
...
{
  if(activePane == ShopPane::Info) infobarScroll.Scroll(-dy, steps);
  else if(activePane == ShopPane::Sidebar) sidebarScroll.Scroll(-dy, steps);
  else mainScroll.Scroll(-dy, steps);
...
{
  if(activePane == ShopPane::Info) infobarScroll = 0.;
  else if(activePane == ShopPane::Sidebar) sidebarScroll = 0.;
  else mainScroll = 0.;
...
{
  if(activePane == ShopPane::Info) infobarScroll = infobarScroll.MaxValue();
  else if(activePane == ShopPane::Sidebar) sidebarScroll = sidebarScroll.MaxValue();
  else mainScroll = mainScroll.MaxValue();
...
void ShopPanel::SideSelect(int count)
{
  ...
  std::vector<std::shared_ptr<Ship>>::const_iterator it = player.Ships().begin();
  for(; it != player.Ships().end(); ++it)
    if((*it).get() == playerShip) break;
  ...
  if(count < 0)
  {
    while(count)
    {
      if(it == player.Ships().begin()) it = player.Ships().end();
      --it;

      if(CanShowInSidebar(**it, here)) ++count;
    }
  }
  else {
    while(count)
    {
      ++it;
      if(it == player.Ships().end()) it = player.Ships().begin();
...
void ShopPanel::SideSelect(Ship *ship, int clicks)
{
  ...
  bool control = (SDL_GetModState() & (SDL_KMOD_CTRL | SDL_KMOD_GUI));

  if(shift)
  {
    bool          on   = false;
    const Planet *here = player.GetPlanet();
    for(const std::shared_ptr<Ship> &other : player.Ships())
    {
      // Skip any ships that are "absent" for whatever reason.
      if(!CanShowInSidebar(*other, here)) continue;

      if(other.get() == ship || other.get() == playerShip) on = !on;
      else if(on) playerShips.insert(other.get());
    }
  }
  else if(!control)
  {
    ...
    if(clicks > 1)
    {
      ...
      {
        if(!CanShowInSidebar(*it, player.GetPlanet())) continue;
        if(it.get() != ship && it->Imitates(*ship)) playerShips.insert(it.get());
      }
    }
  }
  else if(clicks > 1)
  {
    std::vector<Ship *> similarShips;
    ...
    for(Ship *it : similarShips)
      if(unselect) playerShips.erase(it);
      else playerShips.insert(it);
  ...
  else if(playerShips.contains(ship))
  {
    ...
    if(playerShip == ship) playerShip = playerShips.empty() ? nullptr : *playerShips.begin();
    CheckSelection();
    return;
  }
...
void ShopPanel::MainAutoScroll(const std::vector<Zone>::const_iterator &selected)
{
  const int TILE_SIZE = TileSize();
  ...
  if(offTop < 0)
  {
    ...
    if(offBottom > 0) mainScroll += offBottom;
  }
}
...
void ShopPanel::MainLeft()
{
  ...
  std::vector<Zone>::const_iterator it = Selected();
...
void ShopPanel::CheckSelection()
{
  ...
  const std::vector<Zone>::const_iterator oldIt = it;
  ...
  for(; it != zones.end(); ++it)
  {
    ...
    if((ship && HasItem(ship->VariantName())) || (outfit && HasItem(outfit->TrueName()))) break;
  }
...
void ShopPanel::CategoryAdvance(const std::string &category)
{
  std::vector<Zone>::const_iterator it = Selected();
  if(it == zones.end()) return;
  const std::vector<Zone>::const_iterator oldIt = it;
  ...
  for(; it != zones.end(); ++it)
  {
    const Ship   *ship   = it->GetShip();
    const Outfit *outfit = it->GetOutfit();
    if((ship && ship->Attributes().Category() != category) || (outfit && outfit->Category() != category)) break;
  }
...
std::vector<ShopPanel::Zone>::const_iterator ShopPanel::Selected() const
{
  std::vector<Zone>::const_iterator it = zones.begin();
  for(; it != zones.end(); ++it)
    if(it->GetShip() == selectedShip && it->GetOutfit() == selectedOutfit) break;
...
char ShopPanel::CheckButton(int x, int y)
{
  ...
  for(const ClickZone<char> zone : buttonZones)
    if(zone.Contains(clickPoint)) return zone.Value();

  if(x < Screen::Right() - SIDEBAR_WIDTH || y < Screen::Bottom() - ButtonPanelHeight()) return '\0';
