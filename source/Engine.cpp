...
namespace
{
  int RadarType(const Ship &ship, int step)
  {
    if(ship.GetPersonality().IsTarget() && !ship.IsDestroyed())
    {
      ...
      int count = (step / 6) % 7;
      if(count == 0 || count == 2) return Radar::BLINK;
    }
    if(ship.IsDisabled() || (ship.IsOverheated() && ((step / 20) % 2))) return Radar::INACTIVE;
    if(ship.IsYours() || (ship.GetPersonality().IsEscort() && !ship.GetGovernment()->IsEnemy())) return Radar::PLAYER;
    if(!ship.GetGovernment()->IsEnemy()) return Radar::FRIENDLY;
    const auto &target = ship.GetTargetShip();
    if(target && target->IsYours()) return Radar::HOSTILE;
    return Radar::UNFRIENDLY;
  }

  constexpr auto PrunePointers = [](auto &objects)
  { erase_if(objects, [](const auto &obj) { return obj->ShouldBeRemoved(); }); };
  constexpr auto Prune = [](auto &objects)
  { erase_if(objects, [](const auto &obj) { return obj.ShouldBeRemoved(); }); };

  template <class Type>
  void Append(std::vector<Type> &objects, std::vector<Type> &added)
  {
  ...
  void SendMessage(const std::shared_ptr<const Ship> &ship, const std::string &message)
  {
    if(message.empty()) return;

    // If this ship has no name, show its model name instead.
    std::string        tag;
    const std::string &gov = ship->GetGovernment()->DisplayName();
    if(!ship->GivenName().empty()) tag = gov + " " + ship->Noun() + " \"" + ship->GivenName() + "\": ";
    else tag = ship->DisplayModelName() + " (" + gov + "): ";
  ...
  void DrawFlareSprites(
      const Ship                              &ship,
      DrawList                                &draw,
      const std::vector<Ship::EnginePoint>    &enginePoints,
      const std::vector<std::pair<Body, int>> &flareSprites,
      uint8_t                                  side,
      bool                                     reverse)
  {
    ...
    double gimbalDirection =
        (ship.Commands().Has(Command::FORWARD) || ship.Commands().Has(Command::BACK)) * -ship.Commands().Turn();
    ...
    for(const Ship::EnginePoint &point : enginePoints)
    {
      ...
      auto  DrawFlares = [&draw, &pos, &ship, &flareAngle, &point](const std::pair<Body, int> &it, const Point &scale)
      {
      ...
      for(const auto &it : flareSprites)
      {
        if(point.side == side)
        {
          if(point.steering == Ship::EnginePoint::NONE) DrawFlares(it, thrustScale);
          else if(point.steering == Ship::EnginePoint::LEFT && leftTurnScale) DrawFlares(it, leftTurnScale);
          else if(point.steering == Ship::EnginePoint::RIGHT && rightTurnScale) DrawFlares(it, rightTurnScale);
        }
      }
    }
  }
  ...
  {
    if(type == Radar::PLAYER) return *GameData::Colors().Get("ship target outline player");
    else if(type == Radar::FRIENDLY) return *GameData::Colors().Get("ship target outline friendly");
    else if(type == Radar::UNFRIENDLY) return *GameData::Colors().Get("ship target outline unfriendly");
    else if(type == Radar::HOSTILE) return *GameData::Colors().Get("ship target outline hostile");
    else if(type == Radar::SPECIAL) return *GameData::Colors().Get("ship target outline special");
    else if(type == Radar::BLINK) return *GameData::Colors().Get("ship target outline blink");
    else return *GameData::Colors().Get("ship target outline inactive");
  }
  ...
  const Color &GetPlanetTargetPointerColor(const Planet &planet)
  {
    ...
    {
    case Planet::Friendliness::FRIENDLY:   return *GameData::Colors().Get("planet target pointer friendly");
    case Planet::Friendliness::RESTRICTED: return *GameData::Colors().Get("planet target pointer restricted");
    case Planet::Friendliness::HOSTILE:    return *GameData::Colors().Get("planet target pointer hostile");
    case Planet::Friendliness::DOMINATED:  return *GameData::Colors().Get("planet target pointer dominated");
    }
  ...
  {
    if(type == Radar::PLAYER) return *GameData::Colors().Get("ship target pointer player");
    else if(type == Radar::FRIENDLY) return *GameData::Colors().Get("ship target pointer friendly");
    else if(type == Radar::UNFRIENDLY) return *GameData::Colors().Get("ship target pointer unfriendly");
    else if(type == Radar::HOSTILE) return *GameData::Colors().Get("ship target pointer hostile");
    else if(type == Radar::SPECIAL) return *GameData::Colors().Get("ship target pointer special");
    else if(type == Radar::BLINK) return *GameData::Colors().Get("ship target pointer blink");
    else return *GameData::Colors().Get("ship target pointer inactive");
  }

  const Color &GetMinablePointerColor(bool selected)
  {
    if(selected) return *GameData::Colors().Get("minable target pointer selected");
...
Engine::Engine(PlayerInfo &player) :
  player(player),
  ai(player, ships, asteroids.Minables(), flotsam),
  ammoDisplay(player),
  minimap(player),
  shipCollisions(256u, 32u, CollisionType::SHIP)
{
  ...
  if(!player.IsLoaded() || !player.GetSystem()) return;
  ...
  for(const StellarObject &object : player.GetSystem()->Objects())
  {
    ...
    if(object.HasValidPlanet()) SpriteLoadManager::LoadDeferred(queue, object.GetPlanet()->Landscape());
  }
  queue.Wait();
  ...
  if(object) camera.SnapTo(object->Position());
  ...
  for(const StellarObject &object : player.GetSystem()->Objects())
  {
    if(object.HasSprite())
    {
      ...
      double r = std::max(2., object.Radius() * .03 + .5);
  ...
  const std::set<const System *> &links =
...
void Engine::Place()
{
  ...
  for(const std::shared_ptr<Ship> &ship : player.Ships())
    if(!ship->IsParked() && ship->GetSystem()) ships.push_back(ship);
  ...
  std::shared_ptr<Ship> flagship = player.FlagshipPtr();
  ...
  for(const std::shared_ptr<Ship> &ship : ships)
  {
    ...
    if(ship->GetSystem() == system && !ship->IsDisabled())
    {
      ...
      bool launchesWithPlayer = (planet && planet->CanLand(*ship)) && !person.IsStaying() && !person.IsWaiting() &&
                                (!hasOwnPlanet || (ship->IsYours() && ship->GetPlanet() == planet));
      const StellarObject *object = hasOwnPlanet ? ship->GetSystem()->FindStellar(ship->GetPlanet()) : nullptr;
    ...
    else if(!ship->GetSystem())
    {
      // Log this error.
      Logger::Log(
          "Engine::Place: Set fallback system for the NPC \"" + ship->GivenName() + "\" as it had no system.",
  ...
  if(flagship) camera.SnapTo(flagship->Center());
  else if(object) camera.SnapTo(planetPos);
  else camera.SnapTo(camera.Center());
...
void Engine::Place(const std::list<NPC> &npcs, const std::shared_ptr<Ship> &flagship)
{
  for(const NPC &npc : npcs)
  {
    if(!npc.ShouldSpawn()) continue;

    std::map<std::string, std::map<Ship *, int>> carriers;
    for(const std::shared_ptr<Ship> &ship : npc.Ships())
    {
      // Skip ships that have been destroyed.
      if(ship->IsDestroyed() || ship->IsDisabled()) continue;
      ...
      if(ship->HasBays())
      {
        ...
        {
          const std::string &bayType   = cat.Name();
          int                baysTotal = ship->BaysTotal(bayType);
          if(baysTotal) carriers[bayType][&*ship] = baysTotal;
        }
      }
    }

    std::shared_ptr<Ship> npcFlagship;
    for(const std::shared_ptr<Ship> &ship : npc.Ships())
    {
      // Skip ships that have been destroyed.
      ...
      ship->AddCrew(std::max(0, ship->RequiredCrew() - ship->Crew()));
      if(!ship->IsDisabled()) ship->Recharge();

      if(ship->CanBeCarried())
      {
        bool               docked  = false;
        const std::string &bayType = ship->Attributes().Category();
      ...
      if(!npcFlagship) npcFlagship = ship;

      // Only the flagship of an NPC considers the
      // player: the rest of the NPC track it.
      if(npcFlagship && ship != npcFlagship) ship->SetParent(npcFlagship);
      else if(!ship->GetPersonality().IsUninterested()) ship->SetParent(flagship);
      else ship->SetParent(nullptr);
    }
  }
}
...
void Engine::Step(bool isActive)
{
  ...
  {
    for(const StellarObject &object : player.GetSystem()->Objects())
      if(object.HasSprite()) SpriteLoadManager::LoadDeferred(asyncQueue, object.GetSprite());
  }

  // The calculation thread was paused by MainPanel before calling this function, so it is safe to access things.
  const std::shared_ptr<Ship> flagship = player.FlagshipPtr();
  const StellarObject        *object   = player.GetStellarObject();
  if(object)
  {
  ...
  {
    if(isActive && !timePaused) camera.MoveTo(flagship->Center(), hyperspacePercentage);
  ...
  if(isActive)
  {
    ...
    if(!wasActive) activeCommands.Clear();
    else ai.UpdateKeys(player, activeCommands);
  }
  ...
  if(nextZoom) zoom = std::exchange(nextZoom, {});
  ...
  if(isActive)
  {
    ...
    if(zoom.base != zoomTarget)
    {
      ...
      double zoomRatio = std::max(MIN_SPEED, std::min(MAX_SPEED, abs(log2(zoom.base) - log2(zoomTarget)) * ZOOM_SPEED));
      if(zoom.base < zoomTarget) nextZoom.base = std::min(zoomTarget, zoom.base * (1. + zoomRatio));
      else if(zoom.base > zoomTarget) nextZoom.base = std::max(zoomTarget, zoom.base * (1. / (1. + zoomRatio)));
    }
    if(flagship && flagship->Zoom() < 1.)
    {
      if(!nextZoom.base) nextZoom.base = zoom.base;
  ...
  if(Preferences::Has("Cloaked ship outlines"))
  {
    ...
    {
      if(ship->IsParked() || ship->GetSystem() != player.GetSystem() || ship->Cloaking() == 0.) continue;

      outlines.emplace_back(
          ship->GetSprite(),
          (ship->Position() - camera.Center()) * zoom,
          ship->Unit() * zoom,
          ship->GetFrame(),
          Color::Multiply(ship->Cloaking(), cloakColor));
    }
  }
  ...
  if(flagship && flagship->GetPlanet() && isActive) player.SetPlanet(flagship->GetPlanet());

  const System *currentSystem = player.GetSystem();
  // Update this here, for thread safety.
  if(player.HasTravelPlan() && currentSystem == player.TravelPlan().back()) player.PopTravel();
  ...
  if(flagship && player.HasTravelPlan())
  {
    ...
    for(const StellarObject &object : flagship->GetSystem()->Objects())
    {
         ...
         player.CanView(*system))
      {
        ...
        if(&wormhole->WormholeDestination(*flagship->GetSystem()) != system) continue;
    ...
    travelPlanIsValid &= all_of(
        player.TravelPlan().begin(),
        player.TravelPlan().end(),
        [this](const System *system) -> bool { return player.HasSeen(*system); });
    if(!travelPlanIsValid)
    {
      if(flagship->GetTargetSystem() == player.TravelPlan().back()) flagship->SetTargetSystem(nullptr);
  ...
  {
    flash = std::max(0., flash * .99 - .002);
  }

  targets.clear();

  // Update the player's ammo amounts.
  if(flagship) ammoDisplay.Update(*flagship);
  else ammoDisplay.Reset();
  ...
  std::set<std::shared_ptr<Ship>> selected;
  for(const std::weak_ptr<Ship> &ptr : player.SelectedEscorts())
    selected.insert(ptr.lock());
  for(const std::shared_ptr<Ship> &escort : player.Ships())
  {
    ...
    if(Preferences::Has("Show missile overlays"))
    {
      for(const Projectile &projectile : projectiles)
      {
        ...
        if(projectile.MissileStrength() && projectile.GetGovernment()->IsEnemy() &&
           (pos.Length() < std::max(Screen::Width(), Screen::Height()) * .5 / zoom))
        {
    ...
    if(flagship && Preferences::GetTurretOverlays() != Preferences::TurretOverlays::OFF)
    {
      for(const Hardpoint &hardpoint : flagship->Weapons())
      {
        if(!hardpoint.GetBaseAttributes().blindspots.empty())
        {
          ...
          if(Preferences::GetTurretOverlays() == Preferences::TurretOverlays::BLINDSPOTS_ONLY && !isBlind) continue;
  ...
  if(flagship && flagship->IsOverheated()) Messages::Add(*GameData::Messages().Get("overheated"));
  ...
  if(flagship && flagship->Hull())
  {
    ...
    if(Preferences::Has("Rotate flagship in HUD")) shipFacingUnit = flagship->Facing().Unit();

    info.SetSprite(
        "player sprite",
        flagship->GetSprite(),
        shipFacingUnit,
        flagship->GetFrame(step),
        flagship->GetSwizzle());
  }
  if(currentSystem) info.SetString("location", currentSystem->DisplayName());
  info.SetString("date", player.GetDate().ToString());
  if(flagship)
  {
    // Have an alarm label flash up when enemy ships are in the system.
    bool nukeAlert = any_of(
        projectiles.begin(),
        projectiles.end(),
    ...
    if(wasActive)
    {
      if(nukeAlert)
      {
        if(uiStep / 12 % 2) info.SetCondition("nuke alert");
      }
    ...
    if(fuelCap <= MAX_FUEL_DISPLAY) info.SetBar("fuel", flagship->Fuel(), fuelCap * .01);
    else info.SetBar("fuel", flagship->Fuel());
    info.SetBar("energy", flagship->Energy());
    double heat = flagship->Heat();
    info.SetBar("heat", std::min(1., heat));
    // If heat is above 100%, draw a second overlaid bar to indicate the
    // total heat level.
    if(heat > 1.) info.SetBar("overheat", std::min(1., heat - 1.));
    if(wasActive && flagship->IsOverheated() && (uiStep / 20) % 2) info.SetBar("overheat blink", std::min(1., heat));
    info.SetBar("shields", flagship->Shields());
    info.SetBar("hull", flagship->Hull(), 20.);
    info.SetBar("disabled hull", std::min(flagship->Hull(), flagship->DisabledHull()), 20.);
  }
  ...
  else if(flagship && flagship->GetTargetStellar() && !isJumping)
  {
    ...
    std::string          navigationMode = flagship->Commands().Has(Command::LAND) ? "Landing on:"
                                          : object->GetPlanet() && object->GetPlanet()->CanLand(*flagship) ? "Can land on:"
                                                                                                           : "Cannot land on:";
    info.SetString("navigation mode", navigationMode);
    const std::string &name = object->DisplayName();
  ...
  else if(flagship && flagship->GetTargetSystem())
  {
    ...
    else info.SetString("destination", "unexplored system");
  }
  else {
  ...
  std::shared_ptr<const Ship>    target;
  std::shared_ptr<const Minable> targetAsteroid;
  targetVector = Point();
  if(flagship)
  {
  ...
  if(!target) targetSwizzle = nullptr;
  ...
  else if(!target)
  {
    ...
    if(flagship->Attributes().Get("tactical scan power") || flagship->Attributes().Get("strategic scan power"))
    {
      ...
      info.SetString("target range", std::to_string(targetRange));
    }
  }
  else {
    if(target->GetSystem() == player.GetSystem() && !target->IsCloaked()) targetUnit = target->Facing().Unit();
    ...
    if(!target->GetGovernment()) info.SetString("target government", "No Government");
    else info.SetString("target government", target->GetGovernment()->DisplayName());
    ...
    if(blinking) info.SetOutlineColor(GetTargetOutlineColor(Radar::BLINK));
    else info.SetOutlineColor(GetTargetOutlineColor(lastTargetType));

    if(target->GetSystem() == player.GetSystem() && target->IsTargetable())
    {
      info.SetBar("target shields", target->Shields());
      info.SetBar("target hull", target->Hull(), 20.);
      info.SetBar("target disabled hull", std::min(target->Hull(), target->DisabledHull()), 20.);
      ...
      double accelerationScanRange =
          strategicRange + 100. * sqrt(flagship->Attributes().Get("acceleration scan power"));
      ...
      {
        info.SetString("target range", std::to_string(static_cast<int>(round(targetRange))));
        if(strategicRange) info.SetCondition("strategic range display");
        else info.SetCondition("range display");
      }
      ...
      {
        info.SetString("target crew", std::to_string(target->Crew()));
        if(accelerationScanRange || velocityScanRange) info.SetCondition("mobility crew display");
        else info.SetCondition("target crew display");
      }
      if((targetRange <= energyScanRange && scrutable) || (energyScanRange && target->IsYours()))
      {
        ...
        info.SetString("target energy", std::to_string(energy));
      }
      if((targetRange <= fuelScanRange && scrutable) || (fuelScanRange && target->IsYours()))
      {
        ...
        info.SetString("target fuel", std::to_string(fuel));
      }
      ...
      if((targetRange <= weaponScanRange && scrutable) || (weaponScanRange && target->IsYours()))
      {
        ...
        info.SetString("target turret", std::to_string(turretRange) + " ");
        int gunRange = round(target->GetAICache().GunRange());
        info.SetString("target gun", std::to_string(gunRange) + " ");
      }
      ...
      if((targetRange <= crewScanRange && targetRange <= maneuverScanRange && scrutable) ||
         (targetRange <= accelerationScanRange && scrutable) || (mobilityScan && crewScanRange && target->IsYours()))
      {
        info.SetCondition("turn while combined");
        int turnRate = round(60 * target->TrueTurnRate());
        info.SetString("target turnrate", std::to_string(turnRate) + " ");
      }
      else if(
          (targetRange >= crewScanRange && targetRange <= maneuverScanRange && scrutable) ||
      ...
      if((targetRange <= accelerationScanRange && scrutable) || (accelerationScanRange && target->IsYours()))
      {
        ...
        info.SetString("target velocity", std::to_string(presentSpeed) + " ");
      }
      if((targetRange <= velocityScanRange && scrutable) || (velocityScanRange && target->IsYours()))
      {
        ...
        info.SetString("target acceleration", std::to_string(presentAcceleration) + " ");
      }
    }
  }
  if(!Preferences::Has("Ship outlines in HUD")) info.SetCondition("fast hud sprites");
  if(target && target->IsTargetable() && target->GetSystem() == currentSystem &&
     (flagship->CargoScanFraction() || flagship->OutfitScanFraction()))
  {
    double     width         = std::max(target->Width(), target->Height());
    Point      pos           = target->Position() - camera.Center();
    const bool outfitInRange = pos.LengthSquared() <= (flagship->Attributes().Get("outfit scan power") * 10'000);
    const Status::Type outfitOverlayType = outfitInRange ? Status::Type::SCAN : Status::Type::SCAN_OUT_OF_RANGE;
    statuses.emplace_back(
        pos,
        flagship->OutfitScanFraction(),
        0.,
        0.,
        10. + std::max(20., width * .5),
        outfitOverlayType,
        1.f,
        Angle(pos).Degrees() + 180.);
    const bool         cargoInRange = pos.LengthSquared() <= (flagship->Attributes().Get("cargo scan power") * 10'000);
    const Status::Type cargoOverlayType = cargoInRange ? Status::Type::SCAN : Status::Type::SCAN_OUT_OF_RANGE;
    statuses.emplace_back(
        pos,
        0.,
        flagship->CargoScanFraction(),
        0.,
        10. + std::max(20., width * .5),
        cargoOverlayType,
        1.f,
  ...
  if(groupSelect >= 0)
  {
    ...
    if(hasControl) player.SetEscortGroup(groupSelect);
    else player.SelectEscortGroup(groupSelect, hasShift);
    groupSelect = -1;
  }
  if(doClickNextStep)
  {
  ...
  {
    if(uiClickBox.Dimensions()) doClick = !ammoDisplay.Click(uiClickBox);
    else doClick = !ammoDisplay.Click(clickPoint, hasControl);
    doClick = doClick && !player.SelectEscorts(clickBox, hasShift);
    if(doClick)
    {
      const std::vector<std::weak_ptr<Ship>> &stack = escorts.Click(clickPoint);
      if(!stack.empty())
      {
  ...
  for(const std::weak_ptr<Ship> &selected : player.SelectedEscorts())
  {
    std::shared_ptr<Ship> ship = selected.lock();
    if(ship && ship != target && !ship->IsParked() && ship->GetSystem() == player.GetSystem() && !ship->IsDestroyed() &&
       ship->Zoom() > 0.)
    {
  ...
  if(shouldShowAsteroidOverlay || shouldCatalogAsteroids)
  {
    ...
    if(flagship && scanRangeMetric && !flagship->IsHyperspacing())
    {
      ...
      for(const std::shared_ptr<Minable> &minable : asteroids.Minables())
      {
        ...
        if(!shouldShowAsteroidOverlay || !inRange || flagship->GetTargetAsteroid() == minable) continue;

        targets.push_back({offset, minable->Facing(), .8 * minable->Radius(), GetMinablePointerColor(false), 3});
      }
      if(shouldCatalogAsteroids && scanComplete) isAsteroidCatalogComplete = true;
    }
  }
...
bool Engine::IsPaused() const { return timePaused; }


// Give a command on behalf of the player, used for integration tests.
void Engine::GiveCommand(const Command &command) { activeCommands.Set(command); }


// Pass the list of game events to MainPanel for handling by the player, and any
// UI element generation.
std::list<ShipEvent> &Engine::Events() { return events; }
...
void Engine::Draw() const
{
  ...
  if(jumpEffectState != Preferences::ExtendedJumpEffects::OFF)
  {
    motionBlur *=
        baseBlur +
        pow(hyperspacePercentage * (jumpEffectState == Preferences::ExtendedJumpEffects::MEDIUM ? 2.5 : 5.), 2);
  }
  else {
    motionBlur *= baseBlur;
  }

  GameData::Background().Draw(motionBlur, player.Flagship() ? player.Flagship()->GetSystem() : player.GetSystem());
  ...
  for(const auto &it : statuses)
  {
    ...
    if(it.outer > 0.)
      RingShader::Draw(pos, radius + 3., 1.5f, it.outer, Color::Multiply(it.alpha, color[colorIndex]), 0.f, it.angle);
    double dashes  = (it.type >= Status::Type::SCAN) ? 0. : 20. * std::min<double>(1., zoom);
    colorIndex    += static_cast<int>(Status::Type::COUNT);
    if(it.inner > 0.)
      RingShader::Draw(pos, radius, 1.5f, it.inner, Color::Multiply(it.alpha, color[colorIndex]), dashes, it.angle);
    colorIndex += static_cast<int>(Status::Type::COUNT);
    if(it.disabled > 0.)
      RingShader::Draw(pos, radius, 1.5f, it.disabled, Color::Multiply(it.alpha, color[colorIndex]), dashes, it.angle);
  }
  ...
  if(flash) FillShader::Fill(Point(), Screen::Dimensions(), Color(flash, flash));
  ...
  const std::vector<Messages::Entry> &messages          = Messages::Get(uiStep, animationDuration);
  auto                                messageAnimation  = [animationDuration](double age) -> double
  { return std::max(0., 1. - pow((age - animationDuration) / animationDuration, 2)); };
  auto        naturalDecay = [animationDuration](int age) -> float { return (1000 + animationDuration - age) * .001f; };
  ...
  for(auto it = messages.rbegin(); it != messages.rend(); ++it)
  {
    ...
    if(messagesReversed && it == messages.rbegin()) messagePoint.Y() -= height;
    ...
    if(isAnimating) height *= messageAnimation(age);
    if(messagesReversed)
    {
      messagePoint.Y() += height;
      if(messagePoint.Y() > messageBox.Bottom()) break;
    }
    else {
      messagePoint.Y() -= height;
      if(messagePoint.Y() < messageBox.Top()) break;
    }
    float alpha = isAnimating ? isDying ? std::min<double>(messageAnimation(age), naturalDecay(naturalAge))
                                        : messageAnimation(age)
                              : naturalDecay(age);
...
void Engine::Click(const Point &from, const Point &to, bool hasShift, bool hasControl)
{
  ...
  if(Preferences::Has("Clickable radar display") && (from - radarCenter).Length() <= radarRadius) isRadarClick = true;
  else isRadarClick = false;
...
void Engine::BreakTargeting(const Government *gov)
{
  ...
  for(Projectile &projectile : projectiles)
  {
    ...
    if((projectileGov == playerGov && targetGov == gov) || (projectileGov == gov && targetGov == playerGov))
...
void Engine::EnterSystem()
{
  ...
  Messages::Add(
      {"Entering the " + system->DisplayName() + " system on " + today.ToString() +
  ...
  for(const StellarObject &object : system->Objects())
  {
    ...
    if(object.HasValidPlanet())
    {
      ...
      if(object.GetPlanet()->IsWormhole() && !usedWormhole && flagship->Position().Distance(object.Position()) < 1.)
        usedWormhole = &object;
    }
  }
  ...
  if(usedWormhole)
  {
    ...
    if(player.HasTravelPlan())
    {
      ...
      if(system != to && !flagship->JumpNavigation().JumpFuel(to)) player.TravelPlan().clear();
    }
  }
  ...
  for(const System::Asteroid &a : system->Asteroids())
  {
    ...
    if(a.Type()) asteroids.Add(a.Type(), a.Count(), a.Energy(), system->AsteroidBelts());
    else asteroids.Add(a.Name(), a.Count(), a.Energy());
  }
  ...
  for(int i = 0; i < 5; ++i)
  {
    if(fleetMultiplier)
    {
      for(const auto &fleet : system->Fleets())
        if(fleet.Get()->GetGovernment() && Random::Int(fleet.Period() / fleetMultiplier) < 60 && fleet.CanTrigger())
    ...
    if(attraction > 0.)
    {
      for(int i = 0; i < 10; ++i)
      {
        if(Random::Real() < attraction)
        {
          ...
          Messages::Add(
              {"Your fleet has attracted the interest of a " + raidFleet.GetFleet()->GetGovernment()->DisplayName() +
                   " raiding party.",
  ...
  if(entry == SystemEntry::WORMHOLE || entry == SystemEntry::JUMP) GameData::SetBackgroundPosition(camera.Center());
...
void Engine::CalculateStep()
{
  ...
  {
    if(flagship->IsHyperspacing()) hyperspacePercentage = flagship->GetHyperspacePercentage() / 100.;
    else hyperspacePercentage = 0.;
  ...
  for(const StellarObject &object : playerSystem->Objects())
  {
    if(object.HasSprite())
    {
      ...
      if(object.Width() >= 280.) draw[currentCalcBuffer].AddUnblurred(object);
      else draw[currentCalcBuffer].Add(object);
    }
  }
  ...
  if(flagship && showFlagship) DrawShipSprites(*flagship);
...
void Engine::CalculateUnpaused(const Ship *flagship, const System *playerSystem)
{
  ...
  for(const StellarObject &object : playerSystem->Objects())
    if(object.HasValidPlanet()) object.GetPlanet()->DeployDefense(newShips);
  ...
  for(const std::shared_ptr<Ship> &it : ships)
  {
    if(it == player.FlagshipPtr()) continue;
    bool wasUntargetable = !it->IsTargetable();
    MoveShip(it);
    bool isTargetable = it->IsTargetable();
    if(flagshipSystem == it->GetSystem() && ((wasUntargetable && isTargetable) || flagshipBecameTargetable) &&
  ...
  if(!wasHyperspacing && flagship && flagship->IsEnteringHyperspace())
  {
    ...
    const std::map<const Sound *, int> &jumpSounds =
  ...
  if(flagship && playerSystem != flagship->GetSystem())
  {
    ...
    player.SetSystemEntry(
        wormholeEntry                  ? SystemEntry::WORMHOLE
...
void Engine::MoveShip(const std::shared_ptr<Ship> &ship)
{
  ...
  if(ship->ShouldBeRemoved())
  {
    if(ship->IsDestroyed())
    {
      ...
      if(ship->IsYours()) player.DeselectEscort(ship.get());
    }
    return;
  }
  ...
  std::shared_ptr<Ship> victim = ship->Board(autoPlunder, isFlagship);
  if(victim)
  {
  ...
  if(ship->GetSystem() != player.GetSystem()) return;
  ...
  if(ship->HasAntiMissile()) hasAntiMissile.push_back(ship.get());
  if(ship->HasTractorBeam()) hasTractorBeam.push_back(ship.get());
}
...
void Engine::FillCollisionSets()
{
  ...
  for(const std::shared_ptr<Ship> &it : ships)
    if(it->GetSystem() == player.GetSystem() && it->Zoom() == 1.) shipCollisions.Add(*it);
...
void Engine::SpawnFleets()
{
  ...
  for(const auto &fleet : player.GetSystem()->Fleets())
  {
    if(!Random::Int(fleet.Period() / fleetMultiplier) && fleet.CanTrigger())
    {
      ...
      if(enemyStrength && ai.AllyStrength(gov) > 2 * enemyStrength) continue;
...
{
  if(Random::Int(GameData::GetGamerules().PersonSpawnPeriod()) || player.GetSystem()->Links().empty()) return;
  ...
  for(const auto &it : GameData::Persons())
  {
    ...
    if(sum < 0)
    {
      ...
      std::shared_ptr<Ship> parent;
      for(const std::shared_ptr<Ship> &ship : person.Ships())
      {
        ship->Recharge();
        if(ship->GivenName().empty()) ship->SetGivenName(it.first);
...
{
  if(Random::Int(600) || player.IsDead() || ships.empty()) return;

  std::vector<std::shared_ptr<const Ship>> canSend;
  canSend.reserve(ships.size());

  // When deciding who will send a hail, only consider ships that can send hails.
  for(auto &it : ships)
    if(it && it->CanSendHail(player, true)) canSend.push_back(it);
  ...
  std::shared_ptr<const Ship> source = canSend[i];
...
void Engine::HandleKeyboardInputs()
{
  ...
  activeCommands |= keyHeld.And(
      Command::PRIMARY | Command::SECONDARY | Command::SCAN | maneuveringCommands | Command::SHIFT |
  ...
  ++keyInterval;
  if(oldHeld.Has(debouncedCommands)) keyInterval = 0;
...
void Engine::HandleMouseClicks()
{
      ...
      if(object.HasSprite() && object.HasValidPlanet())
      {
        ...
        if(planet->IsAccessible(flagship) && (clickPoint - position).Length() < object.Radius())
        {
          if(&object == flagship->GetTargetStellar())
          {
            ...
            {
              Messages::Add(
                  {"The authorities on " + planet->DisplayName() + " refuse to let you land.",
                   GameData::MessageCategories().Get("high")});
            }
            ...
            if(flagship->Commands().Has(Command::LAND)) ai.DisengageAutopilot();
          }
  ...
  std::shared_ptr<Ship> clickTarget;
  for(std::shared_ptr<Ship> &ship : ships)
  {
    if(ship->GetSystem() == playerSystem && &*ship != flagship && ship->IsTargetable())
    {
      ...
      if(range <= clickRange)
      {
        ...
        if(!range && ship->GetGovernment()->IsEnemy()) break;
      }
    }
  }
  ...
  if(clickTarget)
  {
    ...
    if(mouseButton == MouseButton::RIGHT)
    {
      ...
      if(clickTarget == flagship->GetTargetShip())
      {
        ...
        if(clickTarget->IsYours()) player.SelectEscort(clickTarget.get(), hasShift);
      }
    }
  }
  else if(flagship->Attributes().Get("asteroid scan power"))
  {
    ...
    for(const std::shared_ptr<Minable> &minable : asteroids.Minables())
    {
      ...
      if(position.Length() > scanRange) continue;
      ...
      if(range <= clickRange)
      {
        ...
        if(mouseButton == MouseButton::RIGHT) ai.IssueAsteroidTarget(minable);
      }
    }
  }
...
void Engine::HandleMouseInput(Command &activeCommands)
{
  ...
  if((SDL_GetMouseState(&mousePosX, &mousePosY) & SDL_BUTTON_RMASK) != 0) rightMouseButtonHeld = true;
  ...
  if(!isMouseTurningEnabled) return;
  activeCommands.Set(Command::MOUSE_TURNING_HOLD);

  // Activate firing command.
  if(isMouseTurningEnabled && rightMouseButtonHeld) activeCommands.Set(Command::PRIMARY);
}
...
void Engine::DoCollisions(Projectile &projectile)
{
  ...
  else if(weapon.IsPhasing() && projectile.Target())
  {
    ...
    std::shared_ptr<Body> target = projectile.TargetPtr();
    if(target)
    {
      Point  offset = projectile.Position() - target->Position();
      double range  = target->GetMask(step).Collide(offset, projectile.Velocity(), target->Facing(), 1.);
      if(range < 1.)
      {
        collisions.emplace_back(
            target.get(),
            projectile.IsTargetingShip() ? CollisionType::SHIP : CollisionType::MINABLE,
            range);
      }
    }
  }
    ...
    if(triggerRadius)
    {
      std::vector<Body *> inRadius;
      inRadius.reserve(std::min(static_cast<std::vector<Body *>::size_type>(triggerRadius), ships.size()));
      ...
      for(const Body *body : inRadius)
      {
        ...
        if(body == projectile.Target() ||
           (gov->IsEnemy(body->GetGovernment()) && !ship->IsCloaked() && FighterHitHelper::IsValidTarget(ship)))
        {
          collisions.emplace_back(nullptr, CollisionType::NONE, 0.);
          break;
        }
    ...
    if(collisions.empty())
    {
      if(weapon.CanCollideShips()) shipCollisions.Line(projectile, collisions);
      if(weapon.CanCollideAsteroids()) asteroids.CollideAsteroids(projectile, collisions);
      if(weapon.CanCollideMinables()) asteroids.CollideMinables(projectile, collisions);
    }
  }
  ...
  for(Collision &collision : collisions)
  {
    ...
    std::shared_ptr<Ship> shipHit;
    if(hit && collisionType == CollisionType::SHIP) shipHit = static_cast<Ship *>(hit)->shared_from_this();

    // Don't collide with carried ships that are disabled and not directly targeted.
    if(shipHit && hit != projectile.Target() && !FighterHitHelper::IsValidTarget(shipHit.get())) continue;

    // If the ship is cloaked, and phasing, then skip this ship (during this step).
    if(shipHit && shipHit->Phases(projectile)) continue;
    ...
    if(blastRadius)
    {
      ...
      std::vector<Body *> blastCollisions;
      ...
      for(Body *body : blastCollisions)
      {
        ...
        if((isSafe && !targeted && !gov->IsEnemy(ship->GetGovernment())) || ship->Phases(projectile)) continue;

        // Only directly targeted ships get provoked by blast weapons.
        int eventType = ship->TakeDamage(visuals, damage.CalculateDamage(*ship, ship == hit), targeted ? gov : nullptr);
        if(eventType) eventQueue.emplace_back(gov, ship->shared_from_this(), eventType);
      }
    ...
    else if(hit)
    {
      if(collisionType == CollisionType::SHIP)
      {
        ...
        if(eventType) eventQueue.emplace_back(gov, shipHit, eventType);
      }
    ...
    if(shipHit) DoGrudge(shipHit, gov);
...
void Engine::DoCollection(Flotsam &flotsam)
{
  ...
  if(collector->IsYours())
  {
    ...
    if(flotsamSetting == Preferences::FlotsamCollection::OFF) return;
    if(collectorIsFlagship && flotsamSetting == Preferences::FlotsamCollection::ESCORT) return;
    if(!collectorIsFlagship && flotsamSetting == Preferences::FlotsamCollection::FLAGSHIP) return;
  }
  ...
  if(!collector->IsYours()) return;

  if(!collectorIsFlagship && !Preferences::Has("Extra fleet status messages")) return;

  // One of your ships picked up this flotsam. Describe who it was.
  std::string name =
      (collectorIsFlagship ? "You" : "Your " + collector->Noun() + " \"" + collector->GivenName() + "\"") +
      " picked up ";
  ...
  if(flotsam.OutfitType())
  {
    ...
    if(outfit->Get("minable") > 0.)
    {
      ...
      message =
          name + std::to_string(amount) + " " + (amount == 1 ? outfit->DisplayName() : outfit->PluralName()) + ".";
    }
  }
  else {
  ...
  if(free == total) message += ".)";
  else message += ", " + Format::MassString(total) + " in fleet.)";
...
void Engine::DoScanning(const std::shared_ptr<Ship> &ship)
{
  int scan = ship->Scan(player);
  if(scan)
  {
    std::shared_ptr<Ship> target = ship->GetTargetShip();
    if(target && target->IsTargetable()) eventQueue.emplace_back(ship, target, scan);
  }
}
...
void Engine::FillRadar()
{
  ...
  if(flagship)
  {
    ...
    const std::set<const System *> &links        = (flagship->JumpNavigation().HasJumpDrive())
  ...
  for(std::shared_ptr<Ship> &ship : ships)
  {
    if(ship->GetSystem() == playerSystem)
    {
      ...
      if(ship->IsCloaked() && !isYours) continue;
      ...
      hasHostiles |=
          (!ship->IsDisabled() && ship->GetGovernment()->IsEnemy() && ship->GetTargetShip() &&
  ...
  {
    if(Preferences::PlayAudioAlert()) Audio::Play(Audio::Get("alarm"), SoundCategory::ALERT);
    alarmTime   = 300;
    hadHostiles = true;
  }
  else if(!hasHostiles)
  ...
  for(const Projectile &projectile : projectiles)
  {
    if(!projectile.HasSprite()) continue;

    bool isBlast = projectile.GetWeapon().BlastRadius();
    if(!projectile.MissileStrength() && !isBlast) continue;
    ...
    radar[currentCalcBuffer].Add(
        isEnemy || (isBlast && !isSafe) ? Radar::SPECIAL : Radar::INACTIVE,
        projectile.Position(),
        isBlast ? 1.8 : 1.);
  }
}
...
void Engine::DrawShipSprites(const Ship &ship)
{
  ...
  auto           drawObject   = [&itemsToDraw, cloak, drawCloaked, fancyCloak, cloakSwizzle](const Body &body) -> void
  {
    ...
    if(drawCloaked) itemsToDraw.AddSwizzled(body, cloakSwizzle, fancyCloak ? 0.5 : 0.25);
  ...
  if(hasFighters)
  {
    for(const Ship::Bay &bay : ship.Bays())
      if(bay.side == Ship::Bay::UNDER && bay.ship) drawObject(*bay.ship);
  }
  ...
  auto DrawEngineFlares = [&](uint8_t where)
  {
    if(ship.ThrustHeldFrames(Ship::ThrustKind::FORWARD) && !ship.EnginePoints().empty())
    {
      DrawFlareSprites(
          ship,
          draw[currentCalcBuffer],
          ship.EnginePoints(),
          ship.Attributes().FlareSprites(),
          where,
          false);
    }
    else if(ship.ThrustHeldFrames(Ship::ThrustKind::REVERSE) && !ship.ReverseEnginePoints().empty())
    {
      DrawFlareSprites(
          ship,
          draw[currentCalcBuffer],
          ship.ReverseEnginePoints(),
          ship.Attributes().ReverseFlareSprites(),
          where,
          true);
    }
       ...
       !ship.SteeringEnginePoints().empty())
    {
      DrawFlareSprites(
          ship,
          draw[currentCalcBuffer],
          ship.SteeringEnginePoints(),
          ship.Attributes().SteeringFlareSprites(),
          where,
          false);
    }
  };
  ...
  auto drawHardpoint = [&drawObject, &ship](const Hardpoint &hardpoint) -> void
  {
    ...
    Body body(
        ...
        ship.Zoom());
    if(body.InheritsParentSwizzle()) body.SetSwizzle(ship.GetSwizzle());
    drawObject(body);
  };

  for(const Hardpoint &hardpoint : ship.Weapons())
    if(hardpoint.GetSide() == Hardpoint::Side::UNDER) drawHardpoint(hardpoint);
  drawObject(ship);
  for(const Hardpoint &hardpoint : ship.Weapons())
    if(hardpoint.GetSide() == Hardpoint::Side::OVER) drawHardpoint(hardpoint);
  ...
  {
    for(const Ship::Bay &bay : ship.Bays())
      if(bay.side == Ship::Bay::OVER && bay.ship) drawObject(*bay.ship);
  }
}
...
void Engine::DoGrudge(const std::shared_ptr<Ship> &target, const Government *attacker)
{
  if(attacker->IsPlayer())
  {
    std::shared_ptr<const Ship> previous = grudge[target->GetGovernment()].lock();
  ...
  if(!target || !target->CanSendHail(player)) return;
...
void Engine::CreateStatusOverlays()
{
  ...
  if(overlayAllSetting == Preferences::OverlayState::OFF) return;

  const System *currentSystem = player.GetSystem();
  const auto    flagship      = player.FlagshipPtr();

  static const std::set<Preferences::OverlayType> overlayTypes = {
  ...
  std::map<Preferences::OverlayType, Preferences::OverlayState> overlaySettings;
  ...
  for(const auto &it : ships)
  {
    if(!it->GetGovernment() || it->GetSystem() != currentSystem || (!it->IsYours() && it->Cloaking() == 1.)) continue;
    ...
    if(it == flagship) EmplaceStatusOverlay(it, overlaySettings[FLAGSHIP], Status::Type::FLAGSHIP, it->Cloaking());
    ...
    else EmplaceStatusOverlay(it, overlaySettings[NEUTRAL], Status::Type::NEUTRAL, it->Cloaking());
  }
}


void Engine::EmplaceStatusOverlay(
    const std::shared_ptr<Ship> &it,
    Preferences::OverlayState    overlaySetting,
    Status::Type                 type,
    double                       cloak)
{
  if(overlaySetting == Preferences::OverlayState::OFF) return;

  if(overlaySetting == Preferences::OverlayState::DAMAGED && !it->IsDamaged()) return;

  double width = std::min(it->Width(), it->Height());
  float  alpha = 1.f;
  if(overlaySetting == Preferences::OverlayState::ON_HIT)
  {
    ...
    if(t >= FADE_STEPS) alpha = 1.f;
    else if(t > 0) alpha = static_cast<float>(t) / FADE_STEPS;
    else alpha = 0.f;
  }

  if(it->IsYours()) cloak *= 0.6;

  statuses.emplace_back(
      it->Position() - camera.Center(),
      it->Shields(),
      it->Hull(),
      std::min(it->Hull(), it->DisabledHull()),
      std::max(20., width * .5),
      type,
      alpha * (1. - cloak));
}
