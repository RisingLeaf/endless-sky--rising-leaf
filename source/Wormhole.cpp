...
Wormhole::Wormhole() { linkColor = ExclusiveItem<Color>(GameData::Colors().Get(DEFAULT_WORMHOLE_COLOR)); }
  ...
  for(const DataNode &child : node)
  {
    ...
    if(key == "link" && child.Size() > valueIndex + 1)
    {
      ...
      if(remove)
      {
        ...
        if(it != links.end() && it->second == to) links.erase(from);
        else child.PrintTrace("Unable to remove non-existent link:");
      }
      else {
        links[from] = to;
      }
    ...
    {
      if(remove) displayName = "???";
      else if(hasValue) displayName = value;
      else child.PrintTrace("Missing value for attribute:");
    }
...
bool Wormhole::IsValid() const
{
  ...
  for(auto &&pair : links)
    if(!pair.first->IsValid() || !pair.second->IsValid()) return false;

  return true;
}


const Planet *Wormhole::GetPlanet() const { return planet; }


const std::string &Wormhole::DisplayName() const { return displayName; }


bool Wormhole::IsMappable() const { return mappable; }


const Color *Wormhole::GetLinkColor() const { return &(*linkColor); }


bool Wormhole::IsAutogenerated() const { return isAutogenerated; }


const System &Wormhole::WormholeSource(const System &to) const
{
  using value_type = decltype(links)::value_type;
  auto it          = find_if(links.begin(), links.end(), [&to](const value_type &val) { return val.second == &to; });
...
const std::unordered_map<const System *, const System *> &Wormhole::Links() const { return links; }


void Wormhole::SetPlanet(const Planet &planet) { this->planet = &planet; }
...
void Wormhole::GenerateLinks()
{
  ...
  for(size_t i = 0; i < planet->Systems().size(); ++i)
  {
    ...
    if(planet->Systems()[i]->FindStellar(planet)->GetSprite()) links[planet->Systems()[i]] = planet->Systems()[next];
  }
}
