  ...
  position(position), zoom(zoom)
{
  ...
  if(isDangerous) color = GameData::Colors().Get("missile dangerous");
  else if(isTargetingFlagship) color = GameData::Colors().Get("missile locked");
  else color = GameData::Colors().Get("missile enemy");
...
void AlertLabel::Draw() const
{
  ...
  for(int i = 0; i < 3; i++)
  {
    ...
    if(isTargetingFlagship)
    {
      PointerShader::Draw(
          position * zoom,
          Angle(angle[i] + 30. + rotation).Unit(),
          7.5f,
          (i ? 10.f : 22.f) * zoom,
          radius + (i ? 10.f : 20.f) * zoom,
          *color);
    }
  }
}
