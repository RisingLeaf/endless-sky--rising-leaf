    ...
    const ConditionsStore *playerConditions)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    // is handled below.
    if(!remove && (CoreStartData::LoadChild(child, add) || LoadStateChild(child, unlocked, clearDescription, add)))
      continue;
    ...
    if(remove)
    {
      ...
      {
        if(value == "display") toDisplay = ConditionSet();
        else if(value == "reveal") toReveal = ConditionSet();
        else if(value == "unlock") toUnlock = ConditionSet();
    ...
    else if(key == "ship" && hasValue)
    {
      ...
      if(child.HasChildren() || child.Size() >= add + 3) ships.emplace_back(child, playerConditions);
      // If there's only 2 tokens & there's no child nodes, the created instance would be ill-formed.
      else child.PrintTrace("Skipping unsupported use of a \"stock\" ship (a full definition is required):");
    }
    ...
    {
      if(value == "display") toDisplay.Load(child, globalConditions);
      else if(value == "reveal") toReveal.Load(child, globalConditions);
      else if(value == "unlock") toUnlock.Load(child, globalConditions);
      else child.PrintTrace("Skipping unrecognized attribute:");
    }
    else if(key == "on" && hasValue)
    {
      ...
      if(value == "display") LoadState(child, StartState::DISPLAYED);
      else if(value == "reveal") LoadState(child, StartState::REVEALED);
    }
    else {
  ...
  if(unlocked.description.empty()) unlocked.description = "(No description provided.)";
  if(unlocked.displayName.empty()) unlocked.displayName = "(Unnamed start)";
...
void StartConditions::FinishLoading()
{
  ...
  {
    Logger::Log(
        "The start scenario \"" + Identifier() + "\" (named \"" + unlocked.displayName +
            "\") has an invalid starting conversation." + (reason.empty() ? "" : "\n\t" + std::move(reason)),
...
bool StartConditions::IsValid() const
{
  ...
  if(!planet || !planet->IsValid() || planet->GetSystem() != system) return false;

  // A start must reference a valid "intro" conversation, either stock or custom.
  if(!GetConversation().IsValidIntro() || !GetConversation().Validate().empty()) return false;

  // All ship models must be valid.
  if(any_of(ships.begin(), ships.end(), [](const Ship &it) noexcept -> bool { return !it.IsValid(); })) return false;

  return true;
}


const ConditionAssignments &StartConditions::GetConditions() const noexcept { return conditions; }


const std::vector<Ship> &StartConditions::Ships() const noexcept { return ships; }


const Conversation &StartConditions::GetConversation() const { return *conversation; }
...
{
  auto it = infoByState.find(state);
  if(it == infoByState.end()) return ILLEGAL;
  if(it->second.date) return it->second.date.ToString();
...
bool StartConditions::Visible() const { return toDisplay.Test(); }
...
void StartConditions::SetState()
{
  ...
  {
    if(toReveal.Test())
      if(toUnlock.Test()) state = StartState::UNLOCKED;
      else state = StartState::REVEALED;
    else state = StartState::DISPLAYED;
  }
  else {
...
bool StartConditions::IsUnlocked() const { return state == StartState::UNLOCKED; }
...
bool StartConditions::LoadStateChild(const DataNode &child, StartInfo &info, bool &clearDescription, bool isAdd)
{
  ...
  else if(key == "date" && hasValue)
  {
    ...
    else info.dateString = value;
  }
  // Format credits and debt where applicable.
  else if(key == "credits" && hasValue)
  {
    if(child.IsNumber(value)) info.credits = Format::AbbreviatedNumber(child.Value(value));
    else info.credits = value;
  }
  else if(key == "debt" && hasValue)
  {
    if(child.IsNumber(value)) info.debt = Format::AbbreviatedNumber(child.Value(value));
    else info.debt = value;
  }
  else {
    return false;
  }
...
void StartConditions::FillState(StartState fillState, const Sprite *thumbnail)
{
  ...
  if(!fill.thumbnail) fill.thumbnail = thumbnail;
  if(fill.displayName.empty()) fill.displayName = "???";
  if(fill.description.empty()) fill.description = "???";
  if(fill.system.empty()) fill.system = "???";
  if(fill.planet.empty()) fill.planet = "???";
  if(fill.dateString.empty() && !fill.date) fill.dateString = "???";
  if(fill.credits.empty()) fill.credits = "???";
  if(fill.debt.empty()) fill.debt = "???";
}
