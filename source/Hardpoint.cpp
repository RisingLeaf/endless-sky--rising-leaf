...
namespace
{
  ...
  void
  CreateEffects(const std::map<const Effect *, int> &m, Point pos, Point vel, Angle angle, std::vector<Visual> &visuals)
  {
    for(const auto &it : m)
...
Hardpoint::Hardpoint(const Point &point, const BaseAttributes &attributes, bool isTurret, const Outfit *outfit) :
  ...
  isTurret(isTurret),
  isParallel(baseAttributes.isParallel)
{
  UpdateArc(true);
}
...
double Hardpoint::TurnRate(const Ship &ship) const
{
  ...
  return outfit->GetWeapon()->TurretTurn() *
         (1. + ship.Attributes().GetIndexed(Outfit::INTERNAL_ATTR("turret turn multiplier")) +
          baseAttributes.turnMultiplier);
}


// Find out if this is a turret hardpoint (whether or not it has a turret installed).
bool Hardpoint::IsTurret() const { return isTurret; }


bool Hardpoint::IsParallel() const { return isParallel; }


bool Hardpoint::IsOmnidirectional() const { return isOmnidirectional; }


Hardpoint::Side Hardpoint::GetSide() const { return baseAttributes.side; }


// Find out if this hardpoint has a homing weapon installed.
bool Hardpoint::IsHoming() const { return outfit && outfit->GetWeapon()->Homing(); }
...
bool Hardpoint::CanAim(const Ship &ship) const { return TurnRate(ship); }


// Check if this weapon is ready to fire.
bool Hardpoint::IsReady() const { return outfit && burstReload <= 0. && burstCount && (!IsBlind() || IsSpecial()); }
...
{
  return any_of(
      baseAttributes.blindspots.begin(),
      baseAttributes.blindspots.end(),
...
bool Hardpoint::WasFiring() const { return wasFiring; }


// Get the number of remaining burst shots before a full reload is required.
int Hardpoint::BurstRemaining() const { return burstCount; }
...
void Hardpoint::Aim(const Ship &ship, double amount)
{
  ...
  if(isOmnidirectional)
  {
    ...
    if(add < 0. && minArc.IsInRange(newAngle, angle)) angle = minArc;
    else if(add > 0. && maxArc.IsInRange(angle, newAngle)) angle = maxArc;
    else angle += add;
  }
}
...
bool Hardpoint::FireAntiMissile(Ship &ship, const Projectile &projectile, std::vector<Visual> &visuals)
{
  ...
  if(!FireSpecialSystem(ship, projectile, visuals)) return false;
...
bool Hardpoint::FireTractorBeam(Ship &ship, const Flotsam &flotsam, std::vector<Visual> &visuals)
{
  ...
  if(!FireSpecialSystem(ship, flotsam, visuals)) return false;
...
void Hardpoint::Install(const Outfit *install_outfit)
{
  ...
  if(!install_outfit || !install_outfit->GetWeapon() || (isTurret == !install_outfit->Get("turret mounts")))
  {
    Uninstall();
  }
  else {
    // Reset all the reload counters.
    this->outfit = install_outfit;
    Reload();
    ...
    if(!isParallel && !install_outfit->GetWeapon()->IsParallel())
    {
      ...
      if(!isTurret || isOmnidirectional || harmonized.IsInRange(GetMinArc(), GetMaxArc())) baseAngle = harmonized;
    }
    angle = baseAngle;
  }
}
...
const Hardpoint::BaseAttributes &Hardpoint::GetBaseAttributes() const { return baseAttributes; }
...
bool Hardpoint::FireSpecialSystem(Ship &ship, const Body &body, std::vector<Visual> &visuals)
{
  ...
  if(!isOmnidirectional)
  {
    const Angle curMinArc = GetMinArc() + facing;
    const Angle curMaxArc = GetMaxArc() + facing;
    if(!aim.IsInRange(curMinArc, curMaxArc)) return false;
  }
  angle = aim - facing;
  ...
  visuals.reserve(
      visuals.size() + weapon->FireEffects().size() + weapon->HitEffects().size() + weapon->DieEffects().size());
...
void Hardpoint::Fire(Ship &ship, const Point &start, const Angle &aim)
{
  ...
  if(weaponSound) Audio::Play(weaponSound, start, IsSpecial() ? SoundCategory::ANTI_MISSILE : SoundCategory::WEAPON);
  // Apply any "kick" from firing this weapon.
  double force = weapon->FiringForce();
  if(force) ship.ApplyForce(aim.Unit() * -force);
