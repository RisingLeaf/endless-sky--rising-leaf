...
namespace
{
  ...
  inline bool Check(double probability, double base) { return Random::Real() < base * probability; }
...
Projectile::Projectile(const Ship &parent, Point position, Angle angle, const Weapon *weapon) :
  Body(weapon->WeaponSprite(), position, parent.Velocity(), angle), weapon(weapon), lifetime(weapon->Lifetime())
{
  shared_ptr<Ship> targetShip = parent.GetTargetShip();
  targetIsShip                = static_cast<bool>(targetShip);
  target =
      targetIsShip ? static_pointer_cast<Entity>(targetShip) : static_pointer_cast<Entity>(parent.GetTargetAsteroid());

  government    = parent.GetGovernment();
  hitsRemaining = weapon->PenetrationCount();

  // If you are boarding your target, do not fire on it.
  if(parent.IsBoarding() || parent.Commands().Has(Command::BOARD)) target.reset();
...
Projectile::Projectile(const Projectile &parent, const Point &offset, const Angle &angle, const Weapon *weapon) :
  Body(
      weapon->WeaponSprite(),
      parent.position + parent.velocity + parent.angle.Rotate(offset),
      parent.velocity,
      parent.angle + angle),
  weapon(weapon),
  targetIsShip(parent.targetIsShip),
...
// Ship explosion.
Projectile::Projectile(Point position, const Weapon *weapon) : weapon(weapon) { this->position = std::move(position); }
...
void Projectile::Move(vector<Visual> &visuals, vector<Projectile> &projectiles)
{
      ...
      for(const auto &it : weapon->Submunitions())
      {
        if(lifetime > -100 ? it.spawnOnNaturalDeath : it.spawnOnAntiMissileDeath)
        {
          for(size_t i = 0; i < it.count; ++i)
          {
            ...
            Angle               inaccuracy =
                Distribution::GenerateInaccuracy(subWeapon->Inaccuracy(), subWeapon->InaccuracyDistribution());
  ...
  for(const auto &it : weapon->LiveEffects())
    if(!Random::Int(it.second)) visuals.emplace_back(*it.first, position, velocity, angle);
  ...
  if(turn)
  {
    ...
    if(!Random::Int(ceil(180 / turn))) confusionDirection = Random::Int(2) ? -1 : 1;
    if(homing)
    {
      ...
      else if(currentTarget && hasLock)
      {
        ...
        if(fabs(desiredTurn) > turn) turn = copysign(turn, desiredTurn);
        else turn = desiredTurn;
        ...
        if(weapon->ThrottleControl())
        {
          ...
          if(stepsToFace * 1.5 > stepsToReach) accel = 0.;
        }
      }
  ...
  if(currentTarget && (position - currentTarget->Position()).Length() < weapon->SplitRange()) lifetime = 0;

  // A projectile will begin to fade out when the remaining lifetime is smaller
  // than the specified "fade out" time.
  if(lifetime < weapon->FadeOut()) alpha = static_cast<double>(lifetime) / weapon->FadeOut();
}
...
const Entity *Projectile::Target() const { return cachedTarget; }


const Government *Projectile::TargetGovernment() const { return targetGovernment; }


shared_ptr<Entity> Projectile::TargetPtr() const { return target.lock(); }
...
bool Projectile::IsTargetingShip() const { return targetIsShip; }
...
void Projectile::CheckLock(const Entity *target, bool targetIsShip)
{
  ...
  if(weapon->OpticalTracking())
  {
    ...
    if(opticalJamming) opticalJamming *= RangeFraction(position.Distance(target->Position()), opticalJamming);
  ...
  if(weapon->InfraredTracking())
  {
    ...
    double multiplier = 1.;
    if(distance <= shortRange) multiplier = 2. - distance / shortRange;
  ...
  if(weapon->RadarTracking())
  {
    ...
    if(radarJamming) radarJamming *= RangeFraction(position.Distance(target->Position()), radarJamming);
...
void Projectile::CheckConfused(const Entity &target)
{
  ...
  if(weapon->Tracking()) trackingConfused = Random::Real() > weapon->Tracking();

  if(weapon->InfraredTracking()) infraredConfused = Random::Real() > weapon->InfraredTracking();
...
double Projectile::DistanceTraveled() const { return distanceTraveled; }


bool Projectile::Phases(const Ship &ship) const { return phasedShip == &ship; }


uint16_t Projectile::HitsRemaining() const { return hitsRemaining; }


void Projectile::SetPhases(const Ship *ship) { phasedShip = ship; }


bool Projectile::ShouldExplode() const { return !government || (weapon->IsFused() && lifetime == 1); }
