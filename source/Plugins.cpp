...
namespace
{
  ...
  void LoadSettingsFromFile(const std::filesystem::path &path)
  {
    ...
    {
      if(node.Token(0) != "state") continue;
...
bool Plugin::PluginDependencies::IsEmpty() const { return required.empty() && optional.empty() && conflicted.empty(); }
...
bool Plugin::PluginDependencies::IsValid() const
{
  ...
  for(const std::string &dependency : optional)
    if(required.contains(dependency)) dependencyCollisions += dependency + ", ";
  if(!dependencyCollisions.empty())
  {
    dependencyCollisions.pop_back();
    dependencyCollisions.pop_back();
    Logger::Log(
        "Dependencies named " + dependencyCollisions +
...
{
  std::string text;
  if(!version.empty()) text += "Version: " + version + '\n';
  if(!authors.empty())
  {
    text += "Authors: ";
  ...
  {
    text += "Dependencies:\n";
    if(!dependencies.gameVersion.empty()) text += "  Game Version: " + dependencies.gameVersion + '\n';
  ...
  if(!aboutText.empty()) text += aboutText;
...
bool Plugin::IsValid() const { return !name.empty(); }
...
const Plugin *Plugins::Load(const std::filesystem::path &path)
{
  ...
  for(const DataNode &child : DataFile(pluginFile))
  {
    ...
    else if(key == "dependencies")
    {
      for(const DataNode &grand : child)
      {
        ...
        if(grandKey == "game version" && grandHasValue) dependencies.gameVersion = grand.Token(1);
  ...
  {
    Logger::Log(
        "Skipping plugin located at \"" + path.string() +
            "\", because another plugin with the same name has already been loaded from: \"" + plugin->path.string() +
            "\".",
        Logger::Level::WARNING);
    return nullptr;
  }
  ...
  {
    Logger::Log(
        "Skipping plugin located at \"" + path.string() + "\", because it has errors in its dependencies.",
        Logger::Level::WARNING);
    return nullptr;
  }
...
void Plugins::Save()
{
  ...
  out.BeginChild();
  {
    for(const auto &it : plugins)
      if(it.second.IsValid()) out.Write(it.first, it.second.currentState);
  }
  out.EndChild();
}
...
bool Plugins::IsPlugin(const std::filesystem::path &path)
{
  ...
  return Files::Exists(path / "data") || Files::Exists(path / "images") || Files::Exists(path / "shaders") ||
...
{
  for(const auto &it : plugins)
    if(it.second.enabled != it.second.currentState) return true;
  return false;
}


// Returns the list of plugins that have been identified by the game.
const Set<Plugin> &Plugins::Get() { return plugins; }
