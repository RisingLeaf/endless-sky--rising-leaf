...
void ShipJumpNavigation::Recalibrate(const Ship &ship)
{
  ...
  if(hasJumpMassCost && mass != ship.Mass()) Calibrate(ship);
}


// Pass the current system that the ship is in to the navigation.
void ShipJumpNavigation::SetSystem(const System *system) { currentSystem = system; }
...
double ShipJumpNavigation::JumpFuel(const System *destination) const
{
  ...
  if(!destination) return max(JumpDriveFuel(), HyperdriveFuel());
...
double ShipJumpNavigation::JumpRange() const { return maxJumpRange; }
...
{
  if(!currentSystem || !destination) return make_pair(JumpType::NONE, 0.);
...
{
  if(!from || !to) return make_pair(JumpType::NONE, 0.);
  ...
  else if(hasJumpDrive && canJump) return make_pair(JumpType::JUMP_DRIVE, jumpFuelNeeded);
  else return make_pair(JumpType::NONE, 0.);
}
...
bool ShipJumpNavigation::CanJump(const System *from, const System *to) const
{
  ...
  if(from->Links().contains(to) && (hasHyperdrive || hasJumpDrive)) return true;

  if(!hasJumpDrive) return false;
...
bool ShipJumpNavigation::HasAnyDrive() const { return hasHyperdrive || hasJumpDrive; }


bool ShipJumpNavigation::HasHyperdrive() const { return hasHyperdrive; }


bool ShipJumpNavigation::HasScramDrive() const { return hasScramDrive; }


bool ShipJumpNavigation::HasJumpDrive() const { return hasJumpDrive; }
...
void ShipJumpNavigation::ParseOutfit(const Outfit &outfit)
{
  ...
  if(outfit.Get("hyperdrive") && (!hasScramDrive || outfit.Get("scram drive")))
  {
    ...
    if(!hyperdriveCost || cost < hyperdriveCost) hyperdriveCost = cost;
  }
  if(outfit.Get("jump drive"))
  {
    double distance = outfit.Get("jump range");
    if(distance <= 0.) distance = System::DEFAULT_NEIGHBOR_DISTANCE;
...
{
  if(!maxJumpRange || maxJumpRange < distance) maxJumpRange = distance;
  ...
  if(oit == jumpDriveCosts.end() || !oit->second || oit->second > cost)
  {
    ...
    if(nit != jumpDriveCosts.end() && it->second > nit->second)
    {
      ...
      for(auto sit = jumpDriveCosts.begin(); sit != it; ++sit)
        if(!sit->second || sit->second > it->second) sit->second = it->second;
    }
  }
}
