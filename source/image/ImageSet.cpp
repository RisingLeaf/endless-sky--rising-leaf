...
namespace
{
  ...
  void AddValid(
      const std::map<size_t, std::filesystem::path> &frameData,
      std::vector<std::filesystem::path>            &sequence,
      const std::string                             &prefix,
      bool                                           is2x,
      bool                                           isSwizzleMask) noexcept(false)
  {
    ...
    {
      Logger::Log(
          prefix + "ignored " + (isSwizzleMask ? "mask " : "") + (is2x ? "@2x " : "") + "frame " +
              std::to_string(frameData.begin()->first) + " (" + std::to_string(frameData.size()) +
    ...
    if(next != frameData.end())
    {
      ...
      Logger::Log(
          prefix + "missing " + (isSwizzleMask ? "mask " : "") + (is2x ? "@2x " : "") + "frame " +
              std::to_string(it->first + 1) + " (" + std::to_string(ignored) + (ignored > 1 ? " frames" : " frame") +
              " ignored in total).",
          Logger::Level::WARNING);
    }
  }
...
ImageSet::ImageSet(std::string name) : name(std::move(name)) {}


// Get the name of the sprite for this image set.
const std::string &ImageSet::Name() const { return name; }


// Whether this image set is empty, i.e. has no images.
bool ImageSet::IsEmpty() const { return framePaths[0].empty() && framePaths[1].empty(); }
...
void ImageSet::ValidateFrames() noexcept(false)
{
  ...
  for(int i = 0; i < 4; ++i)
  {
    for(const auto &path : paths[i])
    {
      ...
      {
        Logger::Log(
            "Image sequences must be exclusive; ignoring all but the image sequence data for \"" + name + "\".",
            Logger::Level::WARNING);
        paths[i][0] = path;
        paths[i].resize(1);
        break;
  ...
  auto DropPaths = [&](std::vector<std::filesystem::path> &toResize, const std::string &specifier)
  {
    if(toResize.size() > paths[0].size())
    {
      ...
      {
        Logger::Log(
            prefix + "all frames for the " + specifier +
                " sprite will be ignored, "
                "as it has no corresponding normal resolution frame(s).",
            Logger::Level::WARNING);
      }
      else {
        Logger::Log(
            prefix + std::to_string(toResize.size() - paths[0].size()) + " extra frame(s) for the " + specifier +
                " sprite will be ignored.",
            Logger::Level::WARNING);
      }
...
void ImageSet::Load() noexcept(false)
{
  ...
  {
    Logger::Log(
        "Discarding " + std::to_string(swizzleMaskFrames - 1) +
            " frames of swizzle mask because there"
            " are more frames of animation. Only the first swizzle mask frame will be used.",
        Logger::Level::WARNING);
    swizzleMaskFrames = 1;
  }
  ...
  const auto UpdateFrameCount = [&]()
  {
    ...
    if(makeMasks) masks.resize(frames);
  };
  ...
  for(size_t i = 0; i < paths[0].size(); ++i)
  {
    ...
    if(makeMasks)
    {
      ...
      if(!masks[i].IsLoaded()) Logger::Log("Failed to create collision mask for " + fileName, Logger::Level::WARNING);
    }
  }
  ...
  auto LoadSprites = [&](const std::vector<std::filesystem::path> &toLoad, ImageBuffer &buffer, const std::string &specifier)
  {
    for(size_t i = 0; i < frames && i < toLoad.size(); ++i)
    {
      ...
      {
        Logger::Log("Removing " + specifier + " frames for \"" + name + "\" due to read error", Logger::Level::WARNING);
        buffer.Clear();
        break;
      }
    }
  ...
  {
    Logger::Log(
        "Image \"" + name + "\" will be blurry since width and/or height are not even (" +
