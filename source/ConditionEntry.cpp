...
ConditionEntry::ConditionEntry(const std::string &name) : name(name), value(0), providingEntry(nullptr) {}


const std::string &ConditionEntry::Name() const { return name; }
...
const std::string ConditionEntry::NameWithoutPrefix() const
{
  ...
  if(providingEntry) return name.substr(providingEntry->name.length());
...
ConditionEntry::operator int64_t() const
{
  ...
  if(providingEntry) return providingEntry->getFunction(*this);

  // Named provider; use the local getFunction to get the value.
  if(getFunction) return getFunction(*this);
...
void ConditionEntry::ProvidePrefixed(
...
void ConditionEntry::ProvideNamed(
    std::function<int64_t(const ConditionEntry &)> getFunction,
    std::function<void(ConditionEntry &, int64_t)> setFunction)
{
