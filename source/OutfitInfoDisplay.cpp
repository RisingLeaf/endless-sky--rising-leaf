...
int OutfitInfoDisplay::RequirementsHeight() const { return requirementsHeight; }
    ...
    bool              descriptionCollapsed)
{
  ...
  for(const auto &it : outfit.Attributes())
    if(!count(BEFORE.begin(), BEFORE.end(), it.first)) AddRequirementAttribute(it.first, it.second);
}
...
void OutfitInfoDisplay::AddRequirementAttribute(std::string label, double value)
{
  ...
  if(IsNotRequirement(label)) return;
...
void OutfitInfoDisplay::UpdateAttributes(const Outfit &outfit)
{
  ...
  for(const auto &it : outfit.Attributes())
  {
    if(count(EXPECTED_NEGATIVE.begin(), EXPECTED_NEGATIVE.end(), it.first)) continue;
  ...
  if(velocity == range) attributeValues.emplace_back("instantaneous");
  else attributeValues.emplace_back(Format::Number(velocity * 60.));
  attributesHeight += 20;
  ...
  static const std::vector<std::pair<std::string, std::string>> VALUE_NAMES = {
      ...
      {"heat damage", ""},       {"energy damage", ""},     {"ion damage", ""},       {"scrambling damage", ""},
      {"slowing damage", ""},    {"disruption damage", ""}, {"discharge damage", ""}, {"corrosion damage", ""},
      {"leak damage", ""},       {"burn damage", ""},       {"% shield damage", "%"}, {"% hull damage", "%"},
      {"% minable damage", "%"}, {"% fuel damage", "%"},    {"% heat damage", "%"},   {"% energy damage", "%"},
      {"firing energy", ""},     {"firing heat", ""},       {"firing fuel", ""},      {"firing hull", ""},
      {"firing shields", ""},    {"firing ion", ""},        {"firing scramble", ""},  {"firing slowing", ""},
      {"firing disruption", ""}, {"firing discharge", ""},  {"firing corrosion", ""}, {"firing leak", ""},
      {"firing burn", ""},       {"% firing energy", "%"},  {"% firing heat", "%"},   {"% firing fuel", "%"},
