...
namespace
{
  ...
  Point ParseAlignment(const DataNode &node, int i = 1)
  {
    Point alignment;
    for(; i < node.Size(); ++i)
      if(node.Token(i) == "left") alignment.X() = -1.;
      else if(node.Token(i) == "top") alignment.Y() = -1.;
      else if(node.Token(i) == "right") alignment.X() = 1.;
      else if(node.Token(i) == "bottom") alignment.Y() = 1.;
      else if(node.Token(i) != "center") node.PrintTrace("Skipping unrecognized alignment:");
    return alignment;
  }
} // namespace
...
void Interface::Load(const DataNode &node)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    else if(key == "visible" || key == "active")
    {
      ...
      if(child.Size() >= 3 && child.Token(1) == "if") str = child.Token(2);
      else str.clear();
    }
    else {
      ...
      else if(key == "label" || key == "string" || key == "button" || key == "dynamic button")
      {
        elements.push_back(make_unique<BasicTextElement>(child, anchor));
          ...
          key == "wrapped label" || key == "wrapped string" || key == "wrapped button" ||
      ...
      {
        if(key == "line") child.PrintTrace("\"line\" is deprecated, use \"fill\" instead:");
...
bool Interface::HasPoint(const string &name) const { return points.contains(name); }
...
{
  auto it = points.find(name);
  if(it == points.end()) return Rectangle();
...
Point Interface::AnchoredPoint::Get() const { return position + .5 * Screen::Dimensions() * anchor; }
...
void Interface::Element::Load(const DataNode &node, const Point &globalAnchor)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    {
      if(child.Size() > 3) fromAnchor = toAnchor = ParseAlignment(child, 3);
    ...
    else if(key == "from" && child.Size() >= 6 && child.Token(3) == "to")
    {
      ...
      if(child.Size() > 6) fromAnchor = toAnchor = ParseAlignment(child, 6);
    ...
    {
      // Anything after the coordinates is an anchor point override.
      if(child.Size() > 3) fromAnchor = ParseAlignment(child, 3);
    ...
    {
      // Anything after the coordinates is an anchor point override.
      if(child.Size() > 3) toAnchor = ParseAlignment(child, 3);
...
{
  if(!info.HasCondition(visibleIf)) return;
...
Rectangle Interface::Element::Bounds() const { return Rectangle::WithCorners(from.Get(), to.Get()); }
...
bool Interface::Element::ParseLine(const DataNode &node) { return false; }


// Report the actual dimensions of the object that will be drawn.
Point Interface::Element::NativeDimensions(const Information &info, int state) const { return Bounds().Dimensions(); }
...
Interface::ImageElement::ImageElement(const DataNode &node, const Point &globalAnchor)
{
  ...
  if(key == "sprite") sprite[Element::ACTIVE] = SpriteSet::Get(node.Token(1));
  else name = node.Token(1);
  ...
  {
    if(!sprite[Element::INACTIVE]) sprite[Element::INACTIVE] = sprite[Element::ACTIVE];
    if(!sprite[Element::HOVER]) sprite[Element::HOVER] = sprite[Element::ACTIVE];
  }
}
...
bool Interface::ImageElement::ParseLine(const DataNode &node)
{
  ...
  if(key == "inactive" && hasValue && name.empty()) sprite[Element::INACTIVE] = SpriteSet::Get(node.Token(1));
  else if(key == "hover" && hasValue && name.empty()) sprite[Element::HOVER] = SpriteSet::Get(node.Token(1));
  else if(isOutline && key == "colored") isColored = true;
  else return false;
...
Point Interface::ImageElement::NativeDimensions(const Information &info, int state) const
{
  ...
  if(!sprite || !sprite->Width() || !sprite->Height()) return Point();

  Point     size(sprite->Width(), sprite->Height());
  Rectangle bounds = Bounds();
  if(!bounds.Dimensions()) return size;
...
Interface::TextElement::TextElement(const DataNode &node, const Point &globalAnchor)
{
  ...
  if(key.ends_with("button") || key.ends_with("dynamic button"))
  {
    ...
    if(node.Size() >= 3) str = node.Token(2);
  }
  else {
    str = node.Token(1);
  }
...
bool Interface::TextElement::ParseLine(const DataNode &node)
{
  ...
  {
    if(node.Token(1) == "none") truncate = Truncate::NONE;
    else if(node.Token(1) == "front") truncate = Truncate::FRONT;
    else if(node.Token(1) == "middle") truncate = Truncate::MIDDLE;
    else if(node.Token(1) == "back") truncate = Truncate::BACK;
    else return false;
  }
  else {
    return false;
...
{
  if(buttonKey && panel) panel->AddZone(bounds, buttonKey);
}
...
void Interface::TextElement::FinishLoadingColors()
{
  ...
  if(!color[Element::ACTIVE])
  {
    ...
    if(!color[Element::INACTIVE]) color[Element::INACTIVE] = GameData::Colors().Get("inactive");
    if(!color[Element::HOVER]) color[Element::HOVER] = GameData::Colors().Get("hover");
  }
  else {
    // If a base color was specified, also use it for any unspecified states.
    if(!color[Element::INACTIVE]) color[Element::INACTIVE] = color[Element::ACTIVE];
    if(!color[Element::HOVER]) color[Element::HOVER] = color[Element::ACTIVE];
  }
}
...
{
  if(TextElement::ParseLine(node)) return true;
  if(node.Token(0) == "alignment")
  {
    const string &value = node.Token(1);
    if(value == "left") textAlignment = Alignment::LEFT;
    else if(value == "center") textAlignment = Alignment::CENTER;
    else if(value == "right") textAlignment = Alignment::RIGHT;
    else if(value == "justified") textAlignment = Alignment::JUSTIFIED;
    else return false;
  }
  else {
    return false;
...
Interface::BarElement::BarElement(const DataNode &node, const Point &globalAnchor)
{
  ...
  if(!fromColor) fromColor = toColor = GameData::Colors().Get("active");
}
...
void Interface::BarElement::Draw(const Rectangle &rect, const Information &info, int state) const
{
  ...
  // Avoid crashes for malformed interface elements that are not fully loaded.
  if(!fromColor || !toColor || !width || !value) return;
...
Interface::PointerElement::PointerElement(const DataNode &node, const Point &globalAnchor)
{
  ...
  // Fill in a default color if none is specified.
  if(!color) color = GameData::Colors().Get("medium");

  // Set a default orientation if none is specified.
  if(!orientation) orientation = Point(0., -1.);
}
...
{
  if(node.Token(0) == "color" && node.Size() >= 2) color = GameData::Colors().Get(node.Token(1));
  else return false;
