...
namespace
{
  ...
  void SetRequiredAttributes(const std::set<std::string> &attributes, std::set<std::string> &required)
  {
    ...
    required.clear();
    for_each(
        attributes.lower_bound(PREFIX),
        attributes.lower_bound(PREFIX_END),
...
void Planet::Load(const DataNode &node, Set<Wormhole> &wormholes, const ConditionsStore *playerConditions)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    if(removeAll || overwriteAll)
    {
      ...
      else if(key == "to")
      {
        ...
        else if(value == "access" && child.Size() > valueIndex + 1)
        {
          ...
          if(shop == "shipyard") toAccessShipyard = ConditionSet();
          else if(shop == "outfitter") toAccessOutfitter = ConditionSet();
        }
      }

      // If not in "overwrite" mode, move on to the next node.
      if(overwriteAll) shouldOverwrite.erase(key);
      else continue;
    }
    ...
    {
      if(remove) shipSales.erase(GameData::Shipyards().Get(value));
      else shipSales.insert(GameData::Shipyards().Get(value));
    }
    else if(key == "outfitter")
    {
      if(remove) outfitSales.erase(GameData::Outfitters().Get(value));
      else outfitSales.insert(GameData::Outfitters().Get(value));
    }
    ...
    else if(key == "tribute hails" && child.HasChildren())
    {
      for(const DataNode &grand : child)
      {
        ...
        const std::string &grandKey            = grand.Token(remove);
    ...
    else if(key == "to")
    {
      ...
      {
        const std::string &shop = child.Token(valueIndex + 1);
        if(shop == "shipyard") toAccessShipyard.Load(child, playerConditions);
        else if(shop == "outfitter") toAccessOutfitter.Load(child, playerConditions);
  ...
  {
    if(attributes.contains("uninhabited")) port.LoadUninhabitedSpaceport();
    else port.LoadDefaultSpaceport();
  }
  ...
  for(unsigned i = 0; i < AUTO_ATTRIBUTES.size(); ++i)
    if(autoValues[i]) attributes.insert(AUTO_ATTRIBUTES[i]);
    else attributes.erase(AUTO_ATTRIBUTES[i]);

  // Precalculate commonly used values that can only change due to Load().
  inhabited =
      (HasServices(false) || requiredReputation || !defenseFleets.empty()) && !attributes.contains("uninhabited");
...
void Planet::FinishLoading(Set<Wormhole> &wormholes)
{
  ...
  if(!wormhole && systems.size() > 1)
  {
    ...
    Logger::Log(
        "Deprecated automatic generation of wormhole \"" + trueName + "\" from a multi-system planet.",
...
{
  return isDefined && !systems.empty() &&
         all_of(systems.begin(), systems.end(), [](const System *s) noexcept -> bool { return s->IsValid(); });
}


// Get the name used for this planet in the data files.
const std::string &Planet::TrueName() const { return trueName; }
...
const std::string &Planet::DisplayName() const { return IsWormhole() ? wormhole->DisplayName() : displayName; }


// Get the planet's descriptive text.
const Paragraphs &Planet::Description() const { return description; }


// Get the landscape sprite.
const Sprite *Planet::Landscape() const { return landscape; }


// Get the name of the ambient audio to play on this planet.
const std::string &Planet::MusicName() const { return music; }


// Get the list of "attributes" of the planet.
const std::set<std::string> &Planet::Attributes() const { return attributes; }


// Get the list of "attributes" of the planet.
const std::set<std::string> &Planet::RequiredAttributes() const { return requiredAttributes; }


// Get planet's noun descriptor from attributes
const std::string &Planet::Noun() const
{
  if(IsWormhole()) return WORMHOLE;

  for(const std::string &attribute : attributes)
    if(attribute == "moon" || attribute == "station") return attribute;
...
bool Planet::HasNamedPort() const { return !port.DisplayName().empty(); }


// Get this planet's port.
const Port &Planet::GetPort() const { return port; }


// Check whether there are port services (such as trading, jobs, banking, and hiring)
// available on this planet.
bool Planet::HasServices(bool isPlayer) const { return port.HasServices(isPlayer); }


// Check if this planet is inhabited (i.e. it has a spaceport, and does not
// have the "uninhabited" attribute).
bool Planet::IsInhabited() const { return inhabited; }


// Check if this planet has a permanent shipyard.
bool Planet::HasShipyard() const { return !shipSales.empty() && toAccessShipyard.Test(); }
...
std::set<const Shop<Ship> *> Planet::Shipyards() const
{
  ...
  for(const auto &shop : GameData::Shipyards())
    if(shop.second.CanStock(this)) shops.insert(&shop.second);
  return shops;
}


// Check if this planet has a permanent outfitter.
bool Planet::HasOutfitter() const { return !outfitSales.empty() && toAccessOutfitter.Test(); }
...
double Planet::RequiredReputation() const { return requiredReputation; }
...
double Planet::GetBribeFraction() const { return bribe; }


// This is how likely the planet's authorities are to notice if you are
// doing something illegal.
double Planet::Security() const { return security; }


bool Planet::HasCustomSecurity() const { return customSecurity; }


const System *Planet::GetSystem() const { return systems.empty() ? nullptr : systems.front(); }
...
{
  if(find(systems.begin(), systems.end(), system) == systems.end()) systems.push_back(system);
}
...
const std::vector<const System *> &Planet::Systems() const { return systems; }


// Check if this is a wormhole (that is, it appears in multiple systems).
bool Planet::IsWormhole() const { return wormhole; }


const Wormhole *Planet::GetWormhole() const { return wormhole; }
...
bool Planet::IsAccessible(const Ship *ship) const
{
  ...
  if(ship && ship->IsYours() && !toKnow.Test()) return false;
  ...
  return all_of(
      requiredAttributes.cbegin(),
      requiredAttributes.cend(),
...
bool Planet::IsUnrestricted() const { return requiredAttributes.empty(); }
...
bool Planet::CanBribe() const
{
  ...
  if(bribeThreshold && GameData::GetPolitics().Reputation(government) < bribeThreshold) return false;
...
{
  if(!ship.IsYours()) return IsAccessible(&ship) && GameData::GetPolitics().CanLand(ship, this);

  return IsAccessible(&ship) && GameData::GetPolitics().CanLand(ship, this) &&
         (!port.RequiresBribe() || GameData::GetPolitics().HasClearance(this)) && toLand.Test();
}
...
{
  if(GameData::GetPolitics().HasDominated(this)) return Friendliness::DOMINATED;
  else if(GetGovernment()->IsEnemy()) return Friendliness::HOSTILE;
  else if(CanLand()) return Friendliness::FRIENDLY;
  else return Friendliness::RESTRICTED;
}


bool Planet::CanUseServices() const { return GameData::GetPolitics().CanUseServices(this) && port.CanAccess(); }


void Planet::Bribe(bool fullAccess) const { GameData::GetPolitics().BribePlanet(this, fullAccess); }
...
std::string Planet::DemandTribute(PlayerInfo &player) const
{
  ...
  if(!government) return "Somehow, this planet does not have a government.";
  const auto &playerTribute = player.GetTribute();
  if(playerTribute.find(this) != playerTribute.end())
  {
    if(tributeAlreadyPaying) return tributeAlreadyPaying->Get();
    else if(government->TributeAlreadyPaying()) return government->TributeAlreadyPaying()->Get();
    return "We are already paying you as much as we can afford.";
  }
  if(!tribute || defenseFleets.empty())
  {
    if(tributeUndefined) return tributeUndefined->Get();
    else if(government->TributeUndefined()) return government->TributeUndefined()->Get();
    return "Please don't joke about that sort of thing.";
  }
  if(player.Conditions().Get("combat rating") < defenseThreshold)
  {
    if(tributeUnworthy) return tributeUnworthy->Get();
    else if(government->TributeUnworthy()) return government->TributeUnworthy()->Get();
  ...
  if(!isDefending)
  {
    ...
    for(const auto &gov : toProvoke)
      if(gov) gov->Offend(ShipEvent::PROVOKE);
    ...
    if(tributeFleetLaunching) return tributeFleetLaunching->Get();
    else if(government->TributeFleetLaunching()) return government->TributeFleetLaunching()->Get();
  ...
  if(!isDefeated)
  {
    if(tributeFleetUndefeated) return tributeFleetUndefeated->Get();
    else if(government->TributeFleetUndefeated()) return government->TributeFleetUndefeated()->Get();
    return "We're not ready to surrender yet.";
  }

  player.SetTribute(this, tribute);
  std::string surrenderMessage;
  if(tributeSurrendered) surrenderMessage = tributeSurrendered->Get();
  else if(government->TributeSurrendered()) surrenderMessage = government->TributeSurrendered()->Get();
  else surrenderMessage = "We surrender. We will pay you <credits> per day to leave us alone.";
...
{
  if(!isDefending || Random::Int(60) || defenseDeployed == defenseFleets.size()) return;

  auto end = defenders.begin();
  if(defenseFleets[defenseDeployed]->IsValid()) defenseFleets[defenseDeployed]->Enter(*GetSystem(), defenders, this);
  else Logger::Log("Skipped an incomplete defense fleet of planet \"" + trueName + "\".", Logger::Level::WARNING);
  ...
  for(auto it = defenders.begin(); it != end; ++it)
  {
    ...
    if((**it).HasBays())
    {
      for(auto bay = (**it).Bays().begin(); bay != (**it).Bays().end(); ++bay)
        if(bay->ship) bay->ship->SetPersonality(fighterPersonality);
    }
  }
...
bool Planet::IsDefending() const { return isDefending; }


double Planet::DailyTributePenalty() const { return dailyTributePenalty; }
