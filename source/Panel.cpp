...
bool Panel::IsFullScreen() const noexcept { return isFullScreen; }
...
bool Panel::TrapAllEvents() const noexcept { return trapAllEvents; }


// Check if this panel can be "interrupted" to return to the main menu.
bool Panel::IsInterruptible() const noexcept { return isInterruptible; }


// Clear the list of clickable zones.
void Panel::ClearZones() { zones.clear(); }
...
bool Panel::ZoneClick(const Point &point)
{
  for(auto it = children.rbegin(); it != children.rend(); ++it)
    if((*it)->ZoneClick(point)) return true;
...
bool Panel::AllowsFastForward() const noexcept { return false; }
...
void Panel::AddOrRemove()
{
  for(auto &panel : childrenToAdd)
    if(panel) children.emplace_back(std::move(panel));
  childrenToAdd.clear();
...
bool Panel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress) { return false; }


bool Panel::Click(int x, int y, MouseButton button, int clicks) { return false; }


bool Panel::Hover(int x, int y) { return false; }


bool Panel::Drag(double dx, double dy) { return false; }


bool Panel::Scroll(double dx, double dy) { return false; }


bool Panel::Release(int x, int y, MouseButton button) { return false; }
...
bool Panel::DoHover(int x, int y) { return EventVisit(&Panel::Hover, x, y); }


bool Panel::DoDrag(double dx, double dy) { return EventVisit(&Panel::Drag, dx, dy); }


bool Panel::DoRelease(int x, int y, MouseButton button) { return EventVisit(&Panel::Release, x, y, button); }


bool Panel::DoScroll(double dx, double dy) { return EventVisit(&Panel::Scroll, dx, dy); }
...
void Panel::SetIsFullScreen(bool set) { isFullScreen = set; }


void Panel::SetTrapAllEvents(bool set) { trapAllEvents = set; }


void Panel::SetInterruptible(bool set) { isInterruptible = set; }
...
bool Panel::DoKey(SDL_Keycode key, Uint16 mod) { return KeyDown(key, mod, Command(), true); }
...
int Panel::Modifier()
{
  ...
  int modifier = 1;
  if(mod & SDL_KMOD_ALT) modifier *= 500;
  if(mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI)) modifier *= 20;
  if(mod & SDL_KMOD_SHIFT) modifier *= 5;
...
bool Panel::DoHelp(const std::string &name, bool force) const
{
  ...
  if(!force && Preferences::Has(preference)) return false;
...
void Panel::SetUI(UI *ui) { this->ui = ui; }


const std::vector<std::shared_ptr<Panel>> &Panel::GetChildren() { return children; }


void Panel::AddChild(const std::shared_ptr<Panel> &panel) { childrenToAdd.push_back(panel); }


void Panel::RemoveChild(const Panel *panel) { childrenToRemove.push_back(panel); }
