...
void ShipAICache::Calibrate(const Ship &ship)
{
  ...
  for(const Hardpoint &hardpoint : ship.Weapons())
  {
    ...
    if(!weapon || hardpoint.IsSpecial()) continue;
    ...
    double DPS = (weapon->ShieldDamage() + weapon->HullDamage() + (weapon->RelativeShieldDamage() * ship.MaxShields()) +
  ...
  else if(hasWeapons)
  {
    ...
    useArtilleryAI =
        (artilleryDPS > totalDPS * .75 && (ship.MaxReverseVelocity() || maxTurningRadius < 0.2 * shortestArtillery));
  ...
  for(const auto &hardpoint : ship.Weapons())
  {
    const Weapon *weapon = hardpoint.GetWeapon();
    if(!weapon || hardpoint.IsSpecial()) continue;
    if((weapon->Ammo() && !ship.OutfitCount(weapon->Ammo())) || !weapon->DoesDamage()) continue;
    double weaponRange = weapon->Range() + hardpoint.GetPoint().Length();
    if(hardpoint.IsTurret()) turretRange = std::max(turretRange, weaponRange);
    else gunRange = std::max(gunRange, weaponRange);
  }
}
...
{
  if(mass != ship.Mass()) Calibrate(ship);
}
