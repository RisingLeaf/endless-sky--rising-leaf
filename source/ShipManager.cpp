...
void ShipManager::Load(const DataNode &node)
{
  ...
  if(node.Size() >= 4) name = node.Token(3);
  ...
  for(const DataNode &child : node)
  {
    ...
    else if(key == "count" && hasValue)
    {
      ...
      if(val <= 0) child.PrintTrace("\"count\" must be a non-zero, positive number.");
      else count = val;
    }
    else if(taking)
    {
      if(key == "unconstrained") unconstrained = true;
      else if(key == "with outfits") takeOutfits = true;
      else if(key == "require outfits") requireOutfits = true;
      else child.PrintTrace("Skipping unrecognized token.");
    }
    else {
...
void ShipManager::Save(DataWriter &out) const
{
  ...
  out.BeginChild();
  {
    ...
    if(!id.empty()) out.Write("id", id);
    if(unconstrained) out.Write("unconstrained");
    if(takeOutfits) out.Write("with outfits");
    if(requireOutfits) out.Write("require outfits");
  }
  out.EndChild();
}
...
{
  if(model->TrueModelName().empty()) return;
  ...
  if(Giving())
  {
    ...
    if(toTake.size() == 1) shipName = toTake.begin()->get()->GivenName();
    for(const auto &ship : toTake)
      player.TakeShip(ship.get(), model, takeOutfits);
  }
  Messages::Add(
      {(count == 1 ? "The " + model->DisplayModelName() + " \"" + shipName + "\" was "
...
const Ship *ShipManager::ShipModel() const { return model; }


const string &ShipManager::Id() const { return id; }


bool ShipManager::Giving() const { return !taking; }
...
vector<shared_ptr<Ship>> ShipManager::SatisfyingShips(const PlayerInfo &player) const
{
  ...
  {
    if(ship->TrueModelName() != model->TrueModelName()) continue;
    if(!unconstrained)
    {
      if(ship->GetSystem() != here) continue;
      if(ship->IsDisabled()) continue;
      if(ship->IsParked()) continue;
    }
    if(!id.empty())
    {
      if(!foundShip) continue;
      if(ship->UUID() != shipToTakeId->second) continue;
    }
    if(!name.empty() && name != ship->GivenName()) continue;
    ...
    if(hasRequiredOutfits) satisfyingShips.emplace_back(ship);
    // We do not want any more ships than is specified.
    if(static_cast<int>(satisfyingShips.size()) >= count) break;
  }
