...
VulkanObjects::VulkanBufferInstance::VulkanBufferInstance(const VulkanDeviceInstance *device, const GraphicsTypes::BufferType type, const size_t size) : BufferInstance(), Size(size), Device(device)
{
  ...
  case GraphicsTypes::BufferType::STAGING:         properties = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT; break;
  case GraphicsTypes::BufferType::VERTEX:          properties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT; break;
  case GraphicsTypes::BufferType::VERTEX_DYNAMIC:  properties = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT; break;
  ...
  VulkanHelpers::VK_CHECK_RESULT(vmaCreateBuffer(Device->GetAllocator(), &create_info, &vma_allocation_create_info, &Buffer, &Allocation, nullptr), __LINE__, __FILE__);
}

VulkanObjects::VulkanBufferInstance::~VulkanBufferInstance() { Device->QueueBufferForDeletion(Buffer, Allocation); }
void VulkanObjects::VulkanBufferInstance::Map(void **map_memory) const { vmaMapMemory(Device->GetAllocator(), Allocation, map_memory); }
