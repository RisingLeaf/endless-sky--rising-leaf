...
Color::Color(float r, float g, float b, float a) : color{r, g, b, a}, isLoaded(true) {}
...
bool Color::IsLoaded() const { return isLoaded; }


void Color::SetTrueName(const std::string &name) { this->trueName = name; }


const std::string &Color::TrueName() const { return trueName; }


// Get a float std::vector representing this color, for use by OpenGL.
const float *Color::Get() const { return color; }
...
{
  return Color(scalar * base.color[0], scalar * base.color[1], scalar * base.color[2], scalar * base.color[3]);
}
