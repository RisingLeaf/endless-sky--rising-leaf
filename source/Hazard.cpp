...
bool Hazard::IsValid() const { return !name.empty(); }


// The name of the hazard in the data files.
const std::string &Hazard::Name() const { return name; }


// Does the strength of this hazard deviate over time?
bool Hazard::Deviates() const { return deviates; }


// How often this hazard deals its damage while active.
int Hazard::Period() const { return period; }
...
bool Hazard::SystemWide() const { return systemWide; }


// The minimum and maximum distances from the origin in which this hazard has an effect.
double Hazard::MinRange() const { return minRange; }


double Hazard::MaxRange() const { return maxRange; }


// Visuals to be created while this hazard is active.
const std::map<const Effect *, float> &Hazard::EnvironmentalEffects() const { return environmentalEffects; }
