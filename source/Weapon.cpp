...
Weapon::Weapon(const DataNode &node) { Load(node); }
...
void Weapon::Load(const DataNode &node)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    else if(key == "homing")
    {
      ...
      for(const DataNode &grand : child)
      {
        ...
        if(grandKey == "blindspot") blindspot = true;
        else if(grandKey == "throttle control") throttleControl = true;
        else if(grandKey == "leading") leading = true;
        else grand.PrintTrace("Skipping unknown homing attribute:");
      }
    }
      ...
      for(const DataNode &grand : child)
      {
        ...
        else if(grandKey == "spawn on" && grandHasValue)
        {
          ...
          for(int j = 1; j < grand.Size(); ++j)
            if(grand.Token(j) == "natural") submunitions.back().spawnOnNaturalDeath = true;
            else if(grand.Token(j) == "anti-missile") submunitions.back().spawnOnAntiMissileDeath = true;
        }
        else {
    ...
    else if(key == "inaccuracy")
    {
      ...
      for(const DataNode &grand : child)
      {
        for(int j = 0; j < grand.Size(); ++j)
        {
          ...
          if(token == "inverted") inaccuracyDistribution.second = true;
          else if(token == "triangular") inaccuracyDistribution.first = Distribution::Type::Triangular;
          else if(token == "uniform") inaccuracyDistribution.first = Distribution::Type::Uniform;
          else if(token == "narrow") inaccuracyDistribution.first = Distribution::Type::Narrow;
          else if(token == "medium") inaccuracyDistribution.first = Distribution::Type::Medium;
          else if(token == "wide") inaccuracyDistribution.first = Distribution::Type::Wide;
          else grand.PrintTrace("Skipping unknown distribution attribute:");
        }
      }
    }
    else {
      ...
      else if(key == "hardpoint offset")
      {
        ...
        if(child.Size() == 2) hardpointOffset = Point(0., -value);
        else if(child.Size() == 3) hardpointOffset = Point(value, -child.Value(2));
        else child.PrintTrace("Unsupported \"" + key + "\" specification:");
      }
      else if(key == "turn")
      {
        turn = value;
  ...
  if(!disabledDamageSet) damage[DISABLED_DAMAGE] = damage[HULL_DAMAGE];
  if(!relativeDisabledDamageSet) damage[RELATIVE_DISABLED_DAMAGE] = damage[RELATIVE_HULL_DAMAGE];
  // Minable damage defaults to hull damage instead of 0.
  if(!minableDamageSet) damage[MINABLE_DAMAGE] = damage[HULL_DAMAGE];
  if(!relativeMinableDamageSet) damage[RELATIVE_MINABLE_DAMAGE] = damage[RELATIVE_HULL_DAMAGE];

  // Sanity checks:
  if(burstReload > reload) burstReload = reload;
  if(damageDropoffRange.first > damageDropoffRange.second) damageDropoffRange.second = Range();
  ...
  if(lifetime <= 0) liveEffects.clear();
  ...
  if(!isSafe && blastRadius > 0 && !safeRangeOverriden) safeRange = (blastRadius + triggerRadius);
}


bool Weapon::IsLoaded() const { return isLoaded; }


// Get assets used by this weapon.
const Body &Weapon::WeaponSprite() const { return sprite; }


const Body &Weapon::HardpointSprite() const { return hardpointSprite; }


const Sound *Weapon::WeaponSound() const { return sound; }


const Sound *Weapon::EmptySound() const { return emptySound; }


const Outfit *Weapon::Ammo() const { return ammo.first; }


int Weapon::AmmoUsage() const { return ammo.second; }


bool Weapon::IsParallel() const { return isParallel; }


const Sprite *Weapon::Icon() const { return icon; }


// Effects to be created at the start or end of the weapon's lifetime.
const std::map<const Effect *, int> &Weapon::FireEffects() const { return fireEffects; }


const std::map<const Effect *, int> &Weapon::LiveEffects() const { return liveEffects; }


const std::map<const Effect *, int> &Weapon::HitEffects() const { return hitEffects; }


const std::map<const Effect *, int> &Weapon::TargetEffects() const { return targetEffects; }


const std::map<const Effect *, int> &Weapon::DieEffects() const { return dieEffects; }


const std::vector<Weapon::Submunition> &Weapon::Submunitions() const { return submunitions; }


double Weapon::TotalLifetime() const
{
  if(rangeOverride) return rangeOverride / WeightedVelocity();
  if(totalLifetime < 0.)
  {
    totalLifetime = 0.;
...
double Weapon::Range() const { return (rangeOverride > 0) ? rangeOverride : WeightedVelocity() * TotalLifetime(); }
...
double Weapon::DamageDropoff(double distance) const
{
  ...
  if(distance <= minDropoff) return 1.;
  if(distance >= maxDropoff) return damageDropoffModifier;
...
double Weapon::MaxDropoff() const { return damageDropoffModifier; }


// Return the ranges at which the weapon's damage dropoff begins and ends.
const std::pair<double, double> &Weapon::DropoffRanges() const { return damageDropoffRange; }
...
std::pair<Distribution::Type, bool> Weapon::InaccuracyDistribution() const { return inaccuracyDistribution; }


double Weapon::Inaccuracy() const { return inaccuracy; }
