...
void SpriteShader::Draw(const Sprite *sprite, const Point &position, float zoom, const Swizzle *swizzle, float frame,
                        const Point &unit) {
	if (!sprite)
		return;
...
SpriteShader::Item SpriteShader::Prepare(const Sprite *sprite, const Point &position, float zoom,
...
void SpriteShader::Add(const Item &item, bool withBlur) {
	...
	if (item.swizzleMask)
		texture_list.AddTexture(item.swizzleMask, 1, false);
