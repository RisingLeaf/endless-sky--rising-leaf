...
void Conversation::Load(const DataNode &node, const ConditionsStore *playerConditions)
{
  ...
  if(node.Token(0) != "conversation") return;
  ...
  for(const DataNode &child : node)
  {
    ...
    else if(key == "label" && hasValue)
    {
      ...
      if(!nodes.empty()) nodes.back().canMergeOnto = false;
    ...
    else if(key == "choice")
    {
      ...
      {
        if(!foundErrors) child.PrintTrace("Conversation contains an empty \"choice\" node:");
        nodes.pop_back();
      }
    }
    else if(key == "name")
    ...
    else if(key == "branch")
    {
      ...
      if(child.HasChildren()) nodes.back().conditions.Load(child, playerConditions);
      ...
      for(int i = 1; i <= 2; ++i)
      {
        ...
        if(child.Size() > i)
        {
          ...
          if(!index) Goto(child.Token(i), nodes.size() - 1, i - 1);
          else if(index < 0) nodes.back().elements.back().next = index;
        }
      }
    }
    ...
    else if(hasValue)
    {
      ...
      // node.
      if(nodes.empty() || !nodes.back().canMergeOnto || HasDisplayRestriction(child)) AddNode();
      ...
      if(LoadDestinations(child, playerConditions)) nodes.back().canMergeOnto = false;
    }
  }
...
void Conversation::Save(DataWriter &out) const
{
  ...
  out.BeginChild();
  {
    for(unsigned i = 0; i < nodes.size(); ++i)
    {
      ...
      if(node.scene) out.Write("scene", node.scene->Name());
      if(IsBranch(i))
      {
      ...
      for(const auto &it : node.elements)
      {
        ...
        int index = it.next;
        if(index > 0 && !NodeIsValid(index)) index = Endpoint::DECLINE;

        // Write the node that we go to next after this.
        Endpoint::WriteToken(index, out);
      }
      if(node.isChoice) out.EndChild();
    }
  }
  out.EndChild();
}
...
bool Conversation::IsEmpty() const noexcept { return nodes.empty(); }
...
bool Conversation::IsValidIntro() const noexcept
{
  return any_of(
      nodes.begin(),
      nodes.end(),
      [](const Node &node) noexcept -> bool { return node.isChoice && node.elements.empty(); });
}
...
std::string Conversation::Validate() const
{
  for(const Node &node : nodes)
  {
    if(!node.actions.IsEmpty())
    {
      ...
      if(!reason.empty()) return "conversation action " + std::move(reason);
    }
  }
  return "";
}
...
Conversation Conversation::Instantiate(std::map<std::string, std::string> &subs, int jumps, int payload) const
{
  ...
  for(Node &node : result.nodes)
  {
    ...
    if(!node.actions.IsEmpty()) node.actions = node.actions.Instantiate(subs, jumps, payload);
  }
...
const std::set<const Sprite *> &Conversation::Scenes() const { return scenes; }
...
bool Conversation::HasAnyChoices(int node) const
{
  ...
  if(!nodes[node].isChoice) return false;
  ...
  {
    if(data.toDisplay.IsEmpty()) return true;
    if(data.toDisplay.Test()) return true;
  }
...
{
  if(!NodeIsValid(node) || !IsChoice(node) || !ElementIsValid(node, element)) return false;
...
const ConditionSet &Conversation::Conditions(int node) const
{
  ...
  if(!NodeIsValid(node)) return empty;
...
const std::string &Conversation::Text(int node, int element) const
{
  ...
  if(!NodeIsValid(node) || !ElementIsValid(node, element)) return empty;
...
{
  if(!NodeIsValid(node)) return nullptr;
...
{
  if(!NodeIsValid(node) || !ElementIsValid(node, element)) return Endpoint::DECLINE;
...
int Conversation::StepToNextNode(int node) const
{
  ...
  if(!NodeIsValid(next_node)) return Endpoint::DECLINE;
...
{
  if(!NodeIsValid(node)) return false;
  else if(IsChoice(node) ? !ElementIsValid(node, element) : element != 0) return false;
...
{
  for(const DataNode &child : node)
    if(child.Token(0) == "to" && child.Size() >= 2 && child.Token(1) == "display") return true;
...
void Conversation::Goto(const std::string &label, int node, int element)
{
  ...
  if(it == labels.end()) unresolved.insert({label, {node, element}});
  else nodes[node].elements[element].next = it->second;
}
