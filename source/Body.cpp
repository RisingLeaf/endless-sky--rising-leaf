...
const Mask &Body::GetMask(int step) const
{
  ...
  if(!sprite || current < 0) return EMPTY;
...
const Point &Body::Position() const { return position; }


// Velocity, in pixels per second.
const Point &Body::Velocity() const { return velocity; }


Point Body::Center() const { return -rotatedCenter + position; }


// Direction this Body is facing in.
const Angle &Body::Facing() const { return angle; }
...
Point Body::Unit() const { return angle.Unit() * (.5 * Zoom()); }


// Check if this object is marked for removal from the game.
bool Body::ShouldBeRemoved() const { return shouldBeRemoved; }
...
const Government *Body::GetGovernment() const { return government; }


double Body::Alpha(const Point &drawCenter) const { return alpha * DistanceAlpha(drawCenter); }


double Body::DistanceAlpha(const Point &drawCenter) const
{
  if(!distanceInvisible) return 1.;
...
bool Body::IsVisible(const Point &drawCenter) const { return DistanceAlpha(drawCenter) > 0.; }


// Mark this object to be removed from the game.
void Body::MarkForRemoval() { shouldBeRemoved = true; }


// Mark this object to not be removed from the game.
void Body::UnmarkForRemoval() { shouldBeRemoved = false; }
...
void Body::Turn(const Angle &amount) { Turn(amount.Degrees()); }
