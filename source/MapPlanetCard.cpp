...
MapPlanetCard::MapPlanetCard(
  ...
  parent(parent), number(number), hasVisited(hasVisited), loadingCircle(30.f, 10, 2.), planetName(object.DisplayName())
{
  ...
  if(governmentName != "Uninhabited" && governmentName != systemGovernmentName) hasGovernments = true;
  ...
  if(!hasSpaceport)
  {
    ...
    {
    case Planet::Friendliness::FRIENDLY:   reputationLabel = "Friendly"; break;
    case Planet::Friendliness::RESTRICTED: reputationLabel = "Restricted"; break;
    case Planet::Friendliness::HOSTILE:    reputationLabel = "Hostile"; break;
    case Planet::Friendliness::DOMINATED:  reputationLabel = "Dominated"; break;
    }
  }
  ...
  if(isShown)
  {
    ...
    if(relativeY > 0. && relativeY < AvailableSpace())
    {
      ...
      if(clickAction != ClickAction::SELECTED)
      {
        ...
        if(clickAction == ClickAction::SHOW_SHIPYARD && clicks > 1) clickAction = ClickAction::GOTO_SHIPYARD;
        else if(clickAction == ClickAction::SHOW_OUTFITTER && clicks > 1) clickAction = ClickAction::GOTO_OUTFITTER;
      }
    }
  }
...
bool MapPlanetCard::DrawIfFits(const Point &uiPoint)
{
  ...
  if(isShown)
  {
    ...
    if(hasGovernments && FitsCategory(categories))
    {
      font.Draw(
          governmentName,
          uiPoint + Point(margin, textStart + categorySize),
    ...
    if(FitsCategory(3.))
    {
      font.Draw(
          "Shipyard",
          uiPoint + Point(margin, textStart + categorySize * (2. + hasGovernments)),
    ...
    if(FitsCategory(2.))
    {
      font.Draw(
          "Outfitter",
          uiPoint + Point(margin, textStart + categorySize * (3. + hasGovernments)),
          hasOutfitter ? medium : faint);
    }
    if(FitsCategory(1.))
    {
      font.Draw(
          hasVisited ? "(has been visited)" : "(not yet visited)",
          uiPoint + Point(margin, textStart + categorySize * (4. + hasGovernments)),
          dim);
    }
    ...
    if(isSelected) Highlight(availableBottomSpace);
  }
  else {
...
bool MapPlanetCard::IsShown() const { return AvailableSpace() > 15.; }


bool MapPlanetCard::IsSelected() const { return isSelected; }


double MapPlanetCard::AvailableSpace() const { return std::min(AvailableBottomSpace(), AvailableTopSpace()); }


const Planet *MapPlanetCard::GetPlanet() const { return planet; }


void MapPlanetCard::Select(bool select) { isSelected = select; }
...
void MapPlanetCard::ResetSize() { hasGovernments = false; }
...
double MapPlanetCard::AvailableBottomSpace() const
{
  ...
  return std::min(
      Height(),
      std::max(0., Screen::Top() + std::min(MapDetailPanel::PlanetPanelHeight(), maxPlanetPanelHeight) - yCoordinate));
}
