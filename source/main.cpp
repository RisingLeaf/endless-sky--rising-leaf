...
int main(int argc, char *argv[])
{
  ...
  Logger::SetLogCallback(
      [&hasErrors](const std::string &errorMessage, Logger::Level level)
      {
  ...
  try
  {
    ...
    auto dataFuture = GameData::BeginLoad(
        queue,
        player,
        isConsoleOnly,
        debugMode,
    ...
    // all data now.
    if(isConsoleOnly || checkAssets || isTesting) dataFuture.wait();
    ...
    for(const DataNode &node : globalConditions)
      if(node.Token(0) == "conditions") GameData::GlobalConditions().Load(node);

    if(!GameWindow::Init(isTesting && !debugMode)) return 1;
    ...
    if(isTesting && !noTestMute) Audio::SetVolume(0, SoundCategory::MASTER);
    ...
    bool                debugMode)
{
  ...
  if(!testToRunName.empty()) testContext = TestContext(GameData::Tests().Get(testToRunName));

  const bool isHeadless = (testContext.CurrentTest() && !debugMode);

  auto ProcessEvents =
      [&menuPanels, &gamePanels, &player, &cursorTime, &toggleTimeout, &debugMode, &isDebugPaused, &isFastForward]
  {
    SDL_Event event;
    while(SDL_PollEvent(&event))
    {
      ...
      if(event.type == SDL_EVENT_MOUSE_MOTION) cursorTime = 0;

      if(debugMode && event.type == SDL_EVENT_KEY_DOWN && event.key.key == SDLK_GRAVE)
      {
        isDebugPaused = !isDebugPaused;
        if(isDebugPaused) Audio::Pause();
        else Audio::Resume();
      }
      else if(
          event.type == SDL_EVENT_KEY_DOWN && menuPanels.IsEmpty() && Command(event.key.key).Has(Command::MENU) &&
      ...
      else if(event.type == CustomEvents::GetResize())
      {
        ...
        gamePanels.AdjustViewport();
          ...
          event.type == SDL_EVENT_KEY_DOWN && !toggleTimeout &&
          (Command(event.key.key).Has(Command::FULLSCREEN) ||
           (event.key.key == SDLK_RETURN && (event.key.key & SDL_KMOD_ALT))))
      {
        toggleTimeout = 30;
        Preferences::ToggleScreenMode();
      }
      else if(event.type == SDL_EVENT_KEY_DOWN && Command(event.key.key).Has(Command::PERFORMANCE_DISPLAY))
      {
      ...
      else if(activeUI.Handle(event))
      {
        // The UI handled the event.
          ...
          event.type == SDL_EVENT_KEY_DOWN && !event.key.repeat && (Command(event.key.key).Has(Command::FASTFORWARD)) &&
    ...
    // on keypress.
    if(Command(SDLK_CAPSLOCK).Has(Command::FASTFORWARD)) isFastForward = SDL_GetModState() & SDL_KMOD_CAPS;
  };
  ...
  if(!testContext.CurrentTest())
  {
    ...
    std::chrono::steady_clock::duration cpuLoadSum{};
    std::string                         cpuLoadString;
    std::chrono::steady_clock::duration gpuLoadSum{};
    std::string                         gpuLoadString;
    ...
    while(!menuPanels.IsDone())
    {
      ...
      Font::ShowUnderlines(mod & SDL_KMOD_ALT);
      ...
      ++cursorTime;
      const bool shouldShowCursor = (!GameWindow::IsFullscreen() || cursorTime < 600 || !inFlight);
      if(shouldShowCursor != showCursor)
      {
        showCursor = shouldShowCursor;
        if(showCursor) SDL_ShowCursor();
        else SDL_HideCursor();
      }
      ...
      const bool allowFastForward = !gamePanels.IsEmpty() && gamePanels.Top()->AllowsFastForward();
      ...
      if(mod & SDL_KMOD_CAPS && inFlight && debugMode)
      {
        if(frameRate > 10)
        {
        ...
        {
          cpuLoadSum += std::chrono::steady_clock::now() - start;
          continue;
        }
      }
      ...
      cpuLoadSum += std::chrono::steady_clock::now() - start;
      ++drawStep;
      std::chrono::steady_clock::time_point drawStart = std::chrono::steady_clock::now();

      if(GameWindow::GetInstance()->StartDraw(Screen::Width(), Screen::Height()))
      {
        GraphicsTypes::RenderState state;
        state.DepthTest                = false;
        state.Blending.BlendingEnabled = true;
        state.Blending.SRCAlpha        = GraphicsTypes::BlendFactor::FACTOR_ONE;
        ...
        if(Preferences::Has("Show CPU / GPU load"))
        {
          ...
          if(isPerformanceDisplayReady) performanceInfo.SetCondition("ready");
          ...
          if(drawStep == 60)
          {
            ...
            gpuLoadString = "GPU: " + Format::Number(gpuNano / 6e7, 2, false) + " ms (" +
                            Format::Percentage(gpuNano / 1e9, 0) + ")";
            gpuLoadSum = {};
    ...
    while(!menuPanels.IsDone())
    {
      ...
      if(dataFinishedLoading)
      {
        ...
        if(!integrationStepCounter)
        {
          ...
          if(menuPanels.IsEmpty()) mainPanel->GetEngine().Wait();
          ...
          if(menuPanels.IsEmpty()) mainPanel->GetEngine().GiveCommand(command);
        }
      }
  ...
  if(player.GetPlanet() && gamePanels.CanSave()) player.Save();
}
...
void PrintHelp()
{
  ...
  std::cerr << "    -d, --debug: turn on debugging features (e.g. Caps Lock slows down instead of speeds up)."
            << std::endl;
