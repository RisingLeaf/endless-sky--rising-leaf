...
System::Asteroid::Asteroid(const std::string &name, int count, double energy) : name(name), count(count), energy(energy)
{
}


System::Asteroid::Asteroid(const Minable *type, int count, double energy) : type(type), count(count), energy(energy) {}


const std::string &System::Asteroid::Name() const { return name; }


const Minable *System::Asteroid::Type() const { return type; }


int System::Asteroid::Count() const { return count; }


double System::Asteroid::Energy() const { return energy; }
...
void System::Load(const DataNode &node, Set<Planet> &planets, const ConditionsStore *playerConditions)
{
  ...
  std::set<const Planet *> removedObjectPlanets;
  ...
  for(const DataNode &child : node)
  {
    ...
    if(removeAll || overwriteAll)
    {
      ...
      else if(key == "object")
      {
        ...
        for(StellarObject &object : objects)
          if(object.planet) const_cast<Planet *>(object.planet)->RemoveSystem(this);
      ...
      if(overwriteAll) shouldOverwrite.erase(key == "minables" ? "asteroids" : key);
      else continue;
    }
    ...
    else if(key == "ramscoop")
    {
      for(const DataNode &grand : child)
      {
        ...
        if(key == "universal" && hasValue) universalRamscoop = grand.BoolValue(1);
        else if(key == "addend" && hasValue) ramscoopAddend = grand.Value(1);
        else if(key == "multiplier" && hasValue) ramscoopMultiplier = grand.Value(1);
    ...
    else if(key == "link")
    {
      ...
      if(remove) links.erase(GameData::Systems().Get(value));
      else links.insert(GameData::Systems().Get(value));
    }
    else if(key == "asteroids")
    {
      ...
      else if(child.Size() > valueIndex + 2)
      {
        ...
        child.PrintTrace(
            "Expected " + std::to_string(valueIndex + 3) + " tokens. Found " + std::to_string(child.Size()) + ":");
      }
    }
    ...
    else if(key == "object")
    {
      if(remove)
      {
        ...
        auto removeIt = find_if(
            objects.begin(),
            objects.end(),
            [&toRemoveTemplate](const StellarObject &object)
            {
              if(toRemoveTemplate.GetSprite() != object.GetSprite()) return false;
              if(toRemoveTemplate.distance != object.distance) return false;
              if(toRemoveTemplate.speed != object.speed) return false;
              if(toRemoveTemplate.offset != object.offset) return false;
              return true;
            });
        ...
        for(; last != objects.end() && last->parent >= index; ++last, ++removed)
          if(last->planet) removedObjectPlanets.insert(last->planet);
        if(removeIt->planet) removedObjectPlanets.insert(removeIt->planet);
        last = objects.erase(removeIt, last);

        // Recalculate every parent index.
        for(auto it = last; it != objects.end(); ++it)
          if(it->parent >= index) it->parent -= removed;
      }
      else {
    ...
    else if(key == "arrival")
    {
      ...
      for(const DataNode &grand : child)
      {
        ...
        if(type == "link" && grandHasValue) extraHyperArrivalDistance = grand.Value(1);
        else if(type == "jump" && grandHasValue) extraJumpArrivalDistance = fabs(grand.Value(1));
        else grand.PrintTrace("Skipping unsupported arrival distance limitation:");
      }
    }
    else if(key == "departure")
    {
      ...
      for(const DataNode &grand : child)
      {
        const std::string &type          = grand.Token(0);
        bool               grandHasValue = grand.Size() >= 2;
        if(type == "link" && grandHasValue) hyperDepartureDistance = grand.Value(1);
        else if(type == "jump" && grandHasValue) jumpDepartureDistance = fabs(grand.Value(1));
        else grand.PrintTrace("Skipping unsupported departure distance limitation:");
      }
    }
  ...
  for(StellarObject &object : objects)
  {
    ...
    {
      if(fraction < .5) object.message = &HOTMOON;
      else if(fraction >= 2.) object.message = &COLDMOON;
      else object.message = &UNINHABITEDMOON;
    }
    else {
      if(fraction < .5) object.message = &HOTPLANET;
      else if(fraction >= 2.) object.message = &COLDPLANET;
      else object.message = &UNINHABITEDPLANET;
    }
  }
  ...
  if(!hasPosition) node.PrintTrace("System will be ignored due to missing position:");
  // Systems without an asteroid belt defined default to a radius of 1500.
  if(belts.empty()) belts.emplace_back(1, 1500.);
...
void System::UpdateSystem(const Set<System> &systems, const std::set<double> &neighborDistances)
{
  ...
  if(!IsValid() || inaccessible) return;
  ...
  for(const System *link : links)
    if(link->IsValid() && !link->Inaccessible()) accessibleLinks.insert(link);
  ...
  for(const StellarObject &object : objects)
  {
    ...
    if(starIcon) mapIcons.emplace_back(starIcon);
  }
  ...
  if(IsInhabited(nullptr)) attributes.erase("uninhabited");
  else attributes.insert("uninhabited");
  ...
  if(minimumFleetPeriod == std::numeric_limits<int>::max()) minimumFleetPeriod = 0;
}
...
bool System::IsValid() const { return isDefined && hasPosition; }


const std::string &System::TrueName() const { return trueName; }
...
const std::string &System::DisplayName() const { return displayName; }


// Get this system's position in the star map.
const Point &System::Position() const { return position; }
...
const std::vector<const Sprite *> &System::GetMapIcons() const { return mapIcons; }


// Get the name of the ambient audio to play in this system.
const std::string &System::MusicName() const { return music; }


// Get the list of "attributes" of the planet.
const std::set<std::string> &System::Attributes() const { return attributes; }


// Get a list of systems you can travel to through hyperspace from here.
const std::set<const System *> &System::Links() const { return accessibleLinks; }
...
double System::ExtraHyperArrivalDistance() const
{
  ...
  if(arrivalGamerule.has_value()) return std::max(extraHyperArrivalDistance, *arrivalGamerule);
...
{
  const std::optional<double> arrivalGamerule = GameData::GetGamerules().SystemArrivalMin();
  if(arrivalGamerule.has_value()) return std::max(extraJumpArrivalDistance, *arrivalGamerule);
...
void System::SetDate(const Date &date)
{
  ...
  for(StellarObject &object : objects)
  {
    ...
    if(object.parent >= 0) object.position += objects[object.parent].position;

    if(object.position) object.angle = Angle(object.position);

    if(object.planet) object.planet->ResetDefense();
  }
}


// Get the stellar object locations on the most recently set date.
const std::vector<StellarObject> &System::Objects() const { return objects; }
...
const StellarObject *System::FindStellar(const Planet *planet) const
{
  ...
  {
    for(const StellarObject &object : objects)
      if(object.GetPlanet() == planet) return &object;
  }
...
double System::HabitableZone() const { return habitable; }


// Get the radius of an asteroid belt.
double System::AsteroidBeltRadius() const { return belts.Get(); }


// Get the list of asteroid belts.
const WeightedList<double> &System::AsteroidBelts() const { return belts; }


// Get the system's invisible fence radius.
double System::InvisibleFenceRadius() const { return invisibleFenceRadius; }


// Get how far ships can jump from this system.
double System::JumpRange() const { return jumpRange; }


double System::StarfieldDensity() const { return starfieldDensity; }
...
bool System::IsInhabited(const Ship *ship) const
{
  for(const StellarObject &object : objects)
  {
    if(object.HasSprite() && object.HasValidPlanet())
    {
      ...
      if(!planet.IsWormhole() && planet.IsInhabited() && planet.IsAccessible(ship)) return true;
    }
  }
...
{
  for(const StellarObject &object : objects)
    if(object.HasSprite() && object.HasValidPlanet() && object.GetPlanet()->HasFuelFor(ship)) return true;
...
{
  for(const StellarObject &object : objects)
    if(object.HasSprite() && object.HasValidPlanet() && object.GetPlanet()->HasShipyard()) return true;
...
{
  for(const StellarObject &object : objects)
    if(object.HasSprite() && object.HasValidPlanet() && object.GetPlanet()->HasOutfitter()) return true;
...
const std::vector<System::Asteroid> &System::Asteroids() const { return asteroids; }


// Get a list of all unique payload outfits from minables in this system.
const std::set<const Outfit *> &System::Payloads() const { return payloads; }


// Get the background haze sprite for this system.
const Sprite *System::Haze() const { return haze; }
...
bool System::HasTrade() const { return !trade.empty(); }
...
const std::vector<RandomEvent<Fleet>> &System::Fleets() const { return fleets; }


// Get the probabilities of various hazards in this system.
const std::vector<RandomEvent<Hazard>> &System::Hazards() const { return hazards; }
...
double System::Danger() const
{
  ...
  for(const auto &fleet : fleets)
  {
    ...
    if(gov && gov->IsEnemy()) danger += static_cast<double>(fleet.Get()->Strength()) / fleet.Period();
  }
  return danger;
}


int System::MinimumFleetPeriod() const { return minimumFleetPeriod; }
...
void System::LoadObject(const DataNode &node, Set<Planet> &planets, const ConditionsStore *playerConditions, int parent)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    if(key == "hazard" && child.Size() >= 3)
      object.hazards.emplace_back(GameData::Hazards().Get(child.Token(1)), child.Value(2), child, playerConditions);
    else if(key == "object") LoadObject(child, planets, playerConditions, index);
    else LoadObjectHelper(child, object);
  }
}
...
void System::LoadObjectHelper(const DataNode &node, StellarObject &object, bool removing) const
{
  ...
  else if(key == "visibility" && hasValue)
  {
    ...
    if(node.Size() >= 3) object.distanceVisible = node.Value(2);
  }
...
void System::UpdateNeighbors(const Set<System> &systems, double distance)
{
  ...
  for(const auto &it : systems)
  {
    ...
    if(!other.IsValid() || other.Inaccessible()) continue;

    if(&other != this && other.Position().Distance(position) <= distance) neighborSet.insert(&other);
  }
}
...
void System::Price::Update() { price = base + static_cast<int>(-100. * erf(supply / LIMIT)); }
