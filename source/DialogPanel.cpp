...
namespace
{
  ...
  const map<SDL_Keycode, char> KEY_MAP = {
      {SDLK_KP_0, '0'},      {SDLK_KP_1, '1'},          {SDLK_KP_2, '2'},          {SDLK_KP_3, '3'},
      {SDLK_KP_4, '4'},      {SDLK_KP_5, '5'},          {SDLK_KP_6, '6'},          {SDLK_KP_7, '7'},
      {SDLK_KP_8, '8'},      {SDLK_KP_9, '9'},          {SDLK_KP_AMPERSAND, '&'},  {SDLK_KP_AT, '@'},
      {SDLK_KP_A, 'a'},      {SDLK_KP_B, 'b'},          {SDLK_KP_C, 'c'},          {SDLK_KP_D, 'd'},
      {SDLK_KP_E, 'e'},      {SDLK_KP_F, 'f'},          {SDLK_KP_COLON, ':'},      {SDLK_KP_COMMA, ','},
      {SDLK_KP_DIVIDE, '/'}, {SDLK_KP_EQUALS, '='},     {SDLK_KP_EXCLAM, '!'},     {SDLK_KP_GREATER, '>'},
      {SDLK_KP_HASH, '#'},   {SDLK_KP_LEFTBRACE, '{'},  {SDLK_KP_LEFTPAREN, '('},  {SDLK_KP_LESS, '<'},
      {SDLK_KP_MINUS, '-'},  {SDLK_KP_MULTIPLY, '*'},   {SDLK_KP_PERCENT, '%'},    {SDLK_KP_PERIOD, '.'},
      {SDLK_KP_PLUS, '+'},   {SDLK_KP_POWER, '^'},      {SDLK_KP_RIGHTBRACE, '}'}, {SDLK_KP_RIGHTPAREN, ')'},
      {SDLK_KP_SPACE, ' '},  {SDLK_KP_VERTICALBAR, '|'}};
...
DialogPanel::~DialogPanel() { Audio::Resume(); }
...
DialogPanel *DialogPanel::CallFunctionIfOk(
...
DialogPanel *DialogPanel::MissionOfferDialog(
    std::string   message,
    PlayerInfo   &player,
    const System *system,
    Truncate      truncate,
...
void DialogPanel::Draw()
{
  ...
  if(canCancel)
  {
    ...
    if(numButtons == 3)
    {
      ...
      thirdPos =
          pos + Point(cancelPos.X() - (thirdButtonSprite->Width() + cancel->Width()) / 2 + BUTTON_RIGHT_MARGIN, 0.);
  ...
  if(AcceptsInput())
  {
    ...
    const auto inputText = DisplayText(
        input,
        {static_cast<int>(Width() - HORIZONTAL_PADDING - INPUT_HORIZONTAL_PADDING), Truncate::FRONT});
...
bool DialogPanel::AllowsFastForward() const noexcept { return allowsFastForward; }
...
void DialogPanel::Resize()
{
  ...
  int height = TOP_PADDING + textRectSize.Y() + BOTTOM_PADDING + (realBottomHeight - BOTTOM_PADDING) * AcceptsInput();
  // Determine how many extension panels we need.
  if(height <= realBottomHeight + top->Height()) extensionCount = 0;
  else extensionCount = (height - middle->Height()) / middle->Height();
...
bool DialogPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  bool isCloseRequest = key == SDLK_ESCAPE || (key == 'w' && (mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI)));
  ...
  else if((it != KEY_MAP.end() || (key >= ' ' && key <= '~')) && AcceptsInput() && !isCloseRequest)
  {
    ...
    char c     = ((mod & SDL_KMOD_SHIFT) ? SHIFT[ascii] : ascii);
    // Caps lock should shift letters, but not any other keys.
    if((mod & SDL_KMOD_CAPS) && c >= 'a' && c <= 'z') c += 'A' - 'a';

    if(stringFun) input += c;
    // Integer and double inputs only allow certain characters.
    else if((intFun || doubleFun) && c >= '0' && c <= '9') input += c;
    // Both integer and double input can start with a minus sign.
    else if((intFun || doubleFun) && c == '-' && input.empty()) input += c;
    // Double input should only allow a single decimal point.
    else if(doubleFun && c == '.' && !std::count(input.begin(), input.end(), '.')) input += c;
  ...
  {
    // To the left, 1->2->3->3
    if(activeButton < numButtons) activeButton++;
  }
  else if(key == SDLK_RIGHT)
  {
    // To the right, 3->2->1->1
    if(activeButton > 1) activeButton--;
  }
  else if(
      key == SDLK_RETURN || key == SDLK_KP_ENTER || key == SDLK_SPACE || isCloseRequest ||
      (isMission && (key == 'a' || key == 'd')) || (numButtons == 3 && key == buttonThree.buttonKey))
  {
    // Note: The key shortcuts only work when there is no stringFun defined, else they are being typed out.
    if(key == 'a' || (!canCancel && isCloseRequest)) activeButton = 1;
    if(key == 'd' || (canCancel && isCloseRequest)) activeButton = 2;
    if(key == buttonThree.buttonKey && numButtons == 3) activeButton = 3;
    ...
    else if(activeButton == 3)
    {
      ...
      if(buttonThree.buttonAction && buttonThree.buttonAction(input)) GetUI().Pop(this);
    }
    else {
      GetUI().Pop(this);
    }
...
void DialogPanel::DoCallback(const bool isOk) const
{
  if(isMission)
  {
    if(intFun) intFun(activeButton == 1 ? Endpoint::ACCEPT : Endpoint::DECLINE);
  ...
  if(stringFun) stringFun(input);

  if(voidFun) voidFun();

  if(boolFun) boolFun(isOk);
}
...
bool DialogPanel::AcceptsInput() const { return !isMission && (intFun || doubleFun || stringFun); }


bool DialogPanel::ValidateInput() const
{
  if(validateStringFun) return validateStringFun(input);

  try
  {
    if(validateIntFun) return validateIntFun(stoi(input));
    if(validateDoubleFun) return validateDoubleFun(stod(input));
  }
  catch(...)
  {
    return false;
