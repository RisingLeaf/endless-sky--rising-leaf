...
namespace
{
  ...
  int Precedence(const ConditionSet::ExpressionOp op)
  {
    switch(op)
    {
    case ConditionSet::ExpressionOp::INVALID: return 9;
    case ConditionSet::ExpressionOp::LIT:
    case ConditionSet::ExpressionOp::VAR:     return 8;
    case ConditionSet::ExpressionOp::MUL:
    case ConditionSet::ExpressionOp::DIV:
    case ConditionSet::ExpressionOp::MOD:     return 6;
    case ConditionSet::ExpressionOp::ADD:
    case ConditionSet::ExpressionOp::SUB:     return 5;
    ...
    case ConditionSet::ExpressionOp::LE:      return 3;
    default:
  ...
  pair<ConditionSet::ExpressionOp, uint64_t> ConvertToken(const string &stringToken)
  {
    ...
    if(it != CS_TOKEN_CONVERSION.end()) return it->second;

    // Try number conversion, for if this is a number.
    if(DataNode::IsNumber(stringToken)) return make_pair(ConditionSet::ExpressionOp::LIT, TERMINAL);

    // Try variable name conversion, for if this is a condition variable.
    if(DataNode::IsConditionName(stringToken)) return make_pair(ConditionSet::ExpressionOp::VAR, TERMINAL);
...
ConditionSet::ConditionSet(const ConditionsStore *conditions) { this->conditions = conditions; }


// Construct and Load() at the same time.
ConditionSet::ConditionSet(const DataNode &node, const ConditionsStore *conditions) { Load(node, conditions); }
...
{
  if(!conditions) throw runtime_error("Unable to Load ConditionSet without a pointer to a ConditionsStore!");
  this->conditions = conditions;
...
{
  auto it = find_if(
      CS_TOKEN_CONVERSION.begin(),
      CS_TOKEN_CONVERSION.end(),
...
void ConditionSet::SaveInline(DataWriter &out) const
{
  ...
  {
  case ExpressionOp::INVALID: out.WriteToken("never"); return;
  case ExpressionOp::VAR:     out.WriteToken(conditionName); return;
  case ExpressionOp::LIT:     out.WriteToken(literal); return;
  default:                    break;
  }
  ...
  if((it->second.second & INFIX_OPERATOR) && !children.empty())
  {
    ...
    if(children.size() > 1) out.WriteToken("(");
...
bool ConditionSet::IsEmpty() const
{
  ...
  return (expressionOperator == ExpressionOp::AND && children.size() == 0) ||
         (expressionOperator == ExpressionOp::INVALID);
}


// Check if the conditionset contains valid data
bool ConditionSet::IsValid() const { return expressionOperator != ExpressionOp::INVALID; }


bool ConditionSet::Test() const { return Evaluate(); }
  ...
  case ExpressionOp::VAR:
    {
      ...
      {
        throw runtime_error(
            "Unable to Evaluate ExpressionOp::VAR with condition name \"" + conditionName +
            "\" in ConditionSet without a pointer to a ConditionsStore!");
      }
      return conditions->Get(conditionName);
    }
  case ExpressionOp::LIT: return literal;
...
set<string> ConditionSet::RelevantConditions() const
{
  ...
  if(expressionOperator == ExpressionOp::VAR) result.emplace(conditionName);
...
bool ConditionSet::ParseFromStart(const DataNode &node)
{
  ...
  if(node.HasChildren()) return FailParse(node, "Unexpected child-nodes under toplevel");

  // Special handling for 'never', 'has' and 'not' nodes.
  if(key == "never")
  {
    if(node.Size() > 1) return FailParse(node, "Tokens found after never keyword");
  ...
  int tokenNr = 0;
  if(!ParseNode(node, tokenNr)) return false;
...
bool ConditionSet::ParseNode(const DataNode &node, int &tokenNr)
{
  ...
  // Nodes beyond this point should not have children.
  if(node.HasChildren()) return FailParse(node, "Unexpected child-nodes under arithmetic expression");

  if(!ParseGreedy(node, tokenNr)) return FailParse();

  // Parsing from infix should have consumed and parsed all tokens.
  if(tokenNr < node.Size()) return FailParse(node, "Tokens found after parsing full expression");
...
bool ConditionSet::Optimize(const DataNode &node)
{
  ...
  case ExpressionOp::OR:
    ...
    if(children.size() == 1) *this = children[0];
  ...
  case ExpressionOp::VAR:
  case ExpressionOp::INVALID: break;
  }
  return returnValue;
}
...
bool ConditionSet::ParseChildren(const DataNode &node)
{
  ...
  if(!node.HasChildren()) return FailParse(node, "Child-nodes expected, found none");
  ...
  for(const DataNode &child : node)
  {
    ...
    if(children.back().expressionOperator == ExpressionOp::INVALID) return FailParse();
  }
...
{
  if(!conditions) throw runtime_error("Unable to ParseMini in a ConditionSet without a pointer to a ConditionsStore!");

  if(tokenNr >= node.Size()) return FailParse(node, "Expected terminal or sub-expression, found none");
  ...
  if(node.Token(tokenNr) == "(")
  {
    ++tokenNr;
    if(!ParseGreedy(node, tokenNr)) return FailParse();
    if(tokenNr >= node.Size() || node.Token(tokenNr) != ")") return FailParse(node, "Missing closing bracket");

    // Make sure that this bracketed section gets used as a single terminal.
    if(!PushDownFull(node)) return FailParse();
  ...
  case ExpressionOp::VAR:
    ...
    ++tokenNr;
    break;
  default: return FailParse(node, "Expected terminal or open-bracket");
  }
...
bool ConditionSet::ParseGreedy(const DataNode &node, int &tokenNr)
{
  ...
  if(!ParseMini(node, tokenNr)) return FailParse();

  // If we are at the end here, then we are done
  if(tokenNr >= node.Size()) return true;
  ...
  if(node.Token(tokenNr) == ")" || node.Token(tokenNr) == ",") return true;
  ...
  if(!ParseFromInfix(node, tokenNr, -1)) return FailParse();
...
bool ConditionSet::ParseFunctionBody(const DataNode &node, int &tokenNr)
{
  ...
  if(tokenNr >= node.Size()) return FailParse(node, "Expected function body, found none");

  // Any function body should start with an opening bracket
  if(node.Token(tokenNr) != "(") return FailParse(node, "Expected function body, but missing opening bracket");

  ++tokenNr;
  children.emplace_back(conditions);
  if(!children.back().ParseGreedy(node, tokenNr)) return FailParse();

  while(node.Token(tokenNr) != ")")
  {
    if(node.Token(tokenNr) != ",") return FailParse(node, "Expected function terminator, or separator");
...
bool ConditionSet::ParseFromInfix(const DataNode &node, int &tokenNr, int parentPrecedence)
{
  ...
  while(true)
  {
    ...
    if(tokenNr >= node.Size() || node.Token(tokenNr) == ")" || node.Token(tokenNr) == ",") return true;
    ...
    if(Precedence(infixOp.first) <= parentPrecedence) return true;
    ...
    {
      if(!PushDownLast(node)) return FailParse();
      if(!children.back().ParseFromInfix(node, tokenNr, Precedence(expressionOperator))) return FailParse();
    ...
    if(children.size() == 1) expressionOperator = infixOp.first;
    ...
    {
      ++tokenNr;
      if(!((children.emplace_back(conditions)).ParseMini(node, tokenNr))) return FailParse();
...
bool ConditionSet::PushDownLast(const DataNode &node)
{
  ...
  if(children.empty()) return FailParse(node, "Cannot create sub-expression from void");
