...
const std::string &TestData::Name() const { return dataSetName; }
...
void TestData::Load(const DataNode &node, const std::filesystem::path &sourceDataFilePath)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    {
      if(child.Token(1) == "savegame") dataSetType = Type::SAVEGAME;
      else if(child.Token(1) == "mission") dataSetType = Type::MISSION;
      else child.PrintTrace("Skipping unsupported category:");
    }
  }
}
    ...
    const std::set<const Planet *> *visitedPlanets) const
{
  ...
  if(dataSetName.empty() || sourceDataFile.empty()) return false;

  // Determine data-type and call the relevant function.
  switch(dataSetType)
  {
  case Type::SAVEGAME: return InjectSavegame();
  case Type::MISSION:  return InjectMission(playerConditions, visitedSystems, visitedPlanets);
  ...
  for(const DataNode &rootNode : sourceData)
  {
    ...
    if(rootNode.Size() > 1 && rootNode.Token(0) == "test-data" && rootNode.Token(1) == dataSetName)
    {
      ...
      for(const DataNode &dataNode : rootNode)
        if(dataNode.Token(0) == "contents") return &dataNode;
    }
  }
  return nullptr;
}
