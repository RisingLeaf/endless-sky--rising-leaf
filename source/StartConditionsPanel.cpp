...
void StartConditionsPanel::Draw()
{
  ...
  for(auto it = scenarios.begin(); it != scenarios.end(); ++it, pos += Point(0., entryBox.Height()))
  {
    ...
    if(!(entriesContainer.Contains(zone.TopLeft()) || entriesContainer.Contains(zone.BottomRight()))) continue;
    ...
    if(it == startIt) FillShader::Fill(zone, selectedBackground.Additive(opacity));
...
bool StartConditionsPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool /* isNewPress */)
{
  ...
  else if(!scenarios.empty() && (key == SDLK_UP || key == SDLK_DOWN || key == SDLK_PAGEUP || key == SDLK_PAGEDOWN))
  {
    ...
    if(key == SDLK_UP || key == SDLK_PAGEUP)
      if(startIt == scenarios.begin()) startIt = scenarios.end() - 1;
      else startIt -= std::min(magnitude, distance(scenarios.begin(), startIt));
    else if(startIt + 1 == scenarios.end()) startIt = scenarios.begin();
    else startIt += std::min(magnitude, distance(startIt, scenarios.end() - 1));

    Select(startIt);
  }
  else if(
      startIt != scenarios.end() && (key == 's' || key == 'n' || key == SDLK_KP_ENTER || key == SDLK_RETURN) &&
...
bool StartConditionsPanel::Click(int x, int y, MouseButton button, int /* clicks */)
{
  ...
  if(!entriesContainer.Contains(Point(x, y))) return false;

  for(const auto &it : startConditionsClickZones)
  {
    if(it.Contains(Point(x, y + entriesScroll)))
    {
      if(startIt != it.Value()) Select(it.Value());
      UI::PlaySound(UI::UISound::NORMAL);
      return true;
    }
  }
...
bool StartConditionsPanel::Drag(double /* dx */, double dy)
{
  ...
  {
    entriesScroll =
        std::max(0., std::min(entriesScroll - dy, scenarios.size() * entryBox.Height() - entriesContainer.Height()));
  }
...
bool StartConditionsPanel::Scroll(double /* dx */, double dy) { return Drag(0., dy * Preferences::ScrollSpeed()); }
...
void StartConditionsPanel::Select(StartConditionsList::iterator it)
{
  ...
  if(startIt->IsUnlocked()) info.SetCondition("unlocked start");
  if(startIt->GetThumbnail()) info.SetSprite("thumbnail", startIt->GetThumbnail());
