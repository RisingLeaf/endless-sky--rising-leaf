...
{
  if(powerD.empty() || powerA.empty() || !capture.empty()) return;
...
double CaptureOdds::Odds(int attackingCrew, int defendingCrew) const
{
  ...
  if(attackingCrew < 2 || index < 0) return 0.;
...
double CaptureOdds::AttackerCasualties(int attackingCrew, int defendingCrew) const
{
  ...
  int index = Index(attackingCrew, defendingCrew);
  if(attackingCrew < 2 || !defendingCrew || index < 0) return 0.;
...
{
  if(static_cast<unsigned>(attackingCrew - 1) >= powerA.size()) return 0.;
...
{
  if(static_cast<unsigned>(defendingCrew - 1) >= powerD.size()) return 0.;
...
{
  if(static_cast<unsigned>(attackingCrew - 1) > powerA.size()) return -1;
  if(static_cast<unsigned>(defendingCrew - 1) > powerD.size()) return -1;
...
std::vector<double> CaptureOdds::Power(const Ship &ship, bool isDefender)
{
  ...
  for(const auto &it : ship.Outfits())
  {
    ...
    if(value > 0. && it.second > 0) power.insert(power.end(), it.second, value);
  }
