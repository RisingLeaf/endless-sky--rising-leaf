...
namespace
{
  ...
  template <class Type>
  void
  PrintItemSales(const Set<Type> &items, const Set<Shop<Type>> &sales, const string &itemNoun, const string &saleNoun)
  {
    ...
    {
      if(itemIt.first != ObjectName(itemIt.second)) continue;
  ...
  void PrintObjectsByAttribute(const Set<Type> &objects, const string &name)
  {
    ...
    for(const string &attribute : attributes)
    {
      ...
      {
        const Type &object = it.second;
        if(object.Attributes().contains(attribute)) cout << (index++ ? ';' : ',') << DataWriter::Quote(it.first);
      }
      cout << '\n';
    }
  }
  ...
  void Ships(const char *const *argv)
  {
    auto PrintBaseShipStats = []() -> void
    {
      cout << "model" << ',' << "category" << ',' << DataWriter::Quote("chassis cost") << ','
           ...
           << ',' << DataWriter::Quote("heat dissipation") << ',' << DataWriter::Quote("required crew") << ','
           << "bunks" << ',' << DataWriter::Quote("cargo space") << ',' << "fuel" << ','
           << DataWriter::Quote("outfit space") << ',' << DataWriter::Quote("weapon capacity") << ','
           << DataWriter::Quote("engine capacity") << ',' << DataWriter::Quote("gun mounts") << ','
           << DataWriter::Quote("turret mounts") << ',' << DataWriter::Quote("fighter bays") << ','
           << DataWriter::Quote("drone bays") << '\n';
      ...
      for(auto &it : GameData::Ships())
      {
        ...
        for(auto &hardpoint : ship.Weapons())
          if(hardpoint.IsTurret()) ++numTurrets;
          else ++numGuns;
    ...
    auto PrintLoadedShipStats = [](bool variants) -> void
    {
      ...
      for(auto &it : GameData::Ships())
      {
        ...
        if(it.second.TrueModelName() != it.first && !variants) continue;
        ...
        for(const Hardpoint &hardpoint : ship.Weapons())
        {
          ...
          if(weapon)
          {
            if(weapon->Ammo() && !ship.OutfitCount(weapon->Ammo())) continue;
    ...
    {
      string arg = *it;
      if(arg == "--variants") variants = true;
      else if(arg == "--sales") sales = true;
      else if(arg == "--loaded") loaded = true;
      else if(arg == "--list") list = true;
    }

    if(sales) PrintItemSales(GameData::Ships(), GameData::Shipyards(), "ship", "shipyards");
    else if(loaded) PrintLoadedShipStats(variants);
    else if(list) PrintShipList(variants);
    else PrintBaseShipStats();
  }
  ...
  void Outfits(const char *const *argv)
  {
    auto PrintWeaponStats = []() -> void
    {
      ...
      for(auto &it : GameData::Outfits())
      {
        ...
        if(!weapon || outfit.Category().empty()) continue;
    ...
    auto PrintEngineStats = []() -> void
    {
      ...
      {
        // Skip non-engines.
        if(it.second.Category() != "Engines") continue;
    ...
    auto PrintPowerStats = []() -> void
    {
      ...
      {
        // Skip non-power.
        if(it.second.Category() != "Power") continue;
    ...
    {
      string arg = *it;
      if(arg == "-w" || arg == "--weapons") weapons = true;
      else if(arg == "-e" || arg == "--engines") engines = true;
      else if(arg == "--power") power = true;
      else if(arg == "-s" || arg == "--sales") sales = true;
      else if(arg == "-a" || arg == "--all") all = true;
    }

    if(weapons) PrintWeaponStats();
    else if(engines) PrintEngineStats();
    else if(power) PrintPowerStats();
    else if(sales) PrintItemSales(GameData::Outfits(), GameData::Outfitters(), "outfit", "outfitters");
    else if(all) PrintOutfitsAllStats();
    else PrintObjectList(GameData::Outfits(), "outfit");
  }
  ...
  void Sales(const char *const *argv)
  {
    ...
    {
      string arg = *it;
      if(arg == "-s" || arg == "--ships") ships = true;
      else if(arg == "-o" || arg == "--outfits") outfits = true;
    }
    ...
    if(ships) PrintSales(GameData::Shipyards(), "shipyards", "ships");
    if(outfits) PrintSales(GameData::Outfitters(), "outfitters", "outfits");
  }
  ...
  void Planets(const char *const *argv)
  {
    ...
    {
      string arg = *it;
      if(arg == "--descriptions") descriptions = true;
      else if(arg == "--attributes") attributes = true;
      else if(arg == "--reverse") byAttribute = true;
    }
    if(descriptions) PrintPlanetDescriptions();
    if(attributes && byAttribute) PrintObjectsByAttribute(GameData::Planets(), "planets");
    else if(attributes) PrintObjectAttributes(GameData::Planets(), "planet");
    if(!(descriptions || attributes)) PrintObjectList(GameData::Planets(), "planet");
  }
  ...
  void Systems(const char *const *argv)
  {
    ...
    {
      string arg = *it;
      if(arg == "--attributes") attributes = true;
      else if(arg == "--reverse") byAttribute = true;
    }
    if(attributes && byAttribute) PrintObjectsByAttribute(GameData::Systems(), "systems");
    else if(attributes) PrintObjectAttributes(GameData::Systems(), "system");
    else PrintObjectList(GameData::Systems(), "system");
  }
  ...
  void LocationFilterMatches(const char *const *argv, PlayerInfo &player)
  {
    ...
    for(const auto &it : GameData::Systems())
      if(filter.Matches(&it.second)) cout << it.first << '\n';
  ...
  void Changes()
  {
    ...
    if(recentPath.empty() || !Files::Exists(recentPath)) return;
    ...
    for(const DataNode &node : file)
    {
      if(node.Token(0) != "changes") continue;
  ...
  const set<string> OTHER_VALID_ARGS = {"-s", "--ships", "--sales", "--planets", "--systems", "--matches", "--changes"};
} // namespace
...
bool PrintData::IsPrintDataArgument(const char *const *argv)
{
  ...
  {
    string arg = *it;
    if(OTHER_VALID_ARGS.contains(arg) || OUTFIT_ARGS.contains(arg)) return true;
  }
  return false;
}
...
void PrintData::Help()
{
  ...
  cerr << "            --reverse: prints a table of all planet attributes and which planets have them." << endl;
  ...
  cerr << "    --matches: prints a list of all planets and systems matching a location filter passed in STDIN." << endl;
