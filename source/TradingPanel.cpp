...
namespace
{
  const std::string TRADE_LEVEL[] = {"(very low)", "(low)", "(medium)", "(high)", "(very high)"};
...
TradingPanel::~TradingPanel()
{
  if(profit)
  {
    ...
    if(profit < 0) message += "at a loss of " + Format::CreditString(-profit) + ".";
    else message += "for a total profit of " + Format::CreditString(profit) + ".";
...
void TradingPanel::Step() { DoHelp("trading"); }
...
void TradingPanel::Draw()
{
  ...
  if(player.Cargo().HasOutfits() || missionCargo)
  {
    ...
    if(hasMinables && missionCargo) str += "mission cargo and other items.";
    else if(hasOutfits && missionCargo) str += "outfits and mission cargo.";
    else if(hasOutfits && hasMinables) str += "outfits and special commodities.";
    else if(hasOutfits) str += "outfits.";
    else if(hasMinables) str += "special commodities.";
    else str += "mission cargo.";
  ...
  for(const Trade::Commodity &commodity : GameData::Commodities())
  {
    ...
    if(price)
    {
      ...
      if(level < 0) level = 0;
      else if(level >= (commodity.high - commodity.low)) level = 4;
      else level = (5 * level) / (commodity.high - commodity.low);
  ...
  if(showProfit) font.Draw("Profit", Point(MIN_X + PROFIT_X, FIRST_Y), selected);

  Information info;
  if(sellOutfits) info.SetCondition("can sell outfits");
  else if(player.Cargo().HasOutfits() || canSell) info.SetCondition("can sell");
  if(player.Cargo().Free() > 0 && canBuy) info.SetCondition("can buy");
...
bool TradingPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  else if(key == 'u' || key == 'B' || (key == 'b' && (mod & SDL_KMOD_SHIFT)))
  {
    Buy(1'000'000'000);
  }
  else if(key == 'e' || key == 'S' || (key == 's' && (mod & SDL_KMOD_SHIFT)))
  {
    ...
    for(const auto &it : player.Cargo().Outfits())
    {
      ...
      const int64_t      &amount = it.second;
      if(outfit->Get("minable") <= 0. && !sellOutfits) continue;
...
bool TradingPanel::Click(int x, int y, MouseButton button, int clicks)
{
  ...
  if(x >= MIN_X && x <= MAX_X && y >= FIRST_Y + 25 && y < maxY)
  {
    ...
    if(x >= MIN_X + BUY_X && x < MIN_X + SELL_X) Buy(1);
    else if(x >= MIN_X + SELL_X && x < MIN_X + HOLD_X) Buy(-1);
  }
  else {
    return false;
  }
...
{
  int selectedRow = player.MapColoring();
  if(selectedRow < 0 || selectedRow >= COMMODITY_COUNT) return;
