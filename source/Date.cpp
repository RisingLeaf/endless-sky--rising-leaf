...
namespace
{
  ...
  std::string ZeroPad(int i) { return (i < 10 ? "0" : "") + std::to_string(i); }
...
std::string Date::LongString() const
{
  ...
  if(day / 10 == 1 || day % 10 == 0 || day % 10 > 3) dayString += "th";
  else if(day % 10 == 1) dayString += "st";
  else if(day % 10 == 2) dayString += "nd";
  else dayString += "rd";
  ...
  else if(dateFormat == Preferences::DateFormat::DMY) result = "the " + dayString + " of " + month;
...
int Date::operator-(const Date &other) const { return DaysSinceEpoch() - other.DaysSinceEpoch(); }
...
int Date::DaysSinceEpoch() const
{
  if(date && !daysSinceEpoch)
  {
    ...
    if(month > 2 && !(year % 4) && ((year % 100) || !(year % 400))) ++daysSinceEpoch;
...
{
  if(Year() % 4) return 365 - DaysSinceYearStart();
