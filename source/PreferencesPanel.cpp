      ...
      GameData::Colors().Get("medium"))
{
  ...
  for(const auto &plugin : Plugins::Get())
    if(plugin.second.IsValid()) pluginListHeight += 20;
...
void PreferencesPanel::Draw()
{
  ...
  for(const auto &[bar, category] : volumeBars)
  {
    ...
    if(volume > .75) info.SetCondition(bar + " max");
    else if(volume > .5) info.SetCondition(bar + " medium");
    else if(volume > .25) info.SetCondition(bar + " low");
    else info.SetCondition(bar + " none");
  }

  if(Plugins::HasChanged()) info.SetCondition("show plugins changed");
  if(CONTROLS_PAGE_COUNT > 1) info.SetCondition("multiple controls pages");
  if(currentControlsPage > 0) info.SetCondition("show previous controls");
  if(currentControlsPage + 1 < CONTROLS_PAGE_COUNT) info.SetCondition("show next controls");
  if(SETTINGS_PAGE_COUNT > 1) info.SetCondition("multiple settings pages");
  if(currentSettingsPage > 0) info.SetCondition("show previous settings");
  if(currentSettingsPage + 1 < SETTINGS_PAGE_COUNT) info.SetCondition("show next settings");
...
void PreferencesPanel::UpdateTooltipActivation() { tooltip.UpdateActivationCount(); }


bool PreferencesPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool)
{
  ...
  else if(key == 'o' && page == 'p')
  {
    Files::OpenUserPluginFolder();
      ...
      (key == 'n' || key == SDLK_PAGEUP) && ((page == 'c' && currentControlsPage < CONTROLS_PAGE_COUNT - 1) ||
                                             (page == 's' && currentSettingsPage < SETTINGS_PAGE_COUNT - 1)))
  {
    if(page == 'c') ++currentControlsPage;
    else ++currentSettingsPage;
    selected = 0;
    selectedItem.clear();
  }
  else if(
      (key == 'r' || key == SDLK_PAGEDOWN) &&
...
bool PreferencesPanel::Scroll(double dx, double dy)
{
  ...
  if(page == 's' && !hoverItem.empty())
  {
    if(hoverItem == ZOOM_FACTOR)
    {
      ...
      if(dy < 0. && zoom > ZOOM_FACTOR_MIN) zoom -= ZOOM_FACTOR_INCREMENT;
      if(dy > 0. && zoom < ZOOM_FACTOR_MAX) zoom += ZOOM_FACTOR_INCREMENT;
    ...
    else if(hoverItem == VIEW_ZOOM_FACTOR)
    {
      ...
      else Preferences::ZoomViewIn();
    }
    else if(hoverItem == SCROLL_SPEED)
    {
      int speed = Preferences::ScrollSpeed();
      if(dy < 0.) speed = std::max(10, speed - 10);
      else speed = std::min(60, speed + 10);
...
void PreferencesPanel::EndEditing() { editing = -1; }
...
void PreferencesPanel::DrawControls()
{
  ...
  if(selected != oldSelected) latest = selected;
  if(hover != oldHover) latest = hover;
  ...
  for(const Command &command : COMMANDS)
  {
    ...
    if(!command)
    {
      if(category != end(CATEGORIES)) categoryString = *category++;
      else table.Advance();
    ...
    if(page < currentControlsPage) continue;
    else if(page > currentControlsPage) break;
    if(!command)
    {
...
void PreferencesPanel::DrawSettings()
{
  ...
  for(const std::string &setting : SETTINGS)
  {
    ...
    if(page < currentSettingsPage) continue;
    else if(page > currentSettingsPage) break;
    // Check if this is a category break or column break.
    if(setting.empty() || setting == "\t")
    {
      isCategory = true;
      if(!setting.empty()) table.DrawAt(Point(130, firstY));
      continue;
    }
    ...
    else if(setting == REACTIVATE_HELP)
    {
      ...
      for(const auto &it : help)
      {
        ...
        for(const std::string &str : SPECIAL_HELP)
          if(it.first.find(str) == 0) special = true;
  ...
  if(selectedItem.empty()) selectedItem = prefZones.at(selected).Value();
}
...
void PreferencesPanel::PreDrawPlugins()
{
  ...
  for(const auto &it : Plugins::Get())
  {
    ...
    if(isSelected || plugin.name == hoverItem) table.DrawHighlight(back);
    ...
    if(displayed) AddZone(zoneBounds, [&]() { Plugins::TogglePlugin(plugin.name); });
    if(isSelected) table.Draw(plugin.name, bright);
    else table.Draw(plugin.name, plugin.enabled ? medium : dim);
  }
...
void PreferencesPanel::DrawPlugins()
{
  ...
  pluginListClip->SetFadePadding(pluginListScroll.IsScrollAtMin() ? 0 : 20, pluginListScroll.IsScrollAtMax() ? 0 : 20);
  ...
  if(pluginListScroll.Scrollable())
  {
    ...
    PointerShader::Draw(
        ...
        10.f,
        5.f,
        Color(pluginListScroll.IsScrollAtMin() ? .2f : .8f, 0.f));
...
void PreferencesPanel::RenderPluginDescription(const std::string &pluginName)
{
  ...
  if(plugin) RenderPluginDescription(*plugin);
  else pluginDescriptionBuffer.reset();
}
...
void PreferencesPanel::RenderPluginDescription(const Plugin &plugin)
{
  ...
  if(sprite) descriptionHeight += sprite->Height() + 10;
  ...
  pluginDescriptionBuffer = std::make_unique<RenderBuffer>(Point(box.Width(), descriptionHeight), "plugin_description");
...
void PreferencesPanel::HandleSettingsString(const std::string &str, Point cursorPosition)
{
  ...
  if(str == "Deadline blink by distance" && !player.GetPlanet()) recacheDeadlines = !recacheDeadlines;
}
...
void PreferencesPanel::HandleUp()
{
  ...
  switch(page)
  {
  case 's': selectedItem = prefZones.at(selected).Value(); break;
  case 'p':
...
void PreferencesPanel::HandleDown()
{
  ...
  {
  case 'c':
    if(selected + 1 < static_cast<int>(zones.size())) selected++;
    break;
  case 's':
...
void PreferencesPanel::HandleConfirm()
{
  switch(page)
  {
  case 'c': editing = selected; break;
  case 's': HandleSettingsString(selectedItem, Screen::Dimensions() / 2.); break;
  case 'p': Plugins::TogglePlugin(selectedPlugin); break;
  default:  break;
  }
}
