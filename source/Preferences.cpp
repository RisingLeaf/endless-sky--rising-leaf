...
namespace
{
  ...
  public:
    ...
    void Increment()
    {
      switch(state)
      {
      case Preferences::OverlayState::OFF:      state = Preferences::OverlayState::ON; break;
      case Preferences::OverlayState::ON:       state = Preferences::OverlayState::DAMAGED; break;
      case Preferences::OverlayState::DAMAGED:  state = Preferences::OverlayState::ON_HIT; break;
      case Preferences::OverlayState::ON_HIT:   state = Preferences::OverlayState::OFF; break;
      case Preferences::OverlayState::DISABLED: state = Preferences::OverlayState::OFF; break;
      }
    }
...
void Preferences::Load()
{
  ...
  for(const DataNode &node : prefs)
  {
    ...
    if(key == "window size" && node.Size() >= 3) Screen::SetRaw(node.Value(1), node.Value(2), true);
    else if(key == "zoom" && hasValue) Screen::SetZoom(node.Value(1), true);
    else if(VOLUME_SETTINGS.contains(key) && hasValue)
      Audio::SetVolume(node.Value(1) * VOLUME_SCALE, VOLUME_SETTINGS.at(key));
    else if(key == "scroll speed" && hasValue) scrollSpeed = node.Value(1);
    else if(key == "Tooltip activation time" && hasValue) tooltipActivation = node.Value(1);
    ...
    else if(key == "view zoom") zoomIndex = max(0., node.Value(1));
    else if(key == "vsync") vsyncIndex = max<int>(0, min<int>(node.Value(1), VSYNC_SETTINGS.size() - 1));
    ...
    else if(key == "Show all status overlays") statusOverlaySettings[OverlayType::ALL].SetState(node.Value(1));
    else if(key == "Show flagship overlay") statusOverlaySettings[OverlayType::FLAGSHIP].SetState(node.Value(1));
    else if(key == "Show escort overlays") statusOverlaySettings[OverlayType::ESCORT].SetState(node.Value(1));
    else if(key == "Show enemy overlays") statusOverlaySettings[OverlayType::ENEMY].SetState(node.Value(1));
    else if(key == "Show neutral overlays") statusOverlaySettings[OverlayType::NEUTRAL].SetState(node.Value(1));
    ...
    else if(key == "date format") dateFormatIndex = max<int>(0, min<int>(node.Value(1), DATEFMT_OPTIONS.size() - 1));
    ...
    else if(key == "previous saves" && hasValue) previousSaveCount = max<int>(3, node.Value(1));
    else if(key == "alt-mouse turning") settings["Control ship with mouse"] = (!hasValue || node.Value(1));
...
#endif
    else settings[key] = (node.Size() == 1 || node.Value(1));
  }
  ...
  if(it != settings.end())
  {
    if(!it->second) alertIndicatorIndex = 2;
    settings.erase(it);
  }
  ...
  if(it != settings.end())
  {
    if(it->second) statusOverlaySettings[OverlayType::ALL] = OverlayState::DISABLED;
    settings.erase(it);
  }
  ...
  if(it != settings.end())
  {
    if(!it->second) flotsamIndex = static_cast<int>(FlotsamCollection::ESCORT);
    settings.erase(it);
  }
}
...
#ifdef _WIN32
  if(WinVersion::SupportsDarkTheme()) out.Write("Title bar theme", titleBarThemeIndex);
  if(WinVersion::SupportsWindowRounding()) out.Write("Window rounding", windowRoundingIndex);
#endif
...
void Preferences::Set(const string &name, bool on) { settings[name] = on; }
...
string Preferences::AmmoUsage() { return Has(EXPEND_AMMO) ? Has(FRUGAL_ESCORTS) ? "frugally" : "always" : "never"; }


void Preferences::ToggleDateFormat()
{
  if(dateFormatIndex == static_cast<int>(DATEFMT_OPTIONS.size() - 1)) dateFormatIndex = 0;
  else ++dateFormatIndex;
}


Preferences::DateFormat Preferences::GetDateFormat() { return static_cast<DateFormat>(dateFormatIndex); }


const string &Preferences::DateFormatSetting() { return DATEFMT_OPTIONS[dateFormatIndex]; }


void Preferences::ToggleNotificationSetting()
{
  if(notifOptionsIndex == static_cast<int>(NOTIF_OPTIONS.size() - 1)) notifOptionsIndex = 0;
  else ++notifOptionsIndex;
}
...
const string &Preferences::NotificationSettingString() { return NOTIF_OPTIONS[notifOptionsIndex]; }


// Scroll speed preference.
int Preferences::ScrollSpeed() { return scrollSpeed; }


void Preferences::SetScrollSpeed(int speed) { scrollSpeed = speed; }


int Preferences::TooltipActivation() { return tooltipActivation; }


void Preferences::SetTooltipActivation(int steps) { tooltipActivation = steps; }
...
double Preferences::ViewZoom()
{
  ...
  if(zoomIndex >= zooms.size()) return zooms.empty() ? 1. : zooms.back();
...
{
  const auto &zooms = GameData::Interfaces().Get("main view")->GetList("zooms");
  if(zooms.empty() || zoomIndex >= zooms.size() - 1) return false;
...
{
  const auto &zooms = GameData::Interfaces().Get("main view")->GetList("zooms");
  if(!zoomIndex || zooms.size() <= 1) return false;

  // Make sure that we're actually zooming out. This can happen if the zoom index
  // is out of range.
  if(zoomIndex >= zooms.size()) zoomIndex = zooms.size() - 1;
...
void Preferences::ToggleParallax()
{
  ...
  if(targetIndex == static_cast<int>(PARALLAX_SETTINGS.size())) targetIndex = 0;
...
const string &Preferences::ParallaxSetting() { return PARALLAX_SETTINGS[parallaxIndex]; }


void Preferences::ToggleExtendedJumpEffects()
{
  int targetIndex = extendedJumpEffectIndex + 1;
  if(targetIndex == static_cast<int>(EXTENDED_JUMP_EFFECT_SETTINGS.size())) targetIndex = 0;
...
const string &Preferences::ScreenModeSetting() { return SCREEN_MODE_SETTINGS[screenModeIndex]; }


bool Preferences::ToggleVSync()
{
  int targetIndex = vsyncIndex + 1;
  if(targetIndex == static_cast<int>(VSYNC_SETTINGS.size())) targetIndex = 0;
...
Preferences::VSync Preferences::VSyncState() { return static_cast<VSync>(vsyncIndex); }


const string &Preferences::VSyncSetting() { return VSYNC_SETTINGS[vsyncIndex]; }
...
Preferences::CameraAccel Preferences::CameraAcceleration() { return static_cast<CameraAccel>(cameraAccelerationIndex); }


const string &Preferences::CameraAccelerationSetting() { return CAMERA_ACCELERATION_SETTINGS[cameraAccelerationIndex]; }
...
void Preferences::CycleStatusOverlays(Preferences::OverlayType type)
{
  ...
  // set the all overlay state to DISABLED but do not increment any of the child settings.
    ...
    statusOverlaySettings[OverlayType::ALL] = OverlayState::DISABLED;
  else statusOverlaySettings[type].Increment();
}


Preferences::OverlayState Preferences::StatusOverlaysState(Preferences::OverlayType type)
{
  if(statusOverlaySettings[OverlayType::ALL].IsActive()) return statusOverlaySettings[OverlayType::ALL];
...
const string &Preferences::StatusOverlaysSetting(Preferences::OverlayType type)
{
  ...
  if(allOverlaysSetting.IsActive())
  {
    ...
    if(type != OverlayType::ALL) return DISABLED.ToString();
  }
...
const string &Preferences::TurretOverlaysSetting() { return TURRET_OVERLAYS_SETTINGS[turretOverlaysIndex]; }


void Preferences::ToggleAutoAim() { autoAimIndex = (autoAimIndex + 1) % AUTO_AIM_SETTINGS.size(); }


Preferences::AutoAim Preferences::GetAutoAim() { return static_cast<AutoAim>(autoAimIndex); }


const string &Preferences::AutoAimSetting() { return AUTO_AIM_SETTINGS[autoAimIndex]; }


void Preferences::ToggleAutoFire() { autoFireIndex = (autoFireIndex + 1) % AUTO_FIRE_SETTINGS.size(); }


Preferences::AutoFire Preferences::GetAutoFire() { return static_cast<AutoFire>(autoFireIndex); }


const string &Preferences::AutoFireSetting() { return AUTO_FIRE_SETTINGS[autoFireIndex]; }


void Preferences::ToggleBoarding()
{
  int targetIndex = boardingIndex + 1;
  if(targetIndex == static_cast<int>(BOARDING_SETTINGS.size())) targetIndex = 0;
...
const string &Preferences::BoardingSetting() { return BOARDING_SETTINGS[boardingIndex]; }


void Preferences::ToggleFlotsam() { flotsamIndex = (flotsamIndex + 1) % FLOTSAM_SETTINGS.size(); }
...
const string &Preferences::FlotsamSetting() { return FLOTSAM_SETTINGS[flotsamIndex]; }


void Preferences::ToggleAlert()
{
  if(++alertIndicatorIndex >= static_cast<int>(ALERT_INDICATOR_SETTING.size())) alertIndicatorIndex = 0;
}
...
const string &Preferences::AlertSetting() { return ALERT_INDICATOR_SETTING[alertIndicatorIndex]; }


bool Preferences::PlayAudioAlert() { return DoAlertHelper(AlertIndicator::AUDIO); }


bool Preferences::DisplayVisualAlert() { return DoAlertHelper(AlertIndicator::VISUAL); }


bool Preferences::DoAlertHelper(Preferences::AlertIndicator toDo)
{
  auto value = GetAlertIndicator();
  if(value == AlertIndicator::BOTH) return true;
  else if(value == toDo) return true;
  return false;
}


int Preferences::GetPreviousSaveCount() { return previousSaveCount; }


void Preferences::ToggleMinimapDisplay()
{
  if(++minimapDisplayIndex >= static_cast<int>(MINIMAP_DISPLAY_SETTING.size())) minimapDisplayIndex = 0;
}
...
const string &Preferences::MinimapSetting() { return MINIMAP_DISPLAY_SETTING[minimapDisplayIndex]; }
...
{
  if(++titleBarThemeIndex >= static_cast<int>(TITLE_BAR_THEME_SETTINGS.size())) titleBarThemeIndex = 0;
  GameWindow::UpdateTitleBarTheme();
}


Preferences::TitleBarTheme Preferences::GetTitleBarTheme() { return static_cast<TitleBarTheme>(titleBarThemeIndex); }


const string &Preferences::TitleBarThemeSetting() { return TITLE_BAR_THEME_SETTINGS[titleBarThemeIndex]; }


void Preferences::ToggleWindowRounding()
{
  if(++windowRoundingIndex >= static_cast<int>(WINDOW_ROUNDING_SETTINGS.size())) windowRoundingIndex = 0;
...
const string &Preferences::WindowRoundingSetting() { return WINDOW_ROUNDING_SETTINGS[windowRoundingIndex]; }
#endif
