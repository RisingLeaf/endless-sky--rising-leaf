...
namespace
{
  ...
  int Distance(const System *center, const System *system, int maximum, DistanceCalculationSettings distanceSettings)
  {
    ...
    if(center != previousCenter || maximum > previousMaximum || distanceSettings != previousDistanceSettings)
    {
      ...
      distance =
          DistanceMap(center, distanceSettings.WormholeStrat(), distanceSettings.AssumesJumpDrive(), -1, maximum);
    }
  ...
  {
    return c.empty() || any_of(c.begin(), c.end(), [](const T *item) noexcept -> bool { return item->IsValid(); });
  }
  bool CheckValidity(const list<LocationFilter> &l)
  {
    return l.empty() ||
           any_of(l.begin(), l.end(), [](const LocationFilter &f) noexcept -> bool { return f.IsValid(); });
  }
  bool CheckValidity(const list<set<const Outfit *>> &l)
  {
    ...
    for(auto &&outfits : l)
    {
      for(auto &&outfit : outfits)
        if(outfit->IsDefined()) return true;
    }
    ...
    const set<const Planet *> *visitedPlanets)
{
  for(const DataNode &child : node)
  {
    ...
    if(key == "not" || key == "neighbor")
    {
      ...
      if(child.Size() == 1) filters.back().Load(child, visitedSystems, visitedPlanets);
      else filters.back().LoadChild(child, visitedSystems, visitedPlanets);
    }
    else {
  ...
  isEmpty = planets.empty() && attributes.empty() && systems.empty() && governments.empty() && !center &&
            ...
            shipCategory.empty() && !systemIsVisited && !planetIsVisited;
}
...
void LocationFilter::Save(DataWriter &out) const
{
  out.BeginChild();
  {
    ...
    if(center) out.Write("near", center->TrueName(), centerMinDistance, centerMaxDistance);
    if(planetIsVisited) out.Write("visited", "planet");
    else if(systemIsVisited) out.Write("visited");
  }
  out.EndChild();
}
...
bool LocationFilter::IsEmpty() const { return isEmpty; }
...
bool LocationFilter::IsValid() const
{
  ...
  if(!CheckValidity(planets)) return false;

  // Attributes are always considered valid.

  if(!CheckValidity(systems)) return false;

  // Governments are always considered valid.

  // The "center" of a "near <system>" filter must be valid.
  if(center && !center->IsValid()) return false;

  if(!CheckValidity(outfits)) return false;
  ...
  if(!shipCategory.empty())
  {
    ...
    if(!SetsIntersect(shipCategory, categoriesSet)) return false;
  }

  if(!CheckValidity(notFilters)) return false;

  if(!CheckValidity(neighborFilters)) return false;
...
{
  if(!planet || !planet->IsValid()) return false;
  if(planetIsVisited)
  {
    if(!visitedPlanets)
      throw runtime_error("LocationFilter::Matches called with a null pointer to the player's visited planets!");
    if(!visitedPlanets->contains(planet)) return false;
  }

  // If a ship class was given, do not match planets.
  if(!shipCategory.empty()) return false;

  if(!governments.empty() && !governments.contains(planet->GetGovernment())) return false;

  if(!planets.empty() && !planets.contains(planet)) return false;
  for(const set<string> &attr : attributes)
    if(!SetsIntersect(attr, planet->Attributes())) return false;

  for(const LocationFilter &filter : notFilters)
    if(filter.Matches(planet, origin)) return false;

  // If outfits are specified, make sure they can be bought here.
  for(const set<const Outfit *> &outfitList : outfits)
    if(!SetsIntersect(outfitList, planet->OutfitterStock())) return false;
...
bool LocationFilter::Matches(const Ship &ship) const
{
  ...
  if(!systems.empty() && !systems.contains(origin)) return false;
  if(!governments.empty() && !governments.contains(ship.GetGovernment())) return false;

  if(!shipCategory.empty() && !shipCategory.contains(ship.Attributes().Category())) return false;
  ...
  if(!attributes.empty())
  {
    ...
    for(const auto &attr : ship.Attributes().Attributes())
      if(attr.second > 0.) shipAttributes.insert(shipAttributes.end(), attr.first);
    for(const set<string> &attr : attributes)
      if(!SetsIntersect(attr, shipAttributes)) return false;
  }
  ...
  if(!outfits.empty())
  {
    ...
    for(const auto &oit : ship.Outfits())
      if(oit.second > 0) shipOutfits.insert(shipOutfits.end(), oit.first);
    for(const auto &cit : ship.Cargo().Outfits())
      if(cit.second > 0) shipOutfits.insert(cit.first);
    for(const auto &outfitSet : outfits)
      if(!SetsIntersect(outfitSet, shipOutfits)) return false;
  }

  for(const LocationFilter &filter : notFilters)
    if(filter.Matches(ship)) return false;

  if(!MatchesNeighborFilters(neighborFilters, origin, origin)) return false;
  ...
  if(center && Distance(center, origin, centerMaxDistance, centerDistanceOptions) < centerMinDistance) return false;
...
const System *LocationFilter::PickSystem(const System *origin) const
{
  ...
  for(const auto &it : GameData::Systems())
  {
    ...
    if(Matches(&system, origin)) options.push_back(&system);
  }
...
const Planet *LocationFilter::PickPlanet(const System *origin, bool hasClearance, bool requireSpaceport) const
{
  ...
  for(const auto &it : GameData::Planets())
  {
    ...
    if(!planet.IsValid() || (planet.GetSystem() && planet.GetSystem()->Inaccessible())) continue;
    // Skip planets that do not offer special jobs or missions, unless they were explicitly listed as options.
    if(planet.IsWormhole() || (requireSpaceport && !planet.GetPort().HasService(Port::ServicesType::OffersMissions)) ||
       (!hasClearance && !planet.CanLand()))
    {
      if(planets.empty() || !planets.contains(&planet)) continue;
    }
    if(Matches(&planet, origin)) options.push_back(&planet);
  }
  return options.empty() ? nullptr : options[Random::Int(options.size())];
}
    ...
    const set<const Planet *> *visitedPlanets)
{
  ...
  {
    child.PrintTrace(
        "Skipping unsupported use of 'not' and 'neighbor'."
  ...
  else if(key == "attributes")
  {
    ...
    if(attributes.back().empty()) attributes.pop_back();
  }
  else if(key == "near" && child.Size() >= 1 + valueIndex)
  {
    ...
    if(child.HasChildren()) centerDistanceOptions.Load(child);
  }
  else if(key == "distance" && child.Size() >= 1 + valueIndex)
  {
    ...
    if(child.HasChildren()) originDistanceOptions.Load(child);
  }
  ...
  else if(key == "outfits" && child.Size() >= 2 + isNot)
  {
    ...
    if(outfits.back().empty()) outfits.pop_back();
  }
  else if(key == "visited")
  {
    systemIsVisited = true;
    if(child.Size() >= 2 + isNot && child.Token(valueIndex) == "planet") planetIsVisited = true;
  }
  else {
...
{
  if(!system || !system->IsValid()) return false;
  if(!systems.empty() && !systems.contains(system)) return false;
  if(systemIsVisited)
  {
    if(!visitedSystems)
      throw runtime_error("LocationFilter::Matches called with a null pointer to the player's visited systems!");
    if(!visitedSystems->contains(system)) return false;
  }
  ...
  if(!didPlanet)
  {
    if(!governments.empty() && !governments.contains(system->GetGovernment())) return false;
    ...
    for(const LocationFilter &filter : notFilters)
      if(filter.Matches(system, origin)) return false;
  }

  if(!MatchesNeighborFilters(neighborFilters, system, origin)) return false;

  // Check this system's distance from the desired reference system.
  if(center && Distance(center, system, centerMaxDistance, centerDistanceOptions) < centerMinDistance) return false;
