...
MainPanel::MainPanel(PlayerInfo &player) : player(player), engine(player) { SetIsFullScreen(true); }
...
void MainPanel::Step()
{
  ...
  if(player.IsDead()) show = Command::NONE;
  ...
  if(isActive && player.HasAvailableInflightMissions() && player.Flagship())
  {
    ...
    if(!mission) mission = player.EnteringMission();
    if(mission) mission->Do(Mission::OFFER, player, &GetUI());
    else player.HandleBlockedInflightMissions(GetUI());
  ...
  if(isActive) isActive = !ShowHelp(false);

  engine.Step(isActive);

  if(isActive && !engine.IsPaused()) player.StepMissionTimers(GetUI());
...
void MainPanel::OnBribeCallback(const Government *bribed) { engine.BreakTargeting(bribed); }


bool MainPanel::AllowsFastForward() const noexcept { return true; }


Engine &MainPanel::GetEngine() { return engine; }
...
bool MainPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  else if(command.Has(Command::TURRET_TRACKING))
  {
    ...
    Messages::Add(*GameData::Messages().Get(newValue ? "turret tracking focused" : "turret tracking opportunistic"));
  }
  else if(command.Has(Command::AMMO))
  {
    Preferences::ToggleAmmoUsage();
    Messages::Add(*GameData::Messages().Get(
        Preferences::Has("Escorts expend ammo")
            ? (Preferences::Has("Escorts use ammo frugally") ? "expend ammo frugally" : "expend ammo always")
            : "expend ammo never"));
  }
  ...
  {
    engine.SelectGroup(key - '0', mod & SDL_KMOD_SHIFT, mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI));
  }
  else {
    return false;
  }
...
bool MainPanel::Click(int x, int y, MouseButton button, int clicks)
{
  ...
  case MouseButton::RIGHT:  engine.RightOrMiddleClick(Point(x, y), button); return true;
  ...
  hasShift       = (mod & SDL_KMOD_SHIFT);
  hasControl     = (mod & SDL_KMOD_CTRL);
...
bool MainPanel::Release(int x, int y, MouseButton button)
{
  ...
  {
    dragPoint = Point(x, y);
    if(dragPoint.Distance(dragSource) > 5.) engine.Click(dragSource, dragPoint, hasShift, hasControl);
...
{
  if(dy < 0) Preferences::ZoomViewOut();
  else if(dy > 0) Preferences::ZoomViewIn();
  else return false;
...
void MainPanel::ShowScanDialog(const ShipEvent &event)
{
  ...
  if(event.Type() & ShipEvent::SCAN_CARGO)
  {
    ...
    for(const auto &it : target->Cargo().Commodities())
    {
      if(it.second)
      {
        if(first) out << "This " + target->Noun() + " is carrying:\n";
        first = false;
  ...
  {
    if(!target->Outfits().empty()) out << "This " + target->Noun() + " is equipped with:\n";
    else out << "This " + target->Noun() + " is not equipped with any outfits.\n";
    ...
    for(const auto &it : outfitsByCategory)
    {
      ...
      for(const auto &it2 : it.second)
        if(!it2.first.empty() && it2.second > 0) out << "\t\t" << it2.second << " " << it2.first << "\n";
    }
    ...
    if(!count.empty())
    {
      ...
      for(const auto &it : count)
        if(it.second > 0) out << "\t" << it.second << " " << it.first << "\n";
    }
  }
...
bool MainPanel::ShowHailPanel()
{
  ...
  if(!flagship || flagship->IsDestroyed()) return false;

  // Player cannot hail while landing / departing.
  if(flagship->Zoom() < 1.) return false;

  std::shared_ptr<Ship> target = flagship->GetTargetShip();
  if((SDL_GetModState() & SDL_KMOD_SHIFT) && flagship->GetTargetStellar()) target.reset();
  ...
  else if(target)
  {
    ...
    if(target->Zoom() < 1. || target->IsDestroyed() || target->GetSystem() != player.GetSystem() || target->IsCloaked())
    {
    ...
    {
      Messages::Add(
          {"Unable to send hail: " + target->Noun() + " is entering hyperspace.",
           GameData::MessageCategories().Get("high")});
    }
    else {
      GetUI().Push(
          new HailPanel(player, target, [&](const Government *bribed) { MainPanel::OnBribeCallback(bribed); }));
      return true;
    }
  }
  else if(flagship->GetTargetStellar())
  {
    ...
    else if(planet->IsInhabited())
    {
      ...
      Messages::Add(
          {"Unable to send hail: " + planet->Noun() + " is not inhabited.", GameData::MessageCategories().Get("high")});
    }
  }
  else {
...
bool MainPanel::ShowHelp(bool force)
{
  ...
  {
    if(force) forced.push_back("control ship with mouse");
    else if(DoHelp("control ship with mouse")) return true;
  }
  if(flagship->IsTargetable())
  {
    if(force) forced.push_back("navigation");
    else if(DoHelp("navigation")) return true;
  }
  if(flagship->IsDestroyed())
  {
    if(force) forced.push_back("dead");
    else if(DoHelp("dead")) return true;
  }
  else if(flagship->IsDisabled())
  {
    if(force) forced.push_back("disabled");
    else if(DoHelp("disabled")) return true;
  }
  bool canRefuel = player.GetSystem()->HasFuelFor(*flagship);
  if(!flagship->IsHyperspacing() && !flagship->JumpsRemaining() && !canRefuel)
  {
    if(force) forced.push_back("stranded");
    else if(DoHelp("stranded")) return true;
  }
  std::shared_ptr<Ship> target = flagship->GetTargetShip();
  if(target && target->IsDisabled() && !target->GetGovernment()->IsEnemy())
  {
    if(force) forced.push_back("friendly disabled");
    else if(DoHelp("friendly disabled")) return true;
  }
  if(player.Ships().size() > 1)
  {
    if(force) forced.push_back("multiple ship controls");
    else if(DoHelp("multiple ship controls")) return true;
  }
  if(flagship->IsTargetable() && player.Ships().size() > 1)
  {
    if(force) forced.push_back("fleet harvest tutorial");
    else if(DoHelp("fleet harvest tutorial")) return true;
  }
  if(flagship->IsTargetable() && flagship->Attributes().Get("asteroid scan power") && player.Ships().size() > 1)
  {
  ...
  {
    if(force) forced.push_back("try out fighters transfer cargo");
    else if(DoHelp("try out fighters transfer cargo")) return true;
  }
  if(Preferences::Has("Fighters transfer cargo"))
  {
    if(force) forced.push_back("fighters transfer cargo");
    else if(DoHelp("fighters transfer cargo")) return true;
  }
     ...
     player.GetDate() <= player.StartData().GetDate() + 4)
  {
    ...
    if(count > lostCount && count <= 7)
    {
      ...
      ++lostCount;
      if(force) forced.push_back(message);
      else if(DoHelp(message)) return true;
    }
  }

  if(!force || forced.empty()) return false;
...
void MainPanel::StepEvents(bool &isActive)
{
  while(isActive && !eventQueue.empty())
  {
    ...
    // active missions.
    if(!handledFront) player.HandleEvent(event, GetUI());
    handledFront = true;
    isActive     = (GetUI().Top().get() == this);
    ...
    const Ship *flagship = player.Flagship();
    if((event.Type() & (ShipEvent::BOARD | ShipEvent::ASSIST)) && actor->IsPlayer() && !event.Target()->IsDestroyed() &&
       flagship && event.Actor().get() == flagship)
    {
      ...
      else if(mission)
      {
        player.HandleBlockedMissions(
            (event.Type() & ShipEvent::BOARD) ? Mission::BOARDING : Mission::ASSISTING,
            GetUI());
      }
      // Determine if a Dialog or ConversationPanel is being drawn next frame.
      isActive = (GetUI().Top().get() == this);
    ...
    if((event.Type() & ShipEvent::JUMP) && flagship && event.Actor().get() == flagship) player.CreateEnteringMissions();
