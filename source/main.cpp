		...
		const string &testToRunName, bool debugMode)
{
			...
			&isFastForward]
	{
		...
		while(SDL_PollEvent(&event))
		{
			...
			else if(event.type == SDL_EVENT_KEY_DOWN && menuPanels.IsEmpty()
					&& Command(event.key.key).Has(Command::MENU)
			...
			else if(activeUI.Handle(event))
			{
				// The UI handled the event.
					...
					&& (Command(event.key.key).Has(Command::FASTFORWARD))
		...
		if(Command(SDLK_CAPSLOCK).Has(Command::FASTFORWARD))
			isFastForward = SDL_GetModState() & SDL_KMOD_CAPS;
	};
