...
namespace
{
  ...
  {
    static const Command cancelers(
        Command::LAND | Command::JUMP | Command::FLEET_JUMP | Command::BOARD | Command::AFTERBURNER | Command::BACK |
        Command::FORWARD | Command::LEFT | Command::RIGHT | Command::AUTOSTEER | Command::STOP);
  ...
  {
    return ship.GetSystem() && !ship.IsEnteringHyperspace() && !ship.GetSystem()->HasFuelFor(ship) && ship.NeedsFuel();
  }

  bool NeedsEnergy(const Ship &ship) { return ship.GetSystem() && !ship.IsEnteringHyperspace() && ship.NeedsEnergy(); }

  bool IsStranded(const Ship &ship) { return NeedsFuel(ship) || NeedsEnergy(ship); }

  bool CanBoard(const Ship &ship, const Ship &target)
  {
    if(&ship == &target) return false;
    if(target.IsDestroyed() || !target.IsTargetable() || target.GetSystem() != ship.GetSystem()) return false;
    if(IsStranded(target) && !ship.GetGovernment()->IsEnemy(target.GetGovernment())) return true;
  ...
  {
    if(target.GetPersonality().IsSwarming() || target.IsHyperspacing()) return false;
    if(target.GetGovernment()->IsEnemy(ship.GetGovernment())) return false;
    if(target.GetSystem() != ship.GetSystem()) return false;
  ...
  bool EscortsReadyToJump(const Ship &ship)
  {
    ...
    for(const std::weak_ptr<Ship> &ptr : ship.GetEscorts())
    {
      std::shared_ptr<const Ship> escort = ptr.lock();
      // Skip escorts which are not player-owned and not escort mission NPCs.
      if(!escort || (shipIsYours && !escort->IsYours() &&
  ...
  bool EscortsReadyToLand(const Ship &ship)
  {
    ...
    for(const std::weak_ptr<Ship> &ptr : ship.GetEscorts())
    {
      ...
      if(escort->IsDisabled()) continue;
      if(escort->GetTargetStellar() == ship.GetTargetStellar() && !escort->CanLand()) return false;
    }
    return true;
  }
  ...
  bool IsArmed(const Ship &ship)
  {
    for(const Hardpoint &hardpoint : ship.Weapons())
    {
      ...
      if(weapon && !hardpoint.IsSpecial())
      {
        ...
        if(ammo && !ship.OutfitCount(ammo)) continue;
        return true;
      }
    }
    return false;
  ...
  std::vector<Ship *> GetShipsForFormationCommand(const PlayerInfo &player)
  {
    ...
    if(selectedShips.empty())
    {
      ...
      for(const std::shared_ptr<Ship> &it : player.Ships())
        if(it.get() != player.Flagship() && !it->IsParked() && !it->IsDestroyed() && !it->IsDisabled())
          targetShips.push_back(it.get());
    }
    else {
      targetShips.reserve(selectedShips.size());
      for(const std::weak_ptr<Ship> &it : selectedShips)
      {
        std::shared_ptr<Ship> ship = it.lock();
        if(ship) targetShips.push_back(ship.get());
      }
    }
  ...
  void IssueDeploy(const PlayerInfo &player)
  {
    ...
    auto isCandidate = [](const std::shared_ptr<Ship> &ship) -> bool
    { return ship->CanBeCarried() && !ship->IsParked() && !ship->IsDestroyed(); };

    auto toDeploy = std::vector<Ship *>{};
    auto toRecall = std::vector<Ship *>{};
    {
    ...
    for(const std::weak_ptr<Ship> &it : player.SelectedEscorts())
    {
      std::shared_ptr<Ship> ship = it.lock();
      if(ship && isCandidate(ship)) (ship->HasDeployOrder() ? toRecall : toDeploy).emplace_back(ship.get());
    }
    ...
    if(!toDeploy.empty())
    {
      ...
      std::string ship = (toDeploy.size() == 1 ? "ship" : "ships");
      Messages::Add(
          {"Deployed " + std::to_string(toDeploy.size()) + " carried " + ship + ".",
    ...
    else if(!toRecall.empty())
    {
      ...
      std::string ship = (toRecall.size() == 1 ? "ship" : "ships");
      Messages::Add(
          {"Recalled " + std::to_string(toRecall.size()) + " carried " + ship + ".",
           GameData::MessageCategories().Get("normal")});
    }
  }
  ...
  {
    for(const Ship::Bay &bay : ship.Bays())
      if(bay.ship && bay.ship->HasDeployOrder()) return true;
    return false;
  }
  ...
  bool ShouldRefuel(const Ship &ship, const RoutePlan &route)
  {
    ...
    if(!route.HasRoute()) return false;

    // If the ship is full, no refuel.
    if(ship.Fuel() == 1.) return false;

    // If the ship has nowhere to refuel, no refuel.
    const System *from = ship.GetSystem();
    if(!from->HasFuelFor(ship)) return false;
    ...
    if(!ship.JumpNavigation().JumpFuel()) return false;
  ...
  bool ShouldFlee(Ship &ship)
  {
    ...
    if(personality.IsFleeing()) return true;

    if(personality.IsStaying()) return false;

    const bool lowHealth = ship.Health() < RETREAT_HEALTH + .25 * personality.IsCoward();
    if(!personality.IsDaring() && lowHealth) return true;

    if(ship.GetAICache().NeedsAmmo()) return true;

    if(personality.IsGetaway() && ship.Cargo().Free() == 0 && !ship.GetParent()) return true;
  ...
  {
    const Personality &personality = ship.GetPersonality();
    if(personality.IsStaying()) return true;

    const Government           *government = ship.GetGovernment();
    std::shared_ptr<const Ship> parent     = ship.GetParent();
    if(parent && government && parent->GetGovernment()->IsEnemy(government)) return true;

    if(ship.IsFleeing()) return false;

    // Everything after here is the lingering logic.

    if(!personality.IsLingering()) return false;
    // NPCs that can follow the player do not linger.
    if(ship.IsSpecial() && !personality.IsUninterested()) return false;
    ...
    if(destination && destination != system) return false;
    // Ship cannot linger any longer in this system.
    if(!system || ship.GetLingerSteps() >= system->MinimumFleetPeriod() / 4) return false;
...
{
  // Figure out what ships we are giving orders to
  std::vector<Ship *> targetShips = GetShipsForFormationCommand(player);
  ...
  else if(!multiplePatternsSet)
  {
    ...
    if(it != formationPatterns.end()) ++it;
  ...
  std::string  message  = std::to_string(count) + (count == 1 ? " ship" : " ships") + " will ";
...
void AI::IssueShipTarget(const std::shared_ptr<Ship> &target)
{
  ...
  OrderSingle newOrder(
      isEnemy ? target->IsDisabled() ? Orders::Types::FINISH_OFF : Orders::Types::ATTACK : Orders::Types::KEEP_STATION);
  newOrder.SetTargetShip(target);
  IssueOrder(newOrder, (isEnemy ? "focusing fire on" : "following") + (" \"" + target->GivenName() + "\"."));
}


void AI::IssueAsteroidTarget(const std::shared_ptr<Minable> &targetAsteroid)
{
  OrderSingle newOrder{Orders::Types::MINE};
  newOrder.SetTargetAsteroid(targetAsteroid);
  IssueOrder(newOrder, "focusing fire on " + targetAsteroid->DisplayName() + " " + targetAsteroid->Noun() + ".");
}
...
void AI::IssueMoveTarget(const Point &target, const System *moveToSystem)
{
  ...
  std::string description = "moving to the given location";
...
void AI::UpdateKeys(PlayerInfo &player, const Command &activeCommands)
{
  ...
  if(!flagship || flagship->IsDestroyed()) return;
  ...
  if(activeCommands.Has(AutopilotCancelCommands()))
  {
    ...
    if(canceled) Messages::Add(*GameData::Messages().Get("disengaging autopilot"));
    autoPilot.Clear();
  }
  ...
  if(activeCommands.Has(Command::CLOAK))
  {
    for(const auto &it : player.Ships())
    {
      if(!it->IsParked() && it->CloakingSpeed())
      {
        ...
        Messages::Add(
            *GameData::Messages().Get(isCloaking ? "engaging cloaking device" : "disengaging cloaking device"));
        break;
      }
    }
  }
...
void AI::UpdateEvents(const std::list<ShipEvent> &events)
{
  for(const ShipEvent &event : events)
  {
    ...
    if(event.Actor())
    {
      ...
      if(event.TargetGovernment()) notoriety[event.Actor()][event.TargetGovernment()] |= event.Type();
    }
    ...
    if(actorGovernment)
    {
      ...
      if(actorGovernment->IsPlayer() && event.TargetGovernment())
      {
        ...
        if(event.Type() & ShipEvent::PROVOKE) newActions |= ShipEvent::PROVOKE;
...
void AI::Step(Command &activeCommands)
{
  ...
  std::map<const Government *, int64_t> strength;
  ...
  for(auto it = fenceCount.begin(); it != fenceCount.end();)
  {
    ...
    if(it->second < 0) it = fenceCount.erase(it);
    else ++it;
  }
  for(const auto &it : ships)
  {
    ...
    if(system && it->Position().Length() >= system->InvisibleFenceRadius())
    {
      ...
      value      = std::min(FENCE_MAX, value + FENCE_DECAY + 1);
    }
  }
  ...
  for(const auto &it : ships)
  {
    ...
    if(!it->GetSystem()) continue;

    if(it.get() == flagship)
    {
      // Player cannot do anything if the flagship is landing.
      if(!flagship->IsLanding()) MovePlayer(*it, activeCommands);
      continue;
    }
    ...
    if(isStranded || it->IsDisabled())
    {
      ...
      if(!it->GetPersonality().IsDerelict()) AskForHelp(*it, isStranded, flagship);
    ...
    if(it->IsYours())
    {
      ...
      if(isCloaking) command |= Command::CLOAK;
    }
    ...
    std::shared_ptr<Ship> parent = it->GetParent();
    ...
    std::shared_ptr<Ship>    target         = it->GetTargetShip();
    std::shared_ptr<Minable> targetAsteroid = it->GetTargetAsteroid();
    std::shared_ptr<Flotsam> targetFlotsam  = it->GetTargetFlotsam();
    if(isPresent && it->IsYours() && targetFlotsam && FollowOrders(*it, command)) continue;
    ...
    if(isPresent && !it->IsYours())
    {
      ...
      if((cargoScan || outfitScan) && target && !target->IsDisabled() && !target->GetGovernment()->IsEnemy(gov) &&
         target->GetGovernment() != gov)
      {
        ++scanTime[&*it];
        if(it->CargoScanFraction() == 1.) cargoScans[&*it].insert(&*target);
        if(it->OutfitScanFraction() == 1.) outfitScans[&*it].insert(&*target);
      }
    }
    if(isPresent && !personality.IsSwarming())
    {
      ...
      if(targetTurn == targetStep || !target || target->IsDestroyed() ||
         (target->IsDisabled() && (personality.Disables() ||
                                   (!FighterHitHelper::IsValidTarget(target.get()) && !personality.IsVindictive()))) ||
    ...
    if(isPresent)
    {
      ...
      if(targetAsteroid) AutoFire(*it, firingCommands, *targetAsteroid);
      else AutoFire(*it, firingCommands);
    }
    ...
    if(healthRemaining < RETREAT_HEALTH + .25)
    {
      ...
      if(personality.IsAppeasing() && it->Cargo().Used()) DoAppeasing(it, &appeasementThreshold[it.get()]);
    }

    // If recruited to assist a ship, follow through on the commitment
    // instead of ignoring it due to other personality traits.
    std::shared_ptr<Ship> shipToAssist = it->GetShipToAssist();
    if(shipToAssist)
    {
      if(shipToAssist->IsDestroyed() || shipToAssist->GetSystem() != it->GetSystem() || shipToAssist->IsLanding() ||
         shipToAssist->IsHyperspacing() || shipToAssist->GetGovernment()->IsEnemy(gov) ||
         (!shipToAssist->IsDisabled() && !shipToAssist->NeedsFuel() && !shipToAssist->NeedsEnergy()))
      {
        ...
        shipToAssist.reset();
        it->SetShipToAssist(std::weak_ptr<Ship>());
      }
    ...
    if(target) targetDistance = target->Position().Distance(it->Position());

    // Special case: if the player's flagship tries to board a ship to
    // refuel it, that escort should hold position for boarding.
    isStranded |=
        (flagship && it == flagship->GetTargetShip() && CanBoard(*flagship, *it) && autoPilot.Has(Command::BOARD));

    // Stranded ships that have a helper need to stop and be assisted.
    bool strandedWithHelper = isStranded && (HasHelper(*it, NeedsFuel(*it), NeedsEnergy(*it)) ||
                                             it->GetPersonality().IsDerelict() || it->IsYours());
    ...
    if(isPresent && personality.IsSurveillance() && !strandedWithHelper && (scanPermissions[gov] || it->IsSpecial()))
    {
    ...
    if(isPresent && personality.IsMining() && !shouldFlee && !target && !strandedWithHelper && maxMinerCount)
    {
      ...
      if(it->Cargo().Free() >= 5 && IsArmed(*it) &&
         (it->IsSpecial() || (++miningTime[&*it] < npcMaxMiningTime && ++minerCount < maxMinerCount)))
      {
        if(it->HasBays())
        {
          command |= Command::DEPLOY;
      ...
      {
        const std::shared_ptr<Minable> &minable = parent->GetTargetAsteroid();
    ...
    if(it->CanBeCarried())
    {
      ...
      if(findNewParent && parentHasSpace && it->IsYours()) parentHasSpace = parent->CanCarry(*it);
      if(!hasParent || (!inParentSystem && !it->JumpNavigation().JumpFuel()) || (!parentHasSpace && findNewParent))
      {
        ...
        auto parentChoices = std::vector<std::shared_ptr<Ship>>{};
        parentChoices.reserve(ships.size() * .1);
        auto getParentFrom =
            [&it, &gov, &parentChoices](const std::list<std::shared_ptr<Ship>> &otherShips) -> std::shared_ptr<Ship>
        {
          for(const auto &other : otherShips)
          {
            if(other->GetGovernment() == gov && other->GetSystem() == it->GetSystem() && !other->CanBeCarried())
            {
              if(!other->IsDisabled() && other->CanCarry(*it)) return other;
              else parentChoices.emplace_back(other);
            }
          }
          return std::shared_ptr<Ship>();
        };
        // Mission ships should only pick amongst ships from the same mission.
        auto missionIt = it->IsSpecial() && !it->IsYours() ? find_if(
                                                                 player.Missions().begin(),
                                                                 player.Missions().end(),
                                                                 [&it](const Mission &m) { return m.HasShip(it); })
                                                           : player.Missions().end();

        std::shared_ptr<Ship> newParent;
        ...
        {
          if(parent && parent->IsDestroyed()) parent = parent->GetParent();
        }
        ...
        // bays.
        if(it->IsYours() && parent && parent->GetParent() && !parent->CanCarry(*it)) parent = parent->GetParent();

        if(it->GetParent() != parent) it->SetParent(parent);
      }
    ...
    if(!target && it->HasBays() && !(it->IsYours() ? thisIsLaunching : it->Commands().Has(Command::DEPLOY)))
    {
    ...
    {
      if(personality.IsStaying() || !it->Attributes().Get("fuel capacity")) MoveIndependent(*it, command);
      else MoveEscort(*it, command);
    }
    ...
    else if(parent->GetGovernment()->IsEnemy(gov))
    {
      ...
      if(target) MoveIndependent(*it, command);
      // Otherwise try to find and fight your parent. If your parent
      // can't be both targeted and pursued, then don't follow them.
      else if(parent->IsTargetable() && CanPursue(*it, *parent)) MoveEscort(*it, command);
      else MoveIndependent(*it, command);
    }
    else if(parent->IsDisabled() && !it->CanBeCarried())
    {
      ...
      if(target) MoveIndependent(*it, command);
      else if(!parent->GetPersonality().IsDerelict()) it->SetShipToAssist(parent);
      else CircleAround(*it, command, *parent);
    }
    ...
    // escorts will stay nearby until they have repaired a bit.
        ...
        (personality.IsTimid() || (it->IsYours() && healthRemaining < RETREAT_HEALTH)) &&
...
void AI::SetMousePosition(Point position) { mousePosition = position; }
...
size_t AI::RouteCacheKey::HashFunction::operator()(RouteCacheKey const &key) const
{
  ...
  for(const std::string &k : key.wormholeKeys)
    Hasher::Hash(hash, k);
  return hash;
}
...
{
  // Used by unordered_map to determine equivalence.
  return jumpHash == other.jumpHash && personalityHash == other.personalityHash && to == other.to &&
         isPlayer == other.isPlayer && wormholeKeys == other.wormholeKeys;
}
...
bool AI::CanPursue(const Ship &ship, const Ship &target) const
{
  ...
  if(ship.GetPersonality().IsUnconstrained()) return true;

  // Owned ships ignore fence.
  if(ship.IsYours()) return true;

  // Check if the target is beyond the "invisible fence" for this system.
  const auto fit = fenceCount.find(&target);
  if(fit == fenceCount.end()) return true;
  else return fit->second != FENCE_MAX;
}
  ...
  else if(!Random::Int(30))
  {
    ...
    for(const auto &helper : ships)
    {
      ...
      if(helper->GetGovernment()->IsEnemy(gov) && flagship && system == flagship->GetSystem())
      {
        ...
        bool harmless =
            helper->IsDisabled() || (helper->IsOverheated() && helper->Heat() >= 1.1) || !helper->IsTargetable();
      ...
      if(helper->GetShipToAssist() && helper->GetShipToAssist().get() != &ship) continue;
      // If the ship is mining or chasing flotsam, it cannot help this ship.
      if(helper->GetTargetAsteroid() || helper->GetTargetFlotsam()) continue;
      // Your escorts only help other escorts, and your flagship never helps.
      if((helper->IsYours() && !ship.IsYours()) || helper.get() == flagship) continue;
      ...
      if(!CanHelp(ship, *helper, needsFuel, needsEnergy)) continue;
...
bool AI::CanHelp(const Ship &ship, const Ship &helper, const bool needsFuel, const bool needsEnergy) const
{
  ...
  if(helperList.find(&helper) != helperList.end()) return false;
...
std::shared_ptr<Ship> AI::FindTarget(const Ship &ship) const
{
  // If this ship has no government, it has no enemies.
  std::shared_ptr<Ship> target;
  const Government     *gov = ship.GetGovernment();
  if(!gov || ship.GetPersonality().IsPacifist()) return FindNonHostileTarget(ship);
  ...
  if(isYours)
  {
    ...
    if(it != orders.end())
    {
      ...
      if(it->second.Has(Orders::Types::HOLD_FIRE)) return target;
    }
  }
  ...
  if(!maxRange) return isYours ? target : FindNonHostileTarget(ship);

  const Personality    &person    = ship.GetPersonality();
  std::shared_ptr<Ship> oldTarget = ship.GetTargetShip();
  if(oldTarget && !oldTarget->IsTargetable()) oldTarget.reset();
  ...
  double closest    = person.IsHunting()   ? std::numeric_limits<double>::infinity()
  ...
  if(!person.IsDaring() && strengthIt != shipStrength.end()) maxStrength = 2 * strengthIt->second;
  ...
  for(const auto &foe : enemies)
  {
    ...
    const bool isPotentialNemesis = person.IsNemesis() && (foe->IsYours() || foe->GetPersonality().IsEscort());
    if(hasNemesis && !isPotentialNemesis) continue;
    if(!CanPursue(ship, *foe)) continue;

    // Estimate the range a second from now, so ships prefer foes they are approaching.
    double range = (foe->Position() + 60. * foe->Velocity()).Distance(ship.Position() + 60. * ship.Velocity());
    ...
    if(maxStrength && range > 1000. && !foe->IsDisabled())
    {
      ...
      if(otherStrengthIt != shipStrength.end() && otherStrengthIt->second > maxStrength) continue;
    }

    // Merciful ships do not attack any ships that are trying to escape.
    if(person.IsMerciful() && foe->IsFleeing()) continue;
    ...
    if((person.Disables() || (!person.IsNemesis() && foe != oldTarget.get())) && foe->IsDisabled() &&
       (!canPlunder || Has(ship, foe->weak_from_this(), ShipEvent::BOARD)))
    {
      continue;
    }
    ...
    if(!preferredTarget && targeterCount > 2)
    {
      ...
      if(targeterStrength >= 0.25 * alliedStrength) range += 500;
      // Deprioritize this if it is being targeted by more than twice its strength.
      if(targeterStrength >= 2. * foe->Strength()) range += 500;
    }
    ...
    // While those that do, do so only if no "live" enemies are nearby.
      ...
      if(any_of(
             boarders.begin(),
             boarders.end(),
             [&ship, &foe](auto &it) { return it.first != &ship && it.second == foe; }))
      {
        continue;
      }
    ...
    if(foe->IsOverheated()) range += 3000. * (foe->Heat() - .9);
  ...
  if(!isYours && !target && (ship.IsSpecial() || scanPermissions.at(gov))) target = FindNonHostileTarget(ship);
  ...
  if(!target && person.IsVindictive())
  {
    ...
    if(target && (target->IsCloaked() || target->GetSystem() != ship.GetSystem())) target.reset();
  }
...
std::shared_ptr<Ship> AI::FindNonHostileTarget(const Ship &ship) const
{
  std::shared_ptr<Ship> target;
  ...
  int  searchTime     = isSurveillance ? 9000 : 18'000;
  ...
  if((cargoScan || outfitScan) && shipScanCount < maxScanCount && shipScanTime < forfeitTime)
  {
    ...
    if(oldTarget)
    {
      ...
      if((cargoScanInProgress && range < 2. * cargoScan) || (outfitScanInProgress && range < 2. * outfitScan))
    ...
    else if(shipScanTime < searchTime)
    {
      ...
      double            closest = std::max(cargoScan, outfitScan) * 2.;
      const Government *gov     = ship.GetGovernment();
      for(const auto &it : GetShipsList(ship, false))
      {
        if(it->GetGovernment() != gov)
        {
          std::shared_ptr<Ship> ptr = it->shared_from_this();
...
std::vector<Ship *> AI::GetShipsList(const Ship &ship, bool targetEnemies, double maxRange) const
{
  if(maxRange < 0.) maxRange = std::numeric_limits<double>::infinity();

  auto targets = std::vector<Ship *>();
  ...
  if(it != rosters.end() && !it->second.empty())
  {
    ...
    for(const auto &target : it->second)
    {
      if(target->IsTargetable() && target->GetSystem() == here &&
         ...
         p.Distance(target->Position()) < maxRange && (ship.IsYours() || !target->GetPersonality().IsMarked()) &&
...
bool AI::FollowOrders(Ship &ship, Command &command)
{
  ...
  std::shared_ptr<Ship> parent = ship.GetParent();
  if(!parent) return false;
  // If your parent is jumping or absent, that overrides your orders unless
  // your orders are to hold position.
  if(parent && !(shipOrders.Has(Orders::Types::HOLD_POSITION) || shipOrders.Has(Orders::Types::HOLD_ACTIVE) ||
                 shipOrders.Has(Orders::Types::MOVE_TO)))
  {
    if(parent->GetSystem() != ship.GetSystem()) return false;
    if(parent->Commands().Has(Command::JUMP) && ship.JumpsRemaining()) return false;
  }
  ...
  std::shared_ptr<Ship>    target         = shipOrders.GetTargetShip();
  std::shared_ptr<Minable> targetAsteroid = shipOrders.GetTargetAsteroid();
  ...
  if(shipOrders.Has(Orders::Types::MOVE_TO) && targetSystem && ship.GetSystem() != targetSystem)
  {
    ...
    if((ship.GetTargetSystem() && ship.JumpsRemaining()) || ship.GetTargetStellar()) MoveIndependent(ship, command);
    else return false;
  }
  else if(
      (shipOrders.Has(Orders::Types::MOVE_TO) || shipOrders.Has(Orders::Types::HOLD_ACTIVE)) &&
      ship.Position().Distance(targetPoint) > 20.)
  {
    MoveTo(ship, command, targetPoint, Point(), 10., .1);
  }
  else if(
      shipOrders.Has(Orders::Types::HOLD_POSITION) || shipOrders.Has(Orders::Types::HOLD_ACTIVE) ||
      shipOrders.Has(Orders::Types::MOVE_TO))
  {
  ...
  {
    if(ship.GetFormationPattern()) MoveInFormation(ship, command);
    else CircleAround(ship, command, *target);
  }
  else {
...
void AI::MoveInFormation(Ship &ship, Command &command)
{
  ...
  auto insert = patterns.emplace(
      std::piecewise_construct,
      std::forward_as_tuple(pattern),
      std::forward_as_tuple(formationLead, pattern));
  ...
  if(inPosition)
  {
    ...
    if(abs(facingDeltaDegrees) > FACING_TOLERANCE_DEGREES) command.SetTurn(facingDeltaDegrees);
...
void AI::MoveIndependent(Ship &ship, Command &command)
{
  ...
  std::shared_ptr<const Ship> target = ship.GetTargetShip();
  ...
  if(!ship.IsYours() && !ship.GetPersonality().IsUnconstrained())
  {
    if(target)
    {
      ...
      if(extrapolated.Length() >= invisibleFenceRadius)
      {
        ...
        if(ship.Velocity().Dot(ship.Position()) > 0.) command |= Command::FORWARD;
        return;
      }
    }
  ...
  else if(target && (gov->IsEnemy(target->GetGovernment()) || friendlyOverride))
  {
    ...
    if(target)
    {
      ...
      // Detarget if I cannot scan, or if I already scanned the ship.
          ...
          (!cargoScan || Has(gov, target, ShipEvent::SCAN_CARGO)) &&
      ...
      // Move to (or near) the ship and scan it.
        ...
        if(target->Velocity().Length() > ship.MaxVelocity() * 0.9) CircleAround(ship, command, *target);
        else MoveTo(ship, command, target->Position(), target->Velocity(), 1., 1.);
        if(!ship.IsYours() && (ship.IsSpecial() || scanPermissions.at(gov))) command |= Command::SCAN;
      }
      return;
    }
  }
  ...
  if(!ship.GetTargetSystem() && !ship.GetTargetStellar() && !shouldStay)
  {
    ...
    std::vector<int>                systemWeights;
    int                             totalWeight = 0;
    const std::set<const System *> &links       = ship.JumpNavigation().HasJumpDrive()
    ...
    std::vector<const StellarObject *> planets;
    ...
    if(!totalWeight)
    {
      ...
      if(origin->Objects().empty()) return;
      totalWeight = 1;
      planets.push_back(&origin->Objects().front());
    }

    std::set<const System *>::const_iterator it     = links.begin();
  ...
  if(ship.GetTargetSystem())
  {
    ...
    if(!EscortsReadyToJump(ship)) command |= Command::WAIT;
  }
...
{
  if(ship.GetFormationPattern()) MoveInFormation(ship, command);
  else KeepStation(ship, command, parent);
}
...
void AI::MoveEscort(Ship &ship, Command &command)
{
  ...
  bool          hasFuelCapacity = ship.Attributes().GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity"));
  ...
  const Planet *parentPlanet =
      (parent.GetPlanet() ? parent.GetPlanet()
  ...
  {
    if(parent.GetTargetSystem() != ship.GetTargetSystem()) SelectRoute(ship, parent.GetTargetSystem());
    ...
    {
      MoveToPlanet(ship, command);
      if(parent.IsEnteringHyperspace()) command |= Command::LAND;
    }
    else if(needsFuel)
    {
  ...
  else if((parent.Commands().Has(Command::LAND) || parent.IsLanding()) && parentIsHere && planetIsHere)
  {
    ...
    else if(parentPlanet->IsWormhole())
    {
      ...
      {
        PrepareForHyperspace(ship, command);
        if(parent.IsLanding()) command |= Command::JUMP;
      }
      else if(ship.GetTargetStellar())
      {
        MoveToPlanet(ship, command);
        if(parent.IsLanding()) command |= Command::LAND;
      }
      else if(needsFuel)
      {
        // Return to the system center to maximize solar collection rate.
...
void AI::Refuel(Ship &ship, Command &command)
{
  ...
  if(CanRefuel(ship, parentTarget)) ship.SetTargetStellar(parentTarget);
  else if(!CanRefuel(ship, ship.GetTargetStellar())) ship.SetTargetStellar(FindLandingLocation(ship));
...
bool AI::CanRefuel(const Ship &ship, const StellarObject *target)
{
  ...
  if(!planet->IsInSystem(ship.GetSystem())) return false;

  if(!planet->HasFuelFor(ship)) return false;
...
{
  const System *from = ship.GetSystem();
  if(from == targetSystem || !targetSystem) return;
...
bool AI::ShouldDock(const Ship &ship, const Ship &parent, const System *playerSystem) const
{
  ...
  if(parent.IsDisabled()) return false;
  ...
  if(ship.IsYours())
  {
    if(!ship.HasDeployOrder() || ship.GetSystem() != playerSystem) return true;
  }
  ...
  if(ship.Health() < minHealth && (!ship.IsYours() || Preferences::Has("Damaged fighters retreat"))) return true;
  ...
  auto requiredAmmo = std::set<const Outfit *>{};
  for(const Hardpoint &hardpoint : ship.Weapons())
  {
    const Weapon *weapon = hardpoint.GetWeapon();
    if(weapon && !hardpoint.IsSpecial())
  ...
  if(!requiredAmmo.empty()) return true;
  ...
  if(maxFuel && ship.Fuel() < .005 &&
     parent.JumpNavigation().JumpFuel() < parent.Fuel() * parent.Attributes().Get("fuel capacity") - maxFuel)
  {
    return true;
  }
  ...
  if(!ship.IsYours() || Preferences::Has("Fighters transfer cargo"))
  {
    ...
    if(!hasEnemy && parent.Cargo().Free())
    {
      ...
      if(!cargo.IsEmpty() && cargo.Size() && cargo.Free() < 5) return true;
    }
  }
...
double AI::TurnBackward(const Ship &ship) { return TurnToward(ship, -ship.Velocity()); }
...
double AI::TurnToward(const Ship &ship, const Point &vector, const double precision)
{
  ...
  if(dot > 0.)
  {
    ...
    bool close = false;
    if(precision < 1. && precision > 0. && dot * dot >= precision * vector.LengthSquared()) close = true;
    double angle = asin(std::min(1., std::max(-1., cross / vector.Length()))) * TO_DEG;
...
{
  if(!ship.GetTargetStellar()) return false;
    ...
    double       cruiseSpeed)
{
  ...
  if(isClose && speed < slow) return true;

  bool shouldReverse = false;
  dp                 = targetPosition - StoppingPoint(ship, targetVelocity, shouldReverse);

  // Calculate target std::vector required to get where we want to be.
  Point tv             = dp;
  ...
  if(hasCruiseSpeed)
  {
    ...
    if(tv.LengthSquared() < .01) tv = dp;
  }

  bool isFacing = (tv.Unit().Dot(angle.Unit()) > .95);
  if(!isClose || (!isFacing && !shouldReverse)) command.SetTurn(TurnToward(ship, tv));
  ...
  if(isFacing && (velocity.LengthSquared() <= maxVelocity * maxVelocity || dp.Unit().Dot(velocity.Unit()) < .95))
  {
    ...
    if(!hasCruiseSpeed || !movingTowardsTarget || velocity.Length() < cruiseSpeed) command |= Command::FORWARD;
  }
...
bool AI::Stop(const Ship &ship, Command &command, double maxSpeed, const Point &direction)
{
  ...
  if(speed <= (maxSpeed ? maxSpeed : VELOCITY_ZERO)) return true;
  if(!maxSpeed) command |= Command::STOP;
  ...
  if(ship.Attributes().Get("reverse thrust"))
  {
    ...
    double degreesToTurn  = TO_DEG * acos(std::min(1., std::max(-1., -velocity.Unit().Dot(angle.Unit()))));
    ...
    if(direction)
    {
      ...
      double degreesFromBackwards  = TO_DEG * acos(std::min(1., std::max(-1., direction.Unit().Dot(-velocity.Unit()))));
      ...
      double degreesFromForward   = TO_DEG * acos(std::min(1., std::max(-1., direction.Unit().Dot(angle.Unit()))));
    ...
    {
      command.SetTurn(TurnToward(ship, velocity));
      if(velocity.Unit().Dot(angle.Unit()) > limit) command |= Command::BACK;
      return false;
    }
  }

  command.SetTurn(TurnBackward(ship));
  if(velocity.Unit().Dot(angle.Unit()) < -limit) command |= Command::FORWARD;
...
void AI::PrepareForHyperspace(const Ship &ship, Command &command)
{
  ...
  if(!hasHyperdrive && !hasJumpDrive) return;
  ...
  double departure = isJump ? ship.GetSystem()->JumpDepartureDistance() : ship.GetSystem()->HyperDepartureDistance();
...
void AI::CircleAround(const Ship &ship, Command &command, const Body &target)
{
  ...
  if(length > 200. && ship.Facing().Unit().Dot(direction) >= 0.)
  {
    ...
    if(length > 750. && ShouldUseAfterburner(ship)) command |= Command::AFTERBURNER;
  }
}
...
void AI::Swarm(const Ship &ship, Command &command, const Body &target)
{
  ...
  if(std::isnan(rendezvousTime) || rendezvousTime > 600.) rendezvousTime = 600.;
...
void AI::KeepStation(const Ship &ship, Command &command, const Body &target)
{
  ...
  if(std::isnan(positionTime) || positionTime > MAX_TIME) positionTime = MAX_TIME;
  ...
  if(abs(targetAngle) > 180.) targetAngle += (targetAngle < 0. ? 360. : -360.);
  ...
  if(lastTurn && (changedDirection || (lastTurn < 1. && targetTurn > lastTurn)))
  {
    ...
    double dampedTurn = (changedDirection ? 0. : lastTurn) + std::min(.025, targetTurn);
...
void AI::Attack(const Ship &ship, Command &command, const Ship &target)
{
  ...
  if(minSafeDistance || (useArtilleryAI && shortestRange < weaponDistanceFromTarget))
  {
    ...
    bool   useReverse   = reverseSpeed && (reverseSpeed >= std::min(target.MaxVelocity(), ship.MaxVelocity()) ||
    ...
    if(direction.Length() <
       std::max(minSafeDistance + std::max(slowdownDistance, 0.), useArtilleryAI * .75 * shortestArtillery))
    {
      if(useReverse)
      {
        command.SetTurn(TurnToward(ship, direction));
        if(ship.Facing().Unit().Dot(direction) >= 0.) command |= Command::BACK;
      }
      else {
        command.SetTurn(TurnToward(ship, -direction));
        if(ship.Facing().Unit().Dot(direction) <= 0.) command |= Command::FORWARD;
      }
    }
    else {
...
void AI::MoveToAttack(const Ship &ship, Command &command, const Body &target)
{
  ...
  double diameter        = std::max(200., circumference / PI);
  ...
  else if((facing >= 0. && direction.Length() > diameter) || (ship.Velocity().Dot(direction) < 0. && facing >= .9))
  {
    command |= Command::FORWARD;
    // Use afterburner, if applicable.
    if(direction.Length() > 600. && ShouldUseAfterburner(ship)) command |= Command::AFTERBURNER;
  }
}
...
void AI::PickUp(const Ship &ship, Command &command, const Body &target)
{
  ...
  if(std::isnan(time)) time = p.Length() / vMax;
  double degreesToTurn  = TO_DEG * acos(std::min(1., std::max(-1., p.Unit().Dot(ship.Facing().Unit()))));
  ...
  if(dp > .7) command |= Command::FORWARD;
  ...
  {
    if(dp > std::max(.9, std::min(.9999, 1. - squareDistance / 10000000.))) command |= Command::AFTERBURNER;
  }
}
...
{
  if(!ship.Attributes().Get("afterburner thrust")) return false;
...
void AI::DoAppeasing(const std::shared_ptr<Ship> &ship, double *threshold) const
{
  double health = .5 * ship->Shields() + ship->Hull();
  if(1. - health <= *threshold) return;

  const auto enemies = GetShipsList(*ship, true);
  if(none_of(
         enemies.begin(),
         enemies.end(),
         [&ship](const Ship *foe) noexcept -> bool { return !foe->IsDisabled() && foe->GetTargetShip() == ship; }))
  {
    return;
  }
  ...
  for(auto &&commodity : ship->Cargo().Commodities())
  {
    ...
    {
      int dumped = std::min(commodity.second, toDump);
  ...
  *threshold = (1. - health) + .1;
  ...
  if(ship->GetPersonality().IsMute()) return;
  const Government  *government = ship->GetGovernment();
  const std::string &language   = government->Language();
  if(language.empty() || player.Conditions().Get("language: " + language))
  {
    Messages::Add(
        {government->DisplayName() + " " + ship->Noun() + " \"" + ship->GivenName() +
...
void AI::DoSwarming(Ship &ship, Command &command, std::shared_ptr<Ship> &target)
{
  ...
  if(ship.IsLanding() || !target || !CanSwarm(ship, *target) || !Random::Int(600))
  {
    ...
    ship.SetTargetShip(target);
    if(target) ++swarmCount[target.get()];
  }
  // If a friendly ship to flock with was not found, return to an available planet.
  if(target) Swarm(ship, command, *target);
  else if(ship.Zoom() == 1.) Refuel(ship, command);
}


void AI::DoSurveillance(Ship &ship, Command &command, std::shared_ptr<Ship> &target)
{
  ...
  else if(ship.GetTargetStellar())
  {
    ...
    if(distance < atmosphereScan && !Random::Int(100)) ship.SetTargetStellar(nullptr);
    else if(!isStaying) command |= Command::LAND;
  }
  else if(target)
  {
    ...
    {
      ship.SetTargetShip(std::shared_ptr<Ship>());
    }
    else {
      if(target->Velocity().Length() > ship.MaxVelocity() * 0.9) CircleAround(ship, command, *target);
      else MoveTo(ship, command, target->Position(), target->Velocity(), 1., 1.);
    ...
    shipScanCount                     += outfitScansIt != outfitScans.end() ? outfitScansIt->second.size() : 0;
    int shipScanTime                   = scanTimeIt != scanTime.end() ? scanTimeIt->second : 0;
    if((cargoScan || outfitScan) && shipScanCount < 12 && shipScanTime < 18'000)
    {
      for(const auto &it : GetShipsList(ship, false))
      {
        if(it->GetGovernment() != gov)
        {
          std::weak_ptr<Ship> ptr = it->weak_from_this();
          if((!cargoScan || Has(gov, ptr, ShipEvent::SCAN_CARGO)) &&
             (!outfitScan || Has(gov, ptr, ShipEvent::SCAN_OUTFITS)))
          {
            continue;
          ...
          if(it->IsTargetable()) targetShips.emplace_back(it);
        }
      }
    }

    // Consider scanning any planetary object in the system, if able.
    std::vector<const StellarObject *> targetPlanets;
    double                             atmosphereScan = ship.Attributes().Get("atmosphere scan");
    if(atmosphereScan)
    {
      for(const StellarObject &object : system->Objects())
        if(object.HasSprite() && !object.IsStar() && !object.IsStation()) targetPlanets.push_back(&object);
    }
    ...
    if(ship.JumpsRemaining(false))
    {
      const auto &links = ship.JumpNavigation().HasJumpDrive()
                              ...
                              : system->Links();
      for(const System *link : links)
        if(!ship.IsRestrictedFrom(*link)) targetSystems.push_back(link);
    }
    ...
    if(index < targetShips.size())
    {
      ...
      if(index < targetPlanets.size()) ship.SetTargetStellar(targetPlanets[index]);
      else ship.SetTargetSystem(targetSystems[index - targetPlanets.size()]);
    }
  }
}
...
void AI::DoMining(Ship &ship, Command &command)
{
  ...
  std::shared_ptr<Minable> target = ship.GetTargetAsteroid();
  ...
  if(ship.Velocity().Dot(heading.Unit()) < .7 * ship.MaxVelocity()) command |= Command::FORWARD;
}
...
bool AI::DoHarvesting(Ship &ship, Command &command) const
{
  ...
  std::shared_ptr<Flotsam> target = ship.GetTargetFlotsam();
  // Don't try to chase flotsam that are already being pulled toward the ship by a tractor beam.
  const std::set<const Flotsam *> &avoid = ship.GetTractorFlotsam();
  ...
  if(!target)
  {
    ...
    if(Random::Int(10)) return false;

    // Don't chase anything that will take more than 10 seconds to reach.
    double bestTime = 600.;
    for(const std::shared_ptr<Flotsam> &it : flotsam)
    {
      if(!ship.CanPickUp(*it) || avoid.contains(it.get())) continue;
      ...
      if(range > 800. || (range > 100. && p.Unit().Dot(ship.Facing().Unit()) < .9 && !ship.IsYours())) continue;
...
{
  if(ship.GetPersonality().IsDecloaked()) return false;
  double cloakingSpeed = ship.CloakingSpeed();
  if(!cloakingSpeed) return false;
  ...
  if(cloakingFuel && !attributes.Get("ramscoop"))
  {
    ...
    if(fuel < ship.JumpNavigation().JumpFuel()) return false;
  }

  // If your parent has chosen to cloak, cloak and rendezvous with them.
  const std::shared_ptr<const Ship> &parent                = ship.GetParent();
     ...
     parent->GetSystem() == ship.GetSystem())
  {
    ...
    if(isPlayer && ship.GetGovernment() == parentGovernment) shouldCloakWithParent = true;
    else if(isPlayer && ship.GetPersonality().IsEscort() && !ship.GetPersonality().IsUninterested())
      shouldCloakWithParent = true;
    else if(!isPlayer && !parent->GetGovernment()->IsEnemy(ship.GetGovernment())) shouldCloakWithParent = true;
  }
  ...
  {
    if(attributes.Get("shield generation") > 0.) canRecoverShieldsCloaked = true;
  ...
  {
    if(attributes.Get("hull repair rate") > 0.) canRecoverHullCloaked = true;
    else if(attributes.Get("cloaking repair delay") < 1. && attributes.Get("delayed hull repair") > 0.)
      canRecoverHullCloaked = true;
  }
  bool cloakToRepair =
      (ship.Health() < RETREAT_HEALTH + hysteresis) &&
      ((ship.Shields() < 1. && canRecoverShieldsCloaked) || (ship.Hull() < 1. && canRecoverHullCloaked));
  if(cloakToRepair && (cloakFreely || range < 2000. * (1. + hysteresis)))
  {
    ...
    if(nearestEnemy)
    {
      ...
      else safety = -ship.Position().Unit();
  ...
  if(range == MAX_RANGE && cloakFreely && !ship.GetTargetShip()) command |= Command::CLOAK;
...
void AI::DoPatrol(Ship &ship, Command &command) const
{
  ...
  if(ship.Position().LengthSquared() > radius * radius)
  {
    ...
    if(!ship.GetPersonality().IsStaying() && !Random::Int(10'000))
    {
      std::vector<const StellarObject *> landingTargets;
...
{
  if(!command.Has(Command::FORWARD) && !command.Has(Command::BACK)) return;

  auto &close = closeBy[&ship];
  if(recheckCloseShips)
  {
    close.clear();
    for(const std::shared_ptr<Ship> &other : ships)
    {
      // Do not scatter away from yourself, or ships in other systems.
      if(other.get() == &ship || other->GetSystem() != ship.GetSystem()) continue;
      ...
      if(offset.LengthSquared() > SCATTER_TRACK) continue;
      close.insert(other);
    }
  }
  ...
  {
    std::shared_ptr<const Ship> other = it->lock();
    ...
    else if(fabs(other->TurnRate() / turnRate - 1.) > .05 || fabs(other->Acceleration() / acceleration - 1.) > .05)
    {
...
{
  std::shared_ptr<Ship> scanningShip;
  ...
  {
    if(!ship.GetGovernment()->Trusts(otherShip->GetGovernment()) && otherShip->Commands().Has(Command::SCAN) &&
       otherShip->GetTargetShip().get() == &ship && !otherShip->IsDisabled() && !otherShip->IsDestroyed())
    {
  ...
  if(scanningShip)
  {
    ...
    double maxScanRange = std::max(cargoDistance, outfitDistance);
    ...
    {
      Point away;
      if(ship.GetPersonality().IsUnconstrained() || !fenceCount.contains(&ship)) away = pos - scanningPos;
      else away = -pos;
...
// Instead of coming to a full stop, adjust to a target velocity std::vector
Point AI::StoppingPoint(const Ship &ship, const Point &targetVelocity, bool &shouldReverse)
{
  ...
  {
    if(ship.IsUsingJumpDrive() || ship.IsEnteringHyperspace()) return position;
...
{
  std::shared_ptr<const Ship> target = ship.GetTargetShip();
  if(target) return TargetAim(ship, *target);

  std::shared_ptr<const Minable> targetAsteroid = ship.GetTargetAsteroid();
  if(targetAsteroid) return TargetAim(ship, *targetAsteroid);
...
Point AI::TargetAim(const Ship &ship, const Body &target)
{
  ...
  {
    const Weapon *weapon = hardpoint.GetWeapon();
    if(!weapon || hardpoint.IsHoming() || hardpoint.IsTurret()) continue;
    ...
    if(std::isnan(steps)) continue;

    steps  = std::min(steps, weapon->TotalLifetime());
    p     += steps * v;
    ...
    const std::optional<Point> &targetOverride) const
{
  ...
  std::vector<std::pair<Point, Point>> targets;
  if(!targetOverride)
  {
    // First, get the set of potential hostile ships.
    std::vector<const Body *> targetBodies;
    ...
    if(opportunistic || !currentTarget || !currentTarget->IsTargetable())
    {
      ...
      double maxRange = 0.;
      for(const Hardpoint &hardpoint : ship.Weapons())
        if(hardpoint.CanAim(ship)) maxRange = std::max(maxRange, hardpoint.GetWeapon()->Range());
      ...
      for(auto &&foe : enemies)
        if(!foe->IsDisabled()) targetBodies.emplace_back(foe);
    ...
    if(ship.GetTargetAsteroid()) targetBodies.push_back(ship.GetTargetAsteroid().get());
    ...
    if(targetBodies.empty())
    {
      for(const Hardpoint &hardpoint : ship.Weapons())
      {
        if(hardpoint.CanAim(ship))
        {
          ...
          if(!previous && Random::Int(60)) continue;
          ...
          double       bias         = (centerAngle - hardpoint.GetAngle()).Degrees() / std::min(arcMiddleDegrees, 180.);
  ...
  for(const Hardpoint &hardpoint : ship.Weapons())
  {
    if(hardpoint.CanAim(ship))
    {
      ...
      for(auto [p, v] : targets)
      {
        ...
        if(!weapon->Acceleration()) v -= ship.Velocity();
        // By the time this action is performed, the target will
        // have moved forward one time step.
        p += v;

        double rendezvousTime = std::numeric_limits<double>::quiet_NaN();
        ...
        if(isInstantaneous && distance < vp)
        {
          ...
          if(!isInstantaneous) rendezvousTime = RendezvousTime(p, v, vp);
          ...
          if(std::isnan(rendezvousTime))
            rendezvousTime = std::max(distance / (vp ? vp : 1.), 2 * weapon->TotalLifetime());
          ...
          rendezvousTime = std::max(0., rendezvousTime - weapon->TotalLifetime());
        }

        // Determine how much the turret must turn to face that std::vector.
        double degrees      = 0.;
        ...
        if(hardpoint.IsOmnidirectional())
        {
          ...
          if(!angleToPoint.IsInRange(minArc, maxArc))
          {
            ...
            if(fabs(minDegree) < fabs(maxDegree)) angleToPoint = minArc;
            else angleToPoint = maxArc;
          }
...
{
  const Personality &person = ship.GetPersonality();
  if(person.IsPacifist() || ship.CannotAct(Ship::ActionType::FIRE)) return;
  ...
  if(person.IsFrugal() || (ship.IsYours() && escortsAreFrugal && escortsUseAmmo))
  {
    ...
    if(beFrugal)
    {
      ...
      if(ait != allyStrength.end() && eit != enemyStrength.end() && ait->second < eit->second) beFrugal = false;
    }
  }
  ...
  std::shared_ptr<Ship> currentTarget    = ship.GetTargetShip();
  const Government     *gov              = ship.GetGovernment();
  bool                  friendlyOverride = false;
  ...
  if(ship.IsYours())
  {
    ...
    if(it != orders.end())
    {
      if(it->second.Has(Orders::Types::HOLD_FIRE)) return;
  ...
  bool currentIsEnemy =
      currentTarget && currentTarget->GetGovernment()->IsEnemy(gov) && currentTarget->GetSystem() == ship.GetSystem();
  if(currentTarget && !(currentIsEnemy || friendlyOverride)) currentTarget.reset();
  ...
  for(const Hardpoint &hardpoint : ship.Weapons())
  {
    ...
    {
      const Weapon *weapon = hardpoint.GetWeapon();
      if(!(!currentTarget && hardpoint.IsHoming() && weapon->Ammo()) && !(!secondary && weapon->Icon()) &&
         !(beFrugal && weapon->Ammo()) && !(isWaitingToJump && weapon->FiringForce()))
      {
  ...
  for(const Hardpoint &hardpoint : ship.Weapons())
  {
    ...
    if(!hardpoint.IsReady()) continue;

    // Skip weapons omitted by the "Automatic firing" preference.
    if(isFlagship)
    {
      const Preferences::AutoFire autoFireMode = Preferences::GetAutoFire();
      if(autoFireMode == Preferences::AutoFire::GUNS_ONLY && hardpoint.IsTurret()) continue;
      if(autoFireMode == Preferences::AutoFire::TURRETS_ONLY && !hardpoint.IsTurret()) continue;
    }

    const Weapon *weapon = hardpoint.GetWeapon();
    // Don't expend ammo for homing weapons that have no target selected.
    if(!currentTarget && weapon->Homing() && weapon->Ammo()) continue;
    // Don't fire secondary weapons if told not to.
    if(!secondary && weapon->Icon()) continue;
    // Don't expend ammo if trying to be frugal.
    if(beFrugal && weapon->Ammo()) continue;
    // Don't use weapons with firing force if you are preparing to jump.
    if(isWaitingToJump && weapon->FiringForce()) continue;
    ...
    if(weapon->FiringFuel())
    {
      ...
      if(!secondary || fuel < (isStaying ? 0. : ship.JumpNavigation().JumpFuel())) continue;
    }
    ...
    if(weapon->Homing() && currentTarget)
    {
      ...
      if(currentTarget->IsDisabled() && (disables || (plunders && !hasBoarded)) && !disabledOverride) continue;
      // Don't fire secondary weapons at targets that have started jumping.
      if(weapon->Icon() && currentTarget->IsEnteringHyperspace()) continue;
      ...
      if(!weapon->IsSafe() && p.Length() <= (weapon->BlastRadius() + weapon->TriggerRadius())) continue;
    ...
    for(const auto &target : enemies)
    {
      ...
      if(target->IsDisabled() && (disables || (plunders && !hasBoarded)) && !disabledOverride) continue;
      // Merciful ships let fleeing ships go.
      if(target->IsFleeing() && person.IsMerciful()) continue;
      // Don't hit ships that cannot be hit without targeting
      if(target != currentTarget.get() && !FighterHitHelper::IsValidTarget(target)) continue;
      ...
      // Get the std::vector the weapon will travel along.
...
void AI::AutoFire(const Ship &ship, FireCommand &command, const Body &target) const
{
  ...
  for(const Hardpoint &hardpoint : ship.Weapons())
  {
    ...
    if(!hardpoint.IsReady()) continue;
    const Weapon *weapon = hardpoint.GetWeapon();
    if(weapon->Icon() || weapon->Ammo()) continue;
    ...
    const Mask &mask = target.GetMask(step);
    if(mask.Collide(-p, v, target.Facing(), 1.) < 1.) command.SetFire(index);
  }
}
...
double AI::RendezvousTime(const Point &p, const Point &v, double vp)
{
  ...
  if(discriminant < 0.) return std::numeric_limits<double>::quiet_NaN();
  ...
  if(r1 >= 0. && r2 >= 0.) return std::min(r1, r2);
  else if(r1 >= 0. || r2 >= 0.) return std::max(r1, r2);
...
bool AI::TargetMinable(Ship &ship) const
{
  ...
  if(!scanRangeMetric) return false;
  const bool findClosest       = Preferences::Has("Target asteroid based on");
  auto       bestMinable       = ship.GetTargetAsteroid();
  double     bestScore         = findClosest ? std::numeric_limits<double>::max() : 0.;
  auto       GetDistanceMetric = [&ship](const Minable &minable) -> double
  { return ship.Position().DistanceSquared(minable.Position()); };
  if(bestMinable)
  {
    if(findClosest) bestScore = GetDistanceMetric(*bestMinable);
    else bestScore = bestMinable->GetValue();
  }
  auto MinableStrategy = [&findClosest, &bestMinable, &bestScore, &GetDistanceMetric]()
      -> std::function<void(const std::shared_ptr<Minable> &)>
  {
    if(findClosest)
    {
      return [&bestMinable, &bestScore, &GetDistanceMetric](const std::shared_ptr<Minable> &minable) -> void
      {
      ...
      return [&bestMinable, &bestScore, &GetDistanceMetric](const std::shared_ptr<Minable> &minable) -> void
      {
        ...
        if(newScore > bestScore ||
           (newScore == bestScore && GetDistanceMetric(*minable) < GetDistanceMetric(*bestMinable)))
        {
          bestScore   = newScore;
          bestMinable = minable;
        }
  ...
  {
    if(GetDistanceMetric(*minable) > scanRangeMetric) continue;
    if(bestMinable) UpdateBestMinable(minable);
    else bestMinable = minable;
  }
  if(bestMinable) ship.SetTargetAsteroid(bestMinable);
...
void AI::MovePlayer(Ship &ship, Command &activeCommands)
{
  ...
  if(player.HasTravelPlan())
  {
    ...
    for(const StellarObject &object : ship.GetSystem()->Objects())
    {
      if(object.HasSprite() && object.HasValidPlanet() && object.GetPlanet()->IsWormhole() &&
         object.GetPlanet()->IsAccessible(&ship) && player.HasVisited(*object.GetPlanet()) && player.CanView(*system))
      {
        const auto *wormhole = object.GetPlanet()->GetWormhole();
        if(&wormhole->WormholeDestination(*ship.GetSystem()) != system) continue;

        isWormhole = true;
        if(!ship.GetTargetStellar() || autoPilot.Has(Command::JUMP)) ship.SetTargetStellar(&object);
        break;
      }
    }
    if(!isWormhole) ship.SetTargetSystem(system);
  }
  ...
  if(ship.IsEnteringHyperspace() && !ship.IsHyperspacing())
  {
    ...
    std::set<const Planet *> destinations;
    Date                     deadline;
    ...
    for(const Mission &mission : player.Missions())
    {
      ...
      if(!mission.IsVisible() || mission.IsFailed()) continue;
      ...
      if(mission.Stopovers().empty() && mission.Waypoints().empty() && mission.Destination()->IsInSystem(system) &&
      ...
      for(const Planet *planet : mission.Stopovers())
      {
        if(planet->IsInSystem(system) && planet->IsAccessible(&ship))
        {
          ...
          ++missions;
          if(!bestDestination) bestDestination = planet;
        }
      }
    }
    ...
    {
      std::string message  = "Note: you have ";
      message             += (missions == 1 ? "a mission that requires" : "missions that require");
      message             += " landing on ";
      message             += Format::List<std::set, const Planet *>(
          destinations,
          [](const Planet *const &planet) { return planet->DisplayName(); });
    ...
    if(bestDestination) ship.SetTargetStellar(system->FindStellar(bestDestination));
  }
  ...
  if(activeCommands.Has(Command::NEAREST))
  {
    ...
    for(const std::shared_ptr<Ship> &other : ships)
    {
      if(other.get() != &ship && other->IsTargetable())
      {
        ...
        if((!enemy && !shift) || other->IsYours()) continue;
    ...
    if(!found) TargetMinable(ship);
    else UI::PlaySound(UI::UISound::TARGET);
  }
  else if(activeCommands.Has(Command::TARGET))
  {
    ...
    for(const std::shared_ptr<Ship> &other : ships)
    {
      ...
      if(other.get() == &ship) continue;
      // The default behavior is to ignore your fleet and any friendly escorts.
      bool isPlayer = other->IsYours() || (other->GetPersonality().IsEscort() && !other->GetGovernment()->IsEnemy());
      if(other == target)
      {
        selectNext = true;
      }
      ...
      {
        ship.SetTargetShip(other);
        if(isPlayer) player.SelectEscort(other.get(), false);
        selectNext = false;
        break;
      }
    }
  ...
  else if(activeCommands.Has(Command::BOARD))
  {
    ...
    std::shared_ptr<Ship> target = ship.GetTargetShip();
    if(target && !CanBoard(ship, *target)) target.reset();
    if(!target || activeCommands.Has(Command::WAIT) || (shift && !target->IsYours()))
    {
      if(shift) ship.SetTargetShip(std::shared_ptr<Ship>());

      const auto boardingPriority = Preferences::GetBoardingPriority();
      auto       strategy         = [&]() noexcept -> std::function<double(const Ship &)>
      {
        ...
        case Preferences::BoardingPriority::VALUE:
          return [this, &ship](const Ship &other) noexcept -> double
          {
            ...
            return this->Has(ship, other.weak_from_this(), ShipEvent::SCAN_OUTFITS) ? other.Cost()
                                                                                    : (other.ChassisCost() * 2.);
          };
        case Preferences::BoardingPriority::MIXED:
          return [this, &ship, current](const Ship &other) noexcept -> double
          {
            double cost = this->Has(ship, other.weak_from_this(), ShipEvent::SCAN_OUTFITS) ? other.Cost()
                                                                                           : (other.ChassisCost() * 2.);
        ...
        default:
          return [current](const Ship &other) noexcept -> double { return current.DistanceSquared(other.Position()); };
        }
      }();

      using ShipValue = std::pair<Ship *, double>;
      auto options    = std::vector<ShipValue>{};
      if(shift)
      {
        const auto &owned = governmentRosters[ship.GetGovernment()];
        options.reserve(owned.size());
        for(auto &&escort : owned)
          if(CanBoard(ship, *escort)) options.emplace_back(escort, strategy(*escort));
      }
      else {
        auto ships = GetShipsList(ship, true);
        options.reserve(ships.size());
        // The current target is not considered by GetShipsList.
        if(target) options.emplace_back(target.get(), strategy(*target));

        // First check if we can board enemy ships, then allies.
        for(auto &&enemy : ships)
          if(CanBoard(ship, *enemy)) options.emplace_back(enemy, strategy(*enemy));
        if(options.empty())
        {
          ships = GetShipsList(ship, false);
          options.reserve(ships.size());
          for(auto &&ally : ships)
            if(CanBoard(ship, *ally)) options.emplace_back(ally, strategy(*ally));
        }
      }
      ...
      if(options.empty())
      {
        ...
        sort(
            ...
            {
              if(boardingPriority == Preferences::BoardingPriority::PROXIMITY) return lhs.second > rhs.second;
        ...
        if(it == options.begin()) it = options.end();
  ...
  else if(activeCommands.Has(Command::LAND) && !ship.IsEnteringHyperspace() && ship.Zoom() == 1.)
  {
    ...
    auto landables = std::vector<const StellarObject *>{};
    ...
    for(const StellarObject &object : ship.GetSystem()->Objects())
    {
      ...
      if(object.HasValidPlanet() && object.GetPlanet()->IsAccessible(&ship) && object.IsVisible(ship.Position()))
      {
        ...
        if(isTryingLanding) potentialTarget = &object;
      }
    ...
    if(landIt == landables.cend()) target = nullptr;
    ...
    if(target) message.clear();
    else if(!message.empty()) UI::PlaySound(UI::UISound::FAILURE);
    ...
    else if(message.empty() && target && activeCommands.Has(Command::WAIT))
    {
      ...
      if(++landIt == landables.cend()) landIt = landables.cbegin();
      const StellarObject *next = *landIt;
      ship.SetTargetStellar(next);

      if(!next->GetPlanet()->CanLand())
      {
        message         = "The authorities on this " + next->GetPlanet()->Noun() + " refuse to clear you to land here.";
    ...
    else if(message.empty())
    {
      ...
      if(!target && !landables.empty())
      {
        if(landables.size() == 1)
        {
          ...
          for(const auto &object : landables)
          {
            ...
            if((!planet->CanLand() || !planet->GetPort().CanRecharge(Port::RechargeType::Fuel, ship.IsYours())) &&
               !planet->IsWormhole())
            {
              distance += 10000.;
            }
      ...
      {
        message = "The authorities on this " + target->GetPlanet()->Noun() + " refuse to clear you to land here.";
        messageCategory = GameData::MessageCategories().Get("high");
        UI::PlaySound(UI::UISound::FAILURE);
      }
      else if(!types.empty())
      {
        ...
        std::set<std::string>::const_iterator it  = types.begin();
        message                                  += *it++;
        if(it != types.end())
        {
          std::set<std::string>::const_iterator last = --types.end();
          if(it != last) message += ',';
          while(it != last)
            message += ' ' + *it++ + ',';
          message += " or " + *it;
    ...
    if(!message.empty()) Messages::Add({message, messageCategory});
  }
  else if(activeCommands.Has(Command::JUMP | Command::FLEET_JUMP))
  {
    if(player.TravelPlan().empty() && !isWormhole)
    {
      double      bestMatch = -2.;
      const auto &links =
          (ship.JumpNavigation().HasJumpDrive() ? ship.GetSystem()->JumpNeighbors(ship.JumpNavigation().JumpRange())
      ...
      for(const System *link : links)
      {
        ...
        if(!player.HasSeen(*link)) continue;
    ...
    else if(isWormhole)
    {
      ...
      Messages::Add(
          {"Landing on a local wormhole to navigate to the " + player.TravelPlan().back()->DisplayName() + " system.",
           GameData::MessageCategories().Get("normal")});
    }
    if(ship.GetTargetSystem() && !isWormhole)
    {
      std::string name = "selected star";
      if(player.KnowsName(*ship.GetTargetSystem())) name = ship.GetTargetSystem()->DisplayName();
      ...
      if(activeCommands.Has(Command::FLEET_JUMP))
      {
        ...
        {
          Messages::Add(
              {"Engaging fleet autopilot to jump to the " + name +
                   " system."
        ...
        Messages::Add(
            {"Engaging autopilot to jump to the " + name + " system.", GameData::MessageCategories().Get("normal")});
      }
    }
  }
  ...
  const std::shared_ptr<const Ship> target = ship.GetTargetShip();
  ...
  if(activeCommands)
  {
    if(activeCommands.Has(Command::FORWARD)) command |= Command::FORWARD;
    ...
    {
      if(!activeCommands.Has(Command::FORWARD) && ship.Attributes().Get("reverse thrust")) command |= Command::BACK;
    ...
    if(activeCommands.Has(Command::PRIMARY))
    {
      ...
      for(const Hardpoint &hardpoint : ship.Weapons())
      {
        if(hardpoint.IsReady() && !hardpoint.GetWeapon()->Icon()) firingCommands.SetFire(index);
        ++index;
      }
    }
    ...
    if(activeCommands.Has(Command::AFTERBURNER)) command |= Command::AFTERBURNER;

    if(activeCommands.Has(AutopilotCancelCommands())) autoPilot = activeCommands;
  }
     ...
     !autoPilot.Has(Command::LAND | Command::JUMP | Command::FLEET_JUMP | Command::BOARD))
  {
    if(target && target->GetSystem() == ship.GetSystem() && target->IsTargetable())
      command.SetTurn(TurnToward(ship, TargetAim(ship)));
    else if(ship.GetTargetAsteroid()) command.SetTurn(TurnToward(ship, TargetAim(ship, *ship.GetTargetAsteroid())));
    else if(ship.GetTargetStellar())
      command.SetTurn(TurnToward(ship, ship.GetTargetStellar()->Position() - ship.Position()));
  }
  else if(
      (Preferences::GetAutoAim() == Preferences::AutoAim::ALWAYS_ON ||
  ...
  if(autoPilot.Has(Command::JUMP | Command::FLEET_JUMP) && !(player.HasTravelPlan() || ship.GetTargetSystem()))
  {
    ...
    {
      Messages::Add(
          {"Autopilot: landing on " + planet->DisplayName() + ".", GameData::MessageCategories().Get("normal")});
  ...
  if(autoPilot.Has(Command::LAND) && !ship.GetTargetStellar()) autoPilot.Clear(Command::LAND);
  ...
  else if(autoPilot.Has(Command::STOP))
  {
    ...
    if(Stop(ship, command)) autoPilot.Clear(Command::STOP);
  }
...
bool AI::Has(const Ship &ship, const std::weak_ptr<const Ship> &other, int type) const
{
  auto sit = actions.find(ship.weak_from_this());
  if(sit == actions.end()) return false;

  auto oit = sit->second.find(other);
  if(oit == sit->second.end()) return false;

  return oit->second & type;
}


bool AI::Has(const Government *government, const std::weak_ptr<const Ship> &other, int type) const
{
  auto git = governmentActions.find(government);
  if(git == governmentActions.end()) return false;

  auto oit = git->second.find(other);
  if(oit == git->second.end()) return false;
...
bool AI::Has(const Ship &ship, const Government *government, int type) const
{
  ...
  if(sit == notoriety.end()) return false;

  auto git = sit->second.find(government);
  if(git == sit->second.end()) return false;

  return git->second & type;
}


void AI::UpdateStrengths(std::map<const Government *, int64_t> &strength, const System *playerSystem)
{
  ...
  for(const auto &it : ships)
  {
    if(it->GetGovernment() && it->GetSystem() == playerSystem)
    {
      ...
      if(!it->IsDisabled() && !it->IsOverheated() && !it->IsIonized()) strength[it->GetGovernment()] += it->Strength();
    }
  }
  ...
  for(const auto &it : ships)
  {
    ...
    if(!scanPermissions.contains(gov)) scanPermissions.emplace(gov, gov && gov->CanEnforce(playerSystem));

    // Only have ships update their strength estimate once per second on average.
    if(!gov || it->GetSystem() != playerSystem || it->IsDisabled() || Random::Int(60)) continue;
    ...
    for(const auto &allies : governmentRosters)
    {
      ...
      if(allies.first->AttitudeToward(gov) <= 0.) continue;
      for(const auto &ally : allies.second)
        if(!ally->IsDisabled() && ally->Position().Distance(it->Position()) < 2000.) myStrength += ally->Strength();
    }
  }
}
...
void AI::CacheShipLists()
{
  ...
  {
    allyLists.emplace(git.first, std::vector<Ship *>());
    allyLists.at(git.first).reserve(ships.size());
    enemyLists.emplace(git.first, std::vector<Ship *>());
...
void AI::RegisterDerivedConditions(ConditionsStore &conditions)
{
  ...
  conditions["government strength: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Government *gov      = GameData::Governments().Get(ce.NameWithoutPrefix());
        int64_t           strength = 0;
        for(const Ship *ship : governmentRosters[gov])
          if(ship) strength += ship->Strength();
        return strength;
      });
  conditions["ally strength"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return allyStrength[GameData::PlayerGovernment()]; });
  conditions["enemy strength"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return enemyStrength[GameData::PlayerGovernment()]; });
  conditions["ally strength: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Government *gov = GameData::Governments().Get(ce.NameWithoutPrefix());
        return gov ? allyStrength[gov] : 0.;
      });
  conditions["enemy strength: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Government *gov = GameData::Governments().Get(ce.NameWithoutPrefix());
...
void AI::IssueOrder(const OrderSingle &newOrder, const std::string &description)
{
  // Figure out what ships we are giving orders to.
  std::string               who;
  ...
  const std::vector<std::weak_ptr<Ship>> &selected        = player.SelectedEscorts();
  if(selected.empty())
  {
    for(const std::shared_ptr<Ship> &it : player.Ships())
    {
      ...
      {
        if(it->IsDestroyed()) ++destroyedCount;
        else ships.push_back(it.get());
      }
    }
    who = (ships.empty() ? destroyedCount : ships.size()) > 1 ? "Your fleet is "
          : includeFlagship                                   ? "Your flagship is "
                                                              : "Your escort is ";
  }
  else {
    for(const std::weak_ptr<Ship> &it : selected)
    {
      std::shared_ptr<Ship> ship = it.lock();
      if(!ship) continue;
      if(ship->IsDestroyed()) ++destroyedCount;
      else ships.push_back(ship.get());
    }
    who = (ships.empty() ? destroyedCount : ships.size()) > 1 ? "The selected escorts are " : "The selected escort is ";
  }
  if(ships.empty())
  {
    if(destroyedCount)
    {
      Messages::Add(
          {who + "destroyed and unable to execute your orders.", GameData::MessageCategories().Get("normal")});
    }
    return;
  }
  ...
  if(isMoveOrder)
  {
    ...
    if(squadCount > 1) centerOfGravity /= squadCount;
  }
  ...
  if(isValidTarget)
  {
    for(const Ship *ship : ships)
    {
      ...
      if(ship == newTargetShip) continue;
      ...
      if(isMoveOrder)
      {
        ...
        if(offset.Length() > maxSquadOffset) offset = offset.Unit() * maxSquadOffset;
  ...
  else if(hasMismatch)
  {
    ...
    if(!isValidTarget)
      Messages::Add({who + "unable to and no longer " + description, GameData::MessageCategories().Get("normal")});
    else Messages::Add({who + "no longer " + description, GameData::MessageCategories().Get("normal")});

    // Clear any orders that are now empty.
    for(const Ship *ship : ships)
    {
      auto it = orders.find(ship);
      if(it != orders.end() && it->second.Empty()) orders.erase(it);
    }
  }
}
...
RoutePlan AI::GetRoutePlan(const Ship &ship, const System *targetSystem)
{
  ...
  if(player.RecacheJumpRoutes()) routeCache.clear();
  ...
  for(const auto &requirement : GameData::UniverseWormholeRequirements())
    if(shipAttributes.Get(requirement)) wormholeKeys.emplace_back(requirement);

  auto key = RouteCacheKey(
      ship.JumpNavigation().Hash(),
      personalityHash,
      targetSystem,
      player.Flagship() == &ship,
      wormholeKeys);
