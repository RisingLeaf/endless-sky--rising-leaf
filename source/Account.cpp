  ...
  for(const DataNode &child : node)
  {
    ...
    else if(key == "salaries income")
    {
      for(const DataNode &grand : child)
        if(grand.Size() < 2) grand.PrintTrace("Skipping incomplete salary income:");
        else salariesIncome[grand.Token(0)] = grand.Value(1);
    }
...
void Account::Save(DataWriter &out) const
{
  ...
  out.BeginChild();
  {
    ...
    if(crewSalariesOwed) out.Write("salaries", crewSalariesOwed);
    if(maintenanceDue) out.Write("maintenance", maintenanceDue);
...
int64_t Account::Credits() const { return credits; }
...
void Account::AddCredits(int64_t value) { credits += value; }
...
void Account::PayExtra(int mortgage, int64_t amount)
{
  ...
  if(!mortgages[mortgage].Principal()) mortgages.erase(mortgages.begin() + mortgage);
}
...
std::string Account::Step(int64_t assets, int64_t salaries, int64_t maintenance)
{
  ...
  if(maintenanceDue)
  {
    if(maintenanceDue > credits)
    {
      ...
      if(!missedPayment) out << "You could not pay all your maintenance costs.";
      missedPayment = true;
    }
    else {
  ...
  for(Mortgage &mortgage : mortgages)
  {
    ...
    if(payment > credits)
    {
      ...
      if(!missedPayment) out << "You missed a mortgage payment.";
      missedPayment = true;
    }
    else {
      ...
      if(mortgage.Type() == "Mortgage") mortgagesPaid += payment;
      else if(mortgage.Type() == "Fine") finesPaid += payment;
      else debtPaid += payment;
    }
  ...
  for(auto it = mortgages.begin(); it != mortgages.end();)
    if(!it->Principal()) it = mortgages.erase(it);
    else ++it;

  // Keep track of your net worth over the last HISTORY days.
  if(history.size() > HISTORY) history.erase(history.begin());
  ...
  if(!(salariesPaid + maintenancePaid + mortgagesPaid + finesPaid + debtPaid)) return out.str();
  else if(missedPayment) out << " ";

  out << "You paid ";

  std::map<std::string, int64_t> typesPaid;
  if(salariesPaid) typesPaid["crew salaries"] = salariesPaid;
  if(maintenancePaid) typesPaid["maintenance"] = maintenancePaid;
  if(mortgagesPaid) typesPaid["mortgages"] = mortgagesPaid;
  if(finesPaid) typesPaid["fines"] = finesPaid;
  if(debtPaid) typesPaid["debt"] = debtPaid;

  out << Format::List<std::map, std::string, int64_t>(
      typesPaid,
      [](const std::pair<std::string, int64_t> &it) { return Format::CreditString(it.second) + " in " + it.first; });
  out << '.';
  return out.str();
}


const std::map<std::string, int64_t> &Account::SalariesIncome() const { return salariesIncome; }
...
int64_t Account::SalariesIncomeTotal() const
{
  return accumulate(
      ...
      salariesIncome.end(),
      0,
      [](int64_t value, const std::map<std::string, int64_t>::value_type &salary) { return value + salary.second; });
}


void Account::SetSalaryIncome(const std::string &name, int64_t amount)
{
  if(amount == 0) salariesIncome.erase(name);
  else salariesIncome[name] = amount;
}


int64_t Account::CrewSalariesOwed() const { return crewSalariesOwed; }
...
int64_t Account::MaintenanceDue() const { return maintenanceDue; }
...
const std::vector<Mortgage> &Account::Mortgages() const { return mortgages; }
...
void Account::AddFine(int64_t amount) { mortgages.emplace_back("Fine", amount, 0, 60); }
...
{
  if(interest) mortgages.emplace_back("Debt", amount, *interest, term);
  else mortgages.emplace_back("Debt", amount, creditScore, term);
}
...
int64_t Account::Prequalify() const
{
  ...
  return std::max<int64_t>(
      0,
      std::min(NetWorth() / 3 + 500'000 - liabilities, Mortgage::Maximum(YearlyRevenue(), creditScore, payments)));
}


// Get the player's total net worth (counting all ships and all debts).
int64_t Account::NetWorth() const { return history.empty() ? 0 : history.back(); }


// Find out the player's credit rating.
int Account::CreditScore() const { return creditScore; }
...
int64_t Account::TotalDebt(const std::string &type) const
{
  int64_t total = 0;
  for(const Mortgage &mortgage : mortgages)
    if(type.empty() || mortgage.Type() == type) total += mortgage.Principal();
...
{
  if(history.empty() || history.back() <= history.front()) return 0;
