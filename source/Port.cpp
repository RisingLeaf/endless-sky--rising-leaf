...
void Port::Load(const DataNode &node, const ConditionsStore *playerConditions)
{
  ...
  if(node.Size() > nameIndex) displayName = node.Token(nameIndex);
  ...
  for(const DataNode &child : node)
  {
    ...
    if(key == "recharges" && (child.HasChildren() || hasValue))
    {
      ...
      {
        if(value == "all") recharge |= RechargeType::All;
        else if(value == "shields") recharge |= RechargeType::Shields;
        else if(value == "hull") recharge |= RechargeType::Hull;
        else if(value == "energy") recharge |= RechargeType::Energy;
        else if(value == "fuel") recharge |= RechargeType::Fuel;
        else valueNode.PrintTrace("Skipping unrecognized attribute:");
      };
    ...
    else if(key == "services" && (child.HasChildren() || hasValue))
    {
      ...
      {
        if(value == "all") services |= ServicesType::All;
        else if(value == "trading") services |= ServicesType::Trading;
        else if(value == "job board") services |= ServicesType::JobBoard;
        else if(value == "bank") services |= ServicesType::Bank;
        else if(value == "hire crew") services |= ServicesType::HireCrew;
        else if(value == "offers missions") services |= ServicesType::OffersMissions;
        else valueNode.PrintTrace("Skipping unrecognized attribute:");
      };
      for(int i = 1; i < child.Size(); ++i)
    ...
    else if(key == "description" && hasValue)
    {
      ...
      if(displayName.empty()) displayName = SPACEPORT;
    }
...
bool Port::CustomLoaded() const { return loaded; }


const std::string &Port::DisplayName() const { return displayName; }


const Paragraphs &Port::Description() const { return description; }


bool Port::RequiresBribe() const { return !toRequireBribe.IsEmpty() && toRequireBribe.Test(); }


bool Port::CanAccess() const { return toAccess.Test(); }


// Get all the possible sources that can get recharged at this port.
int Port::GetRecharges(bool isPlayer) const
{
  if(!isPlayer || !recharge) return recharge;

  int retVal = RechargeType::None;
  if(CanRecharge(RechargeType::Shields)) retVal |= RechargeType::Shields;
  if(CanRecharge(RechargeType::Hull)) retVal |= RechargeType::Hull;
  if(CanRecharge(RechargeType::Energy)) retVal |= RechargeType::Energy;
  if(CanRecharge(RechargeType::Fuel)) retVal |= RechargeType::Fuel;
  return retVal;
}
...
bool Port::CanRecharge(int type, bool isPlayer) const
{
  ...
  if(!hasType || !isPlayer || type == RechargeType::All) return hasType;
...
{
  if(!isPlayer || !services) return services;

  return HasService(ServicesType::Trading) || HasService(ServicesType::JobBoard) || HasService(ServicesType::Bank) ||
         HasService(ServicesType::HireCrew) || HasService(ServicesType::OffersMissions);
}
...
bool Port::HasService(int type, bool isPlayer) const
{
  ...
  // The All type shouldn't be used when isPlayer is true.
  // If for some reason it is, behave as if isPlayer was false.
  if(!hasType || !isPlayer || type == ServicesType::All) return hasType;
  if(!CanAccess()) return false;
...
bool Port::HasNews() const { return hasNews; }
