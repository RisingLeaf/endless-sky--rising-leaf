...
namespace
{
  ...
  Point GetOffset(const Point &unit, const Point &dimensions)
  {
    ...
    if(unit.X() == 0.) return Point(0., copysign(box.Y(), unit.Y()));

    const double slope = unit.Y() / unit.X();

    // Left and right sides.
    const double y = fabs(box.X() * slope);
    if(y <= box.Y()) return Point(copysign(box.X(), unit.X()), copysign(y, unit.Y()));
...
void PlanetLabel::Draw() const
{
  ...
  if(offset >= 600. || objectDistanceAlpha == 0.) return;
  ...
  if(!name.empty())
  {
    ...
    FontSet::Get(14).DrawAliased(government, to.X() + governmentOffset.X(), to.Y() + governmentOffset.Y(), labelColor);
  }
}
...
void PlanetLabel::UpdateData(const std::vector<PlanetLabel> &labels, const System &system)
{
  ...
  if(planet.DisplayName() != name) reposition = true;
  ...
  else if(planet.GetGovernment())
  {
    ...
    if(newGovernment != government) reposition = true;
    government = newGovernment;
    color      = Color::Combine(.5f, planet.GetGovernment()->GetColor(), 1.f, Color(.3f));
    if(!planet.CanLand()) hostility = 3 + 2 * planet.GetGovernment()->IsEnemy();
  }
  else {
  ...
  for(const double angle : LINE_ANGLES)
  {
    ...
    if(none_of(
           allZooms.begin(),
           allZooms.end(),
           [&](const double zoom) { return HasOverlaps(labels, system, *object, zoom); }))
    {
      innerAngle = angle;
      break;
    }
...
void PlanetLabel::SetBoundingBox(const Point &labelDimensions, const double angle)
{
  ...
  box = Rectangle(unit * (INNER_SPACE + LINE_GAP + LINE_LENGTH) + GetOffset(unit, labelDimensions), labelDimensions);
}


Rectangle PlanetLabel::GetBoundingBox(const double zoom) const { return box + zoomOffset * zoom; }
    ...
    const double                    zoom) const
{
  ...
  for(const PlanetLabel &label : labels)
    if(boundingBox.Overlaps(label.GetBoundingBox(zoom))) return true;

  for(const StellarObject &other : system.Objects())
    if(&other != &object && boundingBox.Overlaps(other.Position() * zoom, other.Radius() * zoom + MIN_DISTANCE))
      return true;
