...
void GamerulesPanel::Draw()
{
  ...
  if(GAMERULES_PAGE_COUNT > 1) info.SetCondition("multiple gamerules pages");
  if(currentGamerulesPage > 0) info.SetCondition("show previous gamerules");
  if(currentGamerulesPage + 1 < GAMERULES_PAGE_COUNT) info.SetCondition("show next gamerules");
...
void GamerulesPanel::PreDraw()
{
  if(page == 'p')
  {
    Rectangle presetListBox = presetUi->GetBox("preset list");
    presetListClip          = make_unique<RenderBuffer>(presetListBox.Dimensions(), "preset buffer");
    ...
    for(const auto &it : GameData::GamerulesPresets())
    {
      ...
      if(isSelected || name == hoverItem) table.DrawHighlight(back);
      ...
      if(displayed) AddZone(zoneBounds, [&]() { GamerulesPanel::SelectPreset(name); });
      if(isSelected) table.Draw(name, bright);
      else table.Draw(name, medium);
    }

    presetListClip->Deactivate();
...
void GamerulesPanel::UpdateTooltipActivation() { tooltip.UpdateActivationCount(); }
...
bool GamerulesPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  else if(key == 'b' || command.Has(Command::MENU) || (key == 'w' && (mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI))))
  {
    if(existingPilot && gamerules.LockGamerules())
    {
      GetUI().Push(
          DialogPanel::CallFunctionIfOk(
              [this]() -> void { GetUI().Pop(this); },
              "You have set \"Lock Gamerules\" to true, which means that you will not be able "
              "to return to this panel to make further edits after leaving. Continue anyway?",
              false));
    }
    else {
      GetUI().Pop(this);
...
void GamerulesPanel::DrawGamerules()
{
  ...
  if(selectedIndex != oldSelectedIndex) latestIndex = selectedIndex;
  if(hoverIndex != oldHoverIndex) latestIndex = hoverIndex;
  ...
  for(const string &gamerule : GAMERULES)
  {
    ...
    if(page < currentGamerulesPage) continue;
    else if(page > currentGamerulesPage) break;
    // Check if this is a category break or column break.
    if(gamerule.empty() || gamerule == "\t")
    {
      isCategory = true;
      if(!gamerule.empty()) table.DrawAt(Point(130, firstY));
      continue;
    }
    ...
    else if(gamerule == FIGHTERS_HIT_WHEN_DISABLED)
    {
      ...
      {
      case Gamerules::FighterDodgePolicy::ALL:         text = "all"; break;
      case Gamerules::FighterDodgePolicy::ONLY_PLAYER: text = "player"; break;
      case Gamerules::FighterDodgePolicy::NONE:        text = "none"; break;
      }
    }
  ...
  if(selectedItem.empty()) selectedItem = gameruleZones.at(selectedIndex).Value();
}
...
void GamerulesPanel::DrawPresets()
{
  ...
  presetListClip->SetFadePadding(presetListScroll.IsScrollAtMin() ? 0 : 20, presetListScroll.IsScrollAtMax() ? 0 : 20);
  ...
  if(presetListScroll.Scrollable())
  {
    ...
    PointerShader::Draw(
        ...
        10.f,
        5.f,
        Color(presetListScroll.IsScrollAtMin() ? .2f : .8f, 0.f));
    ...
    PointerShader::Draw(
        ...
        10.f,
        5.f,
        Color(presetListScroll.IsScrollAtMax() ? .2f : .8f, 0.f));
  ...
  if(presetDescriptionBuffer)
  {
    ...
    if(presetDescriptionScroll.Scrollable())
    {
      ...
      PointerShader::Draw(
          ...
          10.f,
          5.f,
          Color(presetDescriptionScroll.IsScrollAtMin() ? .2f : .8f, 0.f));
      ...
      PointerShader::Draw(
          ...
          10.f,
          5.f,
          Color(presetDescriptionScroll.IsScrollAtMax() ? .2f : .8f, 0.f));
...
void GamerulesPanel::RenderPresetDescription(const string &name)
{
  ...
  if(preset) RenderPresetDescription(*preset);
  else presetDescriptionBuffer.reset();
}
...
void GamerulesPanel::RenderPresetDescription(const Gamerules &preset)
{
  ...
  if(sprite) descriptionHeight += sprite->Height();
  ...
  if(descriptionHeight < box.Height()) descriptionHeight = box.Height();
  presetDescriptionScroll.SetMaxValue(descriptionHeight);
  presetDescriptionBuffer = make_unique<RenderBuffer>(Point(box.Width(), descriptionHeight), "preset dewscription");
  // Redirect all drawing commands into the offscreen buffer.
  presetDescriptionBuffer->SetTarget();
  ...
  presetDescriptionBuffer->Deactivate();
}
...
void GamerulesPanel::DrawTooltips()
{
  ...
  if(!tooltip.ShouldDraw()) return;

  if(!tooltip.HasText()) tooltip.SetText(GameData::Tooltip(hoverItem));
...
void GamerulesPanel::HandleGamerulesString(const string &str)
{
  if(str == DEPRECIATION_MIN)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestDoubleWithValidation(
            &gamerules,
            &Gamerules::SetDepreciationMin,
            validate,
            message,
            gamerules.DepreciationMin()));
  }
  else if(str == DEPRECIATION_GRACE_PERIOD)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestIntegerWithValidation(
            &gamerules,
            &Gamerules::SetDepreciationGracePeriod,
            validate,
            message,
            gamerules.DepreciationGracePeriod()));
  }
  else if(str == DEPRECIATION_MAX_AGE)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestIntegerWithValidation(
            &gamerules,
            &Gamerules::SetDepreciationMaxAge,
            validate,
            message,
            gamerules.DepreciationMaxAge()));
  }
  else if(str == DEPRECIATION_DAILY)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestDoubleWithValidation(
            &gamerules,
            &Gamerules::SetDepreciationDaily,
            validate,
            message,
            gamerules.DepreciationDaily()));
  }
  else if(str == PERSON_SPAWN_PERIOD)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestIntegerWithValidation(
            &gamerules,
            &Gamerules::SetPersonSpawnPeriod,
            validate,
            message,
            gamerules.PersonSpawnPeriod()));
  }
  else if(str == NO_PERSON_SPAWN_WEIGHT)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestIntegerWithValidation(
            &gamerules,
            &Gamerules::SetNoPersonSpawnWeight,
            validate,
            message,
            gamerules.NoPersonSpawnWeight()));
  }
  else if(str == NPC_MAX_MINING_TIME)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestIntegerWithValidation(
            &gamerules,
            &Gamerules::SetNPCMaxMiningTime,
            validate,
            message,
            gamerules.NPCMaxMiningTime()));
  }
  else if(str == UNIVERSAL_FRUGAL_THRESHOLD)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestDoubleWithValidation(
            &gamerules,
            &Gamerules::SetUniversalFrugalThreshold,
            validate,
            message,
            gamerules.UniversalFrugalThreshold()));
  }
  ...
  else if(str == SYSTEM_DEPARTURE_MIN)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestDoubleWithValidation(
            &gamerules,
            &Gamerules::SetSystemDepartureMin,
            validate,
            message,
            gamerules.SystemDepartureMin()));
  }
  else if(str == SYSTEM_ARRIVAL_MIN)
  {
    string message = "Set the minimum system arrival distance. (Any decimal value.)";
    GetUI().Push(
        OptionalInputDialogPanel::RequestDouble(
            &gamerules,
            &Gamerules::SetSystemArrivalMin,
            message,
  ...
  else if(str == FLEET_MULTIPLIER)
  {
    ...
    GetUI().Push(
        DialogPanel::RequestDoubleWithValidation(
            &gamerules,
            &Gamerules::SetFleetMultiplier,
            validate,
            message,
            gamerules.FleetMultiplier()));
  }
  ...
  else if(str == FIGHTERS_HIT_WHEN_DISABLED)
  {
    ...
    if(value == Gamerules::FighterDodgePolicy::ALL) value = Gamerules::FighterDodgePolicy::NONE;
    else if(value == Gamerules::FighterDodgePolicy::NONE) value = Gamerules::FighterDodgePolicy::ONLY_PLAYER;
    else if(value == Gamerules::FighterDodgePolicy::ONLY_PLAYER) value = Gamerules::FighterDodgePolicy::ALL;
...
void GamerulesPanel::SelectPreset(const string &name) { gamerules.Replace(*GameData::GamerulesPresets().Get(name)); }
...
void GamerulesPanel::HandleUp()
{
  ...
  switch(page)
  {
  case 'g': selectedItem = gameruleZones.at(selectedIndex).Value(); break;
  case 'p':
...
void GamerulesPanel::HandleConfirm()
{
  switch(page)
  {
  case 'g': HandleGamerulesString(selectedItem); break;
  case 'p': SelectPreset(selectedPreset); break;
  default:  break;
  }
}
