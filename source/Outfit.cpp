...
namespace
{
  ...
  void AddFlareSprites(std::vector<std::pair<Body, int>> &thisFlares, const std::pair<Body, int> &it, int count)
  {
    auto oit = find_if(
        ...
        { return it.first.GetSprite() == flare.first.GetSprite() && it.first.Scale() == flare.first.Scale(); });

    if(oit == thisFlares.end()) thisFlares.emplace_back(it.first, count * it.second);
    else oit->second += count * it.second;
  }
  ...
  void MergeMaps(std::map<const T *, int> &thisMap, const std::map<const T *, int> &otherMap, int count)
  {
    for(const auto &it : otherMap)
    {
      ...
      if(thisMap[it.first] == 0) thisMap.erase(it.first);
    }
  }
} // namespace
...
{
  if(node.Size() >= 2) trueName = node.Token(1);
  ...
  for(const DataNode &child : node)
  {
    ...
    {
      if(!weapon) weapon = std::make_shared<Weapon>();
    ...
    else if(key == "jump range" && hasValue)
    {
      ...
      Set("jump range", std::max(0., child.Value(1)));
    }
    else if(hasValue)
    {
      Set(key.c_str(), child.Value(1));
    }
    else {
      (void)child.PrintTrace("Skipping unrecognized attribute:");
    }
  }
  ...
  const bool isHyperdrive = Get("hyperdrive") > 0.;
  const bool isScramDrive = Get("scram drive") > 0.;
  const bool isJumpDrive  = Get("jump drive") > 0.;
  if((isHyperdrive || isScramDrive) && Get("hyperdrive fuel") <= 0.)
  {
    const double jumpFuel = Get("jump fuel");
    Set("hyperdrive fuel",
        jumpFuel > 0.  ? jumpFuel
        : isScramDrive ? DEFAULT_SCRAM_DRIVE_COST
                       : DEFAULT_HYPERDRIVE_COST);
  }
  if(isJumpDrive && Get("jump drive fuel") <= 0.)
  {
    const double jumpFuel = Get("jump fuel");
    Set("jump drive fuel", jumpFuel > 0. ? jumpFuel : DEFAULT_JUMP_DRIVE_COST);
  }
  if(Get("jump fuel")) Set("jump fuel", 0.);
     ...
     !weapon->TractorBeam())
  {
    ...
    weapon               = std::make_shared<Weapon>(std::move(newWeapon));
    (void)node.PrintTrace("Warning: Deprecated use of a turret without specified \"turret turn\":");
  }
  ...
  auto convertScan = [&](std::string &&kind) -> void
  {
    ...
    if(initial)
    {
      ...
      if(!Get(label + " efficiency")) Set(label + " efficiency", 15.);
    }
    ...
    if(initial)
    {
      ...
      Set(kind + " scan efficiency", initial * 15.);
    }
  };
  convertScan("outfit");
  convertScan("cargo");
...
bool Outfit::IsDefined() const { return isDefined; }
...
const std::string &Outfit::TrueName() const { return trueName; }


void Outfit::SetTrueName(const std::string &name) { this->trueName = name; }


const std::string &Outfit::DisplayName() const { return displayName; }


const std::string &Outfit::PluralName() const { return pluralName; }


const std::string &Outfit::Category() const { return category; }


const std::string &Outfit::Series() const { return series; }


int Outfit::Index() const { return index; }


std::string Outfit::Description() const { return description.ToString(); }


// Get the licenses needed to purchase this outfit.
const std::vector<std::string> &Outfit::Licenses() const { return licenses; }


// Get the image to display in the outfitter when buying this item.
const Sprite *Outfit::Thumbnail() const { return thumbnail; }


double Outfit::Get(const std::string_view attribute) const
{
  const auto internal_name = InternalName(attribute);
  return internal_name.has_value() ? QuickLoadDictionary[internal_name.value()] : attributes.Get(attribute.data());
}


std::set<std::string> Outfit::GetPositiveAttributes() const
{
  std::set<std::string> attrs;
  for(size_t i = 0; i < QuickLoadDictionary.size(); i++)
    if(QuickLoadDictionary[i] > 0.) attrs.insert(std::string(AttributeNameTable[i]));
  for(const auto &[name, value] : attributes)
    if(value > 0.) attrs.insert(name);
  return attrs;
}


std::vector<std::pair<std::string, double>> Outfit::Attributes() const
{
...
{
  for(const auto &name : other.Attributes() | std::views::keys)
  {
    ...
    if(name != "required crew") minimum = !(Get("automaton") || other.Get("automaton"));

    const double value = Get(name);
    // Allow for rounding errors:
    if(value + value * count < minimum - EPS) count = (value - minimum) / -value + EPS;
  }
...
void Outfit::Add(const Outfit &other, const int count)
{
  cost += other.cost * count;
  mass += other.mass * count;
  for(const auto &[name, value] : other.attributes)
  {
    attributes[name] += value * count;
    if(fabs(attributes[name]) < EPS) attributes[name] = 0.;
  }

  for(size_t i = 0; i < QuickLoadDictionary.size(); i++)
  {
    QuickLoadDictionary[i] += other.QuickLoadDictionary[i] * count;
    if(fabs(QuickLoadDictionary[i]) < EPS) QuickLoadDictionary[i] = 0.;
  }
...
void Outfit::Set(const std::string_view attribute, const double value)
{
  const auto internal_name = InternalName(attribute);
  if(internal_name.has_value()) QuickLoadDictionary[internal_name.value()] = value;
  else attributes[attribute.data()] = value;
}


const Outfit *Outfit::AmmoStored() const { return ammoStored; }


const Outfit *Outfit::AmmoStoredOrUsed() const { return weapon ? weapon->Ammo() : ammoStored; }


// Get this outfit's engine flare sprite, if any.
const std::vector<std::pair<Body, int>> &Outfit::FlareSprites() const { return flareSprites; }


const std::vector<std::pair<Body, int>> &Outfit::ReverseFlareSprites() const { return reverseFlareSprites; }


const std::vector<std::pair<Body, int>> &Outfit::SteeringFlareSprites() const { return steeringFlareSprites; }


const std::map<const Sound *, int> &Outfit::FlareSounds() const { return flareSounds; }


const std::map<const Sound *, int> &Outfit::ReverseFlareSounds() const { return reverseFlareSounds; }


const std::map<const Sound *, int> &Outfit::SteeringFlareSounds() const { return steeringFlareSounds; }


// Get the afterburner effect, if any.
const std::map<const Effect *, int> &Outfit::AfterburnerEffects() const { return afterburnerEffects; }


// Get this outfit's jump effects and sounds, if any.
const std::map<const Effect *, int> &Outfit::JumpEffects() const { return jumpEffects; }


const std::map<const Sound *, int> &Outfit::HyperSounds() const { return hyperSounds; }


const std::map<const Sound *, int> &Outfit::HyperInSounds() const { return hyperInSounds; }


const std::map<const Sound *, int> &Outfit::HyperOutSounds() const { return hyperOutSounds; }


const std::map<const Sound *, int> &Outfit::JumpSounds() const { return jumpSounds; }


const std::map<const Sound *, int> &Outfit::JumpInSounds() const { return jumpInSounds; }


const std::map<const Sound *, int> &Outfit::JumpOutSounds() const { return jumpOutSounds; }


const std::map<const Sound *, int> &Outfit::CargoScanSounds() const { return cargoScanSounds; }


const std::map<const Sound *, int> &Outfit::OutfitScanSounds() const { return outfitScanSounds; }


// Get the sprite this outfit uses when dumped into space.
const Sprite *Outfit::FlotsamSprite() const { return flotsamSprite; }
...
void Outfit::AddLicense(const std::string &name)
{
  ...
  if(it == licenses.end()) licenses.push_back(name);
}
