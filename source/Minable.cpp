...
Minable::Payload::Payload(const DataNode &node)
{
  ...
  maxDrops = (node.Size() == 2 ? 1 : std::max<int>(1, node.Value(2)));
  ...
  for(const DataNode &child : node)
  {
    ...
    if(!hasValue) child.PrintTrace("Expected key to have a value:");
    else if(key == "max drops") maxDrops = std::max<int>(1, child.Value(1));
    else if(key == "drop rate") dropRate = std::max(0., std::min(child.Value(1), 1.));
    else if(key == "toughness") toughness = std::max(1., child.Value(1));
...
void Minable::Load(const DataNode &node, const ConditionsStore *playerConditions)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    else if(key == "sprite")
    {
      ...
      for(const DataNode &grand : child)
        if(grand.Token(0) == "frame rate" || grand.Token(0) == "frame time") useRandomFrameRate = false;
    }
    else if(key == "hull")
    {
      hull = child.Value(1);
    ...
    {
      randomHull = std::max(0., child.Value(1));
    }
    else if(key == "payload")
    {
  ...
  if(displayName.empty()) displayName = Format::Capitalize(name);
  if(noun.empty()) noun = "Asteroid";
}
...
const std::string &Minable::TrueName() const { return name; }


const std::string &Minable::DisplayName() const { return displayName; }


const std::string &Minable::Noun() const { return noun; }
...
void Minable::Place(double energy, double beltRadius)
{
  ...
  double sMin = std::max(.4 * (1. + eccentricity), .8 * (1. - eccentricity));
  double sMax = std::min(4. * (1. - eccentricity), 1.3 * (1. + eccentricity));
  ...
  if(useRandomFrameRate) SetFrameRate(Random::Real() * 4. * energy + 5.);
...
bool Minable::Move(std::vector<Visual> &visuals, std::list<std::shared_ptr<Flotsam>> &flotsam)
{
  if(hull < 0)
  {
    ...
    for(const Payload &it : payload)
    {
      ...
      // prospecting.
      if(prospecting > 0. && dropRate < 1.) dropRate += (1. - dropRate) / (1. + it.toughness / prospecting);
      if(dropRate <= 0.) continue;
      for(int amount = Random::Binomial(it.maxDrops, dropRate); amount > 0; amount -= Flotsam::TONS_PER_BOX)
      {
        flotsam.emplace_back(new Flotsam(it.outfit, std::min(amount, Flotsam::TONS_PER_BOX)));
...
double Minable::Hull() const { return std::min(1., hull / maxHull); }


double Minable::MaxHull() const { return maxHull; }


double Minable::Mass() const { return attributes.Mass(); }
...
const std::vector<Minable::Payload> &Minable::GetPayload() const { return payload; }


// Get the expected value of the flotsams this minable will create when destroyed.
const int64_t &Minable::GetValue() const { return value; }
...
Minable::LiveEffect::LiveEffect(const DataNode &node)
{
  ...
  for(const DataNode &child : node)
    if(child.Token(0) == "relative to system center") relativeToSystem = true;
    else child.PrintTrace("Skipping unrecognized attribute:");
}
