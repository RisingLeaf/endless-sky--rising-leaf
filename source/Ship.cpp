...
namespace
{
  ...
  const std::vector<std::string> BAY_SIDE       = {"inside", "over", "under"};
  const std::vector<std::string> BAY_FACING     = {"forward", "left", "right", "back"};
  const std::vector<Angle>       BAY_ANGLE      = {Angle(0.), Angle(-90.), Angle(90.), Angle(180.)};

  const std::vector<std::string> ENGINE_SIDE     = {"under", "over"};
  const std::vector<std::string> STEERING_FACING = {"none", "left", "right"};
  ...
  {
    double transfer  = std::max(0., std::min(available, maximum - stat));
    stat            += transfer;
    available       -= transfer;
  }
  ...
  void DoRepair(
      ...
      double  heatCost)
  {
    if(available <= 0. || stat >= maximum) return;

    // Energy, heat, and fuel costs are the energy, fuel, or heat required per unit repaired.
    if(energyCost > 0.) available = std::min(available, energy / energyCost);
    if(fuelCost > 0.) available = std::min(available, fuel / fuelCost);
    if(heatCost < 0.) available = std::min(available, heat / -heatCost);

    double transfer = std::min(available, maximum - stat);
    if(transfer > 0.)
    {
      stat      += transfer;
      available -= transfer;
      ...
      double  heatCost)
  {
    ...
    {
      stat = std::max(0., .99 * stat);
      return;
    }

    // Calculate how much resistance can be used assuming no
    // energy or fuel cost.
    resistance = .99 * stat - std::max(0., .99 * stat - resistance);

    // Limit the resistance by the available energy, heat, and fuel.
    if(energyCost > 0.) resistance = std::min(resistance, energy / energyCost);
    if(fuelCost > 0.) resistance = std::min(resistance, fuel / fuelCost);
    if(heatCost < 0.) resistance = std::min(resistance, heat / -heatCost);

    // Update the stat, energy, heat, and fuel given how much resistance is being used.
    if(resistance > 0.)
    {
      stat    = std::max(0., .99 * stat - resistance);
  ...
  std::map<const Outfit *, int> GetEquipped(const std::vector<Hardpoint> &weapons)
  {
    std::map<const Outfit *, int> equipped;
    for(const Hardpoint &hardpoint : weapons)
      if(hardpoint.GetOutfit()) ++equipped[hardpoint.GetOutfit()];
    return equipped;
  }
  ...
  {
    auto transferred = std::map<const Outfit *, int>{};
  ...
  double CalculateJamChance(double scrambling) { return scrambling > .1 ? 1. - pow(2., -1. * (scrambling / 70.)) : 0.; }
} // namespace


// Construct and Load() at the same time.
Ship::Ship(const DataNode &node, const ConditionsStore *playerConditions) { Load(node, playerConditions); }


void Ship::Load(const DataNode &node, const ConditionsStore *playerConditions)
{
  if(node.Size() >= 2) trueModelName = node.Token(1);
  if(node.Size() >= 3)
  {
  ...
  for(const DataNode &child : node)
  {
    ...
    {
      child.PrintTrace(
          "Skipping invalid use of 'add' with " + (child.Size() < 2 ? "no key." : "key: " + child.Token(1)));
      continue;
    }
    if(key == "sprite")
    {
    ...
    else if((key == "engine" || key == "reverse engine" || key == "steering engine") && child.Size() >= 3)
    {
      ...
      std::vector<EnginePoint> &editPoints =
          (!steering && !reverse) ? enginePoints : (reverse ? reverseEnginePoints : steeringEnginePoints);
      editPoints.emplace_back(Point(child.Value(1), child.Value(2)) * 0.5, child.Size() > 3 ? child.Value(3) : 1.);
      EnginePoint &engine = editPoints.back();
      if(reverse) engine.facing = Angle(180.);
      for(const DataNode &grand : child)
      {
        ...
        else if(grandKey == "gimbal" && grandHasValue)
        {
          ...
          for(unsigned j = 1; j < ENGINE_SIDE.size(); ++j)
            if(grandKey == ENGINE_SIDE[j]) engine.side = j;
          if(steering)
          {
            for(unsigned j = 1; j < STEERING_FACING.size(); ++j)
              if(grandKey == STEERING_FACING[j]) engine.steering = j;
          }
        }
      }
    }
    else if(key == "gun" || key == "turret")
    {
      ...
      if(child.Size() >= 3)
      {
        ...
        if(child.Size() >= 4) outfit = GameData::Outfits().Get(child.Token(3));
      }
      else {
        if(hasValue) outfit = GameData::Outfits().Get(child.Token(1));
      }
      ...
      if(key == "gun") attributes.side = Hardpoint::Side::UNDER;
      ...
      if(key == "gun") armament.AddGunPort(hardpoint, attributes, outfit);
      else armament.AddTurret(hardpoint, attributes, outfit);
    }
    ...
    else if(((key == "fighter" || key == "drone") && child.Size() >= 3) || (key == "bay" && child.Size() >= 4))
    {
      ...
      for(int i = 3 + childOffset; i < child.Size(); ++i)
      {
        for(unsigned j = 1; j < BAY_SIDE.size(); ++j)
          if(child.Token(i) == BAY_SIDE[j]) bay.side = j;
        for(unsigned j = 1; j < BAY_FACING.size(); ++j)
          if(child.Token(i) == BAY_FACING[j]) bay.facing = BAY_ANGLE[j];
      }
      if(child.HasChildren())
      {
        for(const DataNode &grand : child)
        {
          ...
          else if(grandKey == "angle" && grandHasValue)
          {
            ...
            if(!handled) grand.PrintTrace("Skipping unrecognized attribute:");
          }
        }
      }
    }
    else if(key == "leak" && hasValue)
    {
      ...
      if(child.Size() >= 3) leak.openPeriod = child.Value(2);
      if(child.Size() >= 4) leak.closePeriod = child.Value(3);
    ...
    else if(key == "outfits")
    {
      ...
      for(const DataNode &grand : child)
      {
        ...
        if(count > 0) outfits[GameData::Outfits().Get(grand.Token(0))] += count;
        else grand.PrintTrace("Skipping invalid outfit count:");
      }
    ...
    {
      if(child.Token(1) == "bays") removeBays = true;
      else child.PrintTrace("Skipping unsupported \"remove\":");
    }
    else if(key != "actions")
    {
  ...
  {
    if(displayModelName.empty()) displayModelName = trueModelName;
...
void Ship::FinishLoading(bool isNewInstance)
{
  ...
  if(GameData::Ships().Has(trueModelName))
  {
    ...
    if(displayModelName.empty()) displayModelName = model->displayModelName;
    if(pluralModelName.empty()) pluralModelName = model->pluralModelName;
    if(noun.empty()) noun = model->noun;
    if(!thumbnail) thumbnail = model->thumbnail;
  }
  ...
  if(base && base != this)
  {
    ...
    if(enginePoints.empty()) enginePoints = base->enginePoints;
    if(reverseEnginePoints.empty()) reverseEnginePoints = base->reverseEnginePoints;
    if(steeringEnginePoints.empty()) steeringEnginePoints = base->steeringEnginePoints;
    if(explosionEffects.empty())
    {
      explosionEffects = base->explosionEffects;
      explosionTotal   = base->explosionTotal;
    }
    if(finalExplosions.empty()) finalExplosions = base->finalExplosions;
    const bool inheritsOutfits = outfits.empty();
    if(inheritsOutfits) outfits = base->outfits;
    if(description.IsEmpty()) description = base->description;
    ...
    for(const Hardpoint &hardpoint : armament.Get())
      if(hardpoint.GetPoint()) hasHardpoints = true;
  ...
  if(baseAttributes.Category() == "Drone" && !baseAttributes.Get("automaton")) baseAttributes.Set("automaton", 1.);
  ...
  std::vector<std::string> undefinedOutfits;
  for(const auto &it : outfits)
  {
    ...
    if(it.first->GetWeapon())
    {
      ...
      if(eit != equipped.end()) count -= eit->second;
  ...
  for(const Hardpoint &hardpoint : armament.Get())
  {
    ...
    if(outfit && outfit->IsDefined() && (hardpoint.IsTurret() != (outfit->Get("turret mounts") != 0.)))
    {
      std::string warning = (!isYours && !variantName.empty()) ? "variant \"" + variantName + "\"" : trueModelName;
      if(!givenName.empty()) warning += " \"" + givenName + "\"";
  ...
  weaponRadius = 0.;
  for(const Hardpoint &hardpoint : armament.Get())
    weaponRadius = std::max(weaponRadius, hardpoint.GetPoint().Length());
  ...
  for(auto &&attr : std::set<std::string>{"outfit space", "cargo space", "weapon capacity", "engine capacity"})
  {
    const double val = attributes.Get(attr);
    if(val < 0) warning += attr + ": " + Format::Number(val) + "\n";
  }
  if(attributes.Get("drag") <= 0.)
  {
    warning +=
        "Defaulting " + std::string(attributes.Get("drag") ? "invalid" : "missing") + " \"drag\" attribute to 100.0\n";
  ...
  if(!warning.empty())
  {
    ...
    std::string message = (!givenName.empty() ? "Ship \"" + givenName + "\" " : "") + "(" + VariantName() + "):\n";
    std::ostringstream outfitNames;
...
bool Ship::IsValid() const
{
  for(auto &&outfit : outfits)
    if(!outfit.first->IsDefined()) return false;
...
void Ship::Save(DataWriter &out) const
{
  ...
  out.BeginChild();
  {
    ...
    if(displayModelName != trueModelName) out.Write("display name", displayModelName);
    if(pluralModelName != displayModelName + 's') out.Write("plural", pluralModelName);
    if(!noun.empty()) out.Write("noun", noun);
    SaveSprite(out);
    if(thumbnail) out.Write("thumbnail", thumbnail->Name());

    if(neverDisabled) out.Write("never disabled");
    if(!isCapturable) out.Write("uncapturable");
    if(customSwizzle && customSwizzle->IsLoaded()) out.Write("swizzle", customSwizzle->Name());
    ...
    out.BeginChild();
    {
      ...
      for(const auto &it : baseAttributes.Attributes())
        if(it.second) out.Write(it.first, it.second);
    }
    out.EndChild();
    ...
    out.BeginChild();
    {
      using OutfitElement = std::pair<const Outfit *const, int>;
      WriteSorted(
          outfits,
          ...
          {
            if(it.second == 1) out.Write(it.first->TrueName());
            else out.Write(it.first->TrueName(), it.second);
          });
    }
    out.EndChild();
    ...
    for(const Hardpoint &hardpoint : armament.Get())
    {
      ...
      if(hardpoint.GetOutfit())
      {
        out.Write(
            type,
            2. * hardpoint.GetPoint().X(),
            2. * hardpoint.GetPoint().Y(),
      ...
      out.BeginChild();
      {
        if(baseDegree) out.Write("angle", baseDegree);
        if(attributes.isParallel) out.Write("parallel");
        if(!attributes.isOmnidirectional) out.Write("arc", firstArc, secondArc);
        for(const auto &blindspot : attributes.blindspots)
          out.Write("blindspot", blindspot.first.Degrees(), blindspot.second.Degrees());
        if(attributes.turnMultiplier) out.Write("turret turn multiplier", attributes.turnMultiplier);
        if(hardpoint.GetSide() == Hardpoint::Side::OVER) out.Write("over");
        else if(hardpoint.GetSide() == Hardpoint::Side::INSIDE) out.Write("inside");
        else out.Write("under");
      }
      out.EndChild();
    }
    for(const Bay &bay : bays)
    {
      ...
      if(!bay.launchEffects.empty() || bay.facing.Degrees() || bay.side)
      {
        out.BeginChild();
        {
          if(bay.facing.Degrees()) out.Write("angle", bay.facing.Degrees());
          if(bay.side) out.Write(BAY_SIDE[bay.side]);
    ...
    using EffectElement = std::pair<const Effect *const, int>;
    auto effectSort     = [](const EffectElement *lhs, const EffectElement *rhs)
    { return lhs->first->TrueName() < rhs->first->TrueName(); };
    WriteSorted(
        explosionEffects,
        effectSort,
        [&out](const EffectElement &it)
        {
          if(it.second) out.Write("explode", it.first->TrueName(), it.second);
        });
    WriteSorted(
        finalExplosions,
        effectSort,
        [&out](const EffectElement &it)
        {
          if(it.second) out.Write("final explode", it.first->TrueName(), it.second);
        });
    if(formationPattern) out.Write("formation", formationPattern->TrueName());
    if(currentSystem)
    {
      ...
      std::shared_ptr<const Ship> parent = GetParent();
      if(parent && parent->currentSystem) out.Write("system", parent->currentSystem->TrueName());
    }
    if(landingPlanet) out.Write("planet", landingPlanet->TrueName());
    if(targetSystem) out.Write("destination system", targetSystem->TrueName());
    if(isParked) out.Write("parked");
  }
  out.EndChild();
}


const EsUuid &Ship::UUID() const noexcept { return uuid; }


void Ship::SetUUID(const EsUuid &id) { uuid.Clone(id); }


const std::string &Ship::GivenName() const { return givenName; }


// Set the name of this particular ship.
void Ship::SetGivenName(const std::string &name) { this->givenName = name; }


// Set / Get the name of this class of ships, e.g. "Marauder Raven."
void Ship::SetTrueModelName(const std::string &model) { this->trueModelName = model; }


const std::string &Ship::TrueModelName() const { return trueModelName; }


const std::string &Ship::DisplayModelName() const { return displayModelName; }


const std::string &Ship::PluralModelName() const { return pluralModelName; }


// Get the name of this ship as a variant.
const std::string &Ship::VariantName() const { return variantName.empty() ? trueModelName : variantName; }


// Get the variant name to be displayed on the Shipyard tab of the Map screen.
const std::string &Ship::VariantMapShopName() const { return variantMapShopName; }


// Get the generic noun (e.g. "ship") to be used when describing this ship.
const std::string &Ship::Noun() const
{
...
std::string Ship::Description() const { return description.ToString(); }


// Get the shipyard thumbnail for this ship.
const Sprite *Ship::Thumbnail() const { return thumbnail; }


// Get this ship's cost.
int64_t Ship::Cost() const { return attributes.Cost(); }


// Get the cost of this ship's chassis, with no outfits installed.
int64_t Ship::ChassisCost() const { return baseAttributes.Cost(); }


int64_t Ship::Strength() const { return Cost(); }


double Ship::Attraction() const { return attraction; }


double Ship::Deterrence() const { return deterrence; }


// Check if this ship is configured in such a way that it would be difficult
// or impossible to fly.
std::vector<std::string> Ship::FlightCheck() const
{
  auto checks = std::vector<std::string>{};

  const double generation    = attributes.Get("energy generation") - attributes.Get("energy consumption");
  const double consuming     = attributes.Get("fuel energy");
  const double solar         = attributes.Get("solar collection");
  const double battery       = attributes.Get("energy capacity");
  const double energy        = generation + consuming + solar + battery;
  const double fuelChange    = attributes.Get("fuel generation") - attributes.Get("fuel consumption");
  const double fuelCapacity  = attributes.Get("fuel capacity");
  const double fuel          = fuelCapacity + fuelChange;
  const double thrust        = attributes.Get("thrust");
  const double reverseThrust = attributes.Get("reverse thrust");
  const double afterburner   = attributes.Get("afterburner thrust");
  const double thrustEnergy  = attributes.Get("thrusting energy");
  const double thrustHeat    = attributes.Get("thrusting heat");
  const double turn          = attributes.Get("turn");
  const double turnEnergy    = attributes.Get("turning energy");
  const double turnHeat      = attributes.Get("turning heat");
  const double hyperDrive    = navigation.HasHyperdrive();
  const double jumpDrive     = navigation.HasJumpDrive();
  const int    bunks         = attributes.Get("bunks");

  // Report the first error condition that will prevent takeoff:
  if(!bunks && RequiredCrew()) checks.emplace_back("no bunks!");
  else if(IdleHeat() >= MaximumHeat()) checks.emplace_back("overheating!");
  else if(energy <= 0.) checks.emplace_back("no energy!");
  else if((energy - consuming <= 0.) && (fuel <= 0.)) checks.emplace_back("no fuel!");
  else if(!thrust && !reverseThrust && !afterburner) checks.emplace_back("no thruster!");
  else if(!turn) checks.emplace_back("no steering!");

  // If no errors were found, check all warning conditions:
  if(checks.empty())
  {
    if(RequiredCrew() > bunks) checks.emplace_back("insufficient bunks?");
    if(IdleHeat() <= 0. && (thrustHeat < 0. || turnHeat < 0.)) checks.emplace_back("insufficient heat?");
    if(!thrust && !reverseThrust) checks.emplace_back("afterburner only?");
    if(!thrust && !afterburner) checks.emplace_back("reverse only?");
    if(energy <= battery) checks.emplace_back("battery only?");
    if(energy < thrustEnergy) checks.emplace_back("limited thrust?");
    if(energy < turnEnergy) checks.emplace_back("limited turn?");
    if(energy - .8 * solar < .2 * (turnEnergy + thrustEnergy)) checks.emplace_back("solar power?");
    if(fuel < 0.) checks.emplace_back("fuel?");
    if(!canBeCarried)
    {
      if(!hyperDrive && !jumpDrive) checks.emplace_back("no hyperdrive?");
      if(fuelCapacity < navigation.JumpFuel()) checks.emplace_back("no fuel?");
    }
    for(const auto &it : outfits)
    {
...
void Ship::SetPosition(Point position) { this->position = position; }


void Ship::SetVelocity(Point velocity) { this->velocity = velocity; }
...
void Ship::Place(Point position, Point velocity, Angle angle, bool isDeparting)
{
  ...
  if(government && isDeparting)
  {
    ...
    for(const auto &bay : bays)
      if(bay.ship) bay.ship->SetSwizzle(bay.ship->customSwizzle ? bay.ship->customSwizzle : swizzle);
  }
}
...
{
  if(government) SetSwizzle(customSwizzle ? customSwizzle : government->GetSwizzle());
  this->government = government;
}


void Ship::SetIsSpecial(bool special) { isSpecial = special; }


bool Ship::IsSpecial() const { return isSpecial; }


void Ship::SetIsYours(bool yours) { isYours = yours; }


bool Ship::IsYours() const { return isYours; }


void Ship::SetIsParked(bool parked) { isParked = parked; }


bool Ship::IsParked() const { return isParked; }


bool Ship::HasDeployOrder() const { return shouldDeploy; }


void Ship::SetDeployOrder(bool shouldDeploy) { this->shouldDeploy = shouldDeploy; }


const Personality &Ship::GetPersonality() const { return personality; }


void Ship::SetPersonality(const Personality &other) { personality = other; }


const Phrase *Ship::GetHailPhrase() const { return hail; }


void Ship::SetHailPhrase(const Phrase &phrase) { hail = &phrase; }


std::string Ship::GetHail(std::map<std::string, std::string> &&subs) const
{
  std::string hailStr = hail ? hail->Get() : government ? government->GetHail(isDisabled) : "";

  if(hailStr.empty()) return hailStr;
...
const ShipAICache &Ship::GetAICache() const { return aiCache; }
...
bool Ship::CanSendHail(const PlayerInfo &player, bool allowUntranslated) const
{
  ...
  if(!(allowUntranslated && gov->SendUntranslatedHails()) && !gov->Language().empty() &&
...
void Ship::SetCommands(const Command &command) { commands = command; }


void Ship::SetCommands(const FireCommand &firingCommand) { firingCommands.UpdateWith(firingCommand); }


const Command &Ship::Commands() const { return commands; }


const FireCommand &Ship::FiringCommands() const noexcept { return firingCommands; }


// Move this ship. A ship may create effects as it moves, in particular if
// it is in the process of blowing up. If this returns false, the ship
// should be deleted.
void Ship::Move(std::vector<Visual> &visuals, std::list<std::shared_ptr<Flotsam>> &flotsam)
{
  ...
  if(!isBeingDestroyed)
  {
    ...
    if(DoHyperspaceLogic(visuals)) return;

    // Check if we're trying to land.
    // If we landed, we're done.
    if(DoLandingLogic()) return;

    // Move the turrets.
    if(!isDisabled) armament.Aim(*this, firingCommands);
  ...
  if(!isBeingDestroyed) DoEngineVisuals(visuals, isUsingAfterburner);

  // Start fading the damage overlay.
  if(damageOverlayTimer) --damageOverlayTimer;
}


// Launch any ships that are ready to launch.
void Ship::Launch(std::list<std::shared_ptr<Ship>> &ships, std::vector<Visual> &visuals)
{
  ...
  for(Bay &bay : bays)
  {
        ...
        (ejecting && !Random::Int(6))))
    {
      ...
      if(!ejecting)
      {
        ...
        auto toRestock   = std::map<const Outfit *, int>{};
        for(auto &&ammo : restockable)
        {
          int count = OutfitCount(ammo);
          if(count > 0) toRestock.emplace(ammo, count);
        }
        ...
        double maxFuel = bay.ship->attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity"));
        if(maxFuel)
        {
          double spareFuel = fuel - navigation.JumpFuel();
          if(spareFuel > 0.) TransferFuel(spareFuel, bay.ship.get());
...
std::shared_ptr<Ship> Ship::Board(bool autoPlunder, bool nonDocking)
{
  if(!hasBoarded) return std::shared_ptr<Ship>();
  hasBoarded = false;

  std::shared_ptr<Ship> victim = GetTargetShip();
  if(CannotAct(Ship::ActionType::BOARD) || !victim || victim->IsDestroyed() || victim->GetSystem() != GetSystem())
    return std::shared_ptr<Ship>();
  ...
  if(CanBeCarried() && !nonDocking)
  {
    ...
    if(!victim->IsDisabled() && victim->GetGovernment() == government) victim->Carry(shared_from_this());
    return std::shared_ptr<Ship>();
  }
  ...
  if(!victim->IsDisabled()) return std::shared_ptr<Ship>();
  ...
  if(!autoPlunder || personality.Disables()) SetTargetShip(std::shared_ptr<Ship>());
  return victim;
}
...
{
  if(!commands.Has(Command::SCAN) || CannotAct(Ship::ActionType::SCAN)) return 0;

  std::shared_ptr<const Ship> target = GetTargetShip();
  if(!(target && target->IsTargetable())) return 0;
  ...
  if(!cargoDistanceSquared && !outfitDistanceSquared) return 0;

  double cargoSpeed = attributes.Get("cargo scan efficiency");
  if(!cargoSpeed) cargoSpeed = cargoDistanceSquared;

  double outfitSpeed = attributes.Get("outfit scan efficiency");
  if(!outfitSpeed) outfitSpeed = outfitDistanceSquared;
  ...
  const double outfitsSize = target->baseAttributes.GetIndexed(Outfit::INTERNAL_ATTR("outfit space")) +
                             target->attributes.Get("outfit scan opacity");
  const double cargoSize = target->attributes.GetIndexed(Outfit::INTERNAL_ATTR("cargo space")) +
                           target->attributes.Get("cargo scan opacity");
  double outfits = std::max(SCAN_MIN_OUTFIT_SPACE, outfitsSize) * SCAN_OUTFIT_FACTOR;
  double cargo   = std::max(SCAN_MIN_CARGO_SPACE, cargoSize) * SCAN_CARGO_FACTOR;
  ...
  auto doScan          = [&distanceSquared, &startedScanning, &activeScanning, &result](
                    double      &elapsed,
                    const double speed,
                    const double scannerRangeSquared,
                    const double depth,
                    const int    event) -> void
  {
    if(elapsed >= SCAN_TIME) return;
    if(distanceSquared > scannerRangeSquared) return;
    ...
    const double distanceExponent = -distanceSquared / std::max<double>(1e-3, 2. * scannerRangeSquared);
    ...
    {
      elapsed += std::max(MIN_SCAN_STEPS, progress);
    }
    // For fast scan rates, apply an exponential drop-off to prevent insta-scanning.
    else {
      elapsed +=
          SCAN_TIME - (SCAN_TIME - LINEAR_RATE) * exp(-(progress - LINEAR_RATE) / SCAN_TIME / SCAN_DROPOFF_EXPONENT);
    }

    if(elapsed >= SCAN_TIME) result |= event;
  };
  ...
  auto playScanSounds = [](const std::map<const Sound *, int> &sounds, const Point &position)
  {
    if(sounds.empty()) Audio::Play(Audio::Get("scan"), position, SoundCategory::SCAN);
    else
  ...
  {
    if(activeScanning & ShipEvent::SCAN_CARGO) playScanSounds(attributes.CargoScanSounds(), position);
    if(activeScanning & ShipEvent::SCAN_OUTFITS) playScanSounds(attributes.OutfitScanSounds(), position);
  }

  bool isImportant = false;
  if(target->isYours) isImportant = target.get() == player.Flagship() || government->FinesContents(target.get());
  ...
  if(startedScanning && isYours)
  {
    ...
    {
      Messages::Add(
          {"Attempting to scan the " + target->Noun() + " \"" + target->GivenName() + "\".",
           GameData::MessageCategories().Get("low")});
    }
    else {
      Messages::Add(
          {"Attempting to scan the selected " + target->Noun() + ".", GameData::MessageCategories().Get("low")});
    }
    ...
    if(target->GetGovernment()->IsProvokedOnScan() && target->CanSendHail(player))
    {
      ...
      std::string        tag;
      const std::string &gov = target->GetGovernment()->DisplayName();
      if(!target->GivenName().empty()) tag = gov + " " + target->Noun() + " \"" + target->GivenName() + "\": ";
      else tag = target->DisplayModelName() + " (" + gov + "): ";
      Messages::Add(
          {tag + "Please refrain from scanning us or we will be forced to take action.",
  ...
  {
    Messages::Add(
        {"The " + government->DisplayName() + " " + Noun() + " \"" + GivenName() +
             "\" is attempting to scan your ship \"" + target->GivenName() + "\".",
         GameData::MessageCategories().Get("low")});
  }
  ...
  if(target->isYours && !isYours && isImportant)
  {
    if(result & ShipEvent::SCAN_CARGO)
    {
      Messages::Add(
          {"The " + government->DisplayName() + " " + Noun() + " \"" + GivenName() +
               "\" completed its cargo scan of your ship \"" + target->GivenName() + "\".",
           GameData::MessageCategories().Get("normal")});
    }
    if(result & ShipEvent::SCAN_OUTFITS)
    {
      Messages::Add(
          {"The " + government->DisplayName() + " " + Noun() + " \"" + GivenName() +
               "\" completed its outfit scan of your ship \"" + target->GivenName() +
...
double Ship::CargoScanFraction() const { return cargoScan / SCAN_TIME; }


double Ship::OutfitScanFraction() const { return outfitScan / SCAN_TIME; }
...
void Ship::Fire(std::vector<Projectile> &projectiles, std::vector<Visual> &visuals, std::vector<int> *emptySoundsTimer)
{
  isInSystem = true;
  forget     = 0;
  ...
  if(CannotAct(Ship::ActionType::FIRE)) return;
  ...
  const std::vector<Hardpoint> &hardpoints = armament.Get();
  for(unsigned i = 0; i < hardpoints.size(); ++i)
  {
    ...
    if(canFire == CanFireResult::CAN_FIRE)
    {
      ...
      {
        antiMissileRange = std::max(antiMissileRange, weapon->Velocity() + weaponRadius);
      }
      else if(weapon->TractorBeam())
      {
        tractorBeamRange = std::max(tractorBeamRange, weapon->Velocity() + weaponRadius);
      }
      else if(firingCommands.HasFire(i))
      {
        ...
        if(cloak)
        {
          ...
          if(cloakingFiring > 0) cloak -= cloakingFiring;
        }
      }
    }
    else if(
        emptySoundsTimer && !(*emptySoundsTimer)[i] &&
        (canFire == CanFireResult::NO_AMMO || canFire == CanFireResult::NO_FUEL) && firingCommands.HasFire(i) &&
        hardpoints[i].IsReady())
    {
...
bool Ship::HasAntiMissile() const { return antiMissileRange; }


bool Ship::HasTractorBeam() const { return tractorBeamRange; }


// Fire an anti-missile.
bool Ship::FireAntiMissile(const Projectile &projectile, std::vector<Visual> &visuals)
{
  if(projectile.Position().Distance(position) > antiMissileRange) return false;
  if(CannotAct(Ship::ActionType::FIRE)) return false;
  ...
  for(unsigned i = 0; i < hardpoints.size(); ++i)
  {
    ...
    {
      if(armament.FireAntiMissile(i, *this, projectile, visuals, Random::Real() < jamChance)) return true;
    }
  }
...
Point Ship::FireTractorBeam(const Flotsam &flotsam, std::vector<Visual> &visuals)
{
  Point pullVector;
  if(flotsam.Position().Distance(position) > tractorBeamRange) return pullVector;
  if(CannotAct(ActionType::FIRE)) return pullVector;
  // Don't waste energy on flotsams that you can't pick up.
  if(!CanPickUp(flotsam)) return pullVector;
  if(IsYours())
  {
    const auto flotsamSetting = Preferences::GetFlotsamCollection();
    if(flotsamSetting == Preferences::FlotsamCollection::OFF) return pullVector;
    if(!GetParent() && flotsamSetting == Preferences::FlotsamCollection::ESCORT) return pullVector;
    if(GetParent() && flotsamSetting == Preferences::FlotsamCollection::FLAGSHIP) return pullVector;
  }
  ...
  for(unsigned i = 0; i < hardpoints.size(); ++i)
  {
    ...
    if(CanFire(weapon) == CanFireResult::CAN_FIRE)
    {
      if(armament.FireTractorBeam(i, *this, flotsam, visuals, Random::Real() < jamChance))
      {
        ...
        if(personality.IsOpportunistic() || (isYours && opportunisticEscorts)) break;
      }
    }
  }
  return pullVector;
...
const System *Ship::GetSystem() const { return currentSystem; }
...
const Planet *Ship::GetPlanet() const { return zoom ? nullptr : landingPlanet; }


bool Ship::IsCapturable() const { return isCapturable; }


bool Ship::IsTargetable() const
{
  return zoom == 1.f && !explosionRate && !forget && !isInvisible && !IsCloaked() && hull >= 0. && hyperspaceCount < 70;
}


bool Ship::IsOverheated() const { return isOverheated; }


bool Ship::IsIonized() const
{
  if(ionization == 0.) return false;

  // A ship can only be fully ionized if its engines or weapons require energy.
  bool usesEnergy = attributes.GetIndexed(Outfit::INTERNAL_ATTR("thrusting energy")) > 0 ||
                    attributes.GetIndexed(Outfit::INTERNAL_ATTR("reverse thrusting energy")) > 0 ||
                    attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning energy")) > 0 ||
                    any_of(
                        outfits.begin(),
                        outfits.end(),
                        [](const auto &it) -> bool
...
bool Ship::IsBoarding() const { return isBoarding; }


bool Ship::IsLanding() const { return landingPlanet; }


bool Ship::IsFleeing() const { return isFleeing; }


// Check if this ship is currently able to begin landing on its target.
bool Ship::CanLand() const
{
  if(!GetTargetStellar() || !GetTargetStellar()->GetPlanet() || isDisabled || IsDestroyed()) return false;

  if(!GetTargetStellar()->GetPlanet()->CanLand(*this)) return false;

  if(commands.Has(Command::WAIT)) return false;
...
bool Ship::CannotAct(ActionType actionType) const
{
  ...
  if(cloak > 0.)
  {
    switch(actionType)
    {
    case ActionType::AFTERBURNER:   canActCloaked = attributes.Get("cloaked afterburner") > 0.; break;
    case ActionType::BOARD:         canActCloaked = attributes.Get("cloaked boarding") > 0.; break;
    case ActionType::COMMUNICATION: canActCloaked = attributes.Get("cloaked communication") > 0.; break;
    case ActionType::FIRE:          canActCloaked = attributes.Get("cloaked firing") > 0.; break;
    case ActionType::PICKUP:        canActCloaked = attributes.Get("cloaked pickup") > 0.; break;
    case ActionType::SCAN:          canActCloaked = attributes.Get("cloaked scanning") > 0.; break;
    }
  }
  return (cloak == 1. && !canActCloaked) || (cloak != 1. && cloak > 0. && cloakDisruption == 0. && !canActCloaked);
}


double Ship::Cloaking() const { return isInvisible ? 1. : cloak; }


bool Ship::IsEnteringHyperspace() const { return hyperspaceSystem; }


bool Ship::IsHyperspacing() const { return GetHyperspacePercentage() != 0; }


int Ship::GetHyperspacePercentage() const { return hyperspaceCount; }


// Check if this ship is hyperspacing, specifically via a jump drive.
bool Ship::IsUsingJumpDrive() const { return (hyperspaceSystem || hyperspaceCount) && isUsingJumpDrive; }
...
bool Ship::IsRestrictedFrom(const Planet &planet) const
{
  ...
  if(isYours || !government) return false;
  ...
  if(isSpecial) return personality.IsRestricted() && restrictedByGov;
...
bool Ship::IsReadyToJump(bool waitingIsReady) const
{
  ...
  std::pair<JumpType, double> jumpUsed = navigation.GetCheapestJumpType(targetSystem);
  double                      fuelCost = jumpUsed.second;
  if(fuelCost == 0. || fuel < fuelCost) return false;
  ...
  double departure = isJump ? currentSystem->JumpDepartureDistance() * currentSystem->JumpDepartureDistance()
                            : currentSystem->HyperDepartureDistance() * currentSystem->HyperDepartureDistance();
  if(position.LengthSquared() <= departure) return false;


  // The ship can only enter hyperspace if it is traveling slowly enough
  // and pointed in the right direction.
  if(!isJump && scramThreshold > 0.)
  {
    const double deviation = fabs(direction.Unit().Cross(velocity));
    if(deviation > scramThreshold) return false;
  }
  ...
  if(!isJump)
  {
    ...
    if(left == stillLeft && turned != Angle(direction)) return false;
  }
...
const Swizzle *Ship::CustomSwizzle() const { return customSwizzle; }


const std::string &Ship::CustomSwizzleName() const { return customSwizzleName; }


// Check if the ship is thrusting. If so, the engine sound should be played.
bool Ship::IsThrusting() const { return isThrusting; }


bool Ship::IsReversing() const { return isReversing; }


bool Ship::IsSteering() const { return isSteering; }


double Ship::SteeringDirection() const { return steeringDirection; }


// Get the points from which engine flares should be drawn.
const std::vector<Ship::EnginePoint> &Ship::EnginePoints() const { return enginePoints; }


const std::vector<Ship::EnginePoint> &Ship::ReverseEnginePoints() const { return reverseEnginePoints; }


const std::vector<Ship::EnginePoint> &Ship::SteeringEnginePoints() const { return steeringEnginePoints; }
...
{
  shields    = 0.;
  hull       = std::min(hull, .5 * MinimumHull());
  isDisabled = true;
}
...
bool Ship::IsDestroyed() const { return hull < 0.; }
...
void Ship::Recharge(int rechargeType, bool hireCrew)
{
  ...
  if((rechargeType & Port::RechargeType::Shields) ||
     attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield generation")) > 0.)
  {
    shields = MaxShields();
  }
  if((rechargeType & Port::RechargeType::Hull) || attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull repair rate")) > 0.)
    hull = MaxHull();
  if((rechargeType & Port::RechargeType::Energy) ||
     attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy generation")) > 0.)
  {
    energy = attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity"));
  }
...
{
  double toGive = std::min(
      other.attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity")),
      std::max(200., other.attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity")) * 0.2));
...
int Ship::WasCaptured(const std::shared_ptr<Ship> &capturer)
{
  // Repair up to the point where this ship is just barely not disabled.
  hull       = std::min(std::max(hull, MinimumHull() * 1.5), MaxHull());
  isDisabled = false;
  ...
  if(transfer > 0)
  {
    if(totalRequired > capturer->Crew() + crew)
      transfer = std::max(crew ? 0 : 1, (capturer->Crew() * transfer) / totalRequired);
  ...
  for(const Bay &bay : bays)
    if(bay.ship) bay.ship->WasCaptured(capturer);
  // If a flagship is captured, its escorts become independent.
  for(const auto &it : escorts)
  {
    std::shared_ptr<Ship> escort = it.lock();
    if(escort) escort->parent.reset();
  }
...
double Ship::Shields() const
{
  ...
  return maximum > 0. ? std::min(1., shields / maximum) : 0.;
}
...
double Ship::Hull() const
{
  ...
  return maximum > 0. ? std::min(1., hull / maximum) : 1.;
}


double Ship::Fuel() const
{
  double maximum = attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity"));
  return maximum > 0. ? std::min(1., fuel / maximum) : 0.;
}
...
double Ship::Health() const
{
  ...
  if(divisor <= 0. || hullDivisor <= 0.) return 0.;
  ...
  return std::min(spareHull / hullDivisor, (spareHull + shields / (1. + disruption * .01)) / divisor);
}
...
{
  return attributes.GetIndexed(Outfit::INTERNAL_ATTR("shields")) *
         (1 + attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield multiplier")));
}


double Ship::MaxHull() const
{
  return attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull")) *
         (1 + attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull multiplier")));
}


// Get the absolute shield level of the ship.
double Ship::ShieldLevel() const { return shields; }


// Get the absolute hull level of the ship.
double Ship::HullLevel() const { return hull; }


// Get the absolute fuel level of the ship.
double Ship::FuelLevel() const { return fuel; }


// Get how disrupted this ship's shields are.
double Ship::DisruptionLevel() const { return disruption; }
...
double Ship::HullUntilDisabled() const
{
  ...
  return std::max(0., hull + 0.25 - MinimumHull());
}


// Returns the remaining damage timer, for the damage overlay.
int Ship::DamageOverlayTimer() const { return damageOverlayTimer; }


const ShipJumpNavigation &Ship::JumpNavigation() const { return navigation; }
...
int Ship::JumpsRemaining(bool followParent) const
{
  ...
  if(!targetSystem && followParent)
  {
    ...
    auto p = GetParent();
    if(p) jumpFuel = navigation.JumpFuel(p->GetTargetSystem());
  }
  if(jumpFuel == 0.) jumpFuel = navigation.JumpFuel(targetSystem);
  return jumpFuel > 0. ? fuel / jumpFuel : 0.;
}
...
{
  return attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity")) > 0. && energy == 0. &&
         attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy generation")) == 0. &&
         attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel energy")) == 0. &&
         attributes.GetIndexed(Outfit::INTERNAL_ATTR("solar collection")) == 0.;
}
...
double Ship::JumpFuelMissing() const
{
  ...
  if(jumpFuel == 0. || fuel > jumpFuel || jumpFuel > attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity")))
    return 0.;
...
double Ship::IdleHeat() const
{
  ...
  double cooling           = coolingEfficiency * attributes.GetIndexed(Outfit::INTERNAL_ATTR("cooling"));
  double activeCooling     = coolingEfficiency * attributes.GetIndexed(Outfit::INTERNAL_ATTR("active cooling"));
  ...
  double production  = std::max(0., attributes.GetIndexed(Outfit::INTERNAL_ATTR("heat generation")) - cooling);
  double dissipation = HeatDissipation() + activeCooling / MaximumHeat();
  if(dissipation == 0.) return production > 0. ? std::numeric_limits<double>::max() : 0;
  return production / dissipation;
}


// Get the heat dissipation, in heat units per heat unit per frame.
double Ship::HeatDissipation() const { return .001 * attributes.GetIndexed(Outfit::INTERNAL_ATTR("heat dissipation")); }
...
double Ship::MaximumHeat() const
{
  return MAXIMUM_TEMPERATURE *
         (cargo.Used() + attributes.Mass() + attributes.GetIndexed(Outfit::INTERNAL_ATTR("heat capacity")));
}


bool Ship::IsCloaked() const { return Cloaking() == 1.; }


double Ship::CloakingSpeed() const
{
  return attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloak")) +
         attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloak by mass")) * 1000. / Mass();
}


bool Ship::Phases(Projectile &projectile) const
{
  const double &cloak_phasing = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloak phasing"));
  // No Phasing if we are not cloaked, or not having cloak phasing.
  if(!IsCloaked() || cloak_phasing == 0) return false;

  // Check for full phasing first, to avoid more expensive lookups.
  if(cloak_phasing >= 1 || projectile.Phases(*this)) return true;
...
double Ship::CoolingEfficiency() const
{
  ...
  double x = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cooling inefficiency"));
  return 2. + 2. / (1. + exp(x / -2.)) - 4. / (1. + exp(x / -4.));
}


int Ship::Crew() const { return crew; }


// Calculate the drag on this ship. The drag can be no greater than the mass.
double Ship::Drag() const
{
  const double drag = attributes.GetIndexed(Outfit::INTERNAL_ATTR("drag")) /
                      (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("drag reduction")));
...
int Ship::CrewValue() const
{
  ...
  if(attributes.Get("use crew equivalent as crew")) return crewEquivalent;
  return std::max(Crew(), RequiredCrew()) + crewEquivalent;
}


void Ship::AddCrew(int count) { crew = std::min<int>(crew + count, attributes.Get("bunks")); }


// Check if this is a ship that can be used as a flagship.
bool Ship::CanBeFlagship() const { return RequiredCrew() && Crew() && !IsDisabled(); }


double Ship::Mass() const { return carriedMass + cargo.Used() + attributes.Mass(); }
...
{
  return Mass() / (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("inertia reduction")));
}


double Ship::TurnRate() const
{
  return attributes.GetIndexed(Outfit::INTERNAL_ATTR("turn")) / InertialMass() *
         (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("turn multiplier")));
}


double Ship::TrueTurnRate() const { return TurnRate() * 1. / (1. + slowness * .05); }


double Ship::CrewTurnRate() const
{
  // If RequiredCrew() is 0, the ratio is either inf or nan, which should return 1.
  return TurnRate() * std::min(1., static_cast<double>(Crew()) / RequiredCrew());
}


double Ship::Acceleration() const
{
  double thrust = attributes.GetIndexed(Outfit::INTERNAL_ATTR("thrust"));
  return (thrust ? thrust : attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner thrust"))) / InertialMass() *
         (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("acceleration multiplier")));
}


double Ship::TrueAcceleration() const { return Acceleration() * 1. / (1. + slowness * .05); }


double Ship::CrewAcceleration() const
{
  // If RequiredCrew() is 0, the ratio is either inf or nan, which should return 1.
  return Acceleration() * std::min(1., static_cast<double>(Crew()) / RequiredCrew());
}
...
double Ship::MaxVelocity(bool withAfterburner) const
{
  ...
  // v = thrust / drag
  double thrust            = attributes.GetIndexed(Outfit::INTERNAL_ATTR("thrust"));
  double afterburnerThrust = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner thrust"));
...
{
  return attributes.GetIndexed(Outfit::INTERNAL_ATTR("reverse thrust")) / InertialMass() *
         (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("acceleration multiplier")));
}
...
{
  return attributes.GetIndexed(Outfit::INTERNAL_ATTR("reverse thrust")) / Drag();
}
...
uint8_t Ship::ThrustHeldFrames(Ship::ThrustKind kind) const { return thrustHeldFrames[static_cast<size_t>(kind)]; }


double Ship::CurrentSpeed() const { return Velocity().Length(); }
...
int Ship::TakeDamage(std::vector<Visual> &visuals, const DamageDealt &damage, const Government *sourceGovernment)
{
  ...
  {
    int disabledDelay = attributes.GetIndexed(Outfit::INTERNAL_ATTR("depleted shield delay"));
    shieldDelay       = std::max<int>(
        shieldDelay,
        (shields <= 0. && disabledDelay) ? disabledDelay
                                               : attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield delay")));
  }
  hull -= damage.Hull();
  if(damage.Hull() && !isDisabled)
    hullDelay = std::max(hullDelay, static_cast<int>(attributes.GetIndexed(Outfit::INTERNAL_ATTR("repair delay"))));
  ...
  if(damage.HitForce()) ApplyForce(damage.HitForce(), damage.GetWeapon().IsGravitational());

  // Prevent various stats from reaching unallowable values.
  hull    = std::min(hull, MaxHull());
  ...
  if(!wasDisabled && isDisabled)
  {
    ...
    hullDelay  = std::max(hullDelay, static_cast<int>(attributes.Get("disabled repair delay")));
  }
  if(!wasDestroyed && IsDestroyed())
  {
    ...
    {
      Messages::Add(
          {"Your " + DisplayModelName() + " \"" + GivenName() + "\" has been destroyed.",
  ...
  if(sourceGovernment && !sourceGovernment->IsEnemy(government) && !personality.IsPacifist() &&
     (!personality.IsForbearing() || ((damage.Shield() || damage.Discharge()) && Shields() < .9) ||
      ((damage.Hull() || damage.Corrosion()) && Hull() < .9) || ((damage.Heat() || damage.Burn()) && isOverheated) ||
      ((damage.Energy() || damage.Ion()) && Energy() < 0.5) ||
      ((damage.Fuel() || damage.Leak()) && fuel < navigation.JumpFuel() * 2.) ||
      (damage.Scrambling() && CalculateJamChance(scrambling) > 0.1) || (damage.Slowing() && slowness > 10.) ||
...
int Ship::BaysTotal(const std::string &category) const
{
  int count = 0;
  for(const Bay &bay : bays)
...
{
  if(!HasBays() || !ship.CanBeCarried() || (IsYours() && !ship.IsYours())) return false;
  // Check only for the category that we are interested in.
  const std::string &category = ship.attributes.Category();
...
bool Ship::CanBeCarried() const { return canBeCarried; }


bool Ship::Carry(const std::shared_ptr<Ship> &ship)
{
  if(!ship || !ship->CanBeCarried() || ship->IsDisabled()) return false;
  ...
  for(Bay &bay : bays)
  {
    if((bay.category == category) && !bay.ship)
    {
      ...
      if(shouldTransferCargo && cargo.Free() && !ship->Cargo().IsEmpty()) ship->Cargo().TransferAll(cargo);
...
const std::vector<Ship::Bay> &Ship::Bays() const { return bays; }
...
CargoHold &Ship::Cargo() { return cargo; }


const CargoHold &Ship::Cargo() const { return cargo; }


// Display box effects from jettisoning this much cargo.
void Ship::Jettison(const std::string &commodity, int tons, bool wasAppeasing)
{
  ...
  if(isYours) navigation.Recalibrate(*this);
  ...
  for(; tons > 0; tons -= Flotsam::TONS_PER_BOX)
    Jettison(make_shared<Flotsam>(commodity, (Flotsam::TONS_PER_BOX < tons) ? Flotsam::TONS_PER_BOX : tons, notForGov));
}
...
void Ship::Jettison(const Outfit *outfit, int count, bool wasAppeasing)
{
  ...
  while(count > 0)
  {
    Jettison(std::make_shared<Flotsam>(outfit, (perBox < count) ? perBox : count, notForGov));
    count -= perBox;
  }
}
...
const std::map<const Outfit *, int> &Ship::Outfits() const { return outfits; }
...
void Ship::AddOutfit(const Outfit *outfit, int count)
{
  if(outfit && count)
  {
    ...
    if(outfit->GetWeapon())
    {
      ...
      if(isYours) deterrence = CalculateDeterrence();
    }
    ...
    if(outfit->Get("cargo space"))
    {
      cargo.SetSize(attributes.Get("cargo space"));
      // Only the player's ships make use of attraction and deterrence.
      if(isYours) attraction = CalculateAttraction();
    }
    if(outfit->Get("hull")) hull += outfit->Get("hull") * count;
    ...
    if((outfit->Get("hyperdrive") || outfit->Get("jump drive")) && (!before || !after)) navigation.Calibrate(*this);
    ...
    // Non-player ships will recalibrate before they jump.
    else if(isYours) navigation.Recalibrate(*this);
  }
}


// Get the list of weapons.
Armament &Ship::GetArmament() { return armament; }


const std::vector<Hardpoint> &Ship::Weapons() const { return armament.Get(); }
...
Ship::CanFireResult Ship::CanFire(const Weapon *weapon) const
{
  if(weapon->Ammo())
  {
    ...
    if(it == outfits.end() || it->second < weapon->AmmoUsage()) return CanFireResult::NO_AMMO;
  }

  if(weapon->ConsumesEnergy() &&
     energy < weapon->FiringEnergy() +
                  weapon->RelativeFiringEnergy() * attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity")))
  {
    return CanFireResult::NO_ENERGY;
  }
  if(weapon->ConsumesFuel() &&
     fuel < weapon->FiringFuel() +
                weapon->RelativeFiringFuel() * attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity")))
  {
  ...
  if(weapon->ConsumesHeat() &&
     heat <
         -(weapon->FiringHeat() + (!weapon->RelativeFiringHeat() ? 0. : weapon->RelativeFiringHeat() * MaximumHeat())))
  {
    return CanFireResult::NO_HEAT;
  }
  // Repeat this for various effects which shouldn't drop below 0.
  if(weapon->ConsumesIonization() && ionization < -weapon->FiringIon()) return CanFireResult::NO_ION;
  if(weapon->ConsumesDisruption() && disruption < -weapon->FiringDisruption()) return CanFireResult::NO_DISRUPTION;
  if(weapon->ConsumesSlowing() && slowness < -weapon->FiringSlowing()) return CanFireResult::NO_SLOWING;
...
void Ship::ExpendAmmo(const Weapon &weapon)
{
  ...
  const double relativeEnergyChange =
      weapon.RelativeFiringEnergy() * attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity"));
  const double relativeFuelChange =
      weapon.RelativeFiringFuel() * attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity"));
  ...
  if(const Outfit *ammo = weapon.Ammo())
  {
    ...
    if(!OutfitCount(ammo) && ammo->GetWeapon() && ammo->GetWeapon()->AmmoUsage()) aiCache.Calibrate(*this);
  }
...
std::shared_ptr<Ship> Ship::GetTargetShip() const { return targetShip.lock(); }


std::shared_ptr<Ship> Ship::GetShipToAssist() const { return shipToAssist.lock(); }


const StellarObject *Ship::GetTargetStellar() const { return targetPlanet; }


const System *Ship::GetTargetSystem() const { return (targetSystem == currentSystem) ? nullptr : targetSystem; }


// Mining target.
std::shared_ptr<Minable> Ship::GetTargetAsteroid() const { return targetAsteroid.lock(); }


std::shared_ptr<Flotsam> Ship::GetTargetFlotsam() const { return targetFlotsam.lock(); }


const std::set<const Flotsam *> &Ship::GetTractorFlotsam() const { return tractorFlotsam; }


const FormationPattern *Ship::GetFormationPattern() const { return formationPattern; }


void Ship::SetFleeing(bool fleeing) { isFleeing = fleeing; }
...
void Ship::SetTargetSystem(const System *system) { targetSystem = system; }


// Mining target.
void Ship::SetTargetAsteroid(const std::shared_ptr<Minable> &asteroid)
{
...
void Ship::SetTargetFlotsam(const std::shared_ptr<Flotsam> &flotsam) { targetFlotsam = flotsam; }


void Ship::SetParent(const std::shared_ptr<Ship> &ship)
{
  std::shared_ptr<Ship> oldParent = parent.lock();
  if(oldParent) oldParent->RemoveEscort(*this);

  parent = ship;
  if(ship) ship->AddEscort(*this);
}
...
{
  if(this == flotsam.Source()) return false;
  if(government == flotsam.SourceGovernment() && (!personality.Harvests() || personality.IsAppeasing())) return false;
  return cargo.Free() >= flotsam.UnitSize();
}


std::shared_ptr<Ship> Ship::GetParent() const { return parent.lock(); }


const std::vector<std::weak_ptr<Ship>> &Ship::GetEscorts() const { return escorts; }


int Ship::GetLingerSteps() const { return lingerSteps; }


void Ship::Linger() { ++lingerSteps; }
...
bool Ship::StepFlags()
{
  ...
  isInSystem = false;
  if(!fuel || !(navigation.HasHyperdrive() || navigation.HasJumpDrive())) hyperspaceSystem = nullptr;
  return false;
}
...
int Ship::StepDestroyed(std::vector<Visual> &visuals, std::list<std::shared_ptr<Flotsam>> &flotsam)
{
  ...
  if(explosionCount == explosionTotal || forget)
  {
    if(!forget)
    {
      ...
      for(const auto &it : outfits)
      {
        ...
        if(flotsamChance > 0.) Jettison(it.first, Random::Binomial(it.second, flotsamChance));
      ...
      for(std::shared_ptr<Flotsam> &it : jettisoned)
        it->Place(*this);
  ...
  ++explosionRate;
  if(Random::Int(1024) < explosionRate) CreateExplosion(visuals);
  ...
  for(const Leak &leak : leaks)
  {
    if(GetMask().IsLoaded() && leak.openPeriod > 0 && !Random::Int(leak.openPeriod))
    {
      ...
      const std::vector<Point> &outline  = outlines[Random::Int(outlines.size())];
  ...
  for(Leak &leak : activeLeaks)
  {
    if(leak.effect)
    {
      ...
      if(Random::Int(2))
        visuals.emplace_back(*leak.effect, angle.Rotate(leak.location) + position, velocity, leak.angle + angle);

      if(leak.closePeriod > 0 && !Random::Int(leak.closePeriod)) leak.effect = nullptr;
    }
  }
  return -1;
}
...
void Ship::DoGeneration()
{
  ...
  if(!isDisabled)
  {
    ...
    const double hullAvailable =
        (attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull repair rate")) +
         (hullDelay ? 0 : attributes.GetIndexed(Outfit::INTERNAL_ATTR("delayed hull repair rate")))) *
        (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull repair multiplier"))) *
        (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaked repair multiplier")) * Cloaking());
    const double hullEnergy = (attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull energy")) +
                               (hullDelay ? 0 : attributes.GetIndexed(Outfit::INTERNAL_ATTR("delayed hull energy")))) *
                              (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull energy multiplier"))) /
                              hullAvailable;
    const double hullFuel = (attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull fuel")) +
                             (hullDelay ? 0 : attributes.GetIndexed(Outfit::INTERNAL_ATTR("delayed hull fuel")))) *
                            (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull fuel multiplier"))) / hullAvailable;
    const double hullHeat = (attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull heat")) +
                             (hullDelay ? 0 : attributes.GetIndexed(Outfit::INTERNAL_ATTR("delayed hull heat")))) *
                            (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull heat multiplier"))) / hullAvailable;
    double hullRemaining = hullAvailable;
    DoRepair(hull, hullRemaining, MaxHull(), energy, hullEnergy, fuel, hullFuel, heat, hullHeat);

    const double shieldsAvailable =
        (attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield generation")) +
         (shieldDelay ? 0 : attributes.GetIndexed(Outfit::INTERNAL_ATTR("delayed shield generation")))) *
        (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield generation multiplier"))) *
        (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaked regen multiplier")) * Cloaking());
    const double shieldsEnergy =
        (attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield energy")) +
         (shieldDelay ? 0 : attributes.GetIndexed(Outfit::INTERNAL_ATTR("delayed shield energy")))) *
        (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield energy multiplier"))) / shieldsAvailable;
    const double shieldsFuel =
        (attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield fuel")) +
         (shieldDelay ? 0 : attributes.GetIndexed(Outfit::INTERNAL_ATTR("delayed shield fuel")))) *
        (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield fuel multiplier"))) / shieldsAvailable;
    const double shieldsHeat =
        (attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield heat")) +
         (shieldDelay ? 0 : attributes.GetIndexed(Outfit::INTERNAL_ATTR("delayed shield heat")))) *
        (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("shield heat multiplier"))) / shieldsAvailable;
    double shieldsRemaining = shieldsAvailable;
    DoRepair(shields, shieldsRemaining, MaxShields(), energy, shieldsEnergy, fuel, shieldsFuel, heat, shieldsHeat);

    if(!bays.empty())
    {
      // If this ship is carrying fighters, determine their repair priority.
      std::vector<std::pair<double, Ship *>> carried;
      for(const Bay &bay : bays)
        if(bay.ship) carried.emplace_back(1. - bay.ship->Health(), bay.ship.get());
      std::sort(
          carried.begin(),
          carried.end(),
          (isYours && Preferences::Has(FIGHTER_REPAIR))
              // Players may use a parallel strategy, to launch fighters in waves.
              ? [](const std::pair<double, Ship *> &lhs, const std::pair<double, Ship *> &rhs)
      ...
      for(const std::pair<double, Ship *> &it : carried)
      {
        Ship &ship = *it.second;
        if(!hullDelay)
          DoRepair(ship.hull, hullRemaining, ship.MaxHull(), energy, hullEnergy, fuel, hullFuel, heat, hullHeat);
        if(!shieldDelay)
        {
          DoRepair(
              ship.shields,
      ...
      double energyRemaining = energy - attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity"));
      double fuelRemaining   = fuel - attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity"));
      for(const std::pair<double, Ship *> &it : carried)
      {
        Ship &ship = *it.second;
        if(energyRemaining > 0.)
          DoRepair(ship.energy, energyRemaining, ship.attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity")));
        if(fuelRemaining > 0.)
          DoRepair(ship.fuel, fuelRemaining, ship.attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity")));
      }
      ...
      for(const std::pair<double, Ship *> &it : carried)
      {
        ...
        if(ship.HasDeployOrder())
          DoRepair(ship.energy, energy, ship.attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity")));
      }
    }
    // Decrease the shield and hull delays by 1 now that shield generation
    // and hull repair have been skipped over.
    shieldDelay = std::max(0, shieldDelay - 1);
    hullDelay   = std::max(0, hullDelay - 1);
  }
  // Let the ship repair itself when disabled if it has the appropriate attribute.
  if(isDisabled && attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery time")))
  {
    disabledRecoveryCounter     += 1;
    double disabledRepairEnergy  = attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery energy"));
    double disabledRepairFuel    = attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery fuel"));

    // Repair only if the counter has reached the limit and if the ship can meet the energy and fuel costs.
    if(disabledRecoveryCounter >= attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery time")) &&
       energy >= disabledRepairEnergy && fuel >= disabledRepairFuel)
    {
      energy -= disabledRepairEnergy;
      fuel   -= disabledRepairFuel;

      heat       += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery heat"));
      ionization += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery ionization"));
      scrambling += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery scrambling"));
      disruption += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery disruption"));
      slowness   += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery slowing"));
      discharge  += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery discharge"));
      corrosion  += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery corrosion"));
      leakage    += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery leak"));
      burning    += attributes.GetIndexed(Outfit::INTERNAL_ATTR("disabled recovery burning"));
  ...
  energy = std::min(energy, attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity")));
  fuel   = std::min(fuel, attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity")));

  heat -= heat * HeatDissipation();
  if(heat > MaximumHeat())
  {
    isOverheated     = true;
    double heatRatio = Heat() / (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("overheat damage threshold")));
    if(heatRatio > 1.) hull -= attributes.GetIndexed(Outfit::INTERNAL_ATTR("overheat damage rate")) * heatRatio;
  }
  else if(heat < .9 * MaximumHeat())
  {
    isOverheated = false;
  ...
  if(isDisabled)
  {
    ...
    if(currentSystem)
    {
      System::SolarGeneration generation = currentSystem->GetSolarGeneration(
          position,
          attributes.GetIndexed(Outfit::INTERNAL_ATTR("ramscoop")),
          attributes.GetIndexed(Outfit::INTERNAL_ATTR("solar collection")),
          attributes.GetIndexed(Outfit::INTERNAL_ATTR("solar heat")));
    ...
    double coolingEfficiency  = CoolingEfficiency();
    energy                   += attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy generation")) -
              attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy consumption"));
    fuel += attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel generation"));
    heat += attributes.GetIndexed(Outfit::INTERNAL_ATTR("heat generation"));
    heat -= coolingEfficiency * attributes.GetIndexed(Outfit::INTERNAL_ATTR("cooling"));

    // Convert fuel into energy and heat only when the required amount of fuel is available.
    if(attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel consumption")) <= fuel)
    {
      fuel   -= attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel consumption"));
      energy += attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel energy"));
      heat   += attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel heat"));
    }
    ...
    if(activeCooling > 0. && heat > 0. && energy >= 0.)
    {
      ...
      double coolingEnergy = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cooling energy"));
      if(coolingEnergy)
      {
        double spentEnergy  = std::min(energy, coolingEnergy * std::min(1., Heat()));
        heat               -= activeCooling * spentEnergy / coolingEnergy;
        energy             -= spentEnergy;
      }
      else {
        heat -= activeCooling * std::min(1., Heat());
      }
    }
  }

  // Don't allow any levels to drop below zero.
  shields = std::max(0., shields);
  energy  = std::max(0., energy);
  fuel    = std::max(0., fuel);
  heat    = std::max(0., heat);
}
...
void Ship::DoPassiveEffects(std::vector<Visual> &visuals, std::list<std::shared_ptr<Flotsam>> &flotsam)
{
  // Adjust the error in the pilot's targeting.
  personality.UpdateConfusion(firingCommands.IsFiring());

  // Handle ionization effects, etc.
  if(ionization) CreateSparks(visuals, "ion spark", ionization * .05);
  if(scrambling) CreateSparks(visuals, "scramble spark", scrambling * .05);
  if(disruption) CreateSparks(visuals, "disruption spark", disruption * .1);
  if(slowness) CreateSparks(visuals, "slowing spark", slowness * .1);
  if(discharge) CreateSparks(visuals, "discharge spark", discharge * .1);
  if(corrosion) CreateSparks(visuals, "corrosion spark", corrosion * .1);
  if(leakage) CreateSparks(visuals, "leakage spark", leakage * .1);
  if(burning) CreateSparks(visuals, "burning spark", burning * .1);
}


void Ship::DoJettison(std::list<std::shared_ptr<Flotsam>> &flotsam)
{
  if(forget) return;
...
void Ship::DoCloakDecision()
{
  ...
  if(!cloak) cloakDisruption = std::max(0., cloakDisruption - 1.);
  ...
  const double minimalHullForCloak = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloak hull threshold"));
  if(minimalHullForCloak && (hull / attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull")) < minimalHullForCloak))
    cloakDisruption = 1.;

  const double  cloakingSpeed  = CloakingSpeed();
  const double &cloakingFuel   = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaking fuel"));
  const double &cloakingEnergy = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaking energy"));
  const double &cloakingHull   = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaking hull"));
  const double &cloakingShield = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaking shields"));
  const bool    canCloak =
      (!isDisabled && cloakingSpeed > 0. && !cloakDisruption && fuel >= cloakingFuel && energy >= cloakingEnergy &&
       MinimumHull() < hull - cloakingHull && shields >= cloakingShield);
  if(commands.Has(Command::CLOAK) && canCloak)
  {
    cloak                       = std::min(1., std::max(0., cloak + cloakingSpeed));
    fuel                       -= cloakingFuel;
    energy                     -= cloakingEnergy;
    shields                    -= cloakingShield;
    hull                       -= cloakingHull;
    heat                       += attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaking heat"));
    double cloakingShieldDelay  = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaking shield delay"));
    double cloakingHullDelay    = attributes.GetIndexed(Outfit::INTERNAL_ATTR("cloaking repair delay"));
    cloakingShieldDelay  = (cloakingShieldDelay < 1.) ? (Random::Real() <= cloakingShieldDelay) : cloakingShieldDelay;
    cloakingHullDelay    = (cloakingHullDelay < 1.) ? (Random::Real() <= cloakingHullDelay) : cloakingHullDelay;
    shieldDelay         += cloakingShieldDelay;
    hullDelay           += cloakingHullDelay;
  }
  else if(cloakingSpeed)
  {
    cloak = std::max(0., cloak - cloakingSpeed);
    ...
    if(commands.Has(Command::CLOAK)) cloakDisruption = std::max(cloakDisruption, 1.);
  }
  else {
    cloak = 0.;
  }
...
bool Ship::DoHyperspaceLogic(std::vector<Visual> &visuals)
{
  if(!hyperspaceSystem && !hyperspaceCount) return false;
  ...
  if(hyperspaceSystem) fuel -= hyperspaceFuelCost / HYPER_C;
  ...
  if(isUsingJumpDrive && !forget)
  {
    ...
    const std::map<const Effect *, int> &jumpEffects = attributes.JumpEffects();
  ...
  if(hyperspaceCount == HYPER_C)
  {
    ...
    if(!planet || planet->IsWormhole() || !planet->IsInSystem(currentSystem)) targetPlanet = nullptr;
    // Check if your parent has a target planet in this system.
    std::shared_ptr<Ship> parent = GetParent();
    if(!targetPlanet && parent && parent->targetPlanet)
    {
      planet = parent->targetPlanet->GetPlanet();
      if(planet && !planet->IsWormhole() && planet->IsInSystem(currentSystem)) targetPlanet = parent->targetPlanet;
    }
    direction = -1;
    ...
    double extraArrivalDistance =
        isUsingJumpDrive ? currentSystem->ExtraJumpArrivalDistance() : currentSystem->ExtraHyperArrivalDistance();
    ...
    if(extraArrivalDistance == 0)
    {
      if(targetPlanet)
      {
        ...
        {
          if(object.HasSprite() && object.HasValidPlanet() && object.GetPlanet()->HasServices(isYours))
          {
  ...
  if(!isUsingJumpDrive)
  {
    ...
    if(!hyperspaceSystem)
    {
      ...
      double exitV        = std::max(HYPER_A, MaxVelocity());
      ...
      if(discriminant > 0.)
      {
        ...
        if(altV > 0. && altV < exitV) exitV = altV;
      }
  ...
  if(GetParent() && GetParent()->currentSystem == currentSystem)
  {
    ...
    if(length > 1000.) hyperspaceOffset *= 1000. / length;
  }
...
{
  if(!landingPlanet && zoom >= 1.f) return false;
  ...
  if(isDisabled) landingPlanet = nullptr;
  ...
  if(landingPlanet && zoom)
  {
    ...
    if(GetTargetStellar()) position = .97 * position + .03 * GetTargetStellar()->Position();
    zoom -= landingSpeed;
    if(zoom <= 0.f)
    {
      ...
      if(landingPlanet->IsWormhole())
      {
        ...
        for(const StellarObject &object : currentSystem->Objects())
          if(object.GetPlanet() == landingPlanet) position = object.Position();
        SetTargetStellar(nullptr);
        SetTargetSystem(nullptr);
        landingPlanet = nullptr;
      }
      else if(!isSpecial || personality.IsFleeing())
      {
        ...
        for(const auto &it : escorts)
        {
          ...
          if(!escort || escort->IsDestroyed() || escort->IsDisabled() || escort->zoom == 0.f || !escort->GetSystem())
            continue;
          escortsLanded = false;
          break;
        }
        if(escortsLanded) MarkForRemoval();
        return true;
      }
  ...
  // Only refuel if this planet has a spaceport.
      ...
      fuel >= attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity")) || !landingPlanet ||
      !landingPlanet->GetPort().CanRecharge(Port::RechargeType::Fuel, isYours))
  {
    zoom = std::min(1.f, zoom + landingSpeed);
    SetTargetStellar(nullptr);
    landingPlanet = nullptr;
  }
  else {
    fuel = std::min(fuel + 1., attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity")));
  }

  // Move the ship at the velocity it had when it began landing, but
  // scaled based on how small it is now.
  if(zoom > 0.f) position += velocity * zoom;
...
void Ship::DoInitializeMovement()
{
  ...
  else if(commands.Has(Command::JUMP) && IsReadyToJump())
  {
    ...
    std::pair<JumpType, double> jumpUsed = navigation.GetCheapestJumpType(hyperspaceSystem);
...
void Ship::StepPilot()
{
  ...
  else if(requiredCrew && static_cast<int>(Random::Int(requiredCrew)) >= Crew())
  {
    ...
    if(isYours || personality.IsEscort())
    {
      ...
      {
        Messages::Add(
            {"The " + givenName + " is moving erratically because there are not enough crew to pilot it.",
             GameData::MessageCategories().Get("low")});
      }
    }
  }
...
void Ship::DoMovement(bool &isUsingAfterburner)
{
  ...
  const double mass           = InertialMass();
  const double dragForce      = DragForce();
  ...
  else if(!pilotError)
  {
    if(commands.Turn())
    {
      ...
      double cost = attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning energy"));
      if(cost > 0. && energy < cost * fabs(commands.Turn())) commands.SetTurn(copysign(energy / cost, commands.Turn()));

      cost = attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning shields"));
      if(cost > 0. && shields < cost * fabs(commands.Turn()))
        commands.SetTurn(copysign(shields / cost, commands.Turn()));

      cost = attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning hull"));
      if(cost > 0. && hull < cost * fabs(commands.Turn())) commands.SetTurn(copysign(hull / cost, commands.Turn()));

      cost = attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning fuel"));
      if(cost > 0. && fuel < cost * fabs(commands.Turn())) commands.SetTurn(copysign(fuel / cost, commands.Turn()));

      cost = -attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning heat"));
      if(cost > 0. && heat < cost * fabs(commands.Turn())) commands.SetTurn(copysign(heat / cost, commands.Turn()));
      ...
      if(commands.Turn())
      {
        ...
        shields    -= scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning shields"));
        hull       -= scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning hull"));
        energy     -= scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning energy"));
        fuel       -= scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning fuel"));
        heat       += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning heat"));
        discharge  += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning discharge"));
        corrosion  += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning corrosion"));
        ionization += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning ion"));
        scrambling += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning scramble"));
        leakage    += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning leakage"));
        burning    += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning burn"));
        slowness   += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning slowing"));
        disruption += scale * attributes.GetIndexed(Outfit::INTERNAL_ATTR("turning disruption"));
    ...
    if(thrustCommand)
    {
      ...
      double cost = attributes.GetIndexed(
          (thrustCommand > 0.) ? Outfit::INTERNAL_ATTR("thrusting energy")
                               : Outfit::INTERNAL_ATTR("reverse thrusting energy"));
      if(cost > 0. && energy < cost * fabs(thrustCommand)) thrustCommand = copysign(energy / cost, thrustCommand);

      cost = attributes.GetIndexed(
          (thrustCommand > 0.) ? Outfit::INTERNAL_ATTR("thrusting shields")
                               : Outfit::INTERNAL_ATTR("reverse thrusting shields"));
      if(cost > 0. && shields < cost * fabs(thrustCommand)) thrustCommand = copysign(shields / cost, thrustCommand);

      cost = attributes.GetIndexed(
          (thrustCommand > 0.) ? Outfit::INTERNAL_ATTR("thrusting hull")
                               : Outfit::INTERNAL_ATTR("reverse thrusting hull"));
      if(cost > 0. && hull < cost * fabs(thrustCommand)) thrustCommand = copysign(hull / cost, thrustCommand);

      cost = attributes.GetIndexed(
          (thrustCommand > 0.) ? Outfit::INTERNAL_ATTR("thrusting fuel")
                               : Outfit::INTERNAL_ATTR("reverse thrusting fuel"));
      if(cost > 0. && fuel < cost * fabs(thrustCommand)) thrustCommand = copysign(fuel / cost, thrustCommand);

      cost = -attributes.GetIndexed(
          (thrustCommand > 0.) ? Outfit::INTERNAL_ATTR("thrusting heat")
                               : Outfit::INTERNAL_ATTR("reverse thrusting heat"));
      if(cost > 0. && heat < cost * fabs(thrustCommand)) thrustCommand = copysign(heat / cost, thrustCommand);
      ...
      if(thrustCommand)
      {
        ...
        isReversing = !isThrusting && attributes.GetIndexed(Outfit::INTERNAL_ATTR("reverse thrust")) > 0.;
        thrust      = attributes.GetIndexed(
            isThrusting ? Outfit::INTERNAL_ATTR("thrust") : Outfit::INTERNAL_ATTR("reverse thrust"));
        IncrementThrusterHeld(isReversing ? ThrustKind::REVERSE : ThrustKind::FORWARD);
        if(thrust)
        {
          double scale = fabs(thrustCommand);

          shields -= scale * attributes.GetIndexed(
                                 isThrusting ? Outfit::INTERNAL_ATTR("thrusting shields")
                                             : Outfit::INTERNAL_ATTR("reverse thrusting shields"));
          hull -= scale * attributes.GetIndexed(
                              isThrusting ? Outfit::INTERNAL_ATTR("thrusting hull")
                                          : Outfit::INTERNAL_ATTR("reverse thrusting hull"));
          energy -= scale * attributes.GetIndexed(
                                isThrusting ? Outfit::INTERNAL_ATTR("thrusting energy")
                                            : Outfit::INTERNAL_ATTR("reverse thrusting energy"));
          fuel -= scale * attributes.GetIndexed(
                              isThrusting ? Outfit::INTERNAL_ATTR("thrusting fuel")
                                          : Outfit::INTERNAL_ATTR("reverse thrusting fuel"));
          heat += scale * attributes.GetIndexed(
                              isThrusting ? Outfit::INTERNAL_ATTR("thrusting heat")
                                          : Outfit::INTERNAL_ATTR("reverse thrusting heat"));
          discharge += scale * attributes.GetIndexed(
                                   isThrusting ? Outfit::INTERNAL_ATTR("thrusting discharge")
                                               : Outfit::INTERNAL_ATTR("reverse thrusting discharge"));
          corrosion += scale * attributes.GetIndexed(
                                   isThrusting ? Outfit::INTERNAL_ATTR("thrusting corrosion")
                                               : Outfit::INTERNAL_ATTR("reverse thrusting corrosion"));
          ionization += scale * attributes.GetIndexed(
                                    isThrusting ? Outfit::INTERNAL_ATTR("thrusting ion")
                                                : Outfit::INTERNAL_ATTR("reverse thrusting ion"));
          scrambling += scale * attributes.GetIndexed(
                                    isThrusting ? Outfit::INTERNAL_ATTR("thrusting scramble")
                                                : Outfit::INTERNAL_ATTR("reverse thrusting scramble"));
          burning += scale * attributes.GetIndexed(
                                 isThrusting ? Outfit::INTERNAL_ATTR("thrusting burn")
                                             : Outfit::INTERNAL_ATTR("reverse thrusting burn"));
          leakage += scale * attributes.GetIndexed(
                                 isThrusting ? Outfit::INTERNAL_ATTR("thrusting leakage")
                                             : Outfit::INTERNAL_ATTR("reverse thrusting leakage"));
          slowness += scale * attributes.GetIndexed(
                                  isThrusting ? Outfit::INTERNAL_ATTR("thrusting slowing")
                                              : Outfit::INTERNAL_ATTR("reverse thrusting slowing"));
          disruption += scale * attributes.GetIndexed(
                                    isThrusting ? Outfit::INTERNAL_ATTR("thrusting disruption")
                                                : Outfit::INTERNAL_ATTR("reverse thrusting disruption"));
    ...
    if(applyAfterburner)
    {
      thrust            = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner thrust"));
      double shieldCost = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner shields"));
      double hullCost   = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner hull"));
      double energyCost = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner energy"));
      double fuelCost   = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner fuel"));
      double heatCost   = -attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner heat"));

      double dischargeCost  = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner discharge"));
      double corrosionCost  = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner corrosion"));
      double ionCost        = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner ion"));
      double scramblingCost = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner scramble"));
      double leakageCost    = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner leakage"));
      double burningCost    = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner burn"));

      double slownessCost   = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner slowing"));
      double disruptionCost = attributes.GetIndexed(Outfit::INTERNAL_ATTR("afterburner disruption"));

      if(thrust && shields >= shieldCost && hull >= hullCost && energy >= energyCost && fuel >= fuelCost &&
         heat >= heatCost)
      {
        shields -= shieldCost;
        hull    -= hullCost;
        ...
        acceleration += angle.Unit() * (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("acceleration multiplier"))) *
                        thrust / mass;
  ...
  if(acceleration)
  {
    ...
    Point dragAcceleration =
        acceleration -
        velocity * dragForce * (1. + attributes.GetIndexed(Outfit::INTERNAL_ATTR("acceleration multiplier")));
    ...
    if(dragAcceleration)
    {
      ...
      if(commands.Has(Command::STOP))
      {
        ...
        // possible if the acceleration plus drag std::vector is in the
        // opposite direction from the velocity std::vector when both are
        // projected onto the current facing std::vector, and the acceleration
        // std::vector is the larger of the two.
...
void Ship::StepTargeting()
{
  ...
  if(CanBeCarried() && !(target && (target == GetShipToAssist() || isYours))) target = GetParent();
    ...
    if(isBoarding && !CanBeCarried())
    {
      if(!target->IsDisabled() && government->IsEnemy(target->government))
      {
        isBoarding = false;
          ...
          target->IsDestroyed() || target->IsLanding() || target->IsHyperspacing() ||
    ...
    if(isBoarding && !pilotError)
    {
      ...
      if(distance < 10. && speed < 1. && ((CanBeCarried() && government == target->government) || !turn))
      {
        if(cloak && !attributes.Get("cloaked boarding"))
        {
          ...
          if(isYours) Messages::Add(*GameData::Messages().Get("cannot board while cloaked"));
        }
        else {
          isBoarding   = false;
          ...
          {
            Messages::Add(
                {"The " + target->DisplayModelName() + " \"" + target->GivenName() +
  ...
  if(target && target->IsDestroyed() && target->explosionCount >= target->explosionTotal) targetShip.reset();
}


// Finally, move the ship and create any movement visuals.
void Ship::DoEngineVisuals(std::vector<Visual> &visuals, bool isUsingAfterburner)
{
  if(isUsingAfterburner && !Attributes().AfterburnerEffects().empty())
  {
    double gimbalDirection = (Commands().Has(Command::FORWARD) || Commands().Has(Command::BACK)) * -Commands().Turn();
...
{
  if(neverDisabled) return 0.;

  double maximumHull       = MaxHull();
  double absoluteThreshold = attributes.GetIndexed(Outfit::INTERNAL_ATTR("absolute threshold"));
  if(absoluteThreshold > 0.) return absoluteThreshold;

  double thresholdPercent = attributes.GetIndexed(Outfit::INTERNAL_ATTR("threshold percentage"));
  double transition       = 1 / (1 + 0.0005 * maximumHull);
  double minimumHull      = maximumHull * (thresholdPercent > 0. ? std::min(thresholdPercent, 1.)
                                                                 : 0.1 * (1. - transition) + 0.5 * transition);

  return std::max(0., floor(minimumHull + attributes.GetIndexed(Outfit::INTERNAL_ATTR("hull threshold"))));
}


void Ship::CreateExplosion(std::vector<Visual> &visuals, bool spread)
{
  if(!HasSprite() || !GetMask().IsLoaded() || explosionEffects.empty()) return;
...
void Ship::CreateSparks(std::vector<Visual> &visuals, const std::string &name, double amount)
{
  CreateSparks(visuals, GameData::Effects().Get(name), amount);
}


void Ship::CreateSparks(std::vector<Visual> &visuals, const Effect *effect, double amount)
{
  if(forget || amount <= 0.) return;

  // Limit the number of sparks, depending on the size of the sprite.
  // The limit needs to be the first argument in case amount is NaN.
  amount = std::min(Width() * Height() * .0006, amount);
...
{
  return std::max(0., .4 * sqrt(attributes.GetIndexed(Outfit::INTERNAL_ATTR("cargo space"))) - 1.8);
}
...
double Ship::CalculateDeterrence() const
{
  ...
  for(const Hardpoint &hardpoint : Weapons())
  {
    ...
    if(weapon)
    {
      ...
      double shieldFactor =
          weapon->ShieldDamage() + weapon->RelativeShieldDamage() * MaxShields() + weapon->DischargeDamage() * 100.;
      double hullFactor =
          weapon->HullDamage() + weapon->RelativeHullDamage() * MaxHull() + weapon->CorrosionDamage() * 100.;

      // Other damage types don't outright destroy ships, so they aren't considered
      // as heavily in the strength of a weapon.
      double energyFactor =
          weapon->EnergyDamage() +
          weapon->RelativeEnergyDamage() * attributes.GetIndexed(Outfit::INTERNAL_ATTR("energy capacity")) +
          weapon->IonDamage() * 100.;
      double heatFactor =
          weapon->HeatDamage() + weapon->RelativeHeatDamage() * MaximumHeat() + weapon->BurnDamage() * 100.;
      double fuelFactor = weapon->FuelDamage() +
                          weapon->RelativeFuelDamage() * attributes.GetIndexed(Outfit::INTERNAL_ATTR("fuel capacity")) +
      ...
      double strength =
          shieldFactor + hullFactor +
          0.2 * (energyFactor + heatFactor + fuelFactor + scramblingFactor + slowingFactor + disruptionFactor);
...
void Ship::IncrementThrusterHeld(Ship::ThrustKind kind)
{
  ...
  heldFrame          = std::min(MAX_THRUST_HELD_FRAMES, static_cast<uint8_t>(heldFrame + 2));
}


void Ship::Jettison(std::shared_ptr<Flotsam> toJettison)
{
  if(currentSystem)
  {
  ...
  std::shared_ptr<Ship> carrier = parent.lock();
  if(!carrier) return;
  size_t bayIndex = 0;
