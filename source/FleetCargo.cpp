...
namespace
{
  ...
  Sale<Outfit> GetOutfitsForSale(const System *here)
  {
    ...
    if(here)
    {
      for(const StellarObject &object : here->Objects())
      {
        ...
        if(planet && planet->IsValid() && planet->HasOutfitter()) outfits.Add(planet->OutfitterStock());
      }
    }
    return outfits;
  }
  ...
  std::vector<const Outfit *>
  OutfitChoices(const std::set<const Shop<Outfit> *> &outfitters, const System *hub, int maxSize)
  {
      ...
      if(!choices.empty())
      {
        for(const auto outfit : choices)
        {
          ...
          if(mass > 0. && mass < maxSize && outfit->Cost() > 0)
          {
            ...
            if(outfit->Get("outfit space") > 0. || outfit->Get("cargo space") > 0. || outfit->Get("bunks")) continue;
...
void FleetCargo::SetCargo(Ship *ship) const
{
  ...
  for(int i = 0; i < cargo; ++i)
  {
    ...
    {
      outfits.erase(
          remove_if(outfits.begin(), outfits.end(), [&free](const Outfit *a) { return a->Mass() > free; }),
          outfits.end());
    }

    if(canChooseCommodities && canChooseOutfits)
      if(Random::Real() < .8) AddRandomCommodity(*ship, free, commodities);
      else AddRandomOutfit(*ship, free, outfits);
    else if(canChooseCommodities) AddRandomCommodity(*ship, free, commodities);
    else AddRandomOutfit(*ship, free, outfits);

    free = ship->Cargo().Free();
  }
  int extraCrew = ship->Attributes().Get("bunks") - ship->RequiredCrew();
  if(extraCrew > 0) ship->AddCrew(Random::Int(extraCrew + 1));
}
