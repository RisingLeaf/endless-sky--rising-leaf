...
PlanetPanel::~PlanetPanel() { Audio::Resume(); }
...
void PlanetPanel::Step()
{
  ...
  if(player.IsDead())
  {
    ...
    if(selectedPanel) GetUI().Pop(selectedPanel);
    GetUI().Pop(this);
    return;
  }
  ...
  if(!hasLoadedThumbnails || SpriteLoadManager::RecheckThumbnails())
  {
    ...
    if(hasShipyard || hasOutfitter)
    {
      for(const auto &ship : player.Ships())
      {
        ...
        if(!ship || !ship->GetPlanet() || ship->GetSystem() != &system) continue;
    ...
    if(hasOutfitter)
    {
      ...
      for(const auto &license : player.Licenses())
      {
        ...
        if(outfit) SpriteLoadManager::LoadDeferred(queue, outfit->Thumbnail());
      }
    }
  }
  ...
  if(activePanel != spaceport.get() && GetUI().IsTop(activePanel))
  {
    ...
    else player.HandleBlockedMissions(Mission::LANDING, GetUI());
  }
}
...
void PlanetPanel::Draw()
{
  ...
  const Ship *flagship = player.Flagship();
  if(flagship && flagship->CanBeFlagship()) info.SetCondition("has ship");

  if(planet.CanUseServices())
  {
    const Port &port = planet.GetPort();

    if(port.HasService(Port::ServicesType::Bank)) info.SetCondition("has bank");
    if(port.HasService(Port::ServicesType::JobBoard)) info.SetCondition("has job board");
    if(port.HasService(Port::ServicesType::HireCrew)) info.SetCondition("can hire crew");
    if(port.HasService(Port::ServicesType::Trading) && system.HasTrade()) info.SetCondition("has trade");
    ...
    if(hasShipyard) info.SetCondition("has shipyard");

    if(hasOutfitter) info.SetCondition("has outfitter");
  }
  ...
  if(!selectedPanel) description->SetText(planet.Description().ToString());
}
...
bool PlanetPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  else if(key == 't' && hasAccess && planet.GetPort().HasService(Port::ServicesType::Trading) && system.HasTrade())
  {
  ...
  else if(key == 'p' && hasAccess && planet.HasNamedPort())
  {
    ...
    if(isNewPress) spaceport->UpdateNews();
  ...
  if(oldPanel) GetUI().Pop(oldPanel);

  UI::PlaySound(sound);

  if(selectedPanel) RemoveChild(description.get());
  else if(oldPanel) AddChild(description);
...
{
  const Interface &planetInterface =
      *GameData::Interfaces().Get(Screen::Width() < 1280 ? "planet (small screen)" : "planet");
      ...
      if(check.back() == '!')
      {
        ...
        if(result.first->GetSystem() != &system)
        {
          ...
          GetUI().Push(new ConversationPanel(
              player,
              *GameData::Conversations().Get("flight check: " + check),
              nullptr,
              nullptr,
              result.first));
          return;
    ...
    if(!absentCannotFly.empty())
    {
      ...
      GetUI().Push(
          DialogPanel::CallFunctionIfOk(
              this,
              &PlanetPanel::CheckWarningsAndTakeOff,
...
void PlanetPanel::CheckWarningsAndTakeOff()
{
  ...
  for(auto &it : cargo.Outfits())
  {
    ...
    if(it.first->Get("unique")) uniquesToSell[it.first] = it.second;
  }
  // Have you left any unique items at the outfitter?
  std::map<const Outfit *, int> leftUniques;
  for(const auto &it : player.GetStock())
    if(it.second > 0 && it.first->Get("unique")) leftUniques[it.first] = it.second;
  ...
  if(!flightChecks.empty())
  {
    ...
    const auto jumpWarnings = std::set<std::string>{"no bays?", "no fuel?", "no hyperdrive?"};
    for(const auto &result : flightChecks)
    {
     ...
     !leftUniques.empty())
  {
    ...
    {
      out << "\n- launch with ";
      if(nonJumpCount == 1) out << "a ship";
      else out << nonJumpCount << " ships";
