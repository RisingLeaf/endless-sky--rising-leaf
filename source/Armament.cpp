...
int Armament::Add(const Outfit *outfit, int count)
{
  ...
  {
    Logger::Log(
        "Skipping unmountable outfit \"" + outfit->TrueName() +
            "\"."
  ...
  if(!existing)
  {
    ...
    if(added > 0 && weapon->IsStreamed()) streamReload[outfit] = 0;
    else streamReload.erase(outfit);
  }
  return added;
}
...
{
  for(Hardpoint &hardpoint : hardpoints)
    if(hardpoint.GetOutfit()) hardpoint.Install(hardpoint.GetOutfit());
...
void Armament::ReloadAll()
{
  ...
  for(Hardpoint &hardpoint : hardpoints)
  {
    if(hardpoint.GetOutfit())
    {
      ...
      if(outfit->GetWeapon()->IsStreamed()) streamReload[outfit] = 0;
    }
  }
}
...
void Armament::Swap(unsigned first, unsigned second)
{
  ...
  if(first >= hardpoints.size()) return;
  if(second >= hardpoints.size()) return;
  if(hardpoints[first].IsTurret() != hardpoints[second].IsTurret()) return;
...
const std::vector<Hardpoint> &Armament::Get() const { return hardpoints; }


// Determine how many fixed gun hardpoints are on this ship.
int Armament::GunCount() const { return hardpoints.size() - TurretCount(); }
...
std::set<const Outfit *> Armament::RestockableAmmo() const
{
  ...
  for(const Hardpoint &hardpoint : hardpoints)
  {
    ...
    if(weapon)
    {
      ...
      if(ammo && !ammo->GetWeapon()) restockable.emplace(ammo);
    }
  }
  return restockable;
}
    ...
    bool                     jammed)
{
  ...
  if(!CheckHardpoint(index)) return;
  ...
  else hardpoints[index].Fire(ship, projectiles, visuals);
}
...
bool Armament::FireAntiMissile(
    ...
    bool                 jammed)
{
  if(!CheckHardpoint(index, jammed)) return false;
...
{
  if(index >= hardpoints.size() || !hardpoints[index].IsReady()) return false;
