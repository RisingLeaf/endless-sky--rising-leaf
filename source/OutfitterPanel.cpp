...
namespace
{
  ...
  set<const Outfit *> GetRefillableAmmunition(const Ship &ship) noexcept
  {
    ...
    for(auto &&it : ship.Weapons())
    {
      ...
      if(weapon && weapon->Ammo() && weapon->AmmoUsage() > 0) toRefill.emplace(weapon->Ammo());
    }
    ...
    for(auto &&it : ship.Outfits())
    {
      ...
      if(!outfit->GetWeapon() && outfit->AmmoStored()) toRefill.emplace(outfit->AmmoStored());
    }
    return toRefill;
  }
  ...
  string LicenseRoot(const string &name) { return name.substr(0, name.length() - LICENSE.length()); }

  string LocationName(OutfitterPanel::OutfitLocation location)
  {
    switch(location)
    {
    case OutfitterPanel::OutfitLocation::Ship:    return "ship";
...
OutfitterPanel::OutfitterPanel(PlayerInfo &player, Sale<Outfit> stock) : ShopPanel(player, true), outfitter(stock)
{
  ...
  for(auto &ship : player.Ships())
    if(ship->GetPlanet() == planet) ++shipsHere;
}
...
int OutfitterPanel::VisibilityCheckboxesSize() const { return 4 * checkboxSize.Y() + 2. * keyPad.Y(); }


bool OutfitterPanel::HasItem(const string &name) const
{
  const Outfit *outfit = GameData::Outfits().Get(name);
  if(showForSale && (outfitter.Has(outfit) || player.Stock(outfit) > 0)) return true;

  if(showCargo && player.Cargo().Get(outfit)) return true;

  if(showStorage && player.Storage().Get(outfit)) return true;

  if(showInstalled)
  {
    for(const Ship *ship : playerShips)
      if(ship->OutfitCount(outfit)) return true;
  }

  if(showForSale && HasLicense(name)) return true;
...
void OutfitterPanel::DrawItem(const string &name, const Point &point)
{
  ...
  if(point.Y() + OUTFIT_SIZE / 2 < Screen::Top() || point.Y() - OUTFIT_SIZE / 2 > Screen::Bottom()) return;
  ...
  if(playerShip || isLicense || mapSize)
  {
    ...
    else if(mapSize)
    {
      ...
      for(const Ship *ship : playerShips)
      {
        ...
        if(firstModelName.empty()) firstModelName = modelName;
        else highlightDifferences &= (modelName == firstModelName);
    ...
    if(maxCount)
    {
      string label = "installed: " + std::to_string(minCount);
      Color  color = bright;
      if(maxCount > minCount)
      {
        label += " - " + std::to_string(maxCount);
        if(highlightDifferences) color = highlight;
      }
  ...
  int stock = 0;
  if(!outfitter.Has(outfit)) stock = max(0, player.Stock(outfit));
  ...
  else if(cargo && storage) message = "in cargo: " + std::to_string(cargo) + ", in storage: " + std::to_string(storage);
  else if(cargo && stock) message = "in cargo: " + std::to_string(cargo) + ", in stock: " + std::to_string(stock);
  else if(storage && stock) message = "in storage: " + std::to_string(storage) + ", in stock: " + std::to_string(stock);
  else if(cargo) message = "in cargo: " + std::to_string(cargo);
  else if(storage) message = "in storage: " + std::to_string(storage);
  else if(stock) message = "in stock: " + std::to_string(stock);
  else if(!outfitter.Has(outfit)) message = "(not sold here)";
  if(!message.empty())
  {
...
double OutfitterPanel::DrawDetails(const Point &center)
{
  ...
  if(selectedOutfit)
  {
    ...
    const float   tileSize =
        thumbnail ? max(thumbnail->Height(), static_cast<float>(TileSize())) : static_cast<float>(TileSize());
    ...
    if(thumbnail)
    {
      if(thumbnail->IsLoaded()) SpriteShader::Draw(thumbnail, thumbnailCenter);
      else loadingCircle.Draw(thumbnailCenter);
    }
    ...
    if(hasDescription)
    {
      ...
      ClickZone<string> collapseDescription = ClickZone<string>(descriptionCenter, descriptionDimensions, DESCRIPTION);
...
OutfitterPanel::CanMoveOutfit(OutfitLocation fromLocation, OutfitLocation toLocation, const string &actionName) const
{
  if(!planet || !selectedOutfit) return "No outfit selected.";

  // Prevent coding up bad combinations.
  if(fromLocation == toLocation) throw runtime_error("unreachable; to and from are the same");
  ...
  case OutfitLocation::Ship:
    {
      if(!playerShip) return "No ship selected.";
      ...
      for(const Ship *ship : playerShips)
      {
        if(ship->OutfitCount(selectedOutfit) > 0)
        {
          ...
          if(ammo && ship->OutfitCount(ammo)) attributes.Add(*ammo, -ship->OutfitCount(ammo));
          ...
          for(const auto &it : selectedOutfit->Attributes())
          {
            if(attributes.Get(it.first) < it.second)
            {
              ...
              if(errorDetails.empty())
                errorDetails.emplace_back(string("\"") + it.first + "\" value would be reduced to less than zero");
            }
          }

          if(!errorDetails.empty()) dependentOutfitErrors.emplace_back(ship->GivenName(), std::move(errorDetails));
        }
      }
      ...
      if(!foundOutfit) return "You don't have any " + selectedOutfit->PluralName() + " to " + actionName + ".";
      ...
      if(!dependentOutfitErrors.empty())
      {
        ...
        for(const auto &[shipName, errors] : dependentOutfitErrors)
        {
          ...
          {
            errorMessage +=
                to_string(++i) + ". You cannot " + actionName + " this outfit from \"" + shipName + "\", because:\n";
          }
  ...
  case OutfitLocation::Shop:
    {
      ...
      if(licenseCost < 0) return "You cannot buy this outfit, because it requires a license that you don't have.";
  ...
  case OutfitLocation::Ship:
    {
      ...
      if(!canPlace)
      {
        ...
        {
          errors.push_back(
              "Only part of your ship's outfit capacity is usable for weapons. "
              "You cannot install this outfit, because it takes up " +
        ...
        {
          errors.push_back(
              "Only part of your ship's outfit capacity is usable for engines. "
              "You cannot install this outfit, because it takes up " +
        ...
        {
          errors.emplace_back(
              "This weapon is designed to be installed on a turret mount, "
        ...
        {
          errors.emplace_back(
              "This weapon is designed to be installed in a gun port, "
        ...
        if(errors.empty())
        {
          for(const auto &it : selectedOutfit->Attributes())
          {
            ...
            {
              errors.push_back(
                  "You cannot install this outfit, because it requires " +
                  Format::SimplePluralization(outfitRequires, '\'' + string(it.first) + '\'') + ", and this ship has " +
  ...
  case OutfitLocation::Cargo:
    {
      ...
      if(!mass || freeCargo >= mass)
      {
        ...
        return "You cannot load this outfit into cargo, because it takes up " + Format::CargoString(mass, "mass") +
...
ShopPanel::TransactionResult
OutfitterPanel::MoveOutfit(OutfitLocation fromLocation, OutfitLocation toLocation, const string &actionName) const
{
  ...
  if(fromLocation == OutfitLocation::Shop)
  {
    ...
    if(licenseCost)
    {
      ...
      for(const string &licenseName : selectedOutfit->Licenses())
        if(!player.HasLicense(licenseName)) player.AddLicense(licenseName);
    }
    ...
    if(toLocation == OutfitLocation::Ship)
    {
          ...
          --i)
      {
        ...
        if(shipsToOutfit.empty()) break;
        for(Ship *ship : shipsToOutfit)
        {
          if(!(outfitter.Has(selectedOutfit) || player.Stock(selectedOutfit) > 0) || cost > player.Accounts().Credits())
          {
    ...
    {
      if(!outfitter.Has(selectedOutfit)) howManyPer = min(howManyPer, player.Stock(selectedOutfit));
      // Buy up to <modifier> of the selected outfit and place them in fleet cargo.
      double mass = selectedOutfit->Mass();
      if(mass) howManyPer = min(howManyPer, static_cast<int>(player.Cargo().FreePrecise() / mass));
  ...
  else if(fromLocation == OutfitLocation::Ship)
  {
    for(int i = howManyPer; i; --i)
    {
      ...
      for(Ship *ship : shipsToOutfit)
      {
        ...
        ship->AddOutfit(selectedOutfit, -1);
        int required = selectedOutfit->Get("required crew");
        if(required) ship->AddCrew(-required);
        ...
        if(ammo && ship->OutfitCount(ammo))
        {
          ...
          for(const auto &it : ship->Attributes().Attributes())
            if(it.second < 0.) mustUninstall = max<int>(mustUninstall, ceil(it.second / ammo->Get(it.first)));
    ...
    else if(toLocation == OutfitLocation::Ship)
    {
      for(int i = howManyPer; i && hold.Get(selectedOutfit); --i)
      {
        ...
        for(Ship *ship : shipsToOutfit)
        {
          ...
          if(!hold.Remove(selectedOutfit)) break;
...
{
  if(key == 'b') return static_cast<bool>(CanMoveOutfit(OutfitLocation::Shop, OutfitLocation::Ship));
  if(key == 'i')
  {
...
bool OutfitterPanel::ShouldHighlight(const Ship *ship)
{
  ...
  if(!ButtonActive(hoverButton, true)) return false;

  // If we're hovering above a button that can add outfits to a ship then highlight the ship.
  if(hoverButton == 'b' || hoverButton == 'i') return ShipCanAdd(ship, selectedOutfit);
...
{
  if(!ship->OutfitCount(outfit)) return false;
...
void OutfitterPanel::DrawOutfit(const Outfit &outfit, const Point &center, bool isSelected, bool isOwned) const
{
  ...
  if(thumbnail)
  {
    if(thumbnail->IsLoaded()) SpriteShader::Draw(thumbnail, center);
  ...
  font.Draw(
      {name, {OUTFIT_SIZE, Alignment::CENTER, Truncate::MIDDLE}},
      center + offset,
...
void OutfitterPanel::CheckRefill()
{
  ...
  for(const shared_ptr<Ship> &ship : player.Ships())
  {
    ...
    if(ship->GetSystem() != player.GetSystem() || ship->IsDisabled()) continue;
    ...
    for(const Outfit *outfit : toRefill)
    {
      ...
      if(amount > 0)
      {
        ...
        if(available) needed[outfit] += amount;
      }
    }
  }
  ...
  for(auto &it : needed)
  {
    ...
    if(!outfitter.Has(it.first)) it.second = min(it.second, max(0, player.Stock(it.first)));
  ...
  if(!needed.empty() && cost < player.Accounts().Credits())
  {
    ...
    message        += (count == 1) ? "?" : "s?";
    if(cost) message += " It will cost " + Format::CreditString(cost) + ".";
...
const vector<Ship *> OutfitterPanel::GetShipsToOutfit(bool isInstall) const
{
  ...
  {
    if((isInstall && !ShipCanAdd(ship, selectedOutfit)) || (!isInstall && !ShipCanRemove(ship, selectedOutfit)))
      continue;
    ...
    if(count == compareValue) shipsToOutfit.push_back(ship);
  }
...
void OutfitterPanel::DrawButtons()
{
  ...
  const Point creditsPoint(Screen::Right() - SIDEBAR_WIDTH + 10, Screen::Bottom() - ButtonPanelHeight() + 10);
  ...
  const Point cargoPoint(Screen::Right() - SIDEBAR_WIDTH + 10, Screen::Bottom() - ButtonPanelHeight() + 30);
  ...
  if(modifier > 1)
  {
    string mod      = "x " + std::to_string(modifier);
    int    modWidth = font.Width(mod);
    for(int i = -1; i < 2; ++i)
    {
      font.Draw(
          mod,
          Point(buttonCenterX + buttonOffsetX * i, rowBaseY + rowOffsetY * 0) + Point(-.5 * modWidth, 10.),
          dim);
    }
    for(int i = -1; i < 2; ++i)
    {
      font.Draw(
          mod,
          Point(buttonCenterX + buttonOffsetX * i, rowBaseY + rowOffsetY * 1) + Point(-.5 * modWidth, 10.),
          dim);
    }
  }
  ...
  if(creditsBox.Contains(hoverPoint))
  {
    ...
    if(creditsTooltip.ShouldDraw())
    {
      ...
      creditsTooltip.SetText(
          Format::CreditString(player.Accounts().Credits(), false) + '\n' + Format::MassString(player.Cargo().Free()) +
              " free out of " + Format::MassString(player.Cargo().Size()) + " total capacity",
          true);
      creditsTooltip.Draw();
    }
  }
...
ShopPanel::TransactionResult OutfitterPanel::HandleShortcuts(SDL_Keycode key)
{
  ...
  else if(key == 'u')
  {
    ...
    if(!result && MoveOutfit(OutfitLocation::Cargo, OutfitLocation::Storage)) result = true;
  }
