...
FrameTimer::FrameTimer() { next = std::chrono::steady_clock::now(); }
...
FrameTimer::FrameTimer(int fps, int maxLagMsec) :
  step(std::chrono::nanoseconds(1'000'000'000 / fps)), maxLag(std::chrono::milliseconds(maxLagMsec))
{
...
void FrameTimer::Wait()
{
  ...
  if(now < next)
  {
    ...
    if(now + step + maxLag < next) next = now + step;
...
void FrameTimer::SetFrameRate(int fps) { step = std::chrono::nanoseconds(1'000'000'000 / fps); }


// Calculate when the next frame should begin.
void FrameTimer::Step() { next += step; }
