...
DistanceMap::DistanceMap(
    const System    *center,
    WormholeStrategy wormholeStrategy,
    bool             useJumpDrive,
    int              maxSystems,
    int              maxDays) :
  center(center),
...
DistanceMap::DistanceMap(const PlayerInfo &player, const System *center) : player(&player)
{
  ...
  else if(flagship->IsEnteringHyperspace()) this->center = flagship->GetTargetSystem();
  else this->center = flagship->GetSystem();
...
{
  if(player) Init(player->Flagship());
  else Init();
}
...
bool DistanceMap::HasRoute(const System &target) const { return route.contains(&target); }
...
std::vector<const System *> DistanceMap::Plan(const System &target) const
{
  ...
  if(!HasRoute(target)) return plan;
...
{
  if(!center || (ship && ship->IsRestrictedFrom(*center)) || center == destination) return;
  ...
  while(maxSystems && !edgesTodo.empty())
  {
    ...
    if(currentSystem == destination) break;
    ...
    if(player && (!ship || player->Flagship() == ship)) nextEdge.danger += currentSystem->Danger();
    ...
    if(wormholeStrategy != WormholeStrategy::NONE)
    {
      for(const StellarObject &object : currentSystem->Objects())
      {
           ...
           (object.GetPlanet()->IsUnrestricted() || wormholeStrategy == WormholeStrategy::ALL))
        {
          ...
          if(HasBetter(link, nextEdge)) continue;
          ...
          if(ship && (!object.GetPlanet()->IsAccessible(ship) || ship->IsRestrictedFrom(*object.GetPlanet()))) continue;
          if(player && !player->HasVisited(*object.GetPlanet())) continue;
          if(player && !(player->CanView(*currentSystem) && player->CanView(link))) continue;
...
bool DistanceMap::Propagate(const RouteEdge &curEdge)
{
  ...
  for(const System *link : (jumpRangeMax > 0 ? currentSystem->JumpNeighbors(jumpRangeMax) : links))
  {
    ...
    if(!CheckLink(*currentSystem, *link, linked, useJump, fuelCost)) continue;
...
void DistanceMap::Add(const System &to, RouteEdge edge)
{
  ...
  edge.prev = &to;
  if(maxDays < 0 || edge.days < maxDays) edgesTodo.emplace(edge);
}
...
{
  if(!player) return !ship || !ship->IsRestrictedFrom(to);

  // Can never go where you don't know about.
  if(!player->HasSeen(to)) return false;
  ...
  if(player->CanView(from) || player->CanView(to)) return true;
  ...
  if(distance >= jumpRangeMax) return false;
