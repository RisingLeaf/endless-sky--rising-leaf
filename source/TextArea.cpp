...
TextArea::TextArea(const Rectangle &r) : TextArea() { SetRect(r); }
...
void TextArea::SetRect(const Rectangle &r)
{
  ...
  size     = r.Dimensions();
  buffer.reset();
  wrappedText.SetWrapWidth(static_cast<int>(r.Width()));
  scroll.SetDisplaySize(r.Height());
  scrollBar.displaySizeFraction = static_cast<float>(scroll.DisplaySize() / scroll.MaxValue());
  Invalidate();
}
...
{
  if(!buffer) buffer = std::make_unique<RenderBuffer>(size, "text_area_buffer");
  ...
  if(!bufferIsValid || !scroll.IsAnimationDone())
  {
    ...
    buffer->SetFadePadding(scroll.IsScrollAtMin() ? 0 : 20, scroll.IsScrollAtMax() ? 0 : 20);
    bufferIsValid = true;
  }
}
...
void TextArea::Draw()
{
  ...
  {
    constexpr float POINTER_OFFSET   = 5;
    constexpr float SCROLLBAR_OFFSET = 5;
    const Point     topRight(position + Point(buffer->Right() + SCROLLBAR_OFFSET, buffer->Top() + POINTER_OFFSET));
    const Point bottomRight(position + Point(buffer->Right() + SCROLLBAR_OFFSET, buffer->Bottom() - POINTER_OFFSET));
...
bool TextArea::Release(int, int, const MouseButton button)
{
  if(button != MouseButton::LEFT) return false;

  const bool ret = dragging;
  dragging       = false;
  return ret;
}
...
bool TextArea::Scroll(double, const double dy)
{
  if(hovering)
  {
...
void TextArea::Validate(const bool trailingBreak)
{
