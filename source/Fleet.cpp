...
Fleet::Fleet(const DataNode &node) { Load(node); }


void Fleet::Load(const DataNode &node)
{
  if(node.Size() >= 2) fleetName = node.Token(1);
  ...
  for(const DataNode &child : node)
  {
    ...
    {
      if(hasValue) fighterNames = GameData::Phrases().Get(child.Token(1));
      for(const DataNode &grand : child)
      {
        const std::string &fighterKey = grand.Token(0);
        if(fighterKey == "names" && grand.Size() >= 2) fighterNames = GameData::Phrases().Get(grand.Token(1));
        else if(fighterKey == "personality") fighterPersonality.Load(grand);
    ...
    else if(key == "variant")
    {
      ...
      if(!count) child.PrintTrace("Did not find matching variant for specified operation:");
    }
    else {
  ...
  if(variants.empty())
    node.PrintTrace((fleetName.empty() ? "Unnamed fleet" : "Fleet \"" + fleetName + "\"") + " contains no variants:");
}
...
bool Fleet::IsValid(bool requireGovernment) const
{
  ...
  if(requireGovernment && !government) return false;

  if(names && names->IsEmpty()) return false;

  if(fighterNames && fighterNames->IsEmpty()) return false;

  // Any variant a fleet could choose should be valid.
  if(any_of(variants.begin(), variants.end(), [](const Variant &v) noexcept -> bool { return !v.IsValid(); }))
    return false;
...
void Fleet::RemoveInvalidVariants()
{
  ...
  Logger::Log(
      (fleetName.empty() ? "Unnamed fleet" : "Fleet \"" + fleetName + "\"") + ": Removing " + std::to_string(count) +
          " invalid " + (count > 1 ? "variants" : "variant") + " (" + std::to_string(total - variants.TotalWeight()) +
          " of " + std::to_string(total) + " weight).",
      Logger::Level::WARNING);
}


// Get the government of this fleet.
const Government *Fleet::GetGovernment() const { return government; }
...
void Fleet::Enter(const System &system, std::list<std::shared_ptr<Ship>> &ships, const Planet *planet) const
{
  ...
  if(!planet)
  {
    ...
    for(const Ship *ship : variantShips)
    {
      ...
      if(ship->JumpNavigation().HasHyperdrive()) hasHyper = true;
    }
    ...
    if(hasJump || hasHyper)
    {
      ...
      {
        if(!unrestricted && government->IsRestrictedFrom(*neighbor)) continue;
        ...
        if(isWelcomeHere || neighbor->GetGovernment()->IsEnemy(government)) linkVector.push_back(neighbor);
        else linkVector.insert(linkVector.end(), 8, neighbor);
      }
    }
    ...
    if(!options)
    {
      ...
      for(const StellarObject &object : system.Objects())
      {
        if(object.HasValidPlanet() && (unrestricted || !government->IsRestrictedFrom(*object.GetPlanet())) &&
           !object.GetPlanet()->GetGovernment()->IsEnemy(government))
        {
          stellarVector.push_back(&object);
        }
    ...
    if(choice >= linkVector.size())
    {
      ...
      if(!linkVector.empty()) target = linkVector[Random::Int(linkVector.size())];
    }
  ...
  if(planet)
  {
    if(!object)
    {
      ...
      for(const auto &object : system.Objects())
        if(object.GetPlanet() == planet) stellarObjects.push_back(&object);
      ...
      if(stellarObjects.empty())
      {
        // Log this error.
        Logger::Log(
            "Fleet::Enter: Unable to find valid stellar object for planet \"" + planet->TrueName() + "\" in system \"" +
                system.TrueName() + "\".",
            Logger::Level::WARNING);
        return;
      }
    ...
    if(planet->IsUnrestricted() ||
       all_of(
           placed.cbegin(),
           placed.cend(),
           [&](const std::shared_ptr<Ship> &ship) { return ship->GetParent() || planet->IsAccessible(ship.get()); }))
    {
  ...
  for(std::shared_ptr<Ship> &ship : placed)
  {
    ...
    if(target != source) ship->SetTargetSystem(target);

    if(flagship) ship->SetParent(flagship);
    else flagship = ship;
...
void Fleet::Place(const System &system, std::list<std::shared_ptr<Ship>> &ships, bool carried, bool addCargo) const
{
  ...
  for(std::shared_ptr<Ship> &ship : placed)
  {
    ...
    if(carried && PlaceFighter(ship, placed)) continue;

    Angle  angle    = Angle::Random();
    Point  pos      = center.first + Angle::Random().Unit() * OffsetFrom(center);
    double velocity = 0;
    if(!ship->GetPersonality().IsDerelict()) velocity = Random::Real() * ship->MaxVelocity();
    else ship->Disable();
    ...
    if(addCargo) cargo.SetCargo(&*ship);
  }
}
...
const System *Fleet::Enter(const System &system, Ship &ship, const System *source)
{
  bool canEnter =
      (source != nullptr ||
       any_of(
           system.Links().begin(),
           system.Links().end(),
           [&ship](const System *link) noexcept -> bool { return !ship.IsRestrictedFrom(*link); }));
  ...
  if(!source)
  {
    ...
    for(const System *link : system.Links())
      if(!ship.IsRestrictedFrom(*link)) validSystems.emplace_back(link);
...
int64_t Fleet::Strength() const { return variants.Average(std::mem_fn(&Variant::Strength)); }
...
std::pair<Point, double> Fleet::ChooseCenter(const System &system)
{
  ...
  if(centers.empty()) return {Point(), 0.};
...
std::vector<std::shared_ptr<Ship>> Fleet::Instantiate(const std::vector<const Ship *> &ships) const
{
  ...
  for(const Ship *model : ships)
  {
    ...
    {
      Logger::Log(
          "Skipping invalid ship model \"" + model->TrueModelName() + "\" in fleet \"" + fleetName + "\".",
          Logger::Level::WARNING);
      continue;
    }
    ...
    if(phrase) ship->SetGivenName(phrase->Get());
    ship->SetGovernment(government);
    if(canBeCarried && fighterPersonality.IsDefined()) ship->SetPersonality(fighterPersonality);
    else ship->SetPersonality(personality);
...
{
  if(!fighter->CanBeCarried()) return false;

  for(const std::shared_ptr<Ship> &parent : placed)
    if(parent->Carry(fighter)) return true;
