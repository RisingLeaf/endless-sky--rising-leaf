...
void Sale<Item>::LoadSingle(const DataNode &child, const Set<Item> &items, bool preventModifiers)
{
  ...
  if(remove && !hasValue) this->clear();
  else if(remove && hasValue) this->erase(items.Get(child.Token(1)));
  else if(add && hasValue) this->insert(items.Get(child.Token(1)));
  else this->insert(items.Get(token));
}
