...
void Radar::SetCenter(const Point &center) { this->center = center; }
...
void Radar::AddPointer(int type, const Point &position) { pointers.emplace_back(GetColor(type), position.Unit()); }
...
void Radar::Draw(const Point &center, double scale, double radius, double pointerRadius) const
{
  ...
  for(const Object &object : objects)
  {
    ...
    if(length > radius) position *= radius / length;
    position += center;
...
const Color &Radar::GetColor(int type)
{
  ...
  if(static_cast<size_t>(type) >= color.size()) type = INACTIVE;
...
Radar::Line::Line(const Color &color, const Point &base, const Point &vector) : color(color), base(base), vector(vector)
{
}
