    ...
    const std::set<const Planet *> *visitedPlanets)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    else if(key == "ship" && hasValue)
    {
      ...
      if(setName) ships.back()->SetGivenName(child.Token(2));
    }
...
void Person::FinishLoading()
{
  for(const std::shared_ptr<Ship> &ship : ships)
  {
    ...
    if(formationPattern) ship->SetFormationPattern(formationPattern);
  }
}


bool Person::IsLoaded() const { return isLoaded; }


// Prevent this person from being spawned in any system.
void Person::NeverSpawn() { frequency = 0; }
...
int Person::Frequency(const System *system) const
{
  ...
  if(!system || IsDestroyed() || IsPlaced() || system->Links().empty()) return 0;
...
const std::list<std::shared_ptr<Ship>> &Person::Ships() const { return ships; }


const Government *Person::GetGovernment() const { return government; }


const Personality &Person::GetPersonality() const { return personality; }


const Phrase &Person::GetHail() const { return hail; }


bool Person::IsDestroyed() const
{
  if(ships.empty() || !ships.front()) return true;
