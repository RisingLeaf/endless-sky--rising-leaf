...
Minable::Payload::Payload(const DataNode &node)
{
	...
	for(const DataNode &child : node)
	{
		...
		if(!hasValue)
			...
			maxDrops = std::max<int>(1, child.Value(1));
		else if(key == "drop rate")
			dropRate = std::max(0., std::min(child.Value(1), 1.));
		else if(key == "toughness")
			toughness = std::max(1., child.Value(1));
		else
...
const std::string &Minable::TrueName() const
{
	return name;
}
...
const std::string &Minable::DisplayName() const
{
	return displayName;
}
...
const std::string &Minable::Noun() const
{
	return noun;
}
...
const std::vector<Minable::Payload> &Minable::GetPayload() const
{
	return payload;
}
