...
: GraphicsTypes::ShaderInstance(), Info(info), Device(device)
{
  for(const auto &code : shader_code)
  {
    ...
    switch(code.stage)
    {
    case GraphicsTypes::ShaderStage::VULKAN_VERTEX:   shader_module = &VertexShader;   stage_create_info = &VertexShaderStage;   stage = VulkanTranslate::ShaderStage::VERTEX;   break;
    case GraphicsTypes::ShaderStage::VULKAN_FRAGMENT: shader_module = &FragmentShader; stage_create_info = &FragmentShaderStage; stage = VulkanTranslate::ShaderStage::FRAGMENT; break;
    case GraphicsTypes::ShaderStage::VULKAN_COMPUTE:  shader_module = &ComputeShader;  stage_create_info = &ComputeShaderStage;  stage = VulkanTranslate::ShaderStage::COMPUTE;  break;
  ...
  std::vector<VkDescriptorSetLayoutBinding> buffer_bindings = {buffer};
  auto common_buffer_create_info = VulkanBootstrap::GetDescriptorSetLayoutCreate(buffer_bindings);
  vkCreateDescriptorSetLayout(Device->GetDevice(), &common_buffer_create_info, nullptr, &DescriptorSetLayoutUBOCommon);

  auto specific_buffer_create_info = VulkanBootstrap::GetDescriptorSetLayoutCreate(buffer_bindings);
  vkCreateDescriptorSetLayout(Device->GetDevice(), &specific_buffer_create_info, nullptr, &DescriptorSetLayoutUBOSpecial);

  std::vector<VkDescriptorSetLayoutBinding> specific_texture_list_info;
  for(size_t x = 0; x < info.GetSpecificTextureCount(); x++)
  {
    VkDescriptorSetLayoutBinding sampler_layout_binding = VulkanBootstrap::GetDescriptorSetLayoutBinding(x, VulkanTranslate::ShaderStage::ALL, VulkanTranslate::DescriptorType::TEXTURE);
  ...
  vkCreateDescriptorSetLayout(Device->GetDevice(), &specific_texture_create_info, nullptr, &DescriptorSetLayoutTexturesSpecial);
  ...
  VkPipelineLayoutCreateInfo pipeline_layout_info = VulkanBootstrap::GetPipelineLayoutCreate(descriptor_set_layouts, {});
  VulkanHelpers::VK_CHECK_RESULT(vkCreatePipelineLayout(Device->GetDevice(), &pipeline_layout_info, nullptr, &PipelineLayout), __LINE__, __FILE__);
}

VulkanObjects::VulkanShaderInstance::~VulkanShaderInstance()
{
  if(DescriptorSetLayoutUBOCommon)       vkDestroyDescriptorSetLayout(Device->GetDevice(), DescriptorSetLayoutUBOCommon, nullptr);
  if(DescriptorSetLayoutUBOSpecial)      vkDestroyDescriptorSetLayout(Device->GetDevice(), DescriptorSetLayoutUBOSpecial, nullptr);
  if(DescriptorSetLayoutTexturesSpecial) vkDestroyDescriptorSetLayout(Device->GetDevice(), DescriptorSetLayoutTexturesSpecial, nullptr);
  ...
  for(const auto &pipeline : PipelinesWithState) if(pipeline.second) vkDestroyPipeline(Device->GetDevice(), pipeline.second, nullptr);
}
...
VkPipeline VulkanObjects::VulkanShaderInstance::GetPipelineForState(const VulkanPipelineState &state) const
{
  ...
  if(state.Color)
    stages.emplace_back(FragmentShaderStage);
  ...
  VulkanHelpers::VK_CHECK_RESULT(vkCreateGraphicsPipelines(Device->GetDevice(), VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &it.second), __LINE__, __FILE__);
