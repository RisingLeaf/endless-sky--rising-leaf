...
namespace
{
  constexpr std::bitset<static_cast<size_t>(Orders::Types::TYPES_COUNT)> HAS_TARGET_SHIP{
      (1 << static_cast<int>(Orders::Types::KEEP_STATION)) + (1 << static_cast<int>(Orders::Types::GATHER)) +
      (1 << static_cast<int>(Orders::Types::FINISH_OFF))};

  constexpr std::bitset<static_cast<size_t>(Orders::Types::TYPES_COUNT)> HAS_TARGET_ASTEROID{
      (1 << static_cast<int>(Orders::Types::MINE))};

  constexpr std::bitset<static_cast<size_t>(Orders::Types::TYPES_COUNT)> HAS_TARGET_SHIP_OR_ASTEROID{
      (1 << static_cast<int>(Orders::Types::ATTACK))};

  constexpr std::bitset<static_cast<size_t>(Orders::Types::TYPES_COUNT)> HAS_TARGET_LOCATION{
      (1 << static_cast<int>(Orders::Types::MOVE_TO))};

  // Orders not included in the bitset should be removed when the given order is issued.
  constexpr std::array<
      std::bitset<static_cast<size_t>(Orders::Types::TYPES_COUNT)>,
      static_cast<size_t>(Orders::Types::TYPES_COUNT)>
      SIMULTANEOUS{{
          ...
          {},                                                  // ATTACK
          {},                                                  // FINISH_OFF
          {(1 << static_cast<int>(Orders::Types::HOLD_POSITION)) + (1 << static_cast<int>(Orders::Types::HOLD_ACTIVE)) +
           (1 << static_cast<int>(Orders::Types::MOVE_TO)) + (1 << static_cast<int>(Orders::Types::KEEP_STATION)) +
           (1 << static_cast<int>(Orders::Types::GATHER)) +
           (1 << static_cast<int>(Orders::Types::HARVEST))},   // HOLD_FIRE
          {},                                                  // MINE
...
bool OrderSet::Has(Types type) const noexcept { return types[static_cast<size_t>(type)]; }


bool OrderSet::Empty() const noexcept { return types.none(); }
...
void OrderSet::Add(const OrderSingle &newOrder, bool *hasMismatch, bool *alreadyHarvesting)
{
  ...
  if(Has(Types::HOLD_ACTIVE)) Set(Types::HOLD_POSITION);

  std::shared_ptr<Ship> newTargetShip         = newOrder.GetTargetShip();
  ...
  std::shared_ptr<Minable> newTargetAsteroid         = newOrder.GetTargetAsteroid();
  bool                     newTargetAsteroidRelevant = HAS_TARGET_ASTEROID[static_cast<size_t>(newOrder.type)] ||
                                   HAS_TARGET_SHIP_OR_ASTEROID[static_cast<size_t>(newOrder.type)];

  bool individualHasMismatch = !Has(newOrder.type) || (newTargetShipRelevant && GetTargetShip() != newTargetShip) ||
  ...
  if(newTargetShipRelevant) SetTargetShip(newTargetShip);
  if(newTargetAsteroidRelevant) SetTargetAsteroid(newTargetAsteroid);
...
void OrderSet::Validate(const Ship *ship, const System *playerSystem)
{
  ...
  {
    std::shared_ptr<Ship> tShip = GetTargetShip();
    ...
    targetShipInvalid =
        !tShip || (!tShip->IsTargetable() && tShip->GetGovernment() != ship->GetGovernment()) ||
  ...
  if(targetShipInvalid)
  {
    ...
    if(targetAsteroidInvalid) types &= ~HAS_TARGET_SHIP_OR_ASTEROID;
  }
  if(targetAsteroidInvalid) types &= ~HAS_TARGET_ASTEROID;

  // Reset targets that are no longer needed.
  if((types & (HAS_TARGET_SHIP | HAS_TARGET_SHIP_OR_ASTEROID)).none()) targetShip.reset();
  if((types & (HAS_TARGET_ASTEROID | HAS_TARGET_SHIP_OR_ASTEROID)).none()) targetAsteroid.reset();
}
...
void OrderSet::Update(const Ship &ship)
{
  if((Has(Types::MOVE_TO) || Has(Types::HOLD_ACTIVE)) && ship.GetSystem() == targetSystem)
  {
    ...
    if(ship.Position().Distance(targetPoint) < 20. && ship.Velocity().Length() < .001) Set(Types::HOLD_POSITION);
  }
...
void OrderSet::Reset(Types type) noexcept { types.reset(static_cast<size_t>(type)); }
