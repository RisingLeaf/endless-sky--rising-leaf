...
namespace asl
{
  template <typename Type, class allocator = list_allocator_base<Type>>
    requires concepts::inheritance::is_base_of<list_allocator_base<Type>, allocator>
  class list
  {
  public:
    ...
    public:
      ...
      Type *operator->() const
      {
        assert(index < base->alloc.used);
        return &base->alloc.memory[index];
      }
      operator Type *() const
      {
        assert(index < base->alloc.used);
  ...
  public:
    ...
    explicit list(const asl::uint32 size) { alloc.resize(size); }
    ...
    {
      if(size > alloc.get_space()) alloc.resize(size);
    }
    ...
    {
      if(alloc.used == alloc.get_space()) alloc.increase();
      alloc.memory[alloc.used] = t;
      ++alloc.used;
    }

    std::enable_if_t<std::is_default_constructible_v<Type>> emplace_back()
    {
      if(alloc.used == alloc.get_space()) alloc.increase();
