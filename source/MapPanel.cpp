...
namespace
{
  ...
  {
    maximumActive = std::max(
        MapPanel::MAX_MISSION_POINTERS_DRAWN / 2,
  ...
  struct WormholeArrow
  {
    ...
    WormholeArrow(const System *from, const System *to, const Color *color) : from(from), to(to), color(color) {}
  ...
  void TallyEscorts(
      const std::vector<std::shared_ptr<Ship>>              &escorts,
      std::map<const System *, MapPanel::SystemTooltipData> &locations)
  {
    for(const auto &ship : escorts)
    {
      ...
      if(ship->GetSystem())
        if(!ship->IsParked()) ++locations[ship->GetSystem()].activeShips;
        else ++locations[ship->GetSystem()].parkedShips;
      ...
      std::map<const System *, MapPanel::SystemTooltipData> &locations)
  {
    for(const auto &hold : outfits)
    {
      ...
      if(!system || !planet->HasOutfitter()) continue;

      for(const auto &outfit : hold.second.Outfits())
      {
        // Only count a system if it actually stores outfits.
        if(outfit.second) locations[system].outfits[planet] += outfit.second;
      }
    }
  }
  ...
  bool HasMultipleLandablePlanets(const System &system)
  {
    ...
    for(auto &&stellarObject : system.Objects())
    {
      if(stellarObject.HasValidPlanet() && stellarObject.HasSprite() && !stellarObject.GetPlanet()->IsWormhole())
      {
        ...
        if(!firstPlanet) firstPlanet = stellarObject.GetPlanet();
        else if(firstPlanet != stellarObject.GetPlanet()) return true;
      }
    }
  ...
  double DangerFleetTotal(const PlayerInfo &player, const System &system, const bool withRaids)
  {
    ...
    if(withRaids)
      for(const auto &raidFleet : system.GetGovernment()->RaidFleets())
        danger += 10. * player.RaidFleetAttraction(raidFleet, &system) * raidFleet.GetFleet()->Strength();
    return danger;
  }
...
{
  if(++systemCount > MAX_MISSION_POINTERS_DRAWN) return;
  Angle angle = Angle(MISSION_POINTERS_ANGLE_DELTA * systemCount);
  if(drawBack) PointerShader::Draw(position, angle.Unit(), 14.f + bigger, 19.f + 2 * bigger, -4.f, black);
  ...
  fromMission(fromMission)
{
  ...
  {
    escortSystems.clear();
    if(Preferences::Has(SHOW_ESCORT_SYSTEMS)) TallyEscorts(player.Ships(), escortSystems);
    if(Preferences::Has(SHOW_STORED_OUTFITS)) TallyOutfits(player.PlanetaryStorage(), escortSystems);
  }
  ...
  if(systemRange || playerRange) playerJumpDistance = systemRange ? systemRange : playerRange;
  ...
  if(player.GetPlanet()) player.CacheMissionInformation(true);
...
void MapPanel::Step()
{
  ...
  if(needsRecenter) center = mouse / Zoom() - anchor;
}
...
void MapPanel::Draw()
{
  ...
  if(Preferences::Has("Hide unexplored map regions")) FogShader::Draw(center, Zoom(), player);
  ...
  RingShader::Draw(
      Zoom() * (playerSystem.Position() + center),
      System::DEFAULT_NEIGHBOR_DISTANCE * Zoom(),
      2.0f,
      1.0f,
      viewRangeColor);
  ...
  RingShader::Draw(Zoom() * (selectedSystem->Position() + center), 11.f, 9.f, brightColor);

  // Advance a "blink" timer.
  ++step;
  // Update the tooltip timer.
  if(hoverSystem) tooltip.IncrementCount();
  else tooltip.DecrementCount();
...
void MapPanel::FinishDrawing(const std::string &buttonCondition)
{
  ...
  if(player.MapZoom() >= static_cast<int>(mapInterface->GetValue("max zoom"))) info.SetCondition("max zoom");
  if(player.MapZoom() <= static_cast<int>(mapInterface->GetValue("min zoom"))) info.SetCondition("min zoom");
  const Interface *mapButtonUi =
      GameData::Interfaces().Get(Screen::Width() < 1300 ? "map buttons (small screen)" : "map buttons");
  ...
  if(hoverSystem && tooltip.ShouldDraw())
  {
    ...
    if(!tooltip.HasText())
    {
      ...
      {
        if(player.Flagship()) --t.activeShips;
        if(t.activeShips || t.parkedShips || !t.outfits.empty()) text = "You are here, with:\n";
        else text = "You are here.";
      }
      ...
      else if(t.activeShips) text += std::to_string(t.activeShips) + (t.activeShips == 1 ? " escort" : " escorts");
      if(t.parkedShips)
        text += std::to_string(t.parkedShips) + (t.parkedShips == 1 ? " parked escort" : " parked escorts");
      if(!t.outfits.empty())
      {
        if(t.activeShips || t.parkedShips) text += "\n";
  ...
  if(selectedSystem != &playerSystem && !distance.HasRoute(*selectedSystem))
  {
    ...
    if(!flagship) info.SetString("route error", NO_SHIP);
    else if(!flagship->JumpNavigation().HasAnyDrive()) info.SetString("route error", NO_DRIVE);
    else if(player.CanView(*selectedSystem)) info.SetString("route error", UNAVAILABLE);
    else info.SetString("route error", UNKNOWN);
  }
...
bool MapPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  if(command.Has(Command::MAP) || key == 'd' || key == SDLK_ESCAPE ||
     (key == 'w' && (mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI))))
  {
    GetUI().Pop(this);
  }
...
{
  if(escortSystems.empty()) return true;
  ...
  if(hoverSystem)
  {
    ...
    if(pos.Distance(hoverSystem->Position()) <= maxDistance) return true;
  ...
  for(const auto &squad : escortSystems)
  {
    ...
    if(pos.Distance(system->Position()) < maxDistance && (player.HasSeen(*system) || system == specialSystem))
    {
...
{
  if(dy > 0.) IncrementZoom();
  else if(dy < 0.) DecrementZoom();
...
{
  if(std::isnan(value)) return UninhabitedColor();

  value = std::min(1., std::max(-1., value));
  if(value < 0.) return Color(.12 + .12 * value, .48 + .36 * value, .48 - .12 * value, .4);
  else return Color(.12 + .48 * value, .48, .48 - .48 * value, .4);
}
...
Color MapPanel::ReputationColor(double reputation, bool canLand, bool hasDominated)
{
  ...
  if(canLand) reputation = std::max(reputation, 0.);
...
{
  if(!government) return UninhabitedColor();
...
{
  if(std::isnan(danger)) return *GameData::Colors().Get("map danger none");
  else if(danger > .5) return Color(.6, .4 * (2. - 2. * std::min(1., danger)), 0., .4);
  else return MapColor(2. * danger - 1.);
}


Color MapPanel::UninhabitedColor() { return GovernmentColor(GameData::Governments().Get("Uninhabited")); }


Color MapPanel::UnexploredColor() { return *GameData::Colors().Get("map system ring unexplored"); }
...
void MapPanel::Select(const System *system)
{
  ...
  if(!flagship || (!plan.empty() && system == plan.front())) return;
  ...
  bool shift = (mod & SDL_KMOD_SHIFT) && !plan.empty();
  ...
  {
    plan.clear();
    if(!isJumping) flagship->SetTargetSystem(nullptr);
    else plan.push_back(source);
  }
  else if(shift)
  {
    const System *planEnd = plan.front();
    if(system == planEnd) return;

    RoutePlan addedRoute(*planEnd, *system, &player);
    if(!addedRoute.HasRoute()) return;
  ...
  else if(distance.HasRoute(*system))
  {
    ...
    if(isJumping) plan.push_back(source);
  }

  // Reset the travel destination if the final system in the travel plan has changed.
  if(!plan.empty()) player.SetTravelDestination(nullptr);
}
...
// been satisfied.
bool MapPanel::IsSatisfied(const Mission &mission) const { return IsSatisfied(player, mission); }
  ...
  if(!isHyper || wormholeColor)
  {
    for(const StellarObject &object : previous->Objects())
    {
      if(object.HasSprite() && object.HasValidPlanet() && object.GetPlanet()->IsWormhole() &&
         player.HasVisited(*object.GetPlanet()) && player.CanView(*previous) && player.CanView(*next) &&
         &object.GetPlanet()->GetWormhole()->WormholeDestination(*previous) == next)
      {
        ...
        {
          isMappable = true;
          if(wormholeColor) *wormholeColor = *object.GetPlanet()->GetWormhole()->GetLinkColor();
          break;
        }
      }
    }
...
void MapPanel::UpdateCache()
{
  ...
  if(commodity == SHOW_DANGER)
  {
    ...
    for(const auto &it : GameData::Systems())
    {
      ...
      if(!system.IsValid() || system.Inaccessible() || !player.HasVisited(system)) continue;

      const double danger = DangerFleetTotal(player, system, false);
      if(danger > 0.)
      {
        if(dangerMax < danger) dangerMax = danger;
        if(dangerMin > danger) dangerMin = danger;
      }
    }
    if(dangerMax) dangerScale = 1. / log(dangerMin / dangerMax);
  }
  ...
  for(const auto &it : GameData::Systems())
  {
    ...
    if(!system.IsValid() || system.Inaccessible()) continue;
    // Ignore systems the player has never seen, unless they have a pending mission that lets them see it.
    if(!player.HasSeen(system) && &system != specialSystem) continue;
    ...
    if(!player.CanView(system))
    {
      color = UnexploredColor();
        ...
        system.IsInhabited(player.Flagship()) || commodity == SHOW_SPECIAL || commodity == SHOW_VISITED ||
        commodity == SHOW_DANGER)
    {
      if(commodity >= SHOW_SPECIAL)
      {
        ...
        if(commodity >= 0)
        {
          ...
          if(!price) value = std::numeric_limits<double>::quiet_NaN();
          else value = (2. * (price - com.low)) / (com.high - com.low) - 1.;
        }
        else if(commodity == SHOW_SHIPYARD)
        {
          double size = 0;
          for(const StellarObject &object : system.Objects())
            if(object.HasSprite() && object.HasValidPlanet()) size += object.GetPlanet()->ShipyardStock().size();
        ...
        else if(commodity == SHOW_OUTFITTER)
        {
          double size = 0;
          for(const StellarObject &object : system.Objects())
            if(object.HasSprite() && object.HasValidPlanet()) size += object.GetPlanet()->OutfitterStock().size();
        ...
        if(colorSystem) color = MapColor(value);
      }
      ...
      else if(commodity == SHOW_DANGER)
      {
        ...
        if(danger > 0.) color = DangerColor(1. - dangerScale * log(danger / dangerMax));
        else color = DangerColor(std::numeric_limits<double>::quiet_NaN());
      }
      else {
        ...
        for(const StellarObject &object : system.Objects())
        {
          if(object.HasSprite() && object.HasValidPlanet())
          {
            ...
            if(planet->IsWormhole() || !planet->IsAccessible(player.Flagship())) continue;
            canLand      |= planet->CanLand() && hasServices;
            isInhabited  |= planet->IsInhabited();
            hasDominated &= (!planet->IsInhabited() || GameData::GetPolitics().HasDominated(planet));
          }
        }
        ...
        if(hasSpaceport || hasDominated) color = ReputationColor(reputation, canLand, hasDominated);
      }
    }
  ...
  {
    const System *system = &it.second;
    if(!system->IsValid() || !player.HasSeen(*system)) continue;
    ...
    for(const System *link : system->Links())
    {
      if(link < system || !player.HasSeen(*link))
      {
        ...
        if((!player.CanView(*system) && !player.CanView(*link)) || !link->IsValid()) continue;
...
void MapPanel::DrawTravelPlan()
{
  ...
  for(int i = player.TravelPlan().size() - 1; i >= 0; --i, previous = next)
  {
    ...
    Color wormholeColor;
    if(!GetTravelInfo(previous, next, jumpRange, isJump, isWormhole, isMappable, &wormholeColor)) break;
    if(isWormhole && !isMappable) continue;
    ...
    if(isWormhole) drawColor = wormholeColor;
    else if(!stranded) drawColor = withinFleetFuelRangeColor;
    else if(fuel[flagship] >= 0.) drawColor = defaultColor;
    ...
    if(isJump) LineShader::DrawDashed(from, to, unit, 1.6f, drawColor, 11., 4.);
    else LineShader::Draw(from, to, 1.6f, drawColor);
  }
}
...
void MapPanel::DrawSelectedSystem()
{
  ...
  std::string text;
  if(!player.KnowsName(*selectedSystem)) text = "Selected system: unexplored system";
  else text = "Selected system: " + selectedSystem->DisplayName();

  int                                jumps = 0;
  const std::vector<const System *> &plan  = player.TravelPlan();
  auto                               it    = find(plan.begin(), plan.end(), selectedSystem);
  if(it != plan.end()) jumps = plan.end() - it;
  else if(distance.HasRoute(*selectedSystem)) jumps = distance.Days(*selectedSystem);

  if(jumps == 1) text += " (1 jump away)";
  else if(jumps > 0) text += " (" + std::to_string(jumps) + " jumps away)";
...
void MapPanel::DrawWormholes()
{
  ...
  for(auto &&it : GameData::Wormholes())
  {
    ...
    if(!p.IsValid() || !player.HasVisited(p) || !it.second.IsMappable()) continue;
  ...
  for(const WormholeArrow &link : arrowsToDraw)
  {
    ...
    if(link.from < link.to ||
       !count_if(
           arrowsToDraw.begin(),
           arrowsToDraw.end(),
           [&link](const WormholeArrow &cmp) { return cmp.from == link.to && cmp.to == link.from; }))
    {
...
{
  if(commodity != cachedCommodity) UpdateCache();
  ...
  if(commodity == SHOW_GOVERNMENT) closeGovernments.clear();
  ...
  for(const Node &node : nodes)
  {
    ...
    if(commodity != SHOW_STARS)
    {
      ...
      for(int i = 0; i < starsToDraw; ++i)
      {
        ...
        const Body    starBody(
            star,
            pos + zoom * starOffset * starAngle.Unit(),
            Point(0, 0),
            0,
            cbrt(zoom) * 0.6,
            Point(1., 1.),
    ...
    if(commodity == SHOW_GOVERNMENT && node.government && node.government->DisplayName() != "Uninhabited")
    {
      ...
      if(it == closeGovernments.end()) closeGovernments[node.government] = distance;
...
void MapPanel::DrawMissions()
{
  ...
  for(const Mission &mission : player.AvailableJobs())
  {
    ...
    if(mission.CanAccept(player)) ++it.available;
    else ++it.unavailable;
  }
