...
void MiniMap::Step(const std::shared_ptr<Ship> &flagship)
{
  ...
  if(current != flagshipSystem) retargetCenter = true;
  ...
  {
    --displayMinimap;
    if(displayMinimap < 30 && fadeMinimap) --fadeMinimap;
    else if(fadeMinimap < 30) ++fadeMinimap;
  }
  else {
    fadeMinimap = 0;
  }
  ...
  if(retargetCenter)
  {
    ...
    if(target) targetCenter = .5 * (current->Position() + target->Position());
    else targetCenter = current->Position();
  }
...
void MiniMap::Draw(int step) const
{
  ...
  if(pref == Preferences::MinimapDisplay::OFF) return;

  float alpha = .5f;
  if(pref == Preferences::MinimapDisplay::WHEN_JUMPING)
  {
    if(!displayMinimap) return;
  ...
  auto drawSystemLinks = [&](const System &system) -> void
  {
    ...
    if(player.CanView(system) && system.IsInhabited(flagship) && gov) color = gov->GetColor().Additive(alpha);
    RingShader::Draw(from, MapPanel::OUTER, MapPanel::INNER, color);

    // Add a circle around the system that the player is currently in.
    if(&system == current) RingShader::Draw(from, 11.f, 9.f, brightColor);

    for(const System *link : system.Links())
    {
      // Only draw systems known to be attached to the jump systems.
      if(!player.CanView(system) && !player.CanView(*link)) continue;
      ...
      if(drawnSystems.contains(link)) continue;
      drawnSystems.insert(link);

      gov         = link->GetGovernment();
      Color color = Color(.5f * alpha, 0.f);
      if(player.CanView(*link) && link->IsInhabited(flagship) && gov) color = gov->GetColor().Additive(alpha);
    ...
    {
      if(missionCounter >= MapPanel::MAX_MISSION_POINTERS_DRAWN) break;
      ...
      {
        if(missionCounter >= MapPanel::MAX_MISSION_POINTERS_DRAWN) break;
        if(waypoint == &system) MapPanel::DrawPointer(from, missionCounter, waypointColor, false);
      }
      for(const Planet *stopover : mission.Stopovers())
      {
        if(missionCounter >= MapPanel::MAX_MISSION_POINTERS_DRAWN) break;
        if(stopover->IsInSystem(&system)) MapPanel::DrawPointer(from, missionCounter, waypointColor, false);
      }
      for(const System *mark : mission.MarkedSystems())
      {
        if(missionCounter >= MapPanel::MAX_MISSION_POINTERS_DRAWN) break;
        if(mark == &system) MapPanel::DrawPointer(from, missionCounter, waypointColor, false);
      }
  ...
  if(target) drawnSystems.insert(target);
  ...
  if(!current->Links().contains(target)) LineShader::DrawDashed(from, to, unit, MapPanel::LINK_WIDTH, bright, 11., 4.);
