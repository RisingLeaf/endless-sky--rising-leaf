...
namespace
{
  ...
  bool Involves(const Mission &mission, const System *system)
  {
    ...
    for(const System *waypoint : mission.Waypoints())
      if(waypoint == system) return true;

    for(const Planet *stopover : mission.Stopovers())
      if(stopover->IsInSystem(system)) return true;

    for(const System *mark : mission.MarkedSystems())
      if(mark == system) return true;
  ...
  size_t MaxDisplayedMissions(bool onRight)
  {
    return static_cast<unsigned>(
        std::max(0, static_cast<int>(floor((Screen::Height() - (onRight ? 160. : 190.)) / 20.))));
  }

  // Compute the required scroll amount for the given list of jobs/missions.
  void ScrollMissionList(
      const std::list<Mission>                 &missionList,
      const std::list<Mission>::const_iterator &it,
      double                                   &sideScroll,
      bool                                      checkVisibility)
  {
    ...
    if(missionCount < maxViewable)
    {
      ...
      const auto countBefore = static_cast<size_t>(
          checkVisibility ? count_if(missionList.begin(), it, [](const Mission &m) { return m.IsVisible(); })
      ...
      GameData::Colors().Get("medium"))
{
  ...
  if(!FindMissionForSystem(selectedSystem) && player.HasTravelPlan())
  {
    ...
    for(auto it = tp.crbegin(); it != tp.crend(); ++it)
      if(FindMissionForSystem(*it)) break;
  }
      ...
      GameData::Colors().Get("medium"))
{
  ...
  if(!FindMissionForSystem(selectedSystem) &&
     !(player.GetSystem() != selectedSystem && FindMissionForSystem(player.GetSystem())) && player.HasTravelPlan())
  {
    const auto &tp = player.TravelPlan();
    for(auto it = tp.crbegin(); it != tp.crend(); ++it)
      if(FindMissionForSystem(*it)) break;
...
void MissionPanel::Step()
{
  ...
  if(player.ShouldLaunch()) GetUI().Pop(this);
...
void MissionPanel::Draw()
{
  ...
  if(hoverSort >= 0) tooltip.IncrementCount();
  ...
  for(auto it = plan.rbegin(); it != plan.rend(); ++it)
  {
    ...
    if(!GetTravelInfo(previous, next, jumpRange, isJump, isWormhole, isMappable, nullptr)) break;
    ...
    if(isJump) LineShader::DrawDashed(from, to, unit, 3.f, routeColor, 11., 4.);
    else LineShader::Draw(from, to, 3.f, routeColor);
  ...
  {
    pos =
        DrawPanel(Screen::TopLeft() + Point(0., -availableScroll), "Missions available here:", available.size(), true);
    DrawList(available, pos, availableIt, true);
  }

  pos =
      DrawPanel(Screen::TopRight() + Point(-SIDE_WIDTH, -acceptedScroll), "Your current missions:", AcceptedVisible());
...
bool MissionPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  {
    Accept(mod & SDL_KMOD_CTRL);
    return true;
  }
  else if(key == 'A' || (key == 'a' && (mod & SDL_KMOD_SHIFT)))
  {
  ...
  else if(key == SDLK_UP)
  {
    ...
    if(availableIt != available.end())
    {
      ...
      if(availableIt == available.begin()) availableIt = available.end();
      --availableIt;
    }
    else if(acceptedIt != accepted.end())
    {
      ...
      do
      {
        if(acceptedIt == accepted.begin()) acceptedIt = accepted.end();
        --acceptedIt;
  ...
  else if(key == SDLK_DOWN)
  {
    ...
    {
      ++availableIt;
      if(availableIt == available.end()) availableIt = available.begin();
    }
    else if(acceptedIt != accepted.end())
    {
      do
      {
        ++acceptedIt;
        if(acceptedIt == accepted.end()) acceptedIt = accepted.begin();
      } while(!acceptedIt->IsVisible());
    }
  }
  else {
...
{
  if(button != MouseButton::LEFT) return MapPanel::Click(x, y, button, clicks);

  dragSide = 0;

  if(x > Screen::Right() - 80 && y > Screen::Bottom() - 50) return DoKey('p');
  ...
  if(system)
  {
    ...
    {
      if(!available.empty()) availableIt = available.begin();
      else acceptedIt = accepted.begin();
    }
       ...
       (acceptedIt != accepted.end() && !Involves(*acceptedIt, system)))
    {
      auto firstExistingIt =
          find_if(available.begin(), available.end(), [&system](const Mission &m) { return Involves(m, system); });
      ...
      while(options--)
      {
        if(availableIt != available.end())
        {
          ...
          if(availableIt == available.end())
          {
            if(!accepted.empty()) acceptedIt = accepted.begin();
            else availableIt = available.begin();
          }
        }
        else if(acceptedIt != accepted.end())
        {
        ...
        if(acceptedIt != accepted.end() && !acceptedIt->IsVisible()) continue;

        if(availableIt != available.end() && Involves(*availableIt, system)) break;
        if(acceptedIt != accepted.end() && Involves(*acceptedIt, system)) break;
      }
    }
    // Make sure invisible missions are never selected, even if there were
    // no other missions in this system.
    if(acceptedIt != accepted.end() && !acceptedIt->IsVisible()) acceptedIt = accepted.end();
    // Scroll the relevant panel so that the mission highlighted is visible.
    if(availableIt != available.end()) ScrollMissionList(available, availableIt, availableScroll, false);
    else if(acceptedIt != accepted.end()) ScrollMissionList(accepted, acceptedIt, acceptedScroll, true);
  }
...
{
  if(dragSide < 0)
    availableScroll = std::max(0., std::min(available.size() * 20. + 190. - Screen::Height(), availableScroll - dy));
  else if(dragSide > 0)
    acceptedScroll = std::max(0., std::min(accepted.size() * 20. + 160. - Screen::Height(), acceptedScroll - dy));
  else if(canDrag) MapPanel::Drag(dx, dy);
  ...
  if(x < Screen::Left() + SIDE_WIDTH)
  {
    if(index < available.size())
    {
         ...
         x >= Screen::Left() + SIDE_WIDTH - 110)
      {
        ...
        if(hoverSort > 3) hoverSort = -1;
      }
    }
  }
  else if(x >= Screen::Right() - SIDE_WIDTH)
  {
    if(static_cast<int>(index) < AcceptedVisible()) dragSide = 1;
  }

  if(oldSort != hoverSort) tooltip.Clear();
...
{
  if(dragSide) return Drag(0., dy * Preferences::ScrollSpeed());
...
void MissionPanel::ResizeTextArea() const { description->SetRect(missionInterface->GetBox("description")); }
...
{
  static const std::string CONDITIONS[] = {"available", "unavailable", "active", "blocked", "waypoint"};
  int                 selected     = -1;
  if(availableIt != available.end()) selected = 0 + !CanAccept();
  if(acceptedIt != accepted.end() && acceptedIt->Destination()) selected = 2 + !IsSatisfied(*acceptedIt);

  Information info;
  if(selected >= 0) info.SetCondition(CONDITIONS[selected]);
...
Point MissionPanel::DrawPanel(Point pos, const std::string &label, int entries, bool sorter) const
{
  ...
  const Sprite *sortIcon[4] = {
      SpriteSet::Get("ui/sort abc"),
      SpriteSet::Get("ui/sort pay"),
      SpriteSet::Get("ui/sort speed"),
      SpriteSet::Get("ui/sort convenient")};
...
Point MissionPanel::DrawList(
    const std::list<Mission>                 &missionList,
    Point                                     pos,
    const std::list<Mission>::const_iterator &selectIt,
    bool                                      separateDeadlineOrPossible) const
{
  ...
  for(auto it = missionList.begin(); it != missionList.end(); ++it)
  {
    ...
    if(isSelected) FillShader::Fill(pos + Point(.5 * SIDE_WIDTH - 5., 8.), Point(SIDE_WIDTH - 10., 20.), highlight);

    if(it->Deadline()) SpriteShader::Draw(fast, pos + Point(-4., 8.));

    const Color *color     = nullptr;
    bool         canAccept = (&missionList == &available ? it->CanAccept(player) : IsSatisfied(*it));
    if(!canAccept)
      if(it->Unavailable().IsLoaded()) color = &it->Unavailable();
      else color = &dim;
    else if(isSelected)
      if(it->Selected().IsLoaded()) color = &it->Selected();
      else color = &selected;
    else if(it->Unselected().IsLoaded()) color = &it->Unselected();
...
void MissionPanel::DrawMissionInfo()
{
  ...
  if(CanAccept()) info.SetCondition("can accept");
  else if(acceptedIt != accepted.end()) info.SetCondition("can abort");

  if(availableIt != available.end() || acceptedIt != accepted.end()) info.SetCondition("has description");
...
{
  if(hoverSort < 0 || !tooltip.ShouldDraw()) return;
  ...
  if(!tooltip.HasText())
  {
    ...
    else if(hoverSort == 2)
    {
      ...
      {
      case 0: text = "Sort alphabetically"; break;
      case 1: text = "Sort by payment"; break;
      case 2: text = "Sort by distance"; break;
      case 3:
...
{
  if(availableIt == available.end()) return false;
...
void MissionPanel::Accept(bool force)
{
  ...
  int            cargoToSell = 0;
  if(toAccept.CargoSize()) cargoToSell = toAccept.CargoSize() - player.Cargo().Free();
  int crewToFire = 0;
  if(toAccept.Passengers()) crewToFire = toAccept.Passengers() - player.Cargo().BunksFree();
  ...
  if(availableIt == available.end()) --availableIt;
...
void MissionPanel::MakeSpaceAndAccept()
{
  ...
  if(crewToFire > 0) player.Flagship()->AddCrew(-crewToFire);

  for(const auto &it : player.Cargo().Commodities())
  {
    if(cargoToSell <= 0) break;
...
void MissionPanel::AbortMission()
{
  if(acceptedIt != accepted.end())
  {
    ...
    if(acceptedIt == accepted.end() && !accepted.empty()) --acceptedIt;
    if(acceptedIt != accepted.end() && !acceptedIt->IsVisible()) acceptedIt = accepted.end();
  }
}
...
bool MissionPanel::FindMissionForSystem(const System *system)
{
  ...
  for(availableIt = available.begin(); availableIt != available.end(); ++availableIt)
    if(availableIt->Destination()->IsInSystem(system)) return true;
  for(acceptedIt = accepted.begin(); acceptedIt != accepted.end(); ++acceptedIt)
    if(acceptedIt->IsVisible() && acceptedIt->Destination()->IsInSystem(system)) return true;
