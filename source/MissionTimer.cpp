...
namespace
{
  std::string TriggerToText(MissionTimer::TimerTrigger trigger)
  {
    switch(trigger)
    {
    case MissionTimer::TimerTrigger::TIMEUP:       return "on timeup";
    case MissionTimer::TimerTrigger::DEACTIVATION: return "on deactivation";
    ...
    const std::set<const Planet *> *visitedPlanets)
{
  ...
  if(node.Size() > 2) randomWaitTime = std::max<int>(1, node.Value(2));
  ...
  for(const DataNode &child : node)
  {
    ...
    else if(key == "activation requirements" && child.HasChildren())
    {
      ...
      for(const DataNode &grand : child)
      {
        ...
        {
          if(hasValue) system = GameData::Systems().Get(grand.Token(1));
          else if(grand.HasChildren()) systems.Load(grand, visitedSystems, visitedPlanets);
    ...
    else if(key == "triggered actions")
    {
      for(const DataNode &grand : child)
      {
        ...
        if(trigger == "deactivation") triggeredActions.insert(TimerTrigger::DEACTIVATION);
        else grand.PrintTrace("Skipping unrecognized attribute:");
      }
    }
    else {
...
void MissionTimer::Save(DataWriter &out) const
{
  ...
  out.BeginChild();
  {
    ...
    if(optional) out.Write("optional");
    if(pauses) out.Write("pause when inactive");
    if(hasRequirements)
    {
      out.Write("activation requirements");
      out.BeginChild();
      {
        if(requireIdle) out.Write("idle", sqrt(idleMaxSpeed));
        if(requirePeaceful) out.Write("peaceful");
        if(requireCloaked) out.Write("cloaked");
        else if(requireUncloaked) out.Write("uncloaked");
        if(requireSolo) out.Write("solo");
        if(system)
        {
    ...
    if(!triggeredActions.empty())
    {
      ...
      out.BeginChild();
      {
        for(const TimerTrigger &trigger : triggeredActions)
          if(trigger == TimerTrigger::DEACTIVATION) out.Write("deactivation");
      }
      out.EndChild();
    }
...
MissionTimer
MissionTimer::Instantiate(std::map<std::string, std::string> &subs, const System *origin, int jumps, int64_t payload)
    const
{
  MissionTimer result;
  ...
  if(randomWaitTime) result.waitTime += Random::Int(randomWaitTime);
  ...
  {
    Logger::Log(
        "Instantiation Error: Timer action \"" + TriggerToText(ait->first) + "\" uses invalid " + std::move(reason),
...
bool MissionTimer::IsOptional() const { return optional; }


bool MissionTimer::IsComplete() const { return isComplete; }
...
void MissionTimer::Step(PlayerInfo &player, UI &ui, const Mission &mission)
{
  ...
  if(flagship->Zoom() != 1. || flagship->IsHyperspacing()) return;
  ...
  if(++timeElapsed >= waitTime)
  {
    ...
    if(it != actions.end()) it->second.Do(player, &ui, &mission);
    isComplete = true;
  }
}
...
bool MissionTimer::CanActivate(const Ship *flagship, const PlayerInfo &player) const
{
  ...
  if((system && flagshipSystem != system) || (!systems.IsEmpty() && !systems.Matches(flagshipSystem))) return false;
  ...
  if(requireIdle)
  {
    ...
    if(flagship->IsThrusting() || flagship->IsSteering() || flagship->IsReversing()) return false;
    // And their ship's velocity must be below the max speed threshold.
    if(flagship->Velocity().LengthSquared() > idleMaxSpeed) return false;
  }
  ...
  if(requirePeaceful)
  {
    ...
    for(const Hardpoint &hardpoint : flagship->Weapons())
      if(!hardpoint.IsSpecial() && hardpoint.WasFiring()) return false;
  }

  // Does this timer require that the player is cloaked or uncloaked?
  double cloaking = flagship->Cloaking();
  if((requireUncloaked && cloaking) || (requireCloaked && cloaking != 1.)) return false;
