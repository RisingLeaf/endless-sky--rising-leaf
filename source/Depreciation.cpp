...
namespace
{
  ...
  double Min() { return GameData::GetGamerules().DepreciationMin(); }

  int GracePeriod() { return GameData::GetGamerules().DepreciationGracePeriod(); }

  double Daily() { return GameData::GetGamerules().DepreciationDaily(); }

  int MaxAge() { return GameData::GetGamerules().DepreciationMaxAge() + GracePeriod(); }
...
double Depreciation::Full() { return Min(); }
...
void Depreciation::Load(const DataNode &node)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    if(!(isShip || isOutfit) || child.Size() < 2) continue;

    // Figure out which record we're modifying.
    std::map<int, int> &entry =
        isShip ? ships[GameData::Ships().Get(child.Token(1))] : outfits[GameData::Outfits().Get(child.Token(1))];

    // Load any depreciation records for this item.
    for(const DataNode &grand : child)
      if(grand.Size() >= 2) entry[grand.Value(0)] += grand.Value(1);
  }
}
        ...
        [=, this, &out](const ShipElement &sit)
        {
          ...
          out.BeginChild();
          {
            ...
            for(const auto &it : sit.second)
              if(isStock || (it.second && it.first > day - MaxAge())) out.Write(it.first, it.second);
          }
          out.EndChild();
        });
...
bool Depreciation::IsLoaded() const { return isLoaded; }
...
void Depreciation::Buy(const Ship &ship, int day, Depreciation *source, bool chassisOnly)
{
  ...
  if(source)
  {
    ...
    if(it != source->ships.end() && !it->second.empty())
    {
      ...
      if(it->second.empty()) source->ships.erase(it);
    }
    else if(isStock)
    {
...
void Depreciation::Buy(const Outfit *outfit, int day, Depreciation *source)
{
  ...
  if(source)
  {
    ...
    {
      day = source->Sell(it->second);
      if(it->second.empty()) source->outfits.erase(it);
    }
    else if(isStock)
    {
      // If we're a planet buying from the player, and the player has no
...
int64_t Depreciation::Value(const Ship *ship, int day, int count) const
{
  ...
  if(recordIt == ships.end() || recordIt->second.empty()) return DefaultDepreciation() * count * ship->ChassisCost();
...
{
  if(outfit->Get("installable") < 0.) return count * outfit->Cost();

  // Check whether a record exists for this outfit. If not, its value is full
  // if this is  planet's stock, or fully depreciated if this is the player.
  auto recordIt = outfits.find(outfit);
  if(recordIt == outfits.end() || recordIt->second.empty()) return DefaultDepreciation() * count * outfit->Cost();
...
{
  if(record.empty()) return count * DefaultDepreciation();
  ...
  while(true)
  {
    ...
    {
      ++it;
      if(it == record.end()) break;
    }
    else {
      if(it == record.begin()) break;
      --it;
    }
  }
...
{
  if(age <= GracePeriod()) return 1.;

  if(age >= MaxAge()) return Min();
...
double Depreciation::DefaultDepreciation() const { return isStock ? 1. : Min(); }
