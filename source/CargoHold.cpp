...
namespace
{
  ...
  std::vector<const Outfit *> OrderOutfitsBySize(const std::map<const Outfit *, int> &outfits)
  {
    ...
    std::sort(
        sortedOutfits.begin(),
        sortedOutfits.end(),
        [](const Outfit *lhs, const Outfit *rhs) { return lhs->Mass() > rhs->Mass(); });
...
void CargoHold::SetSize(int tons) { size = tons; }


// Get the capacity of this hold.
int CargoHold::Size() const { return size; }


// Get the amount of free cargo space.
int CargoHold::Free() const { return size - Used(); }


double CargoHold::FreePrecise() const { return size - UsedPrecise(); }


// Get the total amount of cargo space used, rounded up to the nearest ton.
// (Some outfits may have non-integral masses.)
int CargoHold::Used() const { return CommoditiesSize() + OutfitsSize() + MissionCargoSize(); }


double CargoHold::UsedPrecise() const { return CommoditiesSize() + OutfitsSizePrecise() + MissionCargoSize(); }
...
int CargoHold::OutfitsSize() const { return ceil(OutfitsSizePrecise()); }
...
bool CargoHold::HasMissionCargo() const { return !missionCargo.empty(); }
...
void CargoHold::SetBunks(int count) { bunks = count; }


// Check how many bunks are free.
int CargoHold::BunksFree() const { return bunks - Passengers(); }
...
const std::map<std::string, int> &CargoHold::Commodities() const { return commodities; }


// Access the outfits map directly.
const std::map<const Outfit *, int> &CargoHold::Outfits() const { return outfits; }


// Access the mission cargo map directly.
const std::map<const Mission *, int> &CargoHold::MissionCargo() const { return missionCargo; }


// Access the mission passenger map directly.
const std::map<const Mission *, int> &CargoHold::PassengerList() const { return passengers; }
...
int CargoHold::Transfer(const Mission *mission, int amount, CargoHold &to)
{
  ...
  if(amount < 0) return -to.Transfer(mission, -amount, *this);
  ...
  if(amount && !existing) return 0;
  amount = std::min(amount, existing);
  if(to.size >= 0) amount = std::max(0, std::min(amount, to.Free()));
  // Don't transfer 0 tons unless that's all that exists.
  if(existing && !amount) return 0;
...
{
  // A negative amount means a transfer in the opposite direction.
  if(amount < 0) return -to.TransferPassengers(mission, -amount, *this);

  // Check if the destination cargo hold has a limit on the number of bunks.
  amount = std::min(amount, GetPassengers(mission));
  if(to.bunks >= 0) amount = std::max(0, std::min(amount, to.BunksFree()));
...
void CargoHold::TransferAll(CargoHold &to, bool transferPassengers)
{
  ...
  while(mit != missionCargo.end())
  {
    ...
    if(!mit->second) mit = missionCargo.erase(mit);
    else ++mit;
  }
...
{
  if(amount < 0) return -Remove(commodity, -amount);

  // If this cargo hold has a size limit, apply it.
  if(size >= 0) amount = std::max(0, std::min(amount, Free()));
...
{
  if(amount < 0) return -Remove(outfit, -amount);

  // If the outfit has mass and this cargo hold has a size limit, apply it.
  double mass = outfit->Mass();
  if(size >= 0 && mass > 0.) amount = std::max(0, std::min(amount, static_cast<int>(FreePrecise() / mass)));
...
{
  if(amount < 0) return Add(commodity, -amount);
...
{
  if(amount < 0) return Add(outfit, -amount);
...
void CargoHold::AddMissionCargo(const Mission *mission)
{
  ...
  if(mission && !mission->Cargo().empty()) missionCargo[mission] += mission->CargoSize();
  if(mission && mission->Passengers()) passengers[mission] += mission->Passengers();
}
...
std::pair<int, const Conversation *> CargoHold::IllegalCargoFine(const Government *government) const
{
  ...
  for(const auto &it : outfits)
  {
    ...
    if(atrocity.isAtrocity) return {-1, atrocity.customDeathSentence};
    int fine = government->Fines(it.first);
    if(fine < 0) return {fine, nullptr};
  ...
  for(const auto &it : missionCargo)
  {
    ...
    if(fine < 0) return {-1 * !government->IgnoresUniversalAtrocities(), nullptr};
    if(!it.first->IsFailed() && !government->IgnoresUniversalIllegals()) totalFine += fine;
  }
...
int CargoHold::IllegalPassengersFine(const Government *government) const
{
  ...
  {
    int fine = it.first->Fine();
    if(fine < 0) return -1 * !government->IgnoresUniversalAtrocities();
    if(!it.first->IsFailed() && !government->IgnoresUniversalIllegals()) totalFine += fine;
  }
...
int CargoHold::IllegalCargoAmount() const
{
  ...
  for(const auto &it : missionCargo)
    if(it.first->Fine()) count += it.second;
