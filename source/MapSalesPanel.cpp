...
bool MapSalesPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  else if((key == SDLK_DOWN || key == SDLK_UP) && !zones.empty())
  {
    ...
    if(selected < 0) selected = zones.size() - 1;
    else if(selected > static_cast<int>(zones.size() - 1)) selected = 0;
...
bool MapSalesPanel::Click(int x, int y, MouseButton button, int clicks)
{
  ...
  if(x < Screen::Left() + WIDTH)
  {
    ...
    const auto  zone =
        find_if(zones.begin(), zones.end(), [&](const ClickZone<int> zone) { return zone.Contains(point); });
...
{
  if(isDragging) scroll = std::min(0., std::max(-maxScroll, scroll + dy));
  else return MapPanel::Drag(dx, dy);
...
{
  if(isDragging) scroll = std::min(0., std::max(-maxScroll, scroll + dy * 2.5 * Preferences::ScrollSpeed()));
  else return MapPanel::Scroll(dx, dy);

  return true;
}


const Swizzle *MapSalesPanel::SelectedSpriteSwizzle() const { return Swizzle::None(); }


const Swizzle *MapSalesPanel::CompareSpriteSwizzle() const { return Swizzle::None(); }


void MapSalesPanel::DrawKey(Information &info) const
{
  info.SetBar("full", 1.);
  if(onlyShowSoldHere) info.SetCondition("only sold here");
  else if(onlyShowStorageHere) info.SetCondition("only stored here");
...
void MapSalesPanel::DrawInfo() const
{
  if(selected >= 0)
  {
    ...
    Point bottomPos =
        leftPos + Point(10. + .5 * (bottom->Width() - left->Width()), .5 * (left->Height() + bottom->Height()));
...
void MapSalesPanel::DrawSprite(const Point &corner, const Sprite *sprite, const Swizzle *swizzle) const
{
  ...
  if(sprite->IsLoaded())
  {
    ...
    if(!swizzle) swizzle = GameData::PlayerGovernment()->GetSwizzle();
    ...
    const std::string &storage)
{
  ...
  {
    if(isSelected) FillShader::Fill(Rectangle::FromCorner(corner, blockSize), selectionColor);
    ...
    if(!variantName.empty()) font.Draw({"\t" + variantName, layout}, corner + variantOffset, textColor);
    font.Draw({price, layout}, corner + priceOffset, textColor);
    font.Draw({info, layout}, corner + infoOffset, textColor);
    if(!storage.empty()) font.Draw({storage, layout}, corner + storageOffset, textColor);
  }
...
{
  if(index < 0 || index >= static_cast<int>(zones.size())) return;

  const ClickZone<int> &it = zones[selected];
  if(it.Bottom() > Screen::Bottom()) scroll += Screen::Bottom() - it.Bottom();
  if(it.Top() < Screen::Top()) scroll += Screen::Top() - it.Top();
}
