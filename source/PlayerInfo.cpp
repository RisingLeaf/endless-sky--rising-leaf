...
namespace
{
  ...
  string EntryToString(SystemEntry entryType)
  {
    ...
    case SystemEntry::JUMP:       return "jump drive";
    case SystemEntry::WORMHOLE:   return "wormhole";
    default:
    case SystemEntry::TAKE_OFF:   return "takeoff";
    }
  }

  SystemEntry StringToEntry(const string &entry)
  {
    if(entry == "hyperdrive") return SystemEntry::HYPERDRIVE;
    else if(entry == "jump drive") return SystemEntry::JUMP;
    else if(entry == "wormhole") return SystemEntry::WORMHOLE;
  ...
  void SortMissions(list<Mission> &missions, bool hasPriorityMissions, unsigned nonBlockingMissions)
  {
    ...
    missions.sort([](const Mission &a, const Mission &b) { return a.OfferPrecedence() > b.OfferPrecedence(); });
    ...
    {
      erase_if(missions, [](const Mission &m) noexcept -> bool { return !m.HasPriority() && !m.IsNonBlocking(); });
    }
    else if(missions.size() > 1 + nonBlockingMissions)
    {
      ...
      while(it != missions.end())
      {
        if(it->IsMinor())
        {
          ...
          if(missions.size() <= 1 + nonBlockingMissions) break;
        }
        else {
          ++it;
        }
...
PlayerInfo::ScheduledEvent::ScheduledEvent(const DataNode &node, const ConditionsStore *playerConditions)
{
  ...
  string eventName;
  if(!conditions.empty()) eventName = conditions.begin()->substr(strlen("event: "));
  ...
  if(eventDef) event = ExclusiveItem<GameEvent>(eventDef);
  else event = ExclusiveItem<GameEvent>(std::move(nodeEvent));
}
...
bool PlayerInfo::ScheduledEvent::operator<(const ScheduledEvent &other) const { return date < other.date; }
...
bool PlayerInfo::IsLoaded() const { return !firstName.empty(); }
...
void PlayerInfo::New(const StartConditions &start, const Gamerules &gamerules)
{
  ...
  for(const auto &it : GameData::Events())
    if(it.second.GetDate()) AddEvent(it.second, it.second.GetDate());
}
...
void PlayerInfo::Load(const filesystem::path &path)
{
  ...
  if(pos != string::npos && pos > namePos) filePath = filePath.substr(0, pos) + ".txt";
  ...
  for(const DataNode &child : file)
  {
    ...
    else if(key == "reputation with")
    {
      for(const DataNode &grand : child)
        if(grand.Size() >= 2)
          reputationChanges.emplace_back(GameData::Governments().Get(grand.Token(0)), grand.Value(1));
    }
    else if(key == "tribute received")
    {
      for(const DataNode &grand : child)
        if(grand.Size() >= 2) tributeReceived[GameData::Planets().Get(grand.Token(0))] = grand.Value(1);
    }
    ...
    {
      for(const DataNode &grand : child)
        if(grand.Size() >= 2) costBasis[grand.Token(0)] += grand.Value(1);
    }
    else if(key == "stock")
    {
      for(const DataNode &grand : child)
        if(grand.Size() >= 2) stock[GameData::Outfits().Get(grand.Token(0))] += grand.Value(1);
    }
    ...
    {
      map<string, map<string, int>> &toDistribute =
          (key == "mission cargo") ? missionCargoToDistribute : missionPassengersToDistribute;
      for(const DataNode &grand : child)
      {
    ...
    else if(key == "harvested")
    {
      for(const DataNode &grand : child)
        if(grand.Size() >= 2)
          harvested.emplace(GameData::Systems().Get(grand.Token(0)), GameData::Outfits().Get(grand.Token(1)));
    }
    else if(key == "logbook")
    {
      for(const DataNode &grand : child)
    ...
    else if(key == "gamerules" && hasValue)
    {
      ...
      {
        child.PrintTrace(
            "The gamerule preset \"" + presetName +
            "\" does not exist. "
      ...
      if(child.HasChildren()) gamerules.Load(child);
    }
    else if(key == "start")
    {
      ...
      if(missionIt != missions.end())
      {
        ...
        for(const auto &shipCargo : it.second)
        {
          ...
          if(shipIt != ships.end())
          {
            ...
            if(passengers) cargo.TransferPassengers(cargoOf, shipCargo.second, destination->Cargo());
            else cargo.Transfer(cargoOf, shipCargo.second, destination->Cargo());
          }
        }
      }
    }
  ...
  if(!depreciation.IsLoaded()) depreciation.Init(ships, date.DaysSinceEpoch());
}


// Reload from the same file from which the current pilot was loaded.
void PlayerInfo::Reload() { Load(filePath); }
...
void PlayerInfo::Save() const
{
  ...
  if(filePath.rfind(".txt") == filePath.length() - 4)
  {
    ...
    if(saved.GetDate() != date.ToString())
    {
      ...
      {
        const string toMove = rootPrevious + std::to_string(i) + ".txt";
        if(Files::Exists(toMove)) Files::Move(toMove, rootPrevious + std::to_string(i + 1) + ".txt");
      }
      if(Files::Exists(filePath)) Files::Move(filePath, rootPrevious + "1.txt");
      if(planet->HasServices()) Save(rootPrevious + "spaceport.txt");
    }
  }
...
void PlayerInfo::AddChanges(list<DataNode> &changes, bool instantChanges)
{
  ...
  if(changedPlanets) GameData::RecomputeWormholeRequirements();
  if((changedPlanets || changedShops) && planet && instantChanges) SpriteLoadManager::RecheckThumbnails();
  if(changedSystems)
  {
    ...
    for(const System *system : visitedSystems)
    {
      ...
      for(const System *neighbor : system->VisibleNeighbors())
        if(!neighbor->Hidden() || system->Links().contains(neighbor)) seen.insert(neighbor);
    }
...
void PlayerInfo::AddEvent(GameEvent event, const Date &date)
{
  ...
  if(date <= this->date)
  {
    ...
    if(!eventChanges.empty()) AddChanges(eventChanges, true);
  }
  else {
    event.SetDate(date);
...
void PlayerInfo::Die(int response, const shared_ptr<Ship> &capturer)
{
  ...
  if(GetPlanet() || !flagship)
  {
    ...
    if(flagship) flagship.get()->SetVelocity(Point());
    ships.clear();
  }
  ...
  else if(!flagship->IsDestroyed())
  {
    ...
    auto it = find(ships.begin(), ships.end(), flagship);
    if(it != ships.end()) ships.erase(it);
  }
}


// Query whether this player is dead.
bool PlayerInfo::IsDead() const { return isDead; }


// Get the player's first name.
const string &PlayerInfo::FirstName() const { return firstName; }


// Get the player's last name.
const string &PlayerInfo::LastName() const { return lastName; }
...
void PlayerInfo::SetName(const string &first, const string &last)
{
  ...
  while(true)
  {
    ...
    if(index++) path += " " + std::to_string(index);
    path += ".txt";
...
const Date &PlayerInfo::GetDate() const { return date; }
...
void PlayerInfo::AdvanceDate(int amount)
{
  ...
  while(amount--)
  {
    ...
    if(!eventChanges.empty()) AddChanges(eventChanges);
    ...
    for(Mission &mission : missions)
    {
      ...
      {
        Messages::Add(
            {"You failed to meet the deadline for the mission \"" + mission.DisplayName() + "\".",
             GameData::MessageCategories().Get("high")});
      }
      if(!mission.IsFailed()) mission.Do(Mission::DAILY, *this);
    }
    DoAccounting();
  }
...
const CoreStartData &PlayerInfo::StartData() const noexcept { return startData; }


void PlayerInfo::SetSystemEntry(SystemEntry entryType) { entry = entryType; }


SystemEntry PlayerInfo::GetSystemEntry() const { return entry; }
...
const System *PlayerInfo::GetSystem() const { return system; }


const System *PlayerInfo::GetPreviousSystem() const { return previousSystem; }


// Set the planet the player is landed on.
void PlayerInfo::SetPlanet(const Planet *planet) { this->planet = planet; }


// Get the planet the player is landed on.
const Planet *PlayerInfo::GetPlanet() const { return planet; }
...
{
  if(!system || !planet) return nullptr;
...
bool PlayerInfo::ShouldLaunch() const { return shouldLaunch; }


// Access the player's account information.
const Account &PlayerInfo::Accounts() const { return accounts; }


// Access the player's account information (and allow modifying it).
Account &PlayerInfo::Accounts() { return accounts; }
...
int64_t PlayerInfo::Salaries() const
{
  ...
  if(flagship) crew = flagship->Crew() - flagship->RequiredCrew();

  // A ship that is "parked" remains on a planet and requires no salaries.
  for(const shared_ptr<Ship> &ship : ships)
    if(!ship->IsParked() && !ship->IsDestroyed()) crew += ship->RequiredCrew();
  if(!crew) return 0;
...
void PlayerInfo::AddLicense(const string &name) { licenses.insert(name); }


void PlayerInfo::RemoveLicense(const string &name) { licenses.erase(name); }


bool PlayerInfo::HasLicense(const string &name) const { return licenses.contains(name); }


const set<string> &PlayerInfo::Licenses() const { return licenses; }


// Get a pointer to the ship that the player controls. This is usually the first
// ship in the list.
const Ship *PlayerInfo::Flagship() const { return const_cast<PlayerInfo *>(this)->FlagshipPtr().get(); }


// Get a pointer to the ship that the player controls. This is usually the first
// ship in the list.
Ship *PlayerInfo::Flagship() { return FlagshipPtr().get(); }
...
const shared_ptr<Ship> &PlayerInfo::FlagshipPtr()
{
  if(!flagship)
  {
    ...
    if(planet) clearance = planet->CanLand() || HasClearance();
    for(const shared_ptr<Ship> &it : ships)
    {
      if(it->IsParked()) continue;
      if(it->GetSystem() != system) continue;
      if(!it->CanBeFlagship()) continue;
...
const vector<shared_ptr<Ship>> &PlayerInfo::Ships() const { return ships; }
...
map<const shared_ptr<Ship>, vector<string>> PlayerInfo::FlightCheck() const
{
  ...
  for(const auto &ship : ships)
  {
    if(ship->GetSystem() && ship->GetPlanet() && !ship->IsParked())
    {
      ...
      if(!checks.empty()) flightChecks.emplace(ship, checks);

      // Only check bays for in-system ships.
      if(ship->GetSystem() != system) continue;
      ...
      if(ship->CanBeCarried()) bayCount.emplace(ship->Attributes().Category(), 0);

      if(ship->CanBeCarried() || !ship->HasBays()) continue;
      ...
      for(auto &bay : ship->Bays())
      {
        ...
        {
          Logger::Log(
              "Expected bay to be empty for " + ship->TrueModelName() + ": " + ship->GivenName(),
  ...
  for(auto &bayType : bayCount)
  {
    ...
    for(const auto &carriable : shipsOfType)
    {
      ...
      else if(bayType.second > 0)
      {
        ...
        if(it != flightChecks.end()) it->second.emplace_back(warning);
        else flightChecks.emplace(carriable, vector<string>{warning});
      }
    }
  }
  return flightChecks;
...
void PlayerInfo::AddShip(const shared_ptr<Ship> &ship)
{
  ...
  ship->SetIsSpecial();
  ship->SetIsYours();
  if(ship->HasBays()) displayCarrierHelp = true;
}
...
void PlayerInfo::BuyShip(const Ship *model, const string &name)
{
  ...
  if(accounts.Credits() >= cost)
  {
    ...
    if(ships.back()->HasBays()) displayCarrierHelp = true;
  }
}
...
const Ship *PlayerInfo::GiftShip(const Ship *model, const string &name, const string &id)
{
  ...
  if(!id.empty()) giftedShips[id].Clone(ships.back()->UUID());
...
void PlayerInfo::SellShip(const Ship *selected, bool storeOutfits)
{
  for(auto it = ships.begin(); it != ships.end(); ++it)
  {
    if(it->get() == selected)
    {
      ...
      if(storeOutfits) cost = depreciation.Value(selected, day);
      else cost = depreciation.Value(*selected, day);
      ...
      if(takeOutfits)
      {
        for(const auto &it : shipToTake->Outfits())
        {
          ...
          if(model)
          {
            ...
            if(outfit != model->Outfits().end()) amountToTake = max(it.second, outfit->second);
          }
...
void PlayerInfo::ReorderShip(int fromIndex, int toIndex)
{
  ...
  if(static_cast<unsigned>(fromIndex) >= ships.size()) return;
  if(static_cast<unsigned>(toIndex) >= ships.size()) return;
...
double PlayerInfo::RaidFleetAttraction(const RaidFleet &raid, const System *system) const
{
  ...
  if(raidGov && raidGov->IsEnemy())
  {
    ...
    if(raid.MaxAttraction() > 0 && factors.first > raid.MaxAttraction()) return 0;
...
CargoHold &PlayerInfo::Cargo() { return cargo; }


// Get cargo information.
const CargoHold &PlayerInfo::Cargo() const { return cargo; }
...
const map<const Planet *, CargoHold> &PlayerInfo::PlanetaryStorage() const { return planetaryStorage; }


// Adjust the cost basis for the given commodity.
void PlayerInfo::AdjustBasis(const string &commodity, int64_t adjustment) { costBasis[commodity] += adjustment; }
...
void PlayerInfo::Land(UI &ui)
{
  ...
  if(!freshlyLoaded) Audio::Play(Audio::Get("landing"), SoundCategory::ENGINE);
  Audio::PlayMusic(planet->MusicName());

  // Mark this planet as visited.
  Visit(*planet);
  if(planet == travelDestination) travelDestination = nullptr;
  ...
  while(it != ships.end())
  {
    if((*it)->IsDestroyed() || !(*it)->IsYours())
    {
      ...
      for(const auto &cargo : (*it)->Cargo().Commodities())
        if(cargo.second) lostCargo[cargo.first] += cargo.second;
    ...
    if(!ship->IsParked() && !ship->IsDisabled())
    {
      if(ship->GetSystem() == system)
      {
        ...
        if(alreadyLanded || planet->CanLand(*ship) || (clearance && planet->IsAccessible(ship.get())))
        {
          ...
          ship->Recharge(
              canUseServices ? port.GetRecharges() : Port::RechargeType::None,
              port.HasService(Port::ServicesType::HireCrew));
          if(!ship->GetPlanet()) ship->SetPlanet(planet);
        }
        ...
        // in the system.
          ...
          if(!landingObject) landingObject = AI::FindLandingLocation(*ship, false);
          if(landingObject)
          {
            const Planet *landingPlanet = landingObject->GetPlanet();
            const Port   &port          = landingPlanet->GetPort();
            ship->Recharge(
                landingPlanet->CanUseServices() ? port.GetRecharges() : Port::RechargeType::None,
                port.HasService(Port::ServicesType::HireCrew));
  ...
  if(!freshlyLoaded) UpdateMissionNPCs();
  ...
  // but if there are many do not name them all (since this would overflow the screen).
    ...
    if(!inactiveMissions.empty())
    {
      ...
      if(mit != inactiveMissions.rend())
        message += " and " + std::to_string(distance(mit, inactiveMissions.rend())) + " more.\n";
    ...
    if(!invalidEvents.empty())
    {
      ...
      if(eit != invalidEvents.rend()) message += " and " + std::to_string(distance(eit, invalidEvents.rend())) + " more.\n";
...
bool PlayerInfo::TakeOff(UI &ui, const bool distributeCargo)
{
  ...
  for(const shared_ptr<Ship> &ship : ships)
  {
    if(!ship->IsParked() && !ship->IsDisabled())
    {
      ...
      if(ship->GetSystem() != system)
      {
        ...
        ship->Recharge(
            canUseServices ? planet->GetPort().GetRecharges() : Port::RechargeType::None,
            planet->GetPort().HasService(Port::ServicesType::HireCrew));
      }
    }
  }

  if(distributeCargo) DistributeCargo();
  ...
  if(cargo.Passengers())
  {
    ...
    if(extra)
    {
      ...
      {
        Messages::Add(
            {"You fired a crew member to free up a bunk for a passenger.",
             GameData::MessageCategories().Get("normal")});
      }
      else {
        Messages::Add(
            {"You fired " + std::to_string(extra) + " crew members to free up bunks for passengers.",
             GameData::MessageCategories().Get("normal")});
      }
  ...
  if(extra > 0)
  {
    ...
    {
      Messages::Add(
          {"You fired a crew member because you have no bunk for them.", GameData::MessageCategories().Get("normal")});
    }
    else {
      Messages::Add(
          {"You fired " + std::to_string(extra) + " crew members because you have no bunks for them.",
           GameData::MessageCategories().Get("normal")});
    }
    flagship->Cargo().SetBunks(flagship->Attributes().Get("bunks") - flagship->Crew());
  }
  ...
  for(auto &ship : ships)
  {
    if(!ship->IsParked() && !ship->IsDisabled())
    {
      if(ship->CanBeCarried() && ship != flagship) toLoad.emplace_back(ship);
      else if(ship->HasBays()) carriers.emplace_back(ship.get());
    }
  }
  if(!toLoad.empty())
  {
  ...
  for(const auto &it : cargo.MissionCargo())
  {
    if(it.second)
    {
      ...
      {
        Messages::Add(
            {"Mission \"" + it.first->DisplayName() + "\" aborted because you do not have space for the cargo.",
             GameData::MessageCategories().Get("high")});
      }
  ...
  for(const auto &it : cargo.PassengerList())
  {
    if(it.second)
    {
      ...
      {
        Messages::Add(
            {"Mission \"" + it.first->DisplayName() + "\" aborted because you do not have enough passenger bunks free.",
             GameData::MessageCategories().Get("high")});
      }
      missionsToRemove.push_back(it.first);
    }
  ...
  if(sold)
  {
    for(const auto &commodity : cargo.Commodities())
    {
      ...
      if(!original || it == costBasis.end() || !it->second) continue;
  ...
  if(sold || stored)
  {
    ...
    if(stored) out << "stored " << Format::CargoString(stored, "outfits") << " you could not carry";
    out << ".";
...
void PlayerInfo::PoolCargo()
{
  ...
  cargo.SetBunks(-1);
  for(const shared_ptr<Ship> &ship : ships)
    if(ship->GetPlanet() == planet && !ship->IsParked()) ship->Cargo().TransferAll(cargo);
  UpdateCargoCapacities();
}
...
double PlayerInfo::GetPlayTime() const noexcept { return playTime; }


void PlayerInfo::AddPlayTime(chrono::nanoseconds timeVal) { playTime += timeVal.count() * .000000001; }


// Get the player's logbook.
const map<Date, BookEntry> &PlayerInfo::Logbook() const { return logbook; }


void PlayerInfo::AddLogEntry(const BookEntry &logbookEntry) { logbook[date].Add(logbookEntry); }


const map<string, map<string, BookEntry>> &PlayerInfo::SpecialLogs() const { return specialLogs; }
...
void PlayerInfo::RemoveSpecialLog(const string &category, const string &heading)
{
  ...
  if(it == specialLogs.end()) return;
  auto &nameMap = it->second;
  auto  eit     = nameMap.find(heading);
  if(eit != nameMap.end()) nameMap.erase(eit);
}
...
void PlayerInfo::RemoveSpecialLog(const string &type)
{
  ...
  if(it != specialLogs.end()) specialLogs.erase(it);
}


bool PlayerInfo::HasLogs() const { return !logbook.empty() || !specialLogs.empty(); }


// Get the list of active missions.
const list<Mission> &PlayerInfo::Missions() const { return missions; }


// Get the list of ordinary jobs that are available on the job board.
const list<Mission> &PlayerInfo::AvailableJobs() const { return availableJobs; }
...
{
  if(!onlyDeadlines) mission.RecalculateTrackedSystems();
     ...
     here.HasRoute(*mission.Destination()->GetSystem()))
  {
    ...
    {
      if(here.HasRoute(*stopover->GetSystem())) toVisit.insert(stopover->GetSystem());
      // Stopovers require you to land on a planet, which takes an extra day.
      --daysLeft;
    }
    for(const System *waypoint : mission.Waypoints())
      if(here.HasRoute(*waypoint)) toVisit.insert(waypoint);
...
const PlayerInfo::SortType PlayerInfo::GetAvailableSortType() const { return availableSortType; }
...
const bool PlayerInfo::ShouldSortAscending() const { return availableSortAsc; }
...
const bool PlayerInfo::ShouldSortSeparateDeadline() const { return sortSeparateDeadline; }
...
const bool PlayerInfo::ShouldSortSeparatePossible() const { return sortSeparatePossible; }
  ...
  availableJobs.sort(
      [&](const Mission &lhs, const Mission &rhs)
      {
        ...
        if(sortSeparateDeadline)
        {
          ...
          if(!lhs.Deadline() && rhs.Deadline()) return availableSortAsc;
          if(lhs.Deadline() && !rhs.Deadline()) return !availableSortAsc;
        }
        // Then, separate greyed-out jobs you can't accept
        if(sortSeparatePossible)
        {
          if(lhs.CanAccept(*this) && !rhs.CanAccept(*this)) return availableSortAsc;
          if(!lhs.CanAccept(*this) && rhs.CanAccept(*this)) return !availableSortAsc;
        }
        ...
        case CONVENIENT:
          {
            ...
            const int lConvenient =
                destinations.count(lhs.Destination()) + destinations.count(lhs.Destination()->GetSystem());
            const int rConvenient =
                destinations.count(rhs.Destination()) + destinations.count(rhs.Destination()->GetSystem());
            if(lConvenient < rConvenient) return true;
            if(lConvenient > rConvenient) return false;
          }
        ...
        case PAY:
          {
            ...
            if(lPay < rPay) return true;
            else if(lPay > rPay) return false;
          }
        // Tiebreaker for equal PAY is ABC.
        case ABC:
          {
            if(lhs.DisplayName() < rhs.DisplayName()) return true;
            else if(lhs.DisplayName() > rhs.DisplayName()) return false;
          }
        // Tiebreaker fallback to keep sorting consistent is unique UUID:
        default: return lhs.UUID() < rhs.UUID();
        }
      });

  if(!availableSortAsc) availableJobs.reverse();
}


// Return a pointer to the mission that was most recently accepted while in-flight.
const Mission *PlayerInfo::ActiveInFlightMission() const { return activeInFlightMission; }
...
void PlayerInfo::AcceptJob(const Mission &mission, UI &ui)
{
  for(auto it = availableJobs.begin(); it != availableJobs.end(); ++it)
  {
    if(&*it == &mission)
    {
      ...
      if(it->IsFailed()) RemoveMission(Mission::Trigger::FAIL, *it, ui);
...
Mission *PlayerInfo::BoardingMission(const shared_ptr<Ship> &ship)
{
  ...
  if(ship->IsSpecial()) return nullptr;
  ...
  Mission::Location location = (ship->GetGovernment()->IsEnemy() ? Mission::BOARDING : Mission::ASSISTING);
  ...
  for(const auto &[name, mission] : GameData::Missions())
  {
    if(mission.IsAtLocation(location) && mission.CanOffer(*this, ship))
    {
      ...
      if(availableBoardingMissions.back().IsFailed()) availableBoardingMissions.pop_back();
      else return &availableBoardingMissions.back();
    }
  }
...
{
  if(ship->IsCapturable()) return false;
  ...
  {
    for(const Mission &mission : Missions())
      if(mission.OverridesCapture() && !mission.IsFailed() && mission.SourceShip() == ship.get()) return true;
  }
...
void PlayerInfo::ClearActiveInFlightMission() { activeInFlightMission = nullptr; }
...
void PlayerInfo::HandleBlockedMissions(Mission::Location location, UI &ui)
{
  ...
  if(ships.empty() || missionList.empty()) return;
...
{
  if(!flagship || (availableEnteringMissions.empty() && availableTransitionMissions.empty())) return;
...
void PlayerInfo::MissionCallback(int response)
{
  ...
  {
    if(!availableMissions.empty()) return availableMissions;
    if(!availableTransitionMissions.empty()) return availableTransitionMissions;
    if(!availableEnteringMissions.empty()) return availableEnteringMissions;
    return availableBoardingMissions;
  };
  list<Mission> &missionList = MissionsOffered();
  if(missionList.empty()) return;
...
void PlayerInfo::HandleEvent(const ShipEvent &event, UI &ui)
{
  ...
  if(event.ActorGovernment() && event.ActorGovernment()->IsPlayer())
  {
    if((event.Type() & ShipEvent::DISABLE) && event.Target() && !event.Target()->IsYours())
    {
      ...
      static const int64_t maxRating = 2'000'000'000;
  ...
  if((event.Type() & ShipEvent::DESTROY) && !ships.empty() && event.Target().get() == Flagship()) Die();
}


// Get mutable access to the player's list of conditions.
ConditionsStore &PlayerInfo::Conditions() { return conditions; }


// Access the player's list of conditions.
const ConditionsStore &PlayerInfo::Conditions() const { return conditions; }


Gamerules &PlayerInfo::GetGamerules() { return gamerules; }


// Uuid for the gifted ships, with the ship class follow by the names they had when they were gifted to the player.
const map<string, EsUuid> &PlayerInfo::GiftedShips() const { return giftedShips; }
...
const map<const Planet *, int64_t> &PlayerInfo::GetTribute() const { return tributeReceived; }
...
int64_t PlayerInfo::GetTributeTotal() const
{
  return accumulate(
      ...
      tributeReceived.end(),
      0,
      [](int64_t value, const map<const Planet *, int64_t>::value_type &tribute) { return value + tribute.second; });
}
...
{
  if(&system == this->system) return true;

  // Shrouded systems have special considerations as to whether they're currently seen or not.
  bool shrouded = system.Shrouded();
  if(!shrouded && seen.contains(&system)) return true;

  auto usesSystem = [&system](const Mission &m) noexcept -> bool
  {
    if(!m.IsVisible()) return false;
    if(m.Waypoints().contains(&system)) return true;
    if(m.MarkedSystems().contains(&system)) return true;
    for(auto &&p : m.Stopovers())
      if(p->IsInSystem(&system)) return true;
    return m.Destination()->IsInSystem(&system);
  };
  if(any_of(availableJobs.begin(), availableJobs.end(), usesSystem)) return true;
  if(any_of(missions.begin(), missions.end(), usesSystem)) return true;
  ...
  if(shrouded)
  {
    ...
    if(!system.VisibleNeighbors().contains(this->system)) return false;
...
bool PlayerInfo::HasVisited(const System &system) const { return visitedSystems.contains(&system); }


// Check if the player has visited the given planet.
bool PlayerInfo::HasVisited(const Planet &planet) const { return visitedPlanets.contains(&planet); }
...
{
  if(CanView(system)) return true;

  for(const Mission &mission : availableJobs)
    if(mission.Destination()->IsInSystem(&system)) return true;

  for(const Mission &mission : missions)
    if(mission.IsVisible() && mission.Destination()->IsInSystem(&system)) return true;
...
void PlayerInfo::Visit(const Planet &planet) { visitedPlanets.insert(&planet); }
...
void PlayerInfo::Unvisit(const System &system)
{
  ...
  for(const StellarObject &object : system.Objects())
    if(object.GetPlanet()) Unvisit(*object.GetPlanet());
}


void PlayerInfo::Unvisit(const Planet &planet) { visitedPlanets.erase(&planet); }


const set<const System *> &PlayerInfo::VisitedSystems() const { return visitedSystems; }


const set<const Planet *> &PlayerInfo::VisitedPlanets() const { return visitedPlanets; }
...
bool PlayerInfo::HasMapped(int mapSize, bool mapMinables) const
{
  ...
  {
    if(!HasVisited(*system)) return false;

    if(mapMinables)
    {
      for(const Outfit *outfit : system->Payloads())
        if(!harvested.contains(make_pair(system, outfit))) return false;
    }
  }
...
void PlayerInfo::Map(int mapSize, bool mapMinables)
{
  ...
  for(const System *system : distance.Systems())
  {
    if(!HasVisited(*system)) Visit(*system);
...
bool PlayerInfo::HasTravelPlan() const { return !travelPlan.empty(); }


// Access the player's travel plan.
const vector<const System *> &PlayerInfo::TravelPlan() const { return travelPlan; }


vector<const System *> &PlayerInfo::TravelPlan() { return travelPlan; }
...
const Planet *PlayerInfo::TravelDestination() const { return travelDestination; }
...
void PlayerInfo::SetTravelDestination(const Planet *planet)
{
  ...
  if(planet && planet->IsInSystem(system) && Flagship()) Flagship()->SetTargetStellar(system->FindStellar(planet));
}


// Check which secondary weapons the player has selected.
const set<const Outfit *> &PlayerInfo::SelectedSecondaryWeapons() const { return selectedWeapons; }


// Cycle through all available secondary weapons.
void PlayerInfo::SelectNextSecondary()
{
  if(!flagship || flagship->Outfits().empty()) return;
  ...
  if(!hadSingleWeaponSelected) return;
  ...
  {
    const Weapon *weapon = it->first->GetWeapon().get();
    if(weapon && weapon->Icon()) selectedWeapons.insert(it->first);
  }
  ...
  if(selectedWeapons.size() == 1) selectedWeapons.clear();
}


void PlayerInfo::DeselectAllSecondaries() { selectedWeapons.clear(); }
...
void PlayerInfo::SelectShips(const vector<weak_ptr<Ship>> &stack, bool hasShift)
{
  ...
  if(matched) UI::PlaySound(UI::UISound::TARGET);
}
...
void PlayerInfo::SelectEscort(const Ship *ship, bool hasShift)
{
  ...
  bool first = !hasShift;
  for(const shared_ptr<Ship> &it : ships)
    if(it.get() == ship) SelectEscort(it, &first);
}
...
void PlayerInfo::SelectEscortGroup(int group, bool hasShift)
{
    ...
    for(const shared_ptr<Ship> &ship : ships)
    {
      if(groups[ship.get()] & bit)
      {
        ...
        for(; it != selectedEscorts.end(); ++it)
          if(it->lock() == ship) break;
        if(it != selectedEscorts.end()) selectedEscorts.erase(it);
        else allWereSelected = false;
      }
    }
    if(allWereSelected) return;
  }
  else {
  ...
  for(const shared_ptr<Ship> &ship : ships)
  {
    if(groups[ship.get()] & bit)
    {
      ...
      if(ship.get() == oldTarget) Flagship()->SetTargetShip(ship);
    }
  }
}
...
set<Ship *> PlayerInfo::GetEscortGroup(int group)
{
  ...
  for(const shared_ptr<Ship> &ship : ships)
  {
    ...
    if(it != groups.end() && (it->second & bit)) result.insert(ship.get());
  }
  return result;
}
...
const map<const Outfit *, int> &PlayerInfo::GetStock() const { return stock; }
...
void PlayerInfo::AddStock(const Outfit *outfit, int count)
{
  ...
  if(count > 0 && stock[outfit] < 0) stock[outfit] = 0;
  stock[outfit] += count;
...
const Depreciation &PlayerInfo::FleetDepreciation() const { return depreciation; }


const Depreciation &PlayerInfo::StockDepreciation() const { return stockDepreciation; }


void PlayerInfo::Harvest(const Outfit *type)
{
  if(type && system) harvested.insert(make_pair(system, type));
}


const set<pair<const System *, const Outfit *>> &PlayerInfo::Harvested() const { return harvested; }


const pair<const System *, Point> &PlayerInfo::GetEscortDestination() const { return interstellarEscortDestination; }
...
bool PlayerInfo::HasEscortDestination() const { return interstellarEscortDestination.first; }


// Get what coloring is currently selected in the map.
int PlayerInfo::MapColoring() const { return mapColoring; }


// Set what the map is being colored by.
void PlayerInfo::SetMapColoring(int index) { mapColoring = index; }


// Get the map zoom level.
int PlayerInfo::MapZoom() const { return mapZoom; }


// Set the map zoom level.
void PlayerInfo::SetMapZoom(int level) { mapZoom = level; }


// Get the set of collapsed categories for the named panel.
set<string> &PlayerInfo::Collapsed(const string &name) { return collapsed[name]; }


bool PlayerInfo::DisplayCarrierHelp() const { return displayCarrierHelp; }
...
void PlayerInfo::ValidateLoad()
{
  ...
  if(planet && !system)
  {
    ...
    Logger::Log(
        "Player system was not specified. Defaulting to the specified planet's system.",
        Logger::Level::WARNING);
  }
  if(!planet || !planet->IsValid() || !system || !system->IsValid())
  {
    ...
    Logger::Log(
        "Player system and/or planet was not valid. Defaulting to the starting location.",
        Logger::Level::WARNING);
  }
  ...
  for(auto &&ship : ships)
  {
    ...
    if(ship->GetSystem() == system && ship->GetPlanet() && !ship->GetPlanet()->IsValid())
    {
      ...
      Logger::Log(
          "In-system player ship \"" + ship->GivenName() +
  ...
  {
    Logger::Log(
        "Removed invalid travel plan destination \"" + travelDestination->TrueName() + "\".",
        Logger::Level::WARNING);
    travelDestination = nullptr;
  }
  if(!travelPlan.empty() && any_of(
                                travelPlan.begin(),
                                travelPlan.end(),
  ...
  if(startData.Identifier().empty())
  {
    ...
    if(startCount >= 1)
    {
      ...
      if(startCount >= 2) conditions["unverified start scenario"] = true;
    }
    else {
      throw runtime_error(
          "Unable to set a starting scenario for an existing pilot. (No valid \"start\" "
  ...
  if(mit != missions.end()) inactiveMissions.splice(inactiveMissions.end(), missions, mit, missions.end());
  ...
  for(const ScheduledEvent &event : scheduledEvents)
    if(!event.event->IsValid().empty()) invalidEvents.insert(event.event->TrueName());
  for(const string &event : triggeredEvents)
    if(!GameData::Events().Get(event)->IsValid().empty()) invalidEvents.insert(event);
}
...
void PlayerInfo::RegisterDerivedConditions()
{
  ...
  conditions["weekday: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        string day    = ce.NameWithoutPrefix();
        int    number = date.WeekdayNumberOffset();
        if(day == "saturday") return number == 0;
        if(day == "sunday") return number == 1;
        if(day == "monday") return number == 2;
        if(day == "tuesday") return number == 3;
        if(day == "wednesday") return number == 4;
        if(day == "thursday") return number == 5;
        if(day == "friday") return number == 6;
        return 0;
      });
  ...
  conditions["days since start"].ProvideNamed(
      [this](const ConditionEntry &ce) { return date.DaysSinceEpoch() - StartData().GetDate().DaysSinceEpoch(); });
  ...
  conditions["credits"].ProvideNamed([this](const ConditionEntry &ce) { return min(limit, accounts.Credits()); });
  ...
  conditions["credit score"].ProvideNamed([this](const ConditionEntry &ce) { return accounts.CreditScore(); });

  // Read/write assets and debts.
  conditions["salary: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const map<string, int64_t> &si = accounts.SalariesIncome();
        auto                        it = si.find(ce.NameWithoutPrefix());
        if(it == si.end()) return 0;
        return it->second;
      },
      [this](ConditionEntry &ce, int64_t value) -> void { accounts.SetSalaryIncome(ce.NameWithoutPrefix(), value); });
  conditions["tribute: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        ...
        if(it == tributeReceived.end()) return 0;

        return it->second;
      },
      [this](ConditionEntry &ce, int64_t value) -> void { SetTribute(ce.NameWithoutPrefix(), value); });

  conditions["license: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t { return HasLicense(ce.NameWithoutPrefix()); },
      [this](ConditionEntry &ce, int64_t value) -> void
      {
        if(!value) RemoveLicense(ce.NameWithoutPrefix());
        else AddLicense(ce.NameWithoutPrefix());
      });

  // Read-only flagship conditions.
  conditions["flagship crew"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return flagship ? flagship->Crew() : 0; });
  conditions["flagship required crew"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return flagship ? flagship->RequiredCrew() : 0; });
  conditions["flagship bunks"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return flagship ? flagship->Attributes().Get("bunks") : 0; });
  conditions["flagship model: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        if(!flagship) return false;
        return !ce.NameWithoutPrefix().compare(flagship->TrueModelName());
      });
  conditions["flagship disabled"].ProvideNamed(
      [this](const ConditionEntry &ce) -> bool { return flagship && flagship->IsDisabled(); });
  ...
  auto shipAttributeHelper = [](const Ship *ship, const string &attribute, bool base) -> int64_t
  {
    ...
    if(attribute == "cost") return attributes.Cost();
    if(attribute == "mass") return round(attributes.Mass() * 1000.);
    return round(attributes.Get(attribute) * 1000.);
  };
  conditions["flagship base attribute: "].ProvidePrefixed(
  ...
  conditions["flagship attribute: "].ProvidePrefixed(
      [this, shipAttributeHelper](const ConditionEntry &ce) -> int64_t
      { return shipAttributeHelper(this->Flagship(), ce.NameWithoutPrefix(), false); });
  conditions["flagship bays: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
  ...
  conditions["flagship bays free: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        if(!flagship) return 0;
        if(GetPlanet())
        {
          Logger::Log(
              "Use of \"flagship bays free: <category>\" condition while landed is unstable behavior.",
              Logger::Level::WARNING);
        }
        return flagship->BaysFree(ce.NameWithoutPrefix());
      });
  conditions["flagship bays"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        if(!flagship) return 0;
  ...
  // The behaviour of this condition while landed is not stable and may change in the future.
  // It should only be used while in-flight.
  conditions["flagship bays free"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        if(!flagship) return 0;
        if(GetPlanet())
          Logger::Log(
              "Use of \"flagship bays free\" condition while landed is unstable behavior.",
              Logger::Level::WARNING);
        const vector<Ship::Bay> &bays = flagship->Bays();
        return count_if(bays.begin(), bays.end(), [](const Ship::Bay &bay) { return !bay.ship; });
      });

  conditions["flagship mass"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return flagship ? flagship->Mass() : 0; });
  conditions["flagship shields"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return flagship ? flagship->ShieldLevel() : 0; });
  conditions["flagship hull"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return flagship ? flagship->HullLevel() : 0; });
  conditions["flagship fuel"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return flagship ? flagship->FuelLevel() : 0; });

  conditions["ship base attribute: "].ProvidePrefixed(
      [this, shipAttributeHelper](const ConditionEntry &ce) -> int64_t
      {
        ...
        for(const shared_ptr<Ship> &ship : ships)
        {
          ...
          if(ship->IsDestroyed() || ship->IsParked() || (planet && ship->GetPlanet() != planet) ||
  ...
  conditions["ship base attribute (all): "].ProvidePrefixed(
      [this, shipAttributeHelper](const ConditionEntry &ce) -> int64_t
      {
        string  attribute = ce.NameWithoutPrefix();
        int64_t retVal    = 0;
  ...
  conditions["ship base attribute (parked): "].ProvidePrefixed(
      [this, shipAttributeHelper](const ConditionEntry &ce) -> int64_t
      {
        ...
        for(const shared_ptr<Ship> &ship : ships)
        {
          if(!ship->IsParked() || ship->GetPlanet() != planet) continue;
          retVal += shipAttributeHelper(ship.get(), attribute, true);
        }
        return retVal;
      });
  conditions["ship attribute: "].ProvidePrefixed(
      [this, shipAttributeHelper](const ConditionEntry &ce) -> int64_t
      {
        string  attribute = ce.NameWithoutPrefix();
        int64_t retVal    = 0;
  ...
  conditions["ship attribute (all): "].ProvidePrefixed(
      [this, shipAttributeHelper](const ConditionEntry &ce) -> int64_t
      {
        string  attribute = ce.NameWithoutPrefix();
        int64_t retVal    = 0;
  ...
  conditions["name: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool { return !ce.NameWithoutPrefix().compare(firstName + " " + lastName); });
  conditions["first name: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool { return !ce.NameWithoutPrefix().compare(firstName); });
  conditions["last name: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool { return !ce.NameWithoutPrefix().compare(lastName); });

  // Conditions for your fleet's attractiveness to pirates.
  conditions["cargo attractiveness"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return RaidFleetFactors().first; });
  conditions["armament deterrence"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return RaidFleetFactors().second; });
  conditions["pirate attraction"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        auto rff = RaidFleetFactors();
        return rff.first - rff.second;
      });
  conditions["raid chance in system: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> double
      {
  ...
  conditions["flagship strength"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t { return flagship ? flagship->Strength() : 0; });
  conditions["player strength"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t strength = 0;
        for(const shared_ptr<Ship> &ship : ships)
  ...
  conditions["cargo space"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal = 0;
        for(const shared_ptr<Ship> &ship : ships)
  ...
  conditions["passenger space"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal = 0;
        for(const shared_ptr<Ship> &ship : ships)
  ...
  conditions["cargo space free"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal = 0;
        for(const shared_ptr<Ship> &ship : ships)
  ...
  conditions["passenger space free"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal = 0;
        for(const shared_ptr<Ship> &ship : ships)
  ...
  conditions["flagship: cargo space"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        if(!flagship) return 0;
        return flagship->Attributes().Get("cargo space");
      });
  conditions["flagship: passenger space"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        if(!flagship) return 0;
        return flagship->Attributes().Get("bunks") - flagship->RequiredCrew();
      });
  conditions["flagship: cargo space free"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        if(!flagship) return 0;
        return flagship->Cargo().Free();
      });
  conditions["flagship: passenger space free"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        if(!flagship) return 0;
  ...
  // (e.g. Heavy Warships).
  conditions["ships: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal   = 0;
  ...
  conditions["ships (all): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal   = 0;
        string  category = ce.NameWithoutPrefix();
        for(const shared_ptr<Ship> &ship : ships)
          if(!ship->IsDestroyed() && !category.compare(ship->Attributes().Category())) ++retVal;
        return retVal;
      });
  // The number of ships the player has of the given model active and present.
  conditions["ship model: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal = 0;
  ...
  conditions["ship model (all): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal = 0;
        string  model  = ce.NameWithoutPrefix();
        for(const shared_ptr<Ship> &ship : ships)
          if(!ship->IsDestroyed() && !model.compare(ship->TrueModelName())) ++retVal;
        return retVal;
      });
  // The total number of ships the player has active and present.
  conditions["total ships"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal = 0;
        for(const shared_ptr<Ship> &ship : ships)
          if(!ship->IsParked() && !ship->IsDisabled() && ship->GetActualSystem() == system) ++retVal;
        return retVal;
      });
  // The total number of ships the player has anywhere.
  conditions["total ships (all)"].ProvideNamed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        int64_t retVal = 0;
        for(const shared_ptr<Ship> &ship : ships)
          if(!ship->IsDestroyed()) ++retVal;
        return retVal;
      });
  ...
  conditions["outfit: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        ...
        if(planet)
        {
          ...
          if(it != planetaryStorage.end()) retVal += it->second.Get(outfit);
        }
        for(const shared_ptr<Ship> &ship : ships)
        {
          // Destroyed and parked ships aren't checked.
  ...
  conditions["outfit (all): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
        if(!outfit) return 0;
  ...
  conditions["outfit (installed): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
        if(!outfit) return 0;
  ...
  conditions["outfit (parked): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        // If the player isn't landed then there can be no parked ships local to them.
        if(!planet) return 0;
  ...
  conditions["outfit (all installed): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
        if(!outfit) return 0;
        int64_t retVal = 0;
        for(const shared_ptr<Ship> &ship : ships)
          if(!ship->IsDestroyed()) retVal += ship->OutfitCount(outfit);
        return retVal;
      });

  // The following condition checks the flagship's installed outfits.
  conditions["outfit (flagship installed): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        if(!flagship) return 0;
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
  ...
  // local to the player.
  conditions["outfit (cargo): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
        if(!outfit) return 0;
        int64_t retVal = 0;
        if(planet) retVal += Cargo().Get(outfit);
        for(const shared_ptr<Ship> &ship : ships)
        {
  ...
  conditions["outfit (all cargo): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
        if(!outfit) return 0;
        ...
        if(planet) retVal += Cargo().Get(outfit);
        for(const shared_ptr<Ship> &ship : ships)
          if(!ship->IsDestroyed()) retVal += ship->Cargo().Get(outfit);
        return retVal;
      });

  // The following condition checks the flagship's cargo or the pooled cargo if landed.
  conditions["outfit (flagship cargo): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
        if(!outfit) return 0;
  ...
  conditions["outfit (storage): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
        if(!outfit) return 0;
        if(planet)
        {
          ...
          for(const StellarObject &object : system->Objects())
          {
            ...
            if(object.HasValidPlanet() && it != planetaryStorage.end()) retVal += it->second.Get(outfit);
          }
          return retVal;
        }
      });
  ...
  conditions["outfit (all storage): "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        const Outfit *outfit = GameData::Outfits().Find(ce.NameWithoutPrefix());
        if(!outfit) return 0;
  ...
  conditions["entered system by: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool { return !ce.NameWithoutPrefix().compare(EntryToString(entry)); });
  // This condition corresponds to the last system the flagship was in.
  conditions["previous system: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        if(!previousSystem) return false;
        return !ce.NameWithoutPrefix().compare(previousSystem->TrueName());
      });
  conditions["previous system government: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        if(!previousSystem || !previousSystem->GetGovernment()) return false;
        return !ce.NameWithoutPrefix().compare(previousSystem->GetGovernment()->TrueName());
      });
  conditions["previous system attribute: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        if(!previousSystem) return false;
        string attribute = ce.NameWithoutPrefix();
  ...
  conditions["flagship system: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        if(!flagship || !flagship->GetSystem()) return false;
        return !ce.NameWithoutPrefix().compare(flagship->GetSystem()->TrueName());
      });
  conditions["flagship landed"].ProvideNamed(
      [this](const ConditionEntry &ce) -> bool { return flagship && flagship->GetPlanet(); });
  conditions["flagship planet: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        if(!flagship || !flagship->GetPlanet()) return false;
        return !ce.NameWithoutPrefix().compare(flagship->GetPlanet()->TrueName());
      });
  conditions["flagship planet attribute: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        if(!flagship || !flagship->GetPlanet()) return false;
        string attribute = ce.NameWithoutPrefix();
  ...
  conditions["visited planet: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        const Planet *planet = GameData::Planets().Find(ce.NameWithoutPrefix());
        return planet ? HasVisited(*planet) : false;
      });
  conditions["visited system: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        const System *system = GameData::Systems().Find(ce.NameWithoutPrefix());
        return system ? HasVisited(*system) : false;
      });
  conditions["landing access: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> bool
      {
        const Planet *planet = GameData::Planets().Find(ce.NameWithoutPrefix());
  ...
  conditions["installed plugin: "].ProvidePrefixed(
      [](const ConditionEntry &ce) -> bool
      {
        const Plugin *plugin = Plugins::Get().Find(ce.NameWithoutPrefix());
        return plugin ? plugin->IsValid() && plugin->enabled : false;
      });

  conditions["person destroyed: "].ProvidePrefixed(
      [](const ConditionEntry &ce) -> bool
      {
  ...
  auto HyperspaceTravelDays = [](const System *origin, const System *destination) -> int
  {
    ...
    if(!distanceMap.HasRoute(*destination)) return -1;
    return distanceMap.Days(*destination);
  };

  conditions["hyperjumps to system: "].ProvidePrefixed(
      [this, HyperspaceTravelDays](const ConditionEntry &ce) -> int
      {
        const System *system = GameData::Systems().Find(ce.NameWithoutPrefix());
        if(!system)
        {
          Logger::Log(
              "System \"" + ce.NameWithoutPrefix() + "\" referred to in condition is not valid.",
              Logger::Level::WARNING);
          return -1;
        }
  ...
  conditions["hyperjumps to planet: "].ProvidePrefixed(
      [this, HyperspaceTravelDays](const ConditionEntry &ce) -> int
      {
        const Planet *planet = GameData::Planets().Find(ce.NameWithoutPrefix());
        if(!planet)
        {
          Logger::Log(
              "Planet \"" + ce.NameWithoutPrefix() + "\" referred to in condition is not valid.",
              Logger::Level::WARNING);
          return -1;
        }
        ...
        {
          Logger::Log(
              "Planet \"" + ce.NameWithoutPrefix() + "\" referred to in condition is not in any system.",
              Logger::Level::WARNING);
          return -1;
        }
        return HyperspaceTravelDays(this->GetSystem(), system);
  ...
  conditions["enemy: "].ProvidePrefixed(
      [](const ConditionEntry &ce) -> int64_t
      {
  ...
  conditions["reputation: "].ProvidePrefixed(
      [](const ConditionEntry &ce) -> int64_t
      {
        string govName = ce.NameWithoutPrefix();
        auto   gov     = GameData::Governments().Get(govName);
  ...
  conditions["random"].ProvideNamed([](const ConditionEntry &ce) -> int64_t { return Random::Int(100); });
  ...
  conditions["roll: "].ProvidePrefixed(
      [this](const ConditionEntry &ce) -> int64_t
      {
        string  input = ce.NameWithoutPrefix();
        int64_t value = 0;
        if(DataNode::IsNumber(input)) value = static_cast<int64_t>(DataNode::Value(input));
        else value = conditions.Get(input);
        if(value <= 1) return 0;
        return Random::Int(value);
      });

  // Gamerule condition getter:
  conditions["gamerule: "].ProvidePrefixed(
      [](const ConditionEntry &ce) -> int64_t { return GameData::GetGamerules().GetValue(ce.NameWithoutPrefix()); });

  // Global conditions setters and getters:
  conditions["global: "].ProvidePrefixed(
      [](const ConditionEntry &ce) -> int64_t
      {
        string globalCondition = ce.NameWithoutPrefix();
        return GameData::GlobalConditions().Get(globalCondition);
      },
      [](ConditionEntry &ce, int64_t value) { GameData::GlobalConditions().Set(ce.NameWithoutPrefix(), value); });
}
...
void PlayerInfo::TriggerEvent(GameEvent event, std::list<DataNode> &eventChanges)
{
  ...
  if(!name.empty()) triggeredEvents.insert(name);
  if(!changes.empty()) eventChanges.splice(eventChanges.end(), changes);
}
...
void PlayerInfo::CreateMissions()
{
  ...
  for(const auto &[name, mission] : GameData::Missions())
  {
    ...
    if(skipJobs && mission.IsAtLocation(Mission::JOB)) continue;

    if(mission.CanOffer(*this))
    {
      list<Mission> &missions = mission.IsAtLocation(Mission::JOB) ? availableJobs : availableMissions;

      Mission newMission = mission.Instantiate(*this);
      if(newMission.IsFailed()) continue;
...
void PlayerInfo::StepMissions(UI &ui)
{
  ...
  for(Mission &mission : missions)
  {
    for(const NPC &npc : mission.NPCs())
    {
      for(const shared_ptr<Ship> &ship : npc.Ships())
        if(ship->IsDestroyed()) mission.Do(ShipEvent(nullptr, ship, ShipEvent::DESTROY), *this, ui);
    }
  }
  ...
  string      visitText;
  int         missionVisits = 0;
  auto        substitutions = map<string, string>{{"<first>", firstName}, {"<last>", lastName}};
  const Ship *flag          = Flagship();
  if(flag)
  {
  ...
  while(mit != missions.end())
  {
    ...
    else if(mission.Destination() == GetPlanet() && !freshlyLoaded)
    {
      ...
      if(mission.IsUnique() || !mission.IsVisible()) continue;
      ...
      if(visitText.empty())
      {
        ...
        if(!text.empty()) visitText = Format::Replace(text, substitutions);
      }
      ++missionVisits;
    }
  }
  ...
  while(mit != missions.end())
  {
    ...
    if(mission.IsFailed()) RemoveMission(Mission::FAIL, mission, ui);
    else if(mission.CanComplete(*this)) RemoveMission(Mission::COMPLETE, mission, ui);
  }
  ...
  vector<const Mission *> missionsToRemove;
  for(const auto &it : cargo.MissionCargo())
    if(!active.contains(it.first)) missionsToRemove.push_back(it.first);
  for(const auto &it : cargo.PassengerList())
    if(!active.contains(it.first)) missionsToRemove.push_back(it.first);
  for(const Mission *mission : missionsToRemove)
    cargo.RemoveMissionCargo(mission);
...
{
  if(!CanBeSaved() || filePath.length() < 4) return;
...
void PlayerInfo::Save(DataWriter &out) const
{
  ...
  if(markedChangesToday) out.Write("marked event changes today");
  out.Write("system entry method", EntryToString(entry));
  if(previousSystem) out.Write("previous system", previousSystem->TrueName());
  if(system) out.Write("system", system->TrueName());
  if(planet) out.Write("planet", planet->TrueName());
  if(planet && planet->CanUseServices()) out.Write("clearance");
  ...
  if(shouldLaunch) out.Write("launching");
  for(const System *system : travelPlan)
    out.Write("travel", system->TrueName());
  if(travelDestination) out.Write("travel destination", travelDestination->TrueName());
  ...
  out.BeginChild();
  {
    for(const auto &it : GameData::Governments())
      if(!it.second.IsPlayer()) out.Write(it.first, it.second.Reputation());
  }
  out.EndChild();
  ...
  out.BeginChild();
  {
    for(const auto &it : tributeReceived)
      if(it.second > 0) out.Write((it.first)->TrueName(), it.second);
  }
  out.EndChild();
  ...
  for(const shared_ptr<Ship> &ship : ships)
  {
    ship->Save(out);
    auto it = groups.find(ship.get());
    if(it != groups.end() && it->second) out.Write("groups", it->second);
  }
  ...
  if(!stock.empty())
  {
    ...
    out.BeginChild();
    {
      ...
      WriteSorted(
          ...
          {
            if(it.second) out.Write(it.first->TrueName(), it.second);
          });
    }
    out.EndChild();
  }
  ...
  for(const auto &it : ships)
  {
    ...
    if(ship.GetPlanet() == planet) continue;
  ...
  {
    if(passengers) out.Write("mission passengers");
    else out.Write("mission cargo");
    out.BeginChild();
    {
  ...
  if(!offWorldMissionCargo.empty()) SaveMissionCargoDistribution(offWorldMissionCargo, false);
  if(!offWorldMissionPassengers.empty()) SaveMissionCargoDistribution(offWorldMissionPassengers, true);
  ...
  if(!availableSortAsc) out.Write("sort descending");
  if(sortSeparateDeadline) out.Write("separate deadline");
  if(sortSeparatePossible) out.Write("separate possible");
  ...
  for(const auto &it : GameData::Persons())
    if(it.second.IsDestroyed()) out.Write("destroyed", it.first);
  ...
  WriteSorted(
      visitedSystems,
      [](const System *const *lhs, const System *const *rhs) { return (*lhs)->TrueName() < (*rhs)->TrueName(); },
      [&out](const System *system) { out.Write("visited", system->TrueName()); });
  ...
  WriteSorted(
      ...
      [&out](const Planet *planet) { out.Write("visited planet", planet->TrueName()); });
    ...
    out.BeginChild();
    {
      ...
      WriteSorted(
          ...
          [](const HarvestLog *lhs, const HarvestLog *rhs) -> bool
          {
            ...
            if(lhs->first != rhs->first) return lhs->first->TrueName() < rhs->first->TrueName();
            else return lhs->second->TrueName() < rhs->second->TrueName();
          },
          [&out](const HarvestLog &it) { out.Write(it.first->TrueName(), it.second->TrueName()); });
    }
    out.EndChild();
  }
  ...
  if(!preset) preset = &GameData::DefaultGamerules();
  ...
  for(const auto &it : Plugins::Get())
  {
    ...
    if(plugin.IsValid() && plugin.enabled) out.Write(plugin.name);
  }
  out.EndChild();
...
void PlayerInfo::Fine(UI &ui)
{
  ...
  if(GameData::GetPolitics().HasDominated(planet) || !(planet->IsInhabited() || planet->HasCustomSecurity())) return;

  // Planets should not fine you if you have mission clearance or are infiltrating.
  for(const Mission &mission : missions)
  {
    if(mission.HasClearance(planet) ||
       (!mission.HasFullClearance() && (mission.Destination() == planet || mission.Stopovers().contains(planet))))
    {
      return;
    }
  }
  ...
  if(!gov->CanEnforce(planet)) return;
...
void PlayerInfo::SetFlagship(Ship &other)
{
  ...
  if(flagship && flagship.get() != &other) flagship->ClearTargetsAndOrders();
  ...
  for(auto it = selectedEscorts.begin(); it != selectedEscorts.end();)
  {
    ...
    if(!ship || ship == flagship) it = selectedEscorts.erase(it);
    else ++it;
  }
}
  ...
  if(shipToForget != giftedShips.end())
  {
    if(failsMissions)
    {
      for(auto &mission : missions)
        if(mission.RequiresGiftedShip(shipToForget->first)) mission.Fail();
    }
...
bool PlayerInfo::CanBeSaved() const { return !isDead && planet && system && !firstName.empty() && !lastName.empty(); }
...
void PlayerInfo::DoAccounting()
{
  ...
  if(salariesIncome) income["salary"] = salariesIncome;
  int64_t tributeIncome = GetTributeTotal();
  if(tributeIncome) income["in tribute"] = tributeIncome;
  FleetBalance balance = MaintenanceAndReturns();
  if(balance.assetsReturns) income["based on outfits and ships"] = balance.assetsReturns;
  if(!income.empty())
  {
    string message =
        "You receive " +
        Format::List<map, string, int64_t>(
            income,
            [](const pair<string, int64_t> &it) { return Format::CreditString(it.second) + ' ' + it.first; }) +
        '.';
    ...
    if(tributeIncome)
    {
      ...
      for(const auto &it : tributeReceived)
      {
        ...
        if(penalty)
        {
          ...
          if(penalty > 0.) governments.insert(gov);
        }
      }
  ...
  if(!message.empty()) Messages::Add({message, GameData::MessageCategories().Get("force log")});
}
...
bool PlayerInfo::HasClearance() const
{
  return any_of(
      missions.begin(),
      missions.end(),
      [this](const Mission &mission) -> bool { return mission.HasClearance(planet); });
}
