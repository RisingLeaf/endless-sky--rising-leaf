...
bool Drawable::HasSprite() const { return sprite && sprite->HasDimensions(); }


// Access the underlying Sprite object.
const Sprite *Drawable::GetSprite() const { return sprite; }
...
double Drawable::Radius() const { return .5 * Point(Width(), Height()).Length(); }


// Which color swizzle should be applied to the sprite?
const Swizzle *Drawable::GetSwizzle() const { return swizzle; }


bool Drawable::InheritsParentSwizzle() const { return inheritsParentSwizzle; }
...
double Drawable::Zoom() const { return max(zoom, 0.f); }


Point Drawable::Scale() const { return scale; }
...
void Drawable::LoadSprite(const DataNode &node)
{
  ...
  if(scale != Point{1., 1.}) GameData::GetMaskManager().RegisterScale(sprite, Scale());
}
...
void Drawable::SaveSprite(DataWriter &out, const string &tag) const
{
  ...
  out.BeginChild();
  {
    ...
    if(delay) out.Write("delay", delay);
    if(scale != Point{1., 1.}) out.Write("scale", scale.X(), scale.Y());
    if(randomize) out.Write("random start frame");
    if(!repeat) out.Write("no repeat");
    if(rewind) out.Write("rewind");
    if(center) out.Write("center", center.X(), center.Y());
    if(inheritsParentSwizzle) out.Write("inherits parent swizzle");
  }
  out.EndChild();
}
...
void Drawable::SetSwizzle(const Swizzle *swizzle) { this->swizzle = swizzle; }


double Drawable::Alpha() const { return alpha; }
...
void Drawable::SetFrameRate(float framesPerSecond) { frameRate = framesPerSecond / 60.f; }


// Add the given amount to the frame rate.
void Drawable::AddFrameRate(float framesPerSecond) { frameRate += framesPerSecond / 60.f; }


void Drawable::PauseAnimation() { ++pause; }
...
void Drawable::SetStep(int step) const
{
  ...
  if(step == currentStep || step < 0 || !sprite || !sprite->IsLoaded()) return;
  currentStep = step;
  ...
  if(repeat) frame = fmod(frame, cycle);
