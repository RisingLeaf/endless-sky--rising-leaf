...
namespace
{
  ...
  void PenaltyHelper(const DataNode &node, std::map<int, Government::PenaltyEffect> &penalties)
  {
    auto loadPenalty = [&penalties](const DataNode &child, int eventType) -> void
    {
      ...
      if(eventType == ShipEvent::PROVOKE) specialPenalty = Government::SpecialPenalty::PROVOKE;
      else if(eventType == ShipEvent::ATROCITY) specialPenalty = Government::SpecialPenalty::ATROCITY;
      if(child.Size() >= 3)
      {
        const std::string &effect = child.Token(2);
        if(effect == "none")
        {
        ...
        else if(effect == "provoke")
        {
          ...
          {
            child.PrintTrace(
                "The \"provoke\" effect will not work"
        ...
        else if(effect == "atrocity")
        {
          ...
          {
            child.PrintTrace(
                "The \"atrocity\" effect will not work"
  ...
  Government::PenaltyEffect PenaltyHelper(int eventType, const std::map<int, Government::PenaltyEffect> &penalties)
  {
    ...
    for(const auto &it : penalties)
    {
      if(eventType & it.first)
      {
        ...
        if(it.second.specialPenalty > penalty.specialPenalty) penalty.specialPenalty = it.second.specialPenalty;
      }
    }
    return penalty;
  }
    ...
    const std::set<const Planet *> *visitedPlanets)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    if(removeAll || overwriteAll)
    {
      ...
      else if(key == "reputation")
      {
        for(const DataNode &grand : child)
        {
          ...
          if(grandKey == "max") reputationMax = std::numeric_limits<double>::max();
          else if(grandKey == "min") reputationMin = std::numeric_limits<double>::lowest();
        }
      }
      else if(key == "raid")
      {
    ...
    else if(key == "trusted")
    {
      ...
      for(const DataNode &grand : child)
      {
        ...
        if(gov)
          if(remove) trusted.erase(gov);
          else trusted.insert(gov);
        else grand.PrintTrace("Skipping unrecognized government:");
      }
    }
    ...
    else if(key == "illegals")
    {
      ...
      for(const DataNode &grand : child)
      {
        ...
        {
          if(grand.Token(1) == "ship" && grand.Size() >= 3) illegalShips[grand.Token(2)] = 0;
          else illegalOutfits[GameData::Outfits().Get(grand.Token(1))] = 0;
        }
    ...
    else if(key == "atrocities")
    {
      ...
      for(const DataNode &grand : child)
      {
        ...
        {
          if(grand.Token(1) == "ignore universal") ignoreUniversalAtrocities = false;
        ...
        {
          if(grand.Token(1) == "ship" && grand.Size() >= 3) atrocityShips[grand.Token(2)].isAtrocity = false;
          else atrocityOutfits[GameData::Outfits().Get(grand.Token(1))].isAtrocity = false;
        }
    ...
    {
      if(add) travelRestrictions.Load(child, visitedSystems, visitedPlanets);
      else travelRestrictions = LocationFilter(child, visitedSystems, visitedPlanets);
    }
  ...
  if(reputationMin > reputationMax) reputationMin = reputationMax;
  SetReputation(Reputation());
}


// Get the display name of this government.
const std::string &Government::DisplayName() const { return displayName; }


// Set / Get the true name used for this government in the data files.
void Government::SetTrueName(const std::string &trueName) { this->trueName = trueName; }


const std::string &Government::TrueName() const { return trueName; }


// Get the color swizzle to use for ships of this government.
const Swizzle *Government::GetSwizzle() const { return swizzle; }


// Get the color to use for displaying this government on the map.
const Color &Government::GetColor() const { return *color; }
...
double Government::AttitudeToward(const Government *other) const
{
  ...
  if(attitudeToward.size() <= other->id) return defaultAttitude;
...
double Government::InitialPlayerReputation() const { return initialPlayerReputation; }
...
Government::PenaltyEffect Government::PenaltyFor(int eventType, const Government *other) const
{
  ...
  if(other == this) return PenaltyHelper(eventType, penaltyFor);
  ...
  if(it == customPenalties.end()) return PenaltyHelper(eventType, penalties);

  std::map<int, PenaltyEffect> tempPenalties = penalties;
...
double Government::GetBribeFraction() const { return bribe; }
...
double Government::GetBribeThreshold() const { return bribeThreshold; }


double Government::GetFineFraction() const { return fine; }
...
bool Government::CanEnforce(const System *system) const
{
  for(const LocationFilter &filter : enforcementZones)
    if(filter.Matches(system)) return true;
...
{
  for(const LocationFilter &filter : enforcementZones)
    if(filter.Matches(planet)) return true;
  return enforcementZones.empty();
}


const Conversation *Government::DeathSentence() const { return deathSentence; }
...
std::string Government::GetHail(bool isDisabled) const
{
  ...
  if(IsEnemy()) phrase = isDisabled ? hostileDisabledHail : hostileHail;
  else phrase = isDisabled ? friendlyDisabledHail : friendlyHail;

  return phrase ? phrase->Get() : "";
}


std::string Government::GetShipBribeAcceptanceHail() const
{
  return shipBribeAcceptanceHail ? shipBribeAcceptanceHail->Get() : "It's a pleasure doing business with you.";
}


std::string Government::GetShipBribeRejectionHail() const
{
  return shipBribeRejectionHail ? shipBribeRejectionHail->Get() : "I do not want your money.";
}


std::string Government::GetPlanetBribeAcceptanceHail() const
{
  return planetBribeAcceptanceHail ? planetBribeAcceptanceHail->Get() : "It's a pleasure doing business with you.";
}


std::string Government::GetPlanetBribeRejectionHail() const
{
  return planetBribeRejectionHail ? planetBribeRejectionHail->Get() : "I do not want your money.";
}


const Phrase *Government::TributeAlreadyPaying() const { return tributeAlreadyPaying; }


const Phrase *Government::TributeUndefined() const { return tributeUndefined; }


const Phrase *Government::TributeUnworthy() const { return tributeUnworthy; }


const Phrase *Government::TributeFleetLaunching() const { return tributeFleetLaunching; }


const Phrase *Government::TributeFleetUndefeated() const { return tributeFleetUndefeated; }


const Phrase *Government::TributeSurrendered() const { return tributeSurrendered; }


// Find out if this government speaks a different language.
const std::string &Government::Language() const { return language; }


// Find out if this government should send custom hails even if the player does not know its language.
bool Government::SendUntranslatedHails() const { return sendUntranslatedHails; }
...
const std::vector<RaidFleet> &Government::RaidFleets() const { return raidFleets; }


// Check if, according to the politics stored by GameData, this government is
// an enemy of the given government right now.
bool Government::IsEnemy(const Government *other) const { return GameData::GetPolitics().IsEnemy(this, other); }


// Check if this government is an enemy of the player.
bool Government::IsEnemy() const { return GameData::GetPolitics().IsEnemy(this, GameData::PlayerGovernment()); }


// Check if this is the player government.
bool Government::IsPlayer() const { return this == GameData::PlayerGovernment(); }
...
void Government::Bribe() const { GameData::GetPolitics().Bribe(this); }


// Check to see if the player has done anything they should be fined for.
// Each government can only fine you once per day.
std::pair<const Conversation *, std::string>
Government::Fine(PlayerInfo &player, int scan, const Ship *target, double security) const
{
...
Government::Atrocity Government::Condemns(const Outfit *outfit) const
{
  ...
  return it != atrocityOutfits.cend()
             ? it->second
...
Government::Atrocity Government::Condemns(const Ship *ship) const
{
  ...
  return it != atrocityShips.cend()
             ? it->second
             : Atrocity{!IgnoresUniversalAtrocities() && ship->BaseAttributes().Get("atrocity") > 0., nullptr};
}


bool Government::IgnoresUniversalAtrocities() const { return ignoreUniversalAtrocities; }
...
int Government::Fines(const Outfit *outfit) const
{
  ...
  for(const auto &it : illegalOutfits)
    if(it.first == outfit) return it.second;
...
int Government::Fines(const Ship *ship) const
{
  ...
  for(const auto &it : illegalShips)
    if(it.first == ship->TrueModelName()) return it.second;
  return IgnoresUniversalIllegals() ? 0 : ship->BaseAttributes().Get("illegal");
}


bool Government::IgnoresUniversalIllegals() const { return ignoreUniversalIllegals; }


bool Government::FinesContents(const Ship *ship) const
{
  for(auto &it : ship->Outfits())
    if(this->Fines(it.first) || this->Condemns(it.first).isAtrocity) return true;
...
double Government::Reputation() const { return GameData::GetPolitics().Reputation(this); }


double Government::ReputationMax() const { return reputationMax; }


double Government::ReputationMin() const { return reputationMin; }


void Government::AddReputation(double value) const { GameData::GetPolitics().AddReputation(this, value); }


void Government::SetReputation(double value) const { GameData::GetPolitics().SetReputation(this, value); }


double Government::CrewAttack() const { return crewAttack; }


double Government::CrewDefense() const { return crewDefense; }


bool Government::IsProvokedOnScan() const { return provokedOnScan; }
