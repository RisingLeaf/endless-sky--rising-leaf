...
Command::Command(const SDL_Keycode keycode)
{
  const auto it = commandForKeycode.find(keycode);
  if(it != commandForKeycode.end()) *this = it->second;
}
...
void Command::ReadKeyboard()
{
  ...
  for(const auto &it : keycodeForCommand)
    if(keyDown[SDL_GetScancodeFromKey(it.second, nullptr)]) *this |= it.first;

  // Check whether the `Shift` modifier key was pressed for this step.
  if(SDL_GetModState() & SDL_KMOD_SHIFT) *this |= SHIFT;
}
...
void Command::SaveSettings(const std::filesystem::path &path)
{
  ...
  for(const auto &it : keycodeForCommand)
  {
    ...
    if(dit != description.end()) out.Write(dit->second, it.second);
  }
}
...
bool Command::HasBinding() const
{
  ...
  if(it == keyName.end()) return false;
...
bool Command::HasConflict() const
{
  ...
  if(it == keycodeForCommand.end()) return false;
...
void Command::Load(const DataNode &node)
{
  for(int i = 1; i < node.Size(); ++i)
  {
    ...
    if(it != lookup.end()) Set(it->second);
    else node.PrintTrace("Skipping unrecognized command \"" + node.Token(i) + "\":");
  }
}
...
void Command::Clear(Command command) { state &= ~command.state; }


// Set any commands that are set in the given command.
void Command::Set(Command command) { state |= command.state; }
...
Command Command::And(Command command) const { return Command(state & command.state); }


// Get the commands that are set in this and not in the given command.
Command Command::AndNot(Command command) const { return Command(state & ~command.state); }


// Set the turn direction and amount to a value between -1 and 1.
void Command::SetTurn(double amount) { turn = std::max(-1., std::min(1., amount)); }
...
Command::operator bool() const { return !!*this; }


// Check whether this command is entirely empty.
bool Command::operator!() const { return !state && !turn; }


// For sorting commands (e.g. so a command can be the key in a map):
bool Command::operator<(const Command &command) const { return state < command.state; }
...
{
  state |= command.state;
  if(command.turn) turn = command.turn;
  return *this;
}
...
Command::Command(uint64_t state, const std::string &text) : state(state)
{
