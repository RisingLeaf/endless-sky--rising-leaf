...
// Get a random angle.
Angle Angle::Random() { return Angle(static_cast<int32_t>(Random::Int(STEPS))); }
...
Angle Angle::operator-() const { return Angle((-angle) & MASK); }


bool Angle::operator==(const Angle &other) const { return angle == other.angle; }
...
Point Angle::Unit() const { return unitCache[angle]; }
...
double Angle::AbsDegrees() const { return angle / DEG_TO_STEP; }
