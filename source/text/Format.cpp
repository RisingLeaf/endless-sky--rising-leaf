...
namespace
{
  ...
  static const vector<pair<const char *, int64_t>> WORD_NUMBERS         = {
      ...
      {"million", K *K},
      {"thousand", K}};
  static const vector<const char *> ONES_NAMES = {"zero ",    "one ",     "two ",       "three ",    "four ",
  ...
  ostream &operator<<(ostream &o, const Wrapped &num)
  {
    ...
    if(num.value < 0) return o << "negative " << Wrapped{-num.value};
    ...
    if(remaining.value < 20) return o << ONES_NAMES[remaining.value];

    o << TENS_NAMES[remaining.value / 10];
    int64_t ones = remaining.value % 10;
    if(ones) return o << '-' << ONES_NAMES[ones];
    return o << ' ';
  }
  ...
  string MLAShorthand(int64_t value)
  {
    ...
    for(size_t magnitude = 0; magnitude < WORD_NUMBERS.size() - 1; ++magnitude)
    {
      ...
      if(above < 1000) continue;
      if(above >= 1'000'000 || !(above % 1000)) break;
      if(below) continue;
      const size_t BUFLEN      = 100;
      char         buf[BUFLEN] = {0};
      snprintf(buf, BUFLEN, "%s%.3f %s", negative ? "negative " : "", above / 1000.0, WORD_NUMBERS[magnitude].first);
      buf[BUFLEN - 1] = '\0';
      return string(buf);
    }
    return string();
  ...
  {
    if(!places || (!fraction && trimTrailingZeros)) return;
  ...
  string StringSubstituter(const string &source, function<const string *(const string &)> SubstitutionFor)
  {
    string target;
      ...
      unordered_set<string>     &keysBeingExpanded)
  {
    ...
    auto SubstitutionFor = [&](const string &request) -> const string *
    {
      ...
      if(hasResult != result.end()) return &hasResult->second;
      // Refuse to traverse a cycle in the graph.
      if(keysBeingExpanded.find(request) != keysBeingExpanded.end()) return nullptr;
      auto hasSource = source.find(request);
      // Undefined key.
      if(hasSource == source.end()) return nullptr;
  ...
  void AppendCondition(
      string                        &result,
      const string                  &source,
      const Format::ConditionGetter &getter,
      size_t                         formatStart,
      size_t                         formatSize,
  ...
  const char *TimestampFormatString()
  {
    ...
    {
    case Preferences::DateFormat::YMD: return DEFAULT_TIMESTAMP_FORMAT;
...
string Format::AbbreviatedNumber(int64_t value)
{
  ...
  static constexpr array<pair<int64_t, char>, 3> THRESHOLD_SUFFIX = {
      {{1'000'000'000'000ll, 'T'}, {1'000'000'000ll, 'B'}, {1'000'000ll, 'M'}}};
...
{
  if(value >= 10'000) return AbbreviatedNumber(value);
...
{
  if(amount == 1) return "1 ton";
  return Format::Number(amount) + " tons";
}


// Creates a string similar to '<amount> tons of <cargo>'.
string Format::CargoString(double amount, const string &cargo) { return MassString(amount) + " of " + cargo; }
...
{
  string result = std::to_string(amount) + ' ' + noun;
  if(amount != 1 && amount != -1) result += 's';
  return result;
}


string Format::StepsToSeconds(size_t steps) { return Number(steps / 60.) + " s"; }
...
string Format::PlayTime(double timeVal)
{
  ...
  do
  {
    ...
    if(i < PERIOD.size()) timeValFormat /= PERIOD[i];
    i++;
...
string Format::TimestampString(filesystem::file_time_type time)
{
  auto sctp = time_point_cast<chrono::system_clock::duration>(
      time - filesystem::file_time_type::clock::now() + chrono::system_clock::now());
...
string Format::AmmoCount(int64_t value)
{
  if(fabs(value) >= SCIENTIFIC_THRESHOLD)
  {
    if(abs(value) == SCIENTIFIC_THRESHOLD) return "1e+15";
    ostringstream out;
    out.precision(1);
    out << static_cast<double>(value);
    return out.str();
  ...
  if(absolute < 10'000) return std::to_string(value);
  ...
  static constexpr array<pair<int64_t, char>, 4> THRESHOLD_SUFFIX = {
      {{1'000'000'000'000ll, 'T'}, {1'000'000'000ll, 'B'}, {1'000'000ll, 'M'}, {1000ll, 'k'}}};
  for(const auto &[threshold, suffix] : THRESHOLD_SUFFIX)
  {
    if(absolute >= threshold)
    {
...
string Format::Number(double value, optional<int> decimalPlaces, bool trimTrailingZeros)
{
  ...
  int places = decimalPlaces.value_or(value >= 10'000 ? 0 : (value >= 1000 ? 1 : 2));
  if(places)
  {
...
string Format::WordForm(int64_t value, bool startOfSentence)
{
  ...
  o << Wrapped{value};
  string result = o.str();
  if(result.size() > 0 && result[result.size() - 1] == ' ') result.resize(result.size() - 1);
  if(!result.empty() && startOfSentence && result[0] >= 'a' && result[0] <= 'z') result[0] -= 32;
  return result;
}
...
{
  if(startOfSentence) return WordForm(value, true);
  if(value < 1000 && value > -1000 && !(value % 100)) return WordForm(value, startOfSentence);
  int64_t above = value, below = 0;
  for(int i = 0; above && i < 6; i++)
  {
    if(below) break;
    else if(above < 100 && above > -100) return WordForm(value, startOfSentence);
    else if(above < 1000 && above > -1000 && !(above % 100)) return WordForm(value, startOfSentence);
    below  = above % 1000;
    above /= 1000;
  }
...
{
  if(startOfSentence) return WordForm(value, true);
  if(value >= -99 && value <= 99) return WordForm(value, startOfSentence);

  // 21350000 => 21.35 million
  string shorthand = MLAShorthand(value);
  if(!shorthand.empty()) return shorthand;

  int64_t above = value, below = 0;
  for(int i = 0; above && i < 6; i++)
  {
    if(below) break;
    else if(above <= 10 && above >= -10) return WordForm(value, startOfSentence);
    else if(above < 100 && above > -100 && !(above % 10)) return WordForm(value, startOfSentence);
    below  = above % 1000;
    above /= 1000;
  }
  return Format::Number(value);
...
double Format::Parse(const string &str)
{
  ...
  if(it != end)
  {
    if(*it == 'k' || *it == 'K') value *= 1e3;
    else if(*it == 'm' || *it == 'M') value *= 1e6;
    else if(*it == 'b' || *it == 'B') value *= 1e9;
    else if(*it == 't' || *it == 'T') value *= 1e12;
  }
...
void Format::Expand(map<string, string> &keys)
{
  ...
  for(auto it = keys.begin(); it != keys.end(); ++it)
    if(newKeys.find(it->first) == newKeys.end()) ExpandInto(it->first, it->second, keys, newKeys, keysBeingExpanded);
  keys.swap(newKeys);
}
...
string Format::Capitalize(const string &str)
{
  ...
  for(char &c : result)
  {
    if(isspace(static_cast<unsigned char>(c)))
    {
      ...
      if(first && islower(static_cast<unsigned char>(c))) c = toupper(static_cast<unsigned char>(c));
      first = false;
    }
  }
  return result;
...
string Format::ExpandConditions(const string &source, const ConditionGetter &getter)
{
  ...
  if(source.find('&') == string::npos) return source;
  ...
  if(start < source.size()) result.append(source, start, string::npos);
  return result;
}
