...
namespace
{
  ...
  std::pair<double, double> SortAngle(const Point &reference, const Point &point)
  {
    ...
    if(!rotated.Y()) angle = copysign(angle, rotated.X() - reference.Dot(reference));
...
MapDetailPanel::MapDetailPanel(const MapPanel &panel, bool isStars) : MapPanel(panel), isStars(isStars)
{
  Audio::Pause();
...
void MapDetailPanel::Step()
{
  ...
  if(selectedSystem != shownSystem) GeneratePlanetCards(*selectedSystem);
  ...
  if(!player.GetPlanet()) DoHelp("map");
}
...
double MapDetailPanel::GetScroll() const { return scroll.AnimatedValue(); }


double MapDetailPanel::PlanetPanelHeight() { return planetPanelHeight; }


bool MapDetailPanel::Hover(int x, int y)
{
  const Interface *planetCardInterface = GameData::Interfaces().Get("map planet card");
  isPlanetViewSelected =
      (x < Screen::Left() + planetCardInterface->GetValue("width") && y < Screen::Top() + PlanetPanelHeight());
...
{
  if(scroll.Scrollable() && scrollbar.SyncDrag(scroll, dx, dy)) return true;
...
bool MapDetailPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
  ...
  if(command.Has(Command::HELP))
  {
    ...
    if(!player.GetPlanet()) DoHelp("map", true);
  }
  else if((key == SDLK_TAB || command.Has(Command::JUMP)) && player.Flagship())
  {
    ...
    if(!plan.empty() && !(mod & SDL_KMOD_SHIFT))
    {
    ...
    const std::set<const System *> &links =
        hasJumpDrive ? source->JumpNeighbors(player.Flagship()->JumpNavigation().JumpRange()) : source->Links();
    ...
    for(const System *it : links)
    {
      ...
      if(!it->IsValid() || it == original) continue;
      if(!player.HasSeen(*it)) continue;
      if(!(hasJumpDrive || player.CanView(*it) || player.CanView(*source))) continue;
  ...
  else if(key == SDLK_DOWN)
  {
    if(!isPlanetViewSelected)
    {
      if(commodity < 0 || commodity == 9) SetCommodity(0);
      else SetCommodity(commodity + 1);
    }
    else {
      ...
      for(auto &card : planetCards)
      {
        if(selectNext)
        {
          ...
          if(space < planetCardHeight) scroll.Scroll(planetCardHeight - space);
          break;
        }
        // We have this one selected, the next one will be selected instead.
        else if(card.IsSelected())
        {
          if(!planetCards.back().IsSelected()) selectNext = true;
          card.Select(false);
        }
      }
  ...
  else if(key == SDLK_UP)
  {
    if(!isPlanetViewSelected)
    {
      ...
      for(auto &card : planetCards)
      {
        if(card.IsSelected())
        {
          if(!planetCards.front().IsSelected()) anySelected = true;
          previousCard->Select();
          card.Select(false);
          double space = previousCard->AvailableSpace();
          if(space < planetCardHeight) scroll.Scroll(space - planetCardHeight);
          break;
        }
        previousCard = &card;
      }
...
{
  if(scroll.Scrollable() && scrollbar.SyncClick(scroll, x, y, button, clicks)) return true;
  ...
  if(button == MouseButton::RIGHT)
  {
    ...
    else if(x >= Screen::Right() - 240 && y >= Screen::Top() + 10 && y <= Screen::Top() + 270)
    {
      ...
      if(uiClick.Length() > 130) return true;

      // Only issue movement orders if the player is in-flight.
      if(player.GetPlanet()) GetUI().Push(DialogPanel::Info("You cannot issue fleet movement orders while docked."));
      ...
      else player.SetEscortDestination(selectedSystem, uiClick / scale);
    }
    return true;
  }
  ...
  if(y <= Screen::Top() + planetPanelHeight + 30 && x <= Screen::Left() + planetCardWidth + arrowOffset + 10)
  {
    for(auto &card : planetCards)
    {
      ...
      else if(clickAction != MapPlanetCard::ClickAction::NONE)
      {
        ...
        if(selectedPlanet && player.Flagship()) player.SetTravelDestination(selectedPlanet);
        if(clickAction != MapPlanetCard::ClickAction::SELECTED) SetCommodity(static_cast<int>(clickAction));
      }
    }
    return true;
  }
    ...
    for(const auto &it : planets)
    {
      ...
      if(d < distance)
      {
        ...
        for(auto &planetCard : planetCards)
        {
          ...
          if(planetCard.IsSelected()) scroll.Set(place * planetCardHeight);
          ++place;
        }
      }
    }
  ...
  if(selectedPlanet && !selectedPlanet->IsInSystem(selectedSystem)) selectedPlanet = nullptr;
  return true;
}


void MapDetailPanel::Resize() { ResizeTextArea(); }
...
void MapDetailPanel::GeneratePlanetCards(const System &system)
{
  ...
  for(const StellarObject &object : system.Objects())
  {
    if(object.HasSprite() && object.HasValidPlanet())
    {
      ...
      if(planet->IsWormhole() || !planet->IsAccessible(player.Flagship()) || shown.contains(planet)) continue;
...
void MapDetailPanel::DrawKey()
{
  ...
  if(commodity >= 0)
  {
    ...
    if(static_cast<unsigned>(commodity) >= commodities.size()) return;
  ...
  else if(commodity >= SHOW_OUTFITTER)
  {
    ...
    static const std::string LABEL[2][4] = {{"10+", "5", "1", "None"}, {"60+", "30", "1", "None"}};
  ...
  {
    static const std::string LABEL[3] = {"All planets", "Some", "None"};
    for(int i = 0; i < 3; ++i)
    {
  ...
  else if(commodity == SHOW_GOVERNMENT)
  {
    ...
    for(const auto &it : distances)
    {
      ...
      auto foundIt = find(alreadyDisplayed.begin(), alreadyDisplayed.end(), make_pair(displayName, displayColor));
      if(foundIt != alreadyDisplayed.end()) continue;
...
void MapDetailPanel::DrawInfo()
{
  ...
  planetPanelHeight =
      canView ? std::min(std::min(minimumSize, maxPlanetPanelHeight), (planetCards.size()) * planetCardHeight) : 0.;
  ...
  if(canView && !planetCards.empty())
  {
    ...
    if(scroll.Scrollable())
    {
      ...
      Point top(Screen::Left() + planetWidth + arrowOffsetX, Screen::Top() + arrowOffsetY);
      Point bottom(Screen::Left() + planetWidth + arrowOffsetX, Screen::Top() - arrowOffsetY + planetPanelHeight);
  ...
  const float   alertScale  = std::min<float>(
      1.f,
      std::min<double>(textMargin, font.Height()) / std::max(alertSprite->Width(), alertSprite->Height()));
  SpriteShader::Draw(alertSprite, uiPoint + Point(-textMargin / 2., -7. + font.Height() / 2.), alertScale);

  std::string systemName = player.KnowsName(*selectedSystem) ? selectedSystem->DisplayName() : "Unexplored System";
  ...
  clickZones.emplace_back(
      Rectangle::FromCorner(Point(Screen::Left(), governmentY - 30), Point(mapInterface->GetValue("text margin"), 30)),
      SHOW_DANGER);
  ...
  clickZones.emplace_back(
      Rectangle::FromCorner(
          ...
          Point(160 - mapInterface->GetValue("text margin"), 30)),
      SHOW_REPUTATION);

  // Add the government click zone.
  clickZones.emplace_back(Rectangle::FromCorner(Point(Screen::Left(), governmentY), Point(160, 25)), SHOW_GOVERNMENT);
  ...
  if(!noCompare && canView && otherIsInhabited)
  {
    for(const Trade::Commodity &commodity : GameData::Commodities())
    {
      ...
      {
        value -= localValue;
        if(value < lowCompare) lowCompare = value;
        if(value > highCompare) highCompare = value;
      }
    }
  }
  ...
  for(const Trade::Commodity &commodity : GameData::Commodities())
  {
    ...
    if(canView && otherIsInhabited)
    {
      ...
      else if(noCompare || player.GetSystem() == selectedSystem || !localValue)
      {
        ...
        value -= localValue;
        if(Preferences::Has("Show parenthesis")) price += "(";
        if(value > 0) price += '+';
        price += std::to_string(value);
        if(Preferences::Has("Show parenthesis")) price += ")";
      }
      ...
      if(!noCompare && player.GetSystem() != selectedSystem)
      {
        ...
        if(halfCompare < 1) halfCompare = 1;
        RingShader::Draw(
            uiPoint + Point(143, 8),
            OUTER,
            INNER,
    ...
    if(isSelected) PointerShader::Draw(uiPoint + Point(0., 7.), Point(1., 0.), 10.f, 10.f, 0.f, color);

    uiPoint.Y() += 20.;
    ++i;
  }

  if(selectedPlanet && !selectedPlanet->Description().IsEmptyFor() && player.HasVisited(*selectedPlanet) &&
     !selectedPlanet->IsWormhole())
  {
    const Sprite *panelSprite = SpriteSet::Get("ui/description panel");
    Point         pos(
        Screen::Right() - descriptionXOffset - .5f * panelSprite->Width(),
  ...
  if(player.HasEscortDestination()) mapInterfaceInfo.SetCondition("has escort destination");
...
{
  if(commodity == SHOW_STARS && !player.CanView(*selectedSystem)) return;
  ...
  if(!player.CanView(*selectedSystem)) return;
  ...
  if(maxDistance > 115.) scale *= 115. / maxDistance;
  ...
  for(const StellarObject &object : selectedSystem->Objects())
  {
    ...
    Color color(
        std::max(0.f, rgb[0] * 1.2f - .2f),
        std::max(0.f, rgb[1] * 1.2f - .2f),
        std::max(0.f, rgb[2] * 1.2f - .2f),
        1.f);
  ...
  for(const StellarObject &object : selectedSystem->Objects())
  {
    ...
    {
      RingShader::Draw(
          orbitCenter + object.Position() * scale,
          object.Radius() * scale + 5.,
          object.Radius() * scale + 4.,
          habitColor[6]);
    }
  }
...
{
  commodity = index;
  if(index != SHOW_STARS) isStars = false;
