...
void MenuPanel::Step()
{
  ...
  {
    scroll += scrollSpeed;
    if(scroll < 0) scroll = (20 * static_cast<long long int>(credits.size()) + 299) * SCROLL_MOD;
    if(scroll >= (20 * static_cast<long long int>(credits.size()) + 300) * SCROLL_MOD) scroll = 0;
  }
}
...
void MenuPanel::Draw()
{
  ...
  if(player.IsLoaded() && !player.IsDead())
  {
    ...
    if(!player.GetGamerules().LockGamerules()) info.SetCondition("gamerules unlocked");
    ...
    if(player.GetSystem()) info.SetString("system", player.GetSystem()->DisplayName());
    if(player.GetPlanet()) info.SetString("planet", player.GetPlanet()->DisplayName());
...
void MenuPanel::DrawCredits() const
{
  ...
  {
    float fade = 1.f;
    if(y < top + 20) fade = std::max(0.f, (y - top) / 20.f);
    else if(y > bottom - 20) fade = std::max(0.f, (bottom - y) / 20.f);
    if(fade)
    {
