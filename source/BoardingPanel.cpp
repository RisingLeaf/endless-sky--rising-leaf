...
BoardingPanel::BoardingPanel(PlayerInfo &player, const std::shared_ptr<Ship> &victim) :
  player(player), you(player.FlagshipPtr()), victim(victim), attackOdds(*you, *victim), defenseOdds(*victim, *you)
{
  Audio::Pause();
  ...
  for(const auto &it : victim->Cargo().Commodities())
    if(it.second) plunder.emplace_back(it.first, it.second, system.Trade(it.first));
  ...
  while(sit != victim->Outfits().end() || cit != victim->Cargo().Outfits().end())
  {
    ...
    bool shipIsFirst =
        (cit == victim->Cargo().Outfits().end() || (sit != victim->Outfits().end() && sit->first <= cit->first));
    bool cargoIsFirst =
        (sit == victim->Outfits().end() || (cit != victim->Cargo().Outfits().end() && cit->first <= sit->first));
    if(shipIsFirst)
    {
      outfit = sit->first;
      // Don't include outfits that are installed and unplunderable. But,
      // "unplunderable" outfits can still be stolen from cargo.
      if(!sit->first->Get("unplunderable")) count += sit->second;
      ++sit;
    }
    if(cargoIsFirst)
    {
    ...
    if(outfit && count) plunder.emplace_back(outfit, count);
  }
...
BoardingPanel::~BoardingPanel() { Audio::Resume(); }


void BoardingPanel::Step() { scroll.Step(); }
...
void BoardingPanel::Draw()
{
  ...
  for(; y < endY && static_cast<unsigned>(index) < plunder.size(); y += 20, ++index)
  {
    ...
    if(isSelected) FillShader::Fill(Point(-155., y + 10.), Point(360., 20.), back);
  ...
  Information info;
  if(CanExit()) info.SetCondition("can exit");
  if(CanTake() == CanTakeResult::CAN_TAKE) info.SetCondition("can take");
  if(CanCapture()) info.SetCondition("can capture");
  if(CanAttack() && (you->Crew() > 1 || !victim->RequiredCrew())) info.SetCondition("can attack");
  if(CanAttack()) info.SetCondition("can defend");
  ...
  if(you)
  {
    ...
    if(canCapture)
    {
      ...
      info.SetString("your attack", Format::Number(attackOdds.AttackerPower(crew), 1, false));
      info.SetString("your defense", Format::Number(defenseOdds.DefenderPower(crew), 1, false));
    }
  }
  ...
  if(victim && (canCapture || victim->IsYours()))
  {
    ...
    info.SetString("enemy attack", Format::Number(defenseOdds.AttackerPower(vCrew), 1, false));
    info.SetString("enemy defense", Format::Number(attackOdds.DefenderPower(vCrew), 1, false));
  }
  if(victim && canCapture && !victim->IsYours())
  {
    ...
    if(!isCapturing) odds *= (1. - victim->Attributes().Get("self destruct"));
    info.SetString("attack odds", Format::Percentage(odds, 1, false));
    info.SetString("attack casualties", Format::Number(attackOdds.AttackerCasualties(crew, vCrew), 1, false));
    info.SetString("defense odds", Format::Percentage(1. - defenseOdds.Odds(vCrew, crew), 1, false));
    info.SetString("defense casualties", Format::Number(defenseOdds.DefenderCasualties(vCrew, crew), 1, false));
  }
  ...
  if(scroll.Scrollable())
    scrollBar.SyncDraw(scroll, plunderList.TopRight() + Point{0., 10.}, plunderList.BottomRight() - Point{0., 10.});
...
{
  if((key == 'd' || key == 'x' || key == SDLK_ESCAPE || (key == 'w' && (mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI)))) &&
     CanExit())
  {
    // When closing the panel, mark the player dead if their ship was captured.
    if(playerDied) player.Die();
    GetUI().Pop(this);
  }
  else if(playerDied)
  {
  ...
  else if(key == 't')
  {
    ...
    {
      std::string message;
      if(canTake == CanTakeResult::TARGET_YOURS) message = "You cannot plunder your own ship.";
      else if(canTake == CanTakeResult::NO_SELECTION) message = "No item selected.";
      ...
      else message = "You cannot plunder now.";
    ...
    if(count == plunder[selected].Count())
    {
      ...
      if(plunder.size() && selected == static_cast<int>(plunder.size())) --selected;
      scroll.SetMaxValue(std::max(0., 20. * plunder.size()));
    }
    else {
      ...
      !isCapturing && (key == SDLK_UP || key == SDLK_DOWN || key == SDLK_PAGEUP || key == SDLK_PAGEDOWN ||
  ...
  else if(key == 'c' && CanCapture())
  {
    ...
    if(Random::Real() < victim->Attributes().Get("self destruct"))
    {
      ...
      GetUI().Push(
          DialogPanel::Info(
              "The moment you blast through the airlock, a series of explosions "
  ...
  else if((key == 'a' || key == 'd' || key == 'D') && CanAttack())
  {
    ...
    if(key == 'a' && !youAttack) return true;
    bool enemyAttacks = defenseOdds.Odds(enemyStartCrew, yourStartCrew) > .5;
    if(isFirstCaptureAction && !youAttack) enemyAttacks = false;
    ...
    if(!youAttack && !enemyAttacks)
    {
      ...
      if(youAttack && enemyAttacks) messages.push_back("You both attack. ");
      else if(youAttack) messages.push_back("You attack. ");
      else if(enemyAttacks) messages.push_back("They attack. ");

      if(yourCasualties && enemyCasualties)
      {
        messages.back() +=
            "You lose " + std::to_string(yourCasualties) + " crew; they lose " + std::to_string(enemyCasualties) + ".";
      }
      ...
      else if(!victim->Crew())
      {
        ...
        if(crewTransferred > 0)
        {
          ...
          if(crewTransferred == 1) transferMessage += " has";
          else transferMessage += "s have";
...
bool BoardingPanel::Click(int x, int y, MouseButton button, int clicks)
{
  ...
  if(x >= -330 && x < 20 && y >= -180 && y < 60)
  {
    ...
    if(static_cast<unsigned>(index) < plunder.size()) selected = index;
    return true;
  }
...
bool BoardingPanel::Scroll(double dx, double dy) { return Drag(0., dy * Preferences::ScrollSpeed()); }
...
int BoardingPanel::Plunder::Count() const { return count; }


// Get the value of each unit of this plunder item.
int64_t BoardingPanel::Plunder::UnitValue() const { return unitValue; }


// Get the name of this item. If it is a commodity, this is its name.
const std::string &BoardingPanel::Plunder::Name() const { return name; }


// Get the mass, in the format "<count> x <unit mass>". If the count is
// 1, only the unit mass is reported.
const std::string &BoardingPanel::Plunder::Size() const { return size; }


// Get the total value (unit value times count) as a string.
const std::string &BoardingPanel::Plunder::Value() const { return value; }


// If this is an outfit, get the outfit. Otherwise, this returns null.
const Outfit *BoardingPanel::Plunder::GetOutfit() const { return outfit; }
...
bool BoardingPanel::Plunder::CanTake(const Ship &ship) const
{
  ...
  if(ship.Cargo().Free() >= mass) return true;
  ...
  if(outfit)
  {
    for(const auto &it : ship.Outfits())
      if(it.first != outfit && it.first->AmmoStoredOrUsed() == outfit && ship.Attributes().CanAdd(*outfit)) return true;
  }
...
{
  double mass = UnitMass();
  if(count == 1) size = Format::Number(mass);
  else size = std::to_string(count) + " x " + Format::Number(mass);
...
double BoardingPanel::Plunder::UnitMass() const { return outfit ? outfit->Mass() : 1.; }


// You can't exit this panel if you're engaged in hand to hand combat.
bool BoardingPanel::CanExit() const { return !isCapturing; }
...
BoardingPanel::CanTakeResult BoardingPanel::CanTake() const
{
  ...
  if(!you->IsYours()) return CanTakeResult::OTHER;
  if(victim->IsYours()) return CanTakeResult::TARGET_YOURS;
  if(isCapturing || playerDied) return CanTakeResult::OTHER;
  if(static_cast<unsigned>(selected) >= plunder.size()) return CanTakeResult::NO_SELECTION;
...
bool BoardingPanel::CanCapture() const
{
  ...
  if(isCapturing || playerDied) return false;

  // If your ship or the other ship has been captured:
  if(!you->IsYours()) return false;
  if(victim->IsYours()) return false;
...
bool BoardingPanel::CanAttack() const { return isCapturing; }
