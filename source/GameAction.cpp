...
namespace
{
  void DoGift(PlayerInfo &player, const Outfit *outfit, int count, UI *ui)
  {
    ...
    if(mapSize > 0)
    {
      ...
      if(!player.HasMapped(mapSize, mapMinables)) player.Map(mapSize, mapMinables);
    ...
    if(!flagship || !count || nameWas.empty()) return;
    ...
    message += nameWas;
    if(count > 0) message += " added to your ";
    else message += " removed from your ";
    ...
    if(didCargo && didShip) message += "cargo hold and your flagship.";
    else if(didCargo) message += "cargo hold.";
    else message += "flagship.";
...
GameAction::GameAction(const DataNode &node, const ConditionsStore *playerConditions) { Load(node, playerConditions); }
...
void GameAction::LoadSingle(const DataNode &child, const ConditionsStore *playerConditions)
{
  ...
  if(key == "remove" && child.Size() >= 3 && child.Token(1) == "log")
  {
    ...
    if(child.Size() > 3) type.push_back(child.Token(3));
  }
  else if(key == "log")
  {
    ...
    if(child.Size() < 3 || (child.Size() == 3 && child.Token(1) == "scene")) logEntries.Load(child, 1);
    else if(child.Size() >= 3) specialLogEntries[child.Token(1)][child.Token(2)].Load(child, 3);
  }
  ...
  else if(key == "outfit" && hasValue)
  {
    ...
    if(count) giftOutfits[GameData::Outfits().Get(child.Token(1))] = count;
    else child.PrintTrace("Skipping invalid outfit quantity:");
  }
  else if(key == "payment")
  {
    if(child.Size() == 1) paymentMultiplier += 150;
    if(child.Size() >= 2) payment += child.Value(1);
    if(child.Size() >= 3) paymentMultiplier += child.Value(2);
  }
  else if(key == "fine" && hasValue)
  {
    int64_t value = child.Value(1);
    if(value > 0) fine += value;
    else child.PrintTrace("Skipping invalid \"fine\" with non-positive value:");
  }
  else if(key == "debt" && hasValue)
  {
    ...
    for(const DataNode &grand : child)
    {
      ...
      if(grandKey == "term" && grandHasValue) debtEntry.term = std::max<int>(1, grand.Value(1));
      else if(grandKey == "interest" && grandHasValue) debtEntry.interest = std::clamp(grand.Value(1), 0., 0.999);
      else grand.PrintTrace("Skipping unrecognized \"debt\" attribute:");
    }
  }
  else if(key == "event" && hasValue)
  {
    ...
    if(maxDays < minDays) std::swap(minDays, maxDays);
  ...
  {
    if(hasValue) messages.push_back(ExclusiveItem<Message>{GameData::Messages().Get(child.Token(1))});
    else messages.push_back(ExclusiveItem<Message>{Message{child}});
  }
  else {
...
void GameAction::Save(DataWriter &out) const
{
  ...
  {
    if(it.second.empty()) out.Write("remove", "log", it.first);
    else
  ...
  if(payment) out.Write("payment", payment);
  if(fine) out.Write("fine", fine);
  for(auto &&debtEntry : debt)
  {
    out.Write("debt", debtEntry.amount);
    out.BeginChild();
    {
      if(debtEntry.interest) out.Write("interest", *debtEntry.interest);
  ...
  for(const std::string &name : fail)
    out.Write("fail", name);
  if(failCaller) out.Write("fail");
  if(music.has_value())
  {
    if(music->empty()) out.Write("mute");
    else out.Write("music", music.value());
  }
...
std::string GameAction::Validate() const
{
  ...
  for(auto &&event : events)
  {
    ...
    if(!reason.empty()) return "event \"" + event.first->TrueName() + "\" - Reason: " + reason;
  }

  // Transferred content must be defined & valid.
  for(auto &&it : giftShips)
    if(!it.ShipModel()->IsValid()) return "gift ship model \"" + it.ShipModel()->VariantName() + "\"";
  for(auto &&outfit : giftOutfits)
    if(!outfit.first->IsDefined()) return "gift outfit \"" + outfit.first->TrueName() + "\"";

  // Marked and unmarked system must be valid.
  for(auto &&system : mark)
    if(!system->IsValid()) return "system \"" + system->TrueName() + "\"";
  for(const auto &[mission, marks] : markOther)
  {
    for(const System *system : marks)
      if(!system->IsValid()) return "system \"" + system->TrueName() + "\"";
  ...
  for(const auto &message : messages)
    if(!message->IsLoaded()) return "message \"" + message->TrueName() + "\"";

  return "";
}


bool GameAction::IsEmpty() const noexcept { return isEmpty; }


int64_t GameAction::Payment() const noexcept { return payment; }


int64_t GameAction::Fine() const noexcept { return fine; }


const std::map<const Outfit *, int> &GameAction::Outfits() const noexcept { return giftOutfits; }


const std::vector<ShipManager> &GameAction::Ships() const noexcept { return giftShips; }


// Perform the specified tasks.
void GameAction::Do(PlayerInfo &player, UI *ui, const Mission *caller) const
{
  if(!logEntries.IsEmpty()) player.AddLogEntry(logEntries);
  ...
  for(auto &&it : specialLogClear)
  {
    if(it.second.empty()) player.RemoveSpecialLog(it.first);
    else
      for(auto &&jt : it.second)
  ...
  for(auto &&it : giftShips)
    if(!it.Giving()) it.Do(player);
  for(auto &&it : giftOutfits)
    if(it.second < 0) DoGift(player, it.first, it.second, ui);
  ...
  if(giftedItem && player.GetPlanet()) SpriteLoadManager::SetRecheckThumbnails();
  ...
  if(payment)
  {
    ...
    if(account + payment >= 0) player.Accounts().AddCredits(payment);
    else if(account > 0) player.Accounts().AddCredits(-account);
  ...
  if(fine) player.Accounts().AddFine(fine);
  ...
  if(!fail.empty() || !markOther.empty() || !unmarkOther.empty())
  {
    for(const Mission &mission : player.Missions())
    {
      ...
      if(fail.contains(mission.TrueName())) player.FailMission(mission);

      auto mit = markOther.find(mission.TrueName());
      if(mit != markOther.end()) mission.Mark(mit->second);

      auto uit = unmarkOther.find(mission.TrueName());
      if(uit != unmarkOther.end()) mission.Unmark(uit->second);
    }
  }
  ...
  if(failCaller && caller) player.FailMission(*caller);
  if(music.has_value())
  {
    if(*music == "<ambient>")
      if(player.GetPlanet()) Audio::PlayMusic(player.GetPlanet()->MusicName());
      else Audio::PlayMusic(player.GetSystem()->MusicName());
    else Audio::PlayMusic(music.value());
  }
...
GameAction GameAction::Instantiate(std::map<std::string, std::string> &subs, int jumps, int payload) const
{
  ...
  if(result.payment) subs["<payment>"] = Format::CreditString(abs(result.payment));

  result.fine = fine;
  if(result.fine) subs["<fine>"] = Format::CreditString(result.fine);
  ...
  for(const auto &it : messages)
    if(it->IsPhrase()) result.messages.push_back(it);
    else result.messages.push_back(ExclusiveItem<Message>{{it->Text(subs), it->GetCategory()}});
