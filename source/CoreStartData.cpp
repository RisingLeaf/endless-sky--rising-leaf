...
void CoreStartData::Load(const DataNode &node)
{
  ...
  for(const DataNode &child : node)
  {
    ...
    if(!LoadChild(child, add)) child.PrintTrace("Skipping unrecognized attribute:");
  }
}
...
Date CoreStartData::GetDate() const { return date ? date : Date(16, 11, 3013); }
...
{
  if(system && system->IsValid()) return *system;
...
const Account &CoreStartData::GetAccounts() const noexcept { return accounts; }


const std::string &CoreStartData::Identifier() const noexcept { return identifier; }
...
bool CoreStartData::LoadChild(const DataNode &child, bool isAdd)
{
  ...
  else if(key == "system" && hasValue) system = GameData::Systems().Get(value);
  else if(key == "planet" && hasValue) planet = GameData::Planets().Get(value);
  else if(key == "account") accounts.Load(child, !isAdd);
  else return false;
