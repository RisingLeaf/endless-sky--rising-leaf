...
FormationPattern::PositionIterator::PositionIterator(const FormationPattern &pattern, double centerBodyRadius) :
...
const Point &FormationPattern::PositionIterator::operator*() const { return currentPoint; }
...
void FormationPattern::PositionIterator::MoveToValidPosition()
{
  ...
  if(atEnd) currentPoint = Point();
  else currentPoint = pattern.Position(ring, line, repeat, position);
}
  ...
  for(const DataNode &child : node)
  {
    ...
    if(key == "flippable" && hasValue)
    {
      for(int i = 1; i < child.Size(); ++i)
      {
        ...
        if(value == "x") flippableX = true;
        else if(value == "y") flippableY = true;
      ...
      for(const DataNode &grand : child)
      {
        ...
        else if(grandKey == "skip")
        {
          for(int i = 1; i < grand.Size(); ++i)
            if(grand.Token(i) == "first") line.skipFirst = true;
            else if(grand.Token(i) == "last") line.skipLast = true;
        ...
        else if(grandKey == "repeat")
        {
          ...
          for(const DataNode &great : grand)
          {
            ...
            if(greatKey == "start" && great.Size() >= 3) repeat.repeatStart.Set(great.Value(1), great.Value(2));
              ...
              repeat.repeatEndOrAnchor.Set(great.Value(1), great.Value(2));
            else if(greatKey == "angle" && greatHasValue && line.isArc) repeat.repeatAngle = great.Value(1);
            else if(greatKey == "positions" && greatHasValue)
              repeat.repeatPositions = static_cast<int>(great.Value(1) + 0.5);
            else great.PrintTrace("Skipping unrecognized attribute:");
          }
        }
        else {
          grand.PrintTrace("Skipping unrecognized attribute:");
...
const std::string &FormationPattern::TrueName() const { return trueName; }


void FormationPattern::SetTrueName(const std::string &name) { this->trueName = name; }
...
unsigned int FormationPattern::Lines() const { return lines.size(); }
...
unsigned int FormationPattern::Positions(unsigned int ring, unsigned int lineNr, unsigned int repeatNr) const
{
  ...
  if(ring > 0)
  {
    ...
    if(repeatNr >= line.repeats.size()) return 0;

    lineRepeatPositions += line.repeats[repeatNr].repeatPositions * ring;
  }

  // If we skip positions, then remove them from the counting.
  if(line.skipFirst && lineRepeatPositions > 0) --lineRepeatPositions;
  if(line.skipLast && lineRepeatPositions > 0) --lineRepeatPositions;

  // If we are in a later ring, then skip lines that don't repeat.
  if(lineRepeatPositions < 0) return 0;
...
Point FormationPattern::Position(
    unsigned int ring,
    unsigned int lineNr,
    unsigned int repeatNr,
    unsigned int linePosition) const
{
  // First check if the inputs result in a valid line or arc position.
  if(lineNr >= lines.size()) return Point();
  const Line &line = lines[lineNr];
  if(ring > 0 && repeatNr >= line.repeats.size()) return Point();
  ...
  if(line.skipFirst) ++linePosition;
  ...
  if(line.isArc)
  {
    ...
    if(repeat) endAngle += repeat->repeatAngle * ring;

    // Apply positions to get the correct position-angle.
    if(positions > 1) endAngle /= positions - 1;
    double positionAngle = startAngle + endAngle * linePosition;

    // Get into the range of 0 to 360 for conversion to angle.
    if(positionAngle < 0) positionAngle = -fmod(-positionAngle, 360) + 360;
    else positionAngle = fmod(positionAngle, 360);
  ...
  if(positions > 1) positionPx /= positions - 1;
...
int FormationPattern::Rotatable() const { return rotatable; }


bool FormationPattern::FlippableY() const { return flippableY; }


bool FormationPattern::FlippableX() const { return flippableX; }
